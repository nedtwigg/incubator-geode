 ...
 *
 * <p>This class returns top N queries based on pagination and filtering criteria if any
 *
  ...
  public ObjectNode execute(final HttpServletRequest request) throws Exception {
    ...
    for (int i = 0; i < stmts.length; ++i) {
      ...
      queryJSON.put(
          ...
          mapper.valueToTree(
              stmts[i].getParseTime() < 0 ? this.VALUE_NA : stmts[i].getParseTime()));
      queryJSON.put(
      ...
      queryJSON.put(
          ...
          mapper.valueToTree(
              stmts[i].getTotalCompilationTime() < 0
                  ? this.VALUE_NA
      ...
      queryJSON.put(
          ...
          mapper.valueToTree(
              stmts[i].getTotalExecutionTime() < 0
                  ? this.VALUE_NA
                  : stmts[i].getTotalExecutionTime()));
      queryJSON.put(
          PulseConstants.MBEAN_ATTRIBUTE_ROWSMODIFICATIONTIME,
          mapper.valueToTree(
              stmts[i].getRowsModificationTime() < 0
                  ? this.VALUE_NA
