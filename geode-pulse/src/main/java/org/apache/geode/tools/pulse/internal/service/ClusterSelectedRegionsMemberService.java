 ...
 *
 * <p>This class contains implementations of getting Cluster's selected region's member specific
...
public class ClusterSelectedRegionsMemberService implements PulseService {
  ...
  private static Comparator<Cluster.RegionOnMember> romEntryCountComparator =
      (m1, m2) -> {
  ...
  public ObjectNode execute(final HttpServletRequest request) throws Exception {
    ...
    responseJSON.put(
        "selectedRegionsMembers", getSelectedRegionsMembersJson(cluster, selectedRegionFullPath));
    // Send json response
    return responseJSON;
  }
  ...
  private ObjectNode getSelectedRegionsMembersJson(Cluster cluster, String selectedRegionFullPath) {
    ...
    if (reg != null) {
      ...
      for (RegionOnMember rom : romList) {
        ...
        LOGGER.finest(
            "calling getSelectedRegionsMembersJson :: rom.getLocalMaxMemory() = "
                + rom.getLocalMaxMemory());

        memberJSON.put(
            "memoryReadsTrend",
            mapper.<JsonNode>valueToTree(
                rom.getRegionOnMemberStatisticTrend(
