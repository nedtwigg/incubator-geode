 ...
 * distributed system.
 *
 * <p>Version tag information which is relevant between multiple distributed systems consistency
 * check is basically dsid and timestamp.
 */
...
public class NewWANConcurrencyCheckForDestroyDUnitTest extends WANTestBase {
  ...
  public void testVersionTagTimestampForDestroy() {
    ...
    vm1.invoke(
        new CacheSerializableRunnable("Putting an entry in ds1") {
    ...
    vm5.invoke(
        () ->
            NewWANConcurrencyCheckForDestroyDUnitTest.verifyTimestampAfterOp(
  ...
  public void testPutAllEventSequenceOnSerialGatewaySenderWithRR() {
    ...
    try {
      ...
      vm1.invoke(
          new CacheSerializableRunnable("Reset Test Hook") {
  ...
  public void testConflictChecksBasedOnDsidAndTimeStamp() {
    ...
    AsyncInvocation asynch =
        vm4.invokeAsync(
            new CacheSerializableRunnable("Putting an entry in ds2 in vm4") {
    ...
    vm3.invoke(
        new CacheSerializableRunnable("Check dsid") {
          ...
          public void run2() throws CacheException {
            ...
            Region.Entry entry =
                ((LocalRegion) region)
                    .getEntry("testKey", /*null,*/ true); //commented while merging revision 43582
            RegionEntry re = null;
