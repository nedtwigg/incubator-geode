...
public class CacheClientNotifierDUnitTest extends WANTestBase {
  ...
  public static void createClientWithLocator(int port0, String host, String regionName, String clientId, boolean isDurable) {
    ...
    Pool p;
    try {
      p = PoolManager.createFactory().addLocator(host, port0).setPingInterval(250).setSubscriptionEnabled(true).setSubscriptionRedundancy(-1).setReadTimeout(2000).setSocketBufferSize(1000).setMinConnections(6).setMaxConnections(10).setRetryAttempts(3).create(regionName);
    } finally {
    ...
    LogWriterUtils.getLogWriter().info("Distributed Region " + regionName + " created Successfully :" + region.toString() + " in a " + (isDurable ? "durable" : "") + " client");
  }
}
