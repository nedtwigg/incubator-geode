...
public class CacheClientNotifierDUnitTest extends WANTestBase {
  ...
  private int createCacheServerWithCSC(
      VM vm,
      final boolean withCSC,
      final int capacity,
      final String policy,
  ...
  private void checkCacheServer(
      VM vm, final int serverPort, final boolean withCSC, final int capacity) {
    SerializableRunnable checkCacheServer =
        new SerializableRunnable() {
  ...
  private void verifyRegionSize(VM vm, final int expect) {
    SerializableRunnable verifyRegionSize =
        new SerializableRunnable() {
          @Override
          public void run() throws Exception {
  ...
  public void doMultipleCacheServer(boolean durable) throws Exception {
    ...
    vm2.invoke(
        () ->
            createClientWithLocator(
                nyPort, "localhost", getTestMethodName() + "_PR", "123", durable));
    vm3.invoke(
        () ->
            createClientWithLocator(
                nyPort, "localhost", getTestMethodName() + "_PR", "124", durable));
  ...
  public static void createClientWithLocator(
      int port0, String host, String regionName, String clientId, boolean isDurable) {
