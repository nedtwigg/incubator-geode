...
public class PDXNewWanDUnitTest extends WANTestBase {
  ...
  public void testWANPDX_RR_SerialSender3Sites() {
    ...
    vm3.invoke(
        () ->
            WANTestBase.createReplicatedRegion(getTestMethodName() + "_RR", "ny,tk", isOffHeap()));
    vm4.invoke(
        () ->
            WANTestBase.createReplicatedRegion(getTestMethodName() + "_RR", "ln,tk", isOffHeap()));
    vm5.invoke(
        () ->
            WANTestBase.createReplicatedRegion(getTestMethodName() + "_RR", "ln,ny", isOffHeap()));
    ...
    vm3.invoke(
        () ->
            WANTestBase.createSender(
                "ln", 2, false, 100, 10, false, false, new PDXGatewayEventFilter(), true));
  ...
  public void testWANPDX_PR_MultipleVM_ParallelSenderWithFilter() {
    ...
    vm3.invoke(
        () ->
            WANTestBase.createSender(
                "ln", 2, true, 100, 10, false, false, new PDXGatewayEventFilter(), true));
    vm4.invoke(
        () ->
            WANTestBase.createSender(
                "ln", 2, true, 100, 10, false, false, new PDXGatewayEventFilter(), true));
