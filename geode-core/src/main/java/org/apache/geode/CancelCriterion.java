 ...
 *
 * <p>Creators of services or threads should implement a subclass of CancelCriterion, and implement
 * the two methods - cancelInProgress, and generateCancelledException(e).
 *
 * <p>Code inside the service can check to see if the service is cancelled by calling {@link
 ...
 * and check for cancelation inside a long loop. Eg. <code>
 * while(true) {
...
public abstract class CancelCriterion {
  ...
  /**
   * See if the current operation is being cancelled. If so, it either throws a {@link
   * RuntimeException} (usually a {@link CancelException}).
   *
  ...
  /**
   * Template factory method for generating the exception to be thrown by {@link
   * #checkCancelInProgress(Throwable)}. Override this to specify different exception for
   * checkCancelInProgress() to throw.
   *
   * <p>This method should wrap the exception in a service specific CancelationException (eg
