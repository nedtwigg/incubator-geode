...
public abstract class ListExecutor extends AbstractExecutor {
  ...
  @Override
  protected Region<Integer, ByteArrayWrapper> getOrCreateRegion(
      ExecutionHandlerContext context, ByteArrayWrapper key, RedisDataType type) {
    return (Region<Integer, ByteArrayWrapper>)
        context.getRegionProvider().getOrCreateRegion(key, type, context);
  }

  @SuppressWarnings("unchecked")
  protected Region<Integer, ByteArrayWrapper> getRegion(
   ...
   */
  protected void pushElements(
      ByteArrayWrapper key,
      List<byte[]> commandElems,
      int startIndex,
      int endIndex,
      Region keyRegion,
      ListDirection pushType,
    ...
    if (index != opp)
      index +=
          pushType == ListDirection.LEFT ? -1 : 1; // Subtract index if left push, add if right push

    /**
     * Multi push command
     *
     * <p>For every element that needs to be added
     */
    for (int i = startIndex; i < endIndex; i++) {
      ...
      do {
        ...
        if (oldValue != null) {
          index +=
              pushType == ListDirection.LEFT
                  ? -1
                  : 1; // Subtract index if left push, add if right push
        }
      } while (oldValue != null);
       ...
       *
       * <p>********************** LPUSH/LPUSH *************************** Push occurring at the
       * same time, further index update first | This push | | | | V V [-4] [-3] [-2] [-1] [0] [1]
       * [2]
       *
       * <p>In this case, -4 would already exist in the meta data region, therefore we do not try to
       * put -3 in the meta data region because a further index is already there.
       * ***************************************************************
       *
       * <p>Another example
       *
       * <p>********************** LPUSH/LPOP ***************************** This push | Simultaneous
       * LPOP, meta data head index already updated to -2 | | | | V V [-4] [X] [-2] [-1] [0] [1] [2]
       *
       * <p>In this case, -2 would already exist in the meta data region, but we need to make sure
       * the element at -4 is visible to all other threads so we will attempt to change the index to
       * -4 as long as it is greater than -4
       * ***************************************************************
       */
