...
public abstract class PopExecutor extends ListExecutor implements Extendable {
  ...
  public void executeCommand(Command command, ExecutionHandlerContext context) {
    ...
    do {
      ...
      if (valueWrapper != null) removed = keyRegion.remove(index, valueWrapper);
       ...
       *
       * <p>************************** FIRST MISS *********************************** Push occurring
       * at the same time, further index update first | This is location of miss | | | | V V [-4]
       * [X] [-2] [-1] [0] [1] [2] <-- Direction of index update If this is the first miss, the
       * index is re obtained from the meta region and that index is trying. However, if the index
       * in the meta data region is not further out, that index is not used and moves on to the
       * second case **************************************************************************
       *
       * <p>************************* SUBSEQUENT MISSES ****************************** Push
       * occurring at the same time, further index update first | This is location of miss | | | | V
       * V [-4] [X] [-2] [-1] [0] [1] [2] Direction of index update --> If this is not the first
       * miss then we move down to the other end of the list which means the next not empty index
       * will be attempted to be removed
       * **************************************************************************
       *
       * <p>If it is the case that the list is empty, it will exit this loop
       */
      index += incr;
      Integer metaIndex = (Integer) keyRegion.get(indexKey);
      if (i < 1
          && (popType() == ListDirection.LEFT && metaIndex < originalIndex
              || popType() == ListDirection.RIGHT && metaIndex > originalIndex)) index = metaIndex;
      i++;
