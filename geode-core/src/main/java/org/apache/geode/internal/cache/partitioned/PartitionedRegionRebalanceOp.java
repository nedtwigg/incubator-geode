 ...
 *
 * <p>There are three main classes involved in the rebalance - this class, the
 ...
 *
 * <p>The PartitionedRegionLoadModel model of the system that is constructed by this class. It
 * contains information about what buckets are where and how big they are. It has methods to find
 * low redundancy buckets or determine where the best place to move a bucket is.
 *
 * <p>The RebalanceDirector is responsible for actually deciding what to do at each step of a
 ...
 *
 * <p>There is also a FPRDirector that creates buckets and moves primaries for fixed partititioned
 * regions.
 */
...
public class PartitionedRegionRebalanceOp {
   ...
   *     recovery
   */
  public PartitionedRegionRebalanceOp(
  ...
  public Set<PartitionRebalanceInfo> execute() {
    ...
    try {
      ...
      ParallelBucketOperator parallelOperator =
          new ParallelBucketOperator(
   ...
   */
  private PartitionedRegionLoadModel buildModel(
      BucketOperator operator,
      Map<PartitionedRegion, InternalPRInfo> detailsMap,
      InternalResourceManager resourceManager) {
    ...
    debug(
        "Building Model for rebalancing "
            + leaderRegion
            + ". redundantCopies="
            + redundantCopies
            + ", totalNumBuckets="
      ...
      for (PartitionMemberInfo memberDetails : details.getPartitionMemberInfo()) {
        debug(
            "For Region: "
                + region
                + ", Member: "
                + memberDetails.getDistributedMember()
                + "LOAD="
                + ((InternalPartitionDetails) memberDetails).getPRLoad()
                + ", equivalentMembers="
                + dm.getMembersInSameZone(
  ...
  public boolean createRedundantBucketForRegion(InternalDistributedMember target, int bucketId) {
    return getLeaderRegion()
        ...
        .createBackupBucketOnMember(bucketId, target, isRebalance, replaceOfflineData, null, true);
  }
   ...
   */
  public boolean moveBucketForRegion(
