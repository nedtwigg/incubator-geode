...
public class FunctionExecutionNodePruner {
  ...
  public static HashMap<InternalDistributedMember, HashSet<Integer>> pruneNodes(
    ...
    try {
      for (Integer bucketId : buckets) {
        ...
        if (isDebugEnabled) {
          logger.debug(
              "FunctionExecutionNodePruner: The buckets owners of the bucket: {} are: {}",
              bucketId,
              nodes);
        }
     ...
     *
     * <p>Second Logic: Give highest preference to the local node and after that use First Logic. //
     ...
     *
     * <p>Third Logic: After including local node, choose random nodes among the remaining nodes in
    ...
    while (!currentBucketSet.equals(buckets)) {
      ...
      if (!bucketSet.isEmpty()) {
        ...
        if (isDebugEnabled) {
          logger.debug(
              "FunctionExecutionNodePruner: Adding the node: {} and buckets {} to prunedMap",
              node,
              bucketSet);
        }
      }
    ...
    if (isDebugEnabled) {
      logger.debug(
          "FunctionExecutionNodePruner: The final prunedNodeToBucket calculated is: {}",
  ...
  private static InternalDistributedMember findNextNode(
    ...
    return (nodesOfEqualSize.size() > 0
        ? nodesOfEqualSize.get(PartitionedRegion.rand.nextInt(nodesOfEqualSize.size()))
        : null);
  }

  public static HashMap<Integer, HashSet> groupByBucket(
      PartitionedRegion pr,
      Set routingKeys,
      final boolean primaryMembersNeeded,
          ...
          bucketId =
              Integer.valueOf(
                  PartitionedRegionHelper.getHashKey(
                      pr, Operation.FUNCTION_EXECUTION, key, null, null));
        }
      }
  ...
  public static HashSet<Integer> getBucketSet(
      PartitionedRegion pr,
      Set routingKeys,
      final boolean hasRoutingObjects,
