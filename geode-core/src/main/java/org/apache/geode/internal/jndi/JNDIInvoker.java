 ...
 *
 * <p>Datasources are bound to jndi tree after getting initialised. The initialisation parameters
 * are read from cache.xml.
 *
 * <p>If there is a pre-existing TransactionManager/UserTransaction (possible in presence of
...
public class JNDIInvoker {
   ...
   *
   * <p>If there is pre-existing JNDI tree in the system and TransactionManager / UserTransaction is
  ...
  private static void doTransactionLookup() throws NamingException {
    ...
    for (int i = 0; i < knownJNDIManagers.length; i++) {
      try {
        ...
        String exception =
            "JNDIInvoker::doTransactionLookup::Couldn't lookup ["
                + knownJNDIManagers[i][0]
                + " ("
      ...
      if (jndiObject instanceof TransactionManager) {
        ...
        String exception =
            "JNDIInvoker::doTransactionLookup::Found TransactionManager for "
                + knownJNDIManagers[i][1];
        if (writer.fineEnabled()) writer.fine(exception);
        return;
      } else {
        String exception =
            "JNDIInvoker::doTransactionLookup::Found TransactionManager of class "
    ...
    try {
      ...
      if (writer.fineEnabled())
        writer.fine(
            "JNDIInvoker::doTransactionLookup::Found WebSphere 5.1: " + WS_FACTORY_CLASS_5_1);
