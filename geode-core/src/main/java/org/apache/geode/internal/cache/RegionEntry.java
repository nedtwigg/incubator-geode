 ...
 *
 * @since GemFire 3.5.1
 *     <p>Implementations: (unshared) ThinEntry protected volatile long lastModified; StatsEntry
 *     extends ThinEntry private volatile long lastAccessed; private volatile int hitCount; private
 *     volatile int missCount; DiskThinEntry extends ThinEntry private volatile long id;
...
public interface RegionEntry {
   ...
   * those other events.
   *
   * <p>This is done within the RegionEntry synchronization lock.
   */
   ...
   *
   * <p>This method returns tombstones so that they can be transferred with the initial image.
   *
   ...
   *
   * <p>This is only retained in off-heap subclasses. However, it's marked as Retained here so that
  ...
  /**
   * Returns true if the entry value is equal to {@link Token#INVALID} or {@link
   * Token#LOCAL_INVALID}.
   */
  ...
  /**
   * Returns true if the entry value is equal to {@link Token#DESTROYED} or {@link
   * Token#REMOVED_PHASE1} or {@link Token#REMOVED_PHASE2} or {@link Token#TOMBSTONE}.
   */
