...
public class EventTracker {
   ...
   * org.apache.geode.internal.cache.EventTracker.EventSeqnoHolder}.
   */
   ...
   * this cache.
   *
   * <p>Keys are instances of @link {@link ThreadIdentifier}, values are instances of {@link
   * BulkOpProcessed}.
   */
  ...
  /**
   * a mapping of originator to bulkOperation's last version tags. This map differs from {@link
   * #recordedBulkOps} in that the thread identifier used here is the base member id and thread id
   * of the bulk op, as opposed to the fake thread id which is assigned for each bucket.
   *
   * <p>recordedBulkOps are also only tracked on the secondary for partitioned regions
  ...
  public Map<ThreadIdentifier, EventSeqnoHolder> getState() {
    ...
    for (Iterator<Map.Entry<ThreadIdentifier, EventSeqnoHolder>> it =
            recordedEvents.entrySet().iterator();
        it.hasNext();
        ) {
      ...
      result.put(
          entry.getKey(),
          new EventSeqnoHolder(
              holder.lastSeqno,
              null)); // don't transfer version tags - adds too much bulk just so we can do client tag recovery
    }
    return result;
  }
  ...
  public void recordState(
      InternalDistributedMember provider, Map<ThreadIdentifier, EventSeqnoHolder> state) {
    ...
    for (Iterator<Map.Entry<ThreadIdentifier, EventSeqnoHolder>> it = state.entrySet().iterator();
        it.hasNext();
        ) {
      Map.Entry<ThreadIdentifier, EventSeqnoHolder> entry = it.next();
      if (sb != null) {
  ...
  public void recordEvent(InternalCacheEvent event) {
    ...
    VersionTag tag = null;
    if (lr.getServerProxy()
        == null /* && event.hasClientOrigin()*/) { // clients do not need to store version tags for replayed events
  ...
  private void recordBulkOpEvent(InternalCacheEvent event, ThreadIdentifier tid) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "recording bulkOp event {} {} {} op={}",
          tid.expensiveToString(),
          eventID,
          tag,
          event.getOperation());
    }
  ...
  public boolean hasSeenEvent(EventID eventID, InternalCacheEvent tagHolder) {
    ...
    synchronized (evh) {
      ...
      if (logger.isTraceEnabled(LogMarker.DISTRIBUTION_BRIDGE_SERVER)) {
        logger.trace(
            LogMarker.DISTRIBUTION_BRIDGE_SERVER,
  ...
  public VersionTag findVersionTag(EventID eventID) {
    ...
    if (evh == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "search for version tag failed as no event is recorded for {}",
    ...
    synchronized (evh) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "search for version tag located last event for {}: {}",
            threadID.expensiveToString(),
            evh);
      }
  ...
  public VersionTag findVersionTagForBulkOp(EventID eventID) {
    ...
    if (evh == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "search for version tag failed as no events are recorded for {}",
            threadID.expensiveToString());
      }
      return null;
    }
    ...
    synchronized (evh) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "search for version tag located event holder for {}: {}",
            threadID.expensiveToString(),
            evh);
      }
