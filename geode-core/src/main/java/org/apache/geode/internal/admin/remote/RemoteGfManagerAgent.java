 ...
 * communicate with other members of the distributed system. Because it is a <code>
 * MembershipListener</code> it is alerted when members join and leave the distributed system. It
 * also implements support for {@link JoinLeaveListener}s as well suport for collecting and
...
final class RemoteGfManagerAgent implements GfManagerAgent {
      ...
      if (this.isConnected() || (this.membersMap.size() > 0)) { //Hot fix from 6.6.3
        ...
        for (int i = 0; i < apps.length; i++) {
          try {
            apps[i]
                .disconnect(
                    removeListener); // hit NPE here in ConsoleDistributionManagerTest so fixed listApplications to exclude nulls returned from future
  ...
  public ApplicationVM[] listApplications(boolean disconnected) { //Hot fix from 6.6.3
    if (isConnected() || (this.membersMap.size() > 0 && disconnected)) {
      ...
      VMS:
      for (Iterator iter = this.membersMap.values().iterator(); iter.hasNext(); ) {
  ...
  private static ThreadLocal sending =
      new ThreadLocal() {
        @Override
  ...
  AdminResponse sendAndWait(AdminRequest msg) {
    try {
      ...
      if (isConnected()) {
        ...
        throw new RuntimeAdminException(
            LocalizedStrings.RemoteGfManagerAgent_0_IS_NOT_CURRENTLY_CONNECTED.toLocalizedString(
                this));
      }
  ...
  /**
   * Returns the distributed system member (application VM or system VM) with the given <code>id
   * </code>.
   */
  ...
  protected void callCacheCollector(
      CacheSnapshot results, InternalDistributedMember sender, int snapshotId) {
    if (!isListening()) {
      return;
    }
    if (collector != null) {
  ...
  protected void connectToDS() {
    ...
    this.system.addDisconnectListener(
        new InternalDistributedSystem.DisconnectListener() {
          ...
          public void onDisconnect(InternalDistributedSystem sys) {
            ...
            if (!reconnect) {
              ...
              if (RemoteGfManagerAgent.this
                  .disconnect()) { // returns true if disconnected during this call
                if (listener != null) {
    ...
    InternalDistributedSystem.addReconnectListener(
        new ReconnectListener() {
          ...
          public void onReconnect(
              InternalDistributedSystem oldsys, InternalDistributedSystem newsys) {
            if (logger.isDebugEnabled()) {
              logger.debug(
                  "RemoteGfManagerAgent.onReconnect attempting to join new distributed system");
            }
            join();
          }
        });
  ...
  /**
   * A daemon thread that reads {@link SnapshotResultMessage}s from a queue and invokes the <code>
   * CacheCollector</code> accordingly.
   */
        ...
        try {
          ...
          logger.warn(
              LocalizedMessage.create(
                  LocalizedStrings.RemoteGfManagerAgent_IGNORING_STRANGE_INTERRUPT),
              ignore);
        } catch (Exception ex) {
  ...
  protected void handleJoined(InternalDistributedMember id) {
    ...
    try {
      ...
      switch (id.getVmKind()) {
        ...
        default:
          throw new IllegalArgumentException(
              LocalizedStrings.RemoteGfManagerAgent_UNKNOWN_VM_KIND_0.toLocalizedString(
      ...
      if (member != null) {
        ...
        for (Iterator it = this.listeners.iterator(); it.hasNext(); ) {
          ...
          try {
            ...
            logger.warn(
                LocalizedMessage.create(
                    LocalizedStrings.RemoteGfManagerAgent_LISTENER_THREW_AN_EXCEPTION),
                e);
          }
        }
      }
      ...
      while (!this.shutDown) {
        ...
        try {
          ...
          if (this.paused || noPendingJoins) { //fix for #39893
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                  "JoinProcessor was interrupted when it was paused, now resuming ...", ignore);
            }
          ...
          logger.error(
              LocalizedMessage.create(
                  LocalizedStrings.RemoteGfManagerAgent_JOINPROCESSOR_CAUGHT_EXCEPTION),
              e);
        } // catch Throwable
      } // while !shutDown
    }
