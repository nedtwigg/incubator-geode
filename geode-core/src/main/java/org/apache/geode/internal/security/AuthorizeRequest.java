...
public class AuthorizeRequest {
  ...
  public AuthorizeRequest(
      String authzFactoryName, DistributedMember dm, Principal principal, Cache cache)
  ...
  public AuthorizeRequest(
      String authzFactoryName, ClientProxyMembershipID id, Principal principal, Cache cache)
      throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException,
          InvocationTargetException, NotAuthorizedException {
  ...
  public PutOperationContext putAuthorize(
      String regionName, Object key, Object value, boolean isObject, Object callbackArg)
      throws NotAuthorizedException {
  ...
  public RemoveAllOperationContext removeAllAuthorize(
      String regionName, Collection<?> keys, Object callbackArg) throws NotAuthorizedException {
    ...
    if (!this.authzCallback.authorizeOperation(regionName, removeAllContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString()
                + ": Authorized to perform removeAll operation on region ["
                + regionName
                + ']');
      }
    }
  ...
  public QueryOperationContext queryAuthorize(
      String queryString, Set regionNames, Object[] queryParams) throws NotAuthorizedException {
  ...
  public ExecuteCQOperationContext executeCQAuthorize(
      String cqName, String queryString, Set regionNames) throws NotAuthorizedException {
    ...
    if (!this.authzCallback.authorizeOperation(null, executeCQContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString()
                + ": Authorized to perform EXECUTE_CQ operation ["
                + queryString
                + "] on cache");
      }
    }
  ...
  public void stopCQAuthorize(String cqName, String queryString, Set regionNames)
    ...
    if (!this.authzCallback.authorizeOperation(null, stopCQContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString()
                + ": Authorized to perform STOP_CQ operation ["
                + cqName
                + ','
                + queryString
                + "] on cache");
  ...
  public void closeCQAuthorize(String cqName, String queryString, Set regionNames)
    ...
    if (!this.authzCallback.authorizeOperation(null, closeCQContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString()
                + ": Authorized to perform CLOSE_CQ operation ["
                + cqName
                + ','
                + queryString
                + "] on cache");
  ...
  public void getDurableCQsAuthorize() throws NotAuthorizedException {
    ...
    if (!this.authzCallback.authorizeOperation(null, getDurableCQsContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString() + ": Authorized to perform GET_DURABLE_CQS operation on cache");
      }
    }
  }
  ...
  public RegisterInterestOperationContext registerInterestAuthorize(
      String regionName, Object key, int interestType, InterestResultPolicy policy)
      throws NotAuthorizedException {

    RegisterInterestOperationContext registerInterestContext =
        new RegisterInterestOperationContext(
  ...
  public RegisterInterestOperationContext registerInterestListAuthorize(
      String regionName, List keys, InterestResultPolicy policy) throws NotAuthorizedException {
  ...
  public ExecuteFunctionOperationContext executeFunctionAuthorize(
    ...
    ExecuteFunctionOperationContext executeContext =
        new ExecuteFunctionOperationContext(
    ...
    if (!this.authzCallback.authorizeOperation(region, executeContext)) {
      ...
      if (this.logger.finestEnabled()) {
        this.logger.finest(
            toString() + ": Authorized to perform EXECUTE_REGION_FUNCTION operation ");
      }
    }
    return executeContext;
  }

  public InvalidateOperationContext invalidateAuthorize(
      String regionName, Object key, Object callbackArg) throws NotAuthorizedException {
