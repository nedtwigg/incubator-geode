 ...
 *
 * <p>This class is useful if you have a task that you want to make sure runs at least once after an
 * event, but if the event happens repeatedly before the task runs, the task should still only run
 * once.
 *
 * <p>In many cases it might make sense to have multiple decorators, or to submit one kind of task
 * through the decorator and other tasks directly through the executor.
 *
 * <p>For example, a task that recovers redundancy for all buckets of a PR when a member crashes
 * only needs to be run once, no matter how many members crash before the task starts. But after the
 * task has started, we want to make sure we schedule another execution for the next crash.
 *
 * <p>If the a new task is scheduled to run sooner than the task that is currently in the queue, the
