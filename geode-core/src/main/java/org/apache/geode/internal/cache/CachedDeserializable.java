...
public interface CachedDeserializable extends Sizeable {
   ...
   * value: 1) read the byte array off the stream {@link
   * DataSerializer#readByteArray(java.io.DataInput)} 2) de-serialize the byte array using {@link
   * BlobHelper#deserializeBlob(byte[])} into an object. The idea is to delay de-serialization until
   * the last possible moment to provide better parallelism
   *
