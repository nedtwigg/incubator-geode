...
public class InternalInstantiator {
  ...
  private static boolean checkForThread() {
    ...
    return !(name.startsWith(CacheClientUpdater.CLIENT_UPDATER_THREAD_NAME) || name.startsWith(SERVER_CONNECTION_THREAD));
  }
  ...
  public static void register(String instantiatorClass, String instantiatedClass, int id, boolean distribute) {
    ...
    if (checkForThread()) {
      register(instantiatorClass, new InstantiatorAttributesHolder(instantiatorClass, instantiatedClass, id), distribute);
    }
  }
  ...
  public static void register(String instantiatorClass, String instantiatedClass, int id, boolean distribute, EventID eventId, ClientProxyMembershipID context) {
    register(instantiatorClass, new InstantiatorAttributesHolder(instantiatorClass, instantiatedClass, id, eventId, context), distribute);
  }
  ...
  private static void register(String instantiatorClassName, InstantiatorAttributesHolder holder, boolean distribute) {
    ...
    synchronized (InternalInstantiator.class) {
      ...
      if (inst == null) {
        ...
        if (iah != null && iah.getId() != holder.getId()) {
          throw new IllegalStateException(LocalizedStrings.InternalInstantiator_CLASS_0_IS_ALREADY_REGISTERED_WITH_ID_1_SO_IT_CANNOT_BE_REGISTERED_WTH_ID_2.toLocalizedString(new Object[] { instantiatorClassName, iah.getId(), holder.getId() }));
        }
    ...
    if (inst instanceof Marker) {
      ...
      synchronized (InternalInstantiator.class) {
        ...
        if (inst2 == inst) {
          register(instantiatorClass, instantiatedClass, holder.getId(), distribute, holder.getEventId(), holder.getContext());
        } else {
    ...
    public void fromData(DataInput in) throws IOException, ClassNotFoundException {
      ...
      if (CacheClientNotifier.getInstance() != null) {
        ...
        try {
          ...
          fromDataProblem(LocalizedStrings.InternalInstantiator_COULD_NOT_LOAD_INSTANTIATOR_CLASS_0.toLocalizedString(ex));
        ...
        try {
          ...
          fromDataProblem(LocalizedStrings.InternalInstantiator_COULD_NOT_LOAD_INSTANTIATED_CLASS_0.toLocalizedString(ex));
