...
public class GlobalTransaction {
   ...
   *
   * <p>Concurrency: Some paths invoke this method after taking a lock on "this" while other paths
  ...
  public void rollback() throws IllegalStateException, SystemException {
    ...
    try {
      ...
      if (xar1 != null) xar1.rollback(xid);
      status = Status.STATUS_ROLLEDBACK;
      if (VERBOSE) writer.fine("Transaction rolled back successfully");
    } catch (Exception e) {
      // we will throw an error later, make sure that the synchronizations rollback
      status = Status.STATUS_ROLLEDBACK;
      String exception =
   ...
   *
   * <p>Concurrency: The order of acquiring lock will be lock on "this" followed by lock on
   ...
   *
   * <p>In the current implementation this call will never be made by the application server. The
  ...
  public boolean delistResource(XAResource xaRes, int flag)
    ...
    try {
      ...
      String exception =
          LocalizedStrings.GlobalTransaction_ERROR_WHILE_DELISTING_XARESOURCE_0_1.toLocalizedString(
              new Object[] {
                e,
                " "
                    + (e instanceof XAException
  ...
  public int compare(GlobalTransaction other) {
    ...
    if (myId < otherId) {
      ...
      throw new IllegalStateException(
          LocalizedStrings.GlobalTransaction_COULD_NOT_COMPARE_0_TO_1.toLocalizedString(
