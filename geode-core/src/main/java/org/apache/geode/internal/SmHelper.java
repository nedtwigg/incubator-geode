...
public class SmHelper {
   ...
   * @throws IllegalAccessException If the zero-argument constructor is not public (only for <code>
   *     Externalizable</code> classes in which <code>c.equals(initClass)</code>
   */
  public static Object allocateJOMObject(Class c, Class initClass)
      throws IllegalAccessException, InvalidClassException {
    if (c.isPrimitive()) {
      throw new InvalidClassException(
          LocalizedStrings.SmHelper_IS_PRIMITIVE.toLocalizedString(), c.getName());

    } else if (initClass.isPrimitive()) {
      throw new InvalidClassException(
          LocalizedStrings.SmHelper_IS_PRIMITIVE.toLocalizedString(), initClass.getName());
    }
   ...
   * of the late, lamented <code>allocateNewObject</code> method of <code>java.io.ObjectInputStream
   * </code> in JDK 1.3.
   */
