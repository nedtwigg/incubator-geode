 ...
 *
 * <p>When register interest responses are deserialized and added to the cache, the following rules
 * are applied
 *
 * <p>1) If the entry exists in the cache with a valid value, we consider it to be more current
 * (either because it made it via the update channel or because the client updated it as part of put
 * or invalidate)
 *
 * <p>2) If the entry is invalid, and the RI response is valid, the valid value is put into the
 * cache
 *
 * <p>3) If an entry is marked destroyed (either from the client operation or via the update
 * channel), it is not updated from the RI response. Destroyed entries are gathered up after the RI
 * response is completed and removed from the system
 *
 * @since GemFire 5.1
 */
public class ServerResponseMatrix {
  ...
  public static boolean checkForValidStateAfterNotification(
      LocalRegion region, Object key, int operation) {
    ...
    if (re == null) {
      ...
      Token token = re.getValueAsToken();
      if (token == Token.DESTROYED
          || token == Token.REMOVED_PHASE1
          || token == Token.REMOVED_PHASE2
