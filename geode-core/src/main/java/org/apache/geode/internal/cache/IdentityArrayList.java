 ...
 * needed when new entries are created (by doubling the current length), but is <em>never</em> made
 * smaller (even on a {@link #clear()}). A family of {@linkplain #trim() trimming methods} lets you
 * control the size of the backing array; this is particularly useful if you reuse instances of this
 * class. Range checks are equivalent to those of <code>java.util</code>'s classes, but they are
 * delayed as much as possible.
 *
 * <p>The backing array is exposed by the {@link #elements()} method. If an instance of this class
 * was created {@linkplain #wrap(Object[],int) by wrapping}, backing-array reallocations will be
 * performed using reflection, so that {@link #elements()} can return an array of the same type of
 * the original array; the comments about efficiency made in {@link
 * it.unimi.dsi.fastutil.objects.ObjectArrays} apply here.
 *
 * <p>This class implements the bulk methods <code>removeElements()</code>, <code>addElements()
 * </code> and <code>getElements()</code> using high-performance system calls (e.g., {@link
...
public class IdentityArrayList<K> extends AbstractObjectList<K>
   ...
   *
   * <p>If this array list was created by wrapping a given array, it is guaranteed that the type of
   * the returned array will be the same. Otherwise, the returned array will be of type {@link
   * Object Object[]} (in spite of the declared return type).
   *
   * <p><strong>Warning</strong>: This behaviour may cause (unfathomable) run-time errors if a
   * method expects an array actually of type <code>K[]</code>, but this methods returns an array of
   * type {@link Object Object[]}.
   *
    ...
    if (wrapped) a = ObjectArrays.grow(a, capacity, size);
      ...
      if (capacity > a.length) {
        final int newLength =
            (int)
                Math.max(
                    Math.min(2L * a.length, it.unimi.dsi.fastutil.Arrays.MAX_ARRAY_SIZE), capacity);
   ...
   *
   * <p>If the current array length is smaller than or equal to <code>n</code>, this method does
   * nothing. Otherwise, it trims the array length to the maximum between <code>n</code> and {@link
   * #size()}.
   *
   ...
   *
   * <p>This method exists only for sake of efficiency. The implementation inherited from the
