 ...
 * gracefully. i.e. Cache.close()</br><br>
...
public class PRHARedundancyProvider {
  ...
  public static String regionStatus(
      PartitionedRegion prRegion, Set allStores, Collection alreadyUsed, boolean forLog) {
      ...
      sb.append(
          newLine
              ...
              + PartitionedRegionHelper.printCollection(
                  prRegion.getDistributionManager().getMembersInThisZone()));
    }
    return sb.toString();
  }
    ...
    final String tooManyRetries =
        LocalizedStrings
                ...
                .toLocalizedString(
                    new Object[] {
                      opString,
                      regionStatus(prRegion, allStores, alreadyUsed, true),
   ...
   */
  private InternalDistributedMember createBucketInstance(
      int bucketId,
    ...
    if (isDebugEnabled) {
      logger.debug(
          "AllStores={} AlreadyUsed={} excluded={} failed={}",
          allStores,
          alreadyUsed,
          excludedMembers,
          failedMembers);
    ...
    if (!(candidate.equals(this.prRegion.getMyId()))) { // myself
      ...
      if (pp == null) {
        if (isDebugEnabled) {
          logger.debug(
              "createBucketInstance: {}: no partition profile for {}",
    ...
    if (isDebugEnabled) {
      logger.debug(
          "createBucketInstance: {}: candidate {} declined to manage bucketId={}: {}",
          this.prRegion.getFullPath(),
          candidate,
  ...
  public static final long INSUFFICIENT_LOGGING_THROTTLE_TIME =
      TimeUnit.SECONDS.toNanos(
          Integer.getInteger(
                  DistributionConfig.GEMFIRE_PREFIX + "InsufficientLoggingThrottleTime", 2)
              .intValue());
  ...
  public InternalDistributedMember createBucketOnDataStore(
      int bucketId, int size, long startTime, RetryTimeKeeper snoozer) {
    ...
    do {
      ...
      try {
        if (isDebugEnabled) {
          logger.debug(
              "Attempting to get data store {} to create the bucket {} for us",
              target,
              this.prRegion.bucketStringForLogs(bucketId));
        }
   ...
   *
   * <p>This method is synchronized to enforce a single threaded ordering, allowing for a more
   * accurate picture of bucket distribution in the face of concurrency. See bug 37275. This method
   * is now slightly misnamed. Another member could be in the process of creating this same bucket
   * at the same time.
   *
   ...
   */
  public InternalDistributedMember createBucketAtomically(
      final int bucketId,
      final int newBucketSize,
      final long startTime,
      final boolean finishIncompleteCreation,
    ...
    synchronized (this) {
      ...
      if (isDebugEnabled) {
        logger.debug(
            "Starting atomic creation of bucketId={}", this.prRegion.bucketStringForLogs(bucketId));
      }
      ...
      try {
        ...
        boolean loggedInsufficentStores =
            false; // track if insufficient data stores have been detected
        for (; ; ) {
          this.prRegion.checkReadiness();
          if (this.prRegion.getCache().isCacheAtShutdownAll()) {
            if (isDebugEnabled) {
          ...
          if (timeLeft < 0) {
            ...
            timedOut(
                this.prRegion,
                getAllStores(partitionName),
                acceptedMembers,
                ALLOCATE_ENOUGH_MEMBERS_TO_HOST_BUCKET.toLocalizedString(),
                computeTimeout());
            // NOTREACHED
          }
          if (isDebugEnabled) {
              ...
              if (!(this.prRegion.getDistributionManager() instanceof LonerDistributionManager)) {
                ...
                logger.warn(
                    LocalizedMessage.create(
                        LocalizedStrings
                            .GemFireCache_ENFORCE_UNIQUE_HOST_NOT_APPLICABLE_FOR_LONER));
              }
            }
          }
          ...
          final int potentialCandidateCount =
              (allStores.size()
   ...
   */
  private void endBucketCreation(
      int bucketId,
      Collection<InternalDistributedMember> acceptedMembers,
      InternalDistributedMember targetPrimary,
      String partitionName) {
  ...
  public void endBucketCreationLocally(int bucketId, InternalDistributedMember newPrimary) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "endBucketCreationLocally: for region {} bucketId={} new primary: {}",
          this.prRegion.getFullPath(),
          bucketId,
          newPrimary);
    }
  ...
  /**
   * Compute timeout for waiting for a bucket. Prefer {@link
   * #DATASTORE_DISCOVERY_TIMEOUT_MILLISECONDS} over {@link PartitionedRegion#getRetryTimeout()}
   *
   ...
   */
  private boolean checkSufficientStores(
      final Set allStores, final boolean loggedInsufficentStores) {
   ...
   */
  public boolean createBackupBucketOnMember(
      final int bucketId,
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "createBackupBucketOnMember for bucketId={} member: {}",
          this.prRegion.bucketStringForLogs(bucketId),
          targetNMember);
    }
    ...
    if (!(targetNMember.equals(this.prRegion.getMyId()))) {
      ...
      try {
        ManageBackupBucketMessage.NodeResponse response =
            ManageBackupBucketMessage.send(
                targetNMember,
                this.prRegion,
                bucketId,
                isRebalance,
      ...
      boolean bucketManaged =
          prDS != null
              && prDS.grabBucket(
                      ...
                      null,
                      false)
                  .equals(CreateBucketResult.CREATED);
      if (!bucketManaged) {
        if (logger.isDebugEnabled()) {
          logger.debug(
   ...
   */
  public ManageBucketRsp createBucketOnMember(
      final int bucketId,
      final InternalDistributedMember targetNMember,
      final int newBucketSize,
      boolean forceCreation) {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "createBucketOnMember for bucketId={} member: {}{}",
          this.prRegion.bucketStringForLogs(bucketId),
          targetNMember,
    ...
    if (!(targetNMember.equals(this.prRegion.getMyId()))) {
      ...
      try {
        ...
        NodeResponse response =
            ManageBucketMessage.send(
                targetNMember, this.prRegion, bucketId, newBucketSize, forceCreation);
        ...
        if (response.waitForAcceptance()) {
          ...
          return response.rejectedDueToInitialization()
              ? ManageBucketRsp.NO_INITIALIZING
              : ManageBucketRsp.NO;
        }
      } catch (PartitionOfflineException e) {
        throw e;
      ...
      boolean bucketManaged =
          prDS != null
              && prDS.handleManageBucketRequest(
                  bucketId, newBucketSize, this.prRegion.getMyId(), forceCreation);
      if (!bucketManaged) {
        if (logger.isDebugEnabled()) {
          logger.debug(
   ...
   *
   * <p>Under concurrent access, the data that this method uses, may be somewhat volatile, note that
  ...
  private InternalDistributedMember getPreferredDataStore(
    ...
    ArrayList<DataStoreBuckets> stores =
        this.prRegion
            .getRegionAdvisor()
    ...
    Comparator<DataStoreBuckets> comparator =
        new Comparator<DataStoreBuckets>() {
  ...
  public static class DataStoreBuckets {
    ...
    public DataStoreBuckets(
        InternalDistributedMember mem, int buckets, int primaryBuckets, int localMaxMemory) {
      this.memberId = mem;
    ...
    public String toString() {
      return "DataStoreBuckets memberId="
          + this.memberId
          + "; numBuckets="
          + this.numBuckets
          + "; numPrimaries="
            ...
            try {
              ...
              final PartitionedRegionRebalanceOp rebalance =
                  new PartitionedRegionRebalanceOp(
                      PRHARedundancyProvider.this.prRegion,
                      false,
                      director,
                      replaceOfflineData,
                      false);
              ...
              logger.error(
                  LocalizedMessage.create(
                      LocalizedStrings
                          .PRHARedundancyProvider_UNEXPECTED_EXCEPTION_DURING_BUCKET_RECOVERY),
                  e);
            }
          }
        };
  ...
  public boolean recoverPersistentBuckets() {
    ...
    if (!ColocationHelper.checkMembersColocation(
        leaderRegion, leaderRegion.getDistributionManager().getDistributionManagerId())) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Skipping persistent recovery of {} because colocation is not complete for {}",
            prRegion,
            leaderRegion);
      }
      return false;
    ...
    MembershipFlushRequest.send(
        peers, this.prRegion.getDistributionManager(), this.prRegion.getFullPath());
    ...
    try {
      if (proxyBucketArray.length > 0) {
        ...
        Thread loggingThread =
            new Thread(
                this.redundancyLogger, "RedundancyLogger for region " + this.prRegion.getName());
        loggingThread.start();
      }
    } catch (RuntimeException e) {
    ...
    for (final ProxyBucketRegion proxyBucket : proxyBucketArray) {
      if (proxyBucket.getPersistenceAdvisor().wasHosting()) {
        final RecoveryRunnable recoveryRunnable =
            new RecoveryRunnable(this) {
  ...
  public InternalPRInfo buildPartitionedRegionInfo(
      final boolean internal, final LoadProbe loadProbe) {
    ...
    if (!datastores.isEmpty()) {
      FetchPartitionDetailsResponse response =
          FetchPartitionDetailsMessage.send(
              datastores, pr, internal, fetchOfflineMembers, loadProbe);
    ...
    InternalPRInfo details =
        new PartitionRegionInfoImpl(
            pr.getFullPath(),
  ...
  public InternalPartitionDetails buildPartitionMemberDetails(
    ...
    for (Iterator<Map.Entry<Integer, Integer>> iter = bucketSizeMap.entrySet().iterator();
        iter.hasNext();
        ) {
  ...
  private class BucketMembershipObserver implements MembershipListener {
    ...
    public BucketMembershipObserver beginMonitoring() {
      int profilesPresent =
          this.bucketToMonitor
              .getBucketAdvisor()
              .addMembershipListenerAndAdviseGeneric(this)
              .size();
    ...
    public BucketMembershipObserverResults waitForOwnersGetPrimary(
        ...
        final Collection<InternalDistributedMember> expectedOwners,
        String partitionName)
        throws InterruptedException {
      ...
      synchronized (this) {
        for (; ; ) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Waiting for bucket {} to finish being created",
          ...
          if (oldArrivals == arrivals.get() && oldDepartures == departures.get()) {
            logger.warn(
                LocalizedMessage.create(
                    ...
                    new Object[] {
                      Integer.valueOf(creationWaitMillis), prRegion.getFullPath(), expectedOwners
                    }));
          }
        } // for (;;)
      } // synchronized
    ...
    public void memberDeparted(final InternalDistributedMember id, final boolean crashed) {
      try {
        ...
        if (!prRegion.isCacheClosing() && !prRegion.isDestroyed() && !dmem.equals(id)) {
          ...
          PartitionedRegionHelper.cleanUpMetaDataForRegion(
              prRegion.getCache(), prRegion.getRegionIdentifier(), id, postRecoveryTask);
        }
      } catch (CancelException e) {
        //ignore
      }
