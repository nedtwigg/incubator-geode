...
public class PRHARedundancyProvider {
  ...
  private static final boolean DISABLE_CREATE_BUCKET_RANDOMNESS = Boolean.getBoolean(DistributionConfig.GEMFIRE_PREFIX + "DISABLE_CREATE_BUCKET_RANDOMNESS");
  ...
  public static final String DATASTORE_DISCOVERY_TIMEOUT_PROPERTY_NAME = DistributionConfig.GEMFIRE_PREFIX + "partitionedRegionDatastoreDiscoveryTimeout";
  static volatile Long DATASTORE_DISCOVERY_TIMEOUT_MILLISECONDS = Long.getLong(DATASTORE_DISCOVERY_TIMEOUT_PROPERTY_NAME);
  ...
  public PRHARedundancyProvider(final PartitionedRegion region) {
    ...
    recoveryExecutor = new OneTaskOnlyExecutor(resourceManager.getExecutor(), new OneTaskOnlyExecutor.ConflatedTaskListener() {
  ...
  static public final StringId TIMEOUT_MSG = LocalizedStrings.PRHARedundancyProvider_IF_YOUR_SYSTEM_HAS_SUFFICIENT_SPACE_PERHAPS_IT_IS_UNDER_MEMBERSHIP_OR_REGION_CREATION_STRESS;
  ...
  public static void timedOut(PartitionedRegion prRegion, Set allStores, Collection alreadyUsed, String opString, long timeOut) {
    final String tooManyRetries = LocalizedStrings.PRHARedundancyProvider_TIMED_OUT_ATTEMPTING_TO_0_IN_THE_PARTITIONED_REGION__1_WAITED_FOR_2_MS.toLocalizedString(new Object[] { opString, regionStatus(prRegion, allStores, alreadyUsed, true), Long.valueOf(timeOut) }) + TIMEOUT_MSG;
   ...
   * available. 
   */
  static public final StringId INSUFFICIENT_STORES_MSG = LocalizedStrings.PRHARedundancyProvider_CONSIDER_STARTING_ANOTHER_MEMBER;

  /**
   * Signature string indicating that there are enough stores
   * available.
   */
  static public final StringId SUFFICIENT_STORES_MSG = LocalizedStrings.PRHARRedundancyProvider_FOUND_A_MEMBER_TO_HOST_A_BUCKET;
  ...
  public static final long INSUFFICIENT_LOGGING_THROTTLE_TIME = TimeUnit.SECONDS.toNanos(Integer.getInteger(DistributionConfig.GEMFIRE_PREFIX + "InsufficientLoggingThrottleTime", 2).intValue());
  ...
  public InternalDistributedMember createBucketOnDataStore(int bucketId, int size, long startTime, RetryTimeKeeper snoozer) {
   ...
   * 
   */
  public InternalDistributedMember createBucketAtomically(final int bucketId, final int newBucketSize, final long startTime, final boolean finishIncompleteCreation, String partitionName) throws PartitionedRegionStorageException, PartitionedRegionException {
    final boolean isDebugEnabled = logger.isDebugEnabled();
  ...
  private void endBucketCreation(int bucketId, Collection<InternalDistributedMember> acceptedMembers, InternalDistributedMember targetPrimary, String partitionName) {
    ...
    if (targetPrimary == null) {
      ...
      targetPrimary = getPreferredDataStore(acceptedMembers, Collections.<InternalDistributedMember> emptySet());
    }
   ...
   * 
   */
  private Set<InternalDistributedMember> getBuddyMembersInZone(final InternalDistributedMember acceptedMember, final Set<InternalDistributedMember> allStores) {
   ...
   */
  private boolean checkSufficientStores(final Set allStores, final boolean loggedInsufficentStores) {
    ...
    if (!loggedInsufficentStores) {
      ...
      if (allStores.size() > 0) {
        ...
        final StringId logStr = LocalizedStrings.PRHARRedundancyProvider_0_IN_THE_PARTITIONED_REGION_REGION_NAME_1;
  ...
  private InternalDistributedMember getColocatedDataStore(Collection<InternalDistributedMember> candidates, Collection<InternalDistributedMember> alreadyUsed, int bucketId, String prName) {
    ...
    PartitionRegionConfig config = (PartitionRegionConfig) prRoot.get(prRegion.getRegionIdentifier());
   ...
   */
  private InternalDistributedMember getPreferredDataStore(Collection<InternalDistributedMember> candidates, final Collection<InternalDistributedMember> alreadyUsed) {
  ...
  public void scheduleCreateMissingBuckets() {
    if (this.prRegion.getColocatedWith() != null && ColocationHelper.isColocationComplete(this.prRegion)) {
      ...
      final InternalResourceManager resourceManager = this.prRegion.getGemFireCache().getResourceManager();
   ...
   */
  public InternalPRInfo buildPartitionedRegionInfo(final boolean internal, final LoadProbe loadProbe) {
    ...
    if (!datastores.isEmpty()) {
      FetchPartitionDetailsResponse response = FetchPartitionDetailsMessage.send(datastores, pr, internal, fetchOfflineMembers, loadProbe);
    ...
    InternalPRInfo details = new PartitionRegionInfoImpl(pr.getFullPath(), configuredBucketCount, createdBucketCount, lowRedundancyBucketCount, configuredRedundantCopies, actualRedundantCopies, memberDetails, colocatedWithPath, offlineMembers);
   ...
   */
  public InternalPartitionDetails buildPartitionMemberDetails(final boolean internal, final LoadProbe loadProbe) {
    ...
    for (Iterator<Map.Entry<Integer, Integer>> iter = bucketSizeMap.entrySet().iterator(); iter.hasNext();) {
    ...
    if (internal) {
      ...
      localDetails = new PartitionMemberInfoImpl(localMember, pr.getLocalMaxMemory() * (1024L * 1024L), size, ds.getBucketsManaged(), ds.getNumberOfPrimaryBucketsManaged(), prLoad, bucketSizes);
    } else {
      localDetails = new PartitionMemberInfoImpl(localMember, pr.getLocalMaxMemory() * (1024L * 1024L), size, ds.getBucketsManaged(), ds.getNumberOfPrimaryBucketsManaged());
    }
    return localDetails;
  }
    ...
    if (recoveryLatch != null) {
      ...
      while (true) {
        try {
          ...
          boolean done = recoveryLatch.await(PartitionedRegionHelper.DEFAULT_WAIT_PER_RETRY_ITERATION, TimeUnit.MILLISECONDS);
          if (done) {
            break;
          }
        } catch (InterruptedException e) {
  ...
  public boolean isPersistentRecoveryComplete() {
    ...
    for (PartitionedRegion region : colocatedRegions.values()) {
      ...
      if (redundancyProvider.allBucketsRecoveredFromDisk != null && redundancyProvider.allBucketsRecoveredFromDisk.getCount() > 0) {
        return false;
      }
    }
  ...
  private class BucketMembershipObserver implements MembershipListener {
     ...
     */
    public BucketMembershipObserverResults waitForOwnersGetPrimary(final int expectedCount, final Collection<InternalDistributedMember> expectedOwners, String partitionName) throws InterruptedException {
      ...
      synchronized (this) {
        for (;;) {
          ...
          if (oldArrivals == arrivals.get() && oldDepartures == departures.get()) {
            logger.warn(LocalizedMessage.create(LocalizedStrings.PRHARedundancyProvider_TIME_OUT_WAITING_0_MS_FOR_CREATION_OF_BUCKET_FOR_PARTITIONED_REGION_1_MEMBERS_REQUESTED_TO_CREATE_THE_BUCKET_ARE_2, new Object[] { Integer.valueOf(creationWaitMillis), prRegion.getFullPath(), expectedOwners }));
          }
        } // for (;;)
      } // synchronized
      if (problematicDeparture) {
    ...
    public void memberDeparted(final InternalDistributedMember id, final boolean crashed) {
      try {
        ...
        if (!prRegion.isCacheClosing() && !prRegion.isDestroyed() && !dmem.equals(id)) {
          ...
          PartitionedRegionHelper.cleanUpMetaDataForRegion(prRegion.getCache(), prRegion.getRegionIdentifier(), id, postRecoveryTask);
        }
      } catch (CancelException e) {
        //ignore
      }
