...
class OverflowOplog implements CompactableOplog, Flushable {
  ...
  public final boolean copyForwardForOverflowCompact(
      DiskEntry entry, byte[] value, int length, byte userBits) {
    try {
      ...
      if (startPos != -1) {
        ...
        if (logger.isTraceEnabled(LogMarker.PERSIST_WRITES)) {
          logger.trace(
              ...
              "writeOpLogBytes startPos={} oplog#{}",
              startPos,
              getOplogId());
        }
  ...
  private BytesAndBits attemptGet(
      DiskRegionView dr, long offsetInOplog, int valueLength, byte userBits) throws IOException {
    synchronized (this.crf) {
  ...
  private BytesAndBits attemptWriteBufferGet(
      long writePosition, long readPosition, int valueLength, byte userBits) {
    BytesAndBits bb = null;
    ...
    if (entry.isValueNull()) {
      ...
      foundData =
          basicGetForCompactor(
              oplogOffset, false, did.getValueLength(), did.getUserBits(), wrapper);
            ...
            LocalizedStrings.Oplog_FAILED_READING_FROM_0_OPLOG_DETAILS_1_2_3_4_5_6
                .toLocalizedString(
                    new Object[] {
                      ...
                      Long.valueOf(this.crf.currSize),
                      Long.valueOf(this.crf.bytesFlushed),
                      Boolean.valueOf(/*!dr.isSync() @todo */ false),
                      Boolean.valueOf(false)
                    }),
            ex,
            getParent().getName());
