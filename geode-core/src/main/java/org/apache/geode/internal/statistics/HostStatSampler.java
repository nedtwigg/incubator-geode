...
public abstract class HostStatSampler
  ...
  public final void start() {
    synchronized (HostStatSampler.class) {
      ...
      this.callbackSampler.start(
          getStatisticsManager(), group, getSampleRate(), TimeUnit.MILLISECONDS);
  ...
  /**
   * Waits for the special statistics to be initialized. For tests, please use {@link
   * #waitForInitialization(long)} instead.
   *
  ...
  /**
   * Waits for the special statistics to be initialized. This overridden version of {@link
   * #waitForInitialization()} should always be used within tests.
   *
   * @see #initSpecialStats
   * @since GemFire 7.0
   */
