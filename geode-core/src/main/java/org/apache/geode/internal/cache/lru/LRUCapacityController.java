 ...
 *
 * <p>This is not supported as the capacity controller of a region with mirroring enabled.
 *
 * <p>LRUCapacityController must be set in the RegionAttributes before the region is created. A
 * Region with LRUCapacityController set will throw an IllegalStateException if an attempt is made
 * to replace the Region's capacity controller. While the capacity controller cannot be replaced, it
 ...
 * can include the following to associate a <code>LRUCapacityController</code> with a region:
 *
 * <pre>
 *  &lt;region-attributes&gt;
 *    &lt;capacity-controller&gt;
...
public final class LRUCapacityController extends LRUAlgorithm implements Declarable {
   ...
   * maximum number of entries and the {@link
   * org.apache.geode.cache.EvictionAction#DEFAULT_EVICTION_ACTION default} eviction action.
   *
   ...
   *     <p>For a region with {@link DataPolicy#PARTITION}, the maximum number of entries allowed in
   ...
   *     copies for this VM, this controller will remove the least recently used entry from the
   *     bucket in which the subsequent <code>put</code> takes place.
   * @param evictionAction The action to perform upon the least recently used entry. See {@link
   *     #EVICTION_ACTION}.
   */
  ...
  protected EnableLRU createLRUHelper() {
    return new AbstractEnableLRU() {
      ...
      public int entrySize(Object key, Object value) throws IllegalArgumentException {
        ...
        if (Token.isRemoved(
            value) /*&& (value != Token.TOMBSTONE)*/) { // un-comment to make tombstones visible
