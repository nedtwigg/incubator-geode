...
public class InternalResourceManager implements ResourceManager {
  ...
  private static String PR_LOAD_PROBE_CLASS =
      System.getProperty(
          DistributionConfig.GEMFIRE_PREFIX + "ResourceManager.PR_LOAD_PROBE_CLASS",
  ...
  private InternalResourceManager(GemFireCacheImpl cache) {
    ...
    ThreadFactory eventProcessorFactory =
        new ThreadFactory() {
          @Override
          public Thread newThread(Runnable r) {
    ...
    for (ResourceType resourceType :
        new ResourceType[] {ResourceType.HEAP_MEMORY, ResourceType.OFFHEAP_MEMORY}) {
  ...
  public void addResourceListener(
      final ResourceType resourceType, final ResourceListener listener) {
  ...
  void stopExecutor(ExecutorService executor) {
    ...
    executor.shutdown();
    final int secToWait =
        Integer.getInteger(DistributionConfig.GEMFIRE_PREFIX + "prrecovery-close-timeout", 120)
            .intValue();
    try {
  ...
  public static interface ResourceObserver {
    ...
    public void movingBucket(
        Region region, int bucketId, DistributedMember source, DistributedMember target);
