...
public class SnapshotOptionsImpl<K, V> implements SnapshotOptions<K, V> {
   ...
   * parallel export.
   *
   * <p>Parallelizing snapshot operations may yield significant performance improvements for large
   * data sets. This is particularly true when each member is reading from or writing to separate
   * physical disks.
   *
   * <p>This flag is ignored for replicated regions.
   *
   * <p>If the mapper is not set explicitly, a default mapping implementation is used that assumes
   * each member can access a non-shared disk volume. The default mapper provides the following
   * behavior:
   *
   * <dl>
   *   <dt>export
   *   <dd>use the supplied path
   *   <dt>import
   *   <dd>if the supplied path is a file, use that file
   *   <dd>if the supplied path is a directory, use all files in the directory
   * </dl>
   *
