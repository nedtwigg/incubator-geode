...
public class GatewaySenderAdvisor extends DistributionAdvisor {
  ...
  private void checkCompatibility(GatewaySenderProfile sp) {
    ...
    if (sp.isBatchConflationEnabled != sender.isBatchConflationEnabled()) {
      throw new IllegalStateException(
          LocalizedStrings
              ...
              .toString(
                  new Object[] {
                    sp.Id, sp.isBatchConflationEnabled, sender.isBatchConflationEnabled()
                  }));
    }
    ...
    if (sp.senderEventListenerClassNames.size() != senderEventListenerClassNames.size()) {
      throw new IllegalStateException(
          LocalizedStrings.GatewaySenderAdvisor_GATEWAY_SENDER_LISTENER_MISMATCH.toString(
              new Object[] {
                sp.Id, sp.senderEventListenerClassNames, senderEventListenerClassNames
              }));
    } else {
  ...
  public boolean volunteerForPrimary() {
    ...
    if (advisePrimaryGatewaySender() == null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Sender : {} no Primary available. So going to acquire distributed lock", this.sender);
      }
  ...
  public void makePrimary() {
    logger.info(
        LocalizedMessage.create(
            LocalizedStrings.SerialGatewaySenderImpl_0__STARTING_AS_PRIMARY, this.sender));
    ...
    synchronized (this.primaryLock) {
      ...
      logger.info(
          LocalizedMessage.create(
              LocalizedStrings.SerialGatewaySenderImpl_0__BECOMING_PRIMARY_GATEWAYSENDER,
              this.sender));
  ...
  public void makeSecondary() {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "{}: Did not obtain the lock on {}. Starting as secondary gateway sender.",
          this.sender,
          this.lockToken);
    }
            ...
            new Runnable() {
              ...
              public void run() {
                ...
                try {
                  ...
                  if (logger.isDebugEnabled()) {
                    logger.debug(
                        "{}: Obtaining the lock on {}", this, GatewaySenderAdvisor.this.lockToken);
                  }

                  if (volunteerForPrimary()) {
                    if (logger.isDebugEnabled()) {
                      logger.debug(
                          "{}: Obtained the lock on {}", this, GatewaySenderAdvisor.this.lockToken);
                    }
                    logger.info(
                        LocalizedMessage.create(
  ...
  public static final class GatewaySenderProfile extends DistributionAdvisor.Profile {
    ...
    public void toData(DataOutput out) throws IOException {
      ...
      if (InternalDataSerializer.getVersionForDataStream(out).compareTo(Version.CURRENT) < 0
        ...
        } else if (orderPolicyName.equals(
            org.apache.geode.cache.util.Gateway.OrderPolicy.THREAD.name())) {
    ...
    public void toDataPre_GFE_8_0_0_0(DataOutput out) throws IOException {
      ...
      out.writeBoolean(isDiskSynchronous);
      //out.writeInt(dispatcherThreads);
      if (isParallel) out.writeInt(1); //it was 1 on previous version of gemfire
      else if (orderPolicy == null) out.writeInt(1); //it was 1 on previous version of gemfire
      else out.writeInt(dispatcherThreads);

      if (isParallel) DataSerializer.writeObject(null, out);
      else DataSerializer.writeObject(orderPolicy, out);
