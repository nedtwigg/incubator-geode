...
public class GatewaySenderAdvisor extends DistributionAdvisor {
  ...
  private boolean adviseEldestGatewaySender() {
    ...
    return (senderStartTimes.isEmpty()) || (this.sender.isRunning() && (this.sender.startTime <= senderStartTimes.first()));
  }
      ...
      public void run() {
        ...
        try {
          ...
          if (volunteerForPrimary()) {
            ...
            logger.info(LocalizedMessage.create(LocalizedStrings.GatewaySender_0_IS_BECOMING_PRIMARY_GATEWAY_Sender, GatewaySenderAdvisor.this));
          ...
          if (!sender.getStopper().isCancelInProgress()) {
            logger.fatal(LocalizedMessage.create(LocalizedStrings.GatewaySenderAdvisor_0_THE_THREAD_TO_OBTAIN_THE_FAILOVER_LOCK_WAS_INTERRUPTED__THIS_GATEWAY_SENDER_WILL_NEVER_BECOME_THE_PRIMARY, GatewaySenderAdvisor.this), e);
          }
        } finally {
  ...
  public static final class GatewaySenderProfile extends DistributionAdvisor.Profile {
    ...
    public void fromData(DataInput in) throws IOException, ClassNotFoundException {
      ...
      if (InternalDataSerializer.getVersionForDataStream(in).compareTo(Version.CURRENT) < 0) {
        org.apache.geode.cache.util.Gateway.OrderPolicy oldOrderPolicy = DataSerializer.readObject(in);
    ...
    public void toData(DataOutput out) throws IOException {
      ...
      if (InternalDataSerializer.getVersionForDataStream(out).compareTo(Version.CURRENT) < 0 && this.orderPolicy != null) {
        ...
        if (orderPolicyName.equals(org.apache.geode.cache.util.Gateway.OrderPolicy.KEY.name())) {
          ...
          DataSerializer.writeObject(org.apache.geode.cache.util.Gateway.OrderPolicy.PARTITION, out);
        }
      } else {
    ...
    public void processIncoming(DistributionManager dm, String adviseePath, boolean removeProfile, boolean exchangeProfiles, final List<Profile> replyProfiles) {
      ...
      if (cache != null) {
        AbstractGatewaySender sender = (AbstractGatewaySender) ((GemFireCacheImpl) cache).getGatewaySender(adviseePath);
