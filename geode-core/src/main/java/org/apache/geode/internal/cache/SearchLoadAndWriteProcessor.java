...
public class SearchLoadAndWriteProcessor implements MembershipListener {
  ...
  private void searchAndLoad(EntryEventImpl event, TXStateInterface txState, Object localValue)
    ...
    if (txState != null) {
      TXEntryState tx =
          txState.txReadEntry(event.getKeyInfo(), region, false, true /*create txEntry is absent*/);
      if (tx != null) {
    ...
    if (advisor != null) {
      ...
      try {
        do { // the only time this loop repeats is when continue is called
          ...
          NetLoadRequestMessage.sendMessage(
              this,
              this.regionName,
              this.key,
  ...
  private Object doLocalLoad(CacheLoader loader, boolean netSearchAllowed)
    ...
    if (loader != null && !this.attemptedLocalLoad) {
      ...
      LoaderHelper loaderHelper =
          this.region.loaderHelperFactory.createLoaderHelper(
              this.key, this.aCallbackArgument, netSearchAllowed, true /*netLoadAllowed*/, this);
  ...
  private boolean netWrite(CacheEvent event, int action, Set writeCandidateSet)
    ...
    do {
      ...
      this.requestInProgress = true;
      this.remoteException = null;
      NetWriteRequestMessage.sendMessage(
          this, this.regionName, this.remainingTimeout, event, set, action);
   ...
   */
  protected synchronized void incomingResponse(
      Object obj,
    ...
    if (this.remoteLoadInProgress) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Ignoring netsearch response from {} because we're now doing a netload", sender);
      }
      return;
    }
  ...
  protected synchronized void sendValueRequest(final InternalDistributedMember sender) {
    ...
    RegionAttributes attrs = this.region.getAttributes();
    NetSearchRequestMessage.sendMessage(
        this,
        this.regionName,
        this.key,
        sender,
   ...
   */
  protected void incomingNetLoadReply(
      Object obj,
      long lastModifiedTime,
      Object callbackArg,
      Exception e,
  ...
  protected synchronized void incomingNetSearchReply(
      byte[] value,
      long lastModifiedTime,
      boolean serialized,
      boolean requestorTimedOut,
  ...
  private int getSearchTimeout() {
    return region
        .getCache()
        .getSearchTimeout(); //CacheFactory.getInstance(((DistributedRegion)this.region).getSystem()).getSearchTimeout();
  }
  ...
  public void testNetSearchMessageDoGet(
      String theRegionName, Object theKey, int theTimeoutMs, int theTtl, int theIdleTime) {
  ...
  public static final class QueryMessage extends SerialDistributionMessage {
    ...
    @Override
    public String toString() {
      return "SearchLoadAndWriteProcessor.QueryMessage for \""
          + this.key
          + "\" in region \""
          + this.regionName
          + "\", processorId "
              ...
              synchronized (entry) {
                assert region.isInitialized();
                {
                  if (dm.cacheTimeMillis() - startTime < timeoutMs) {
                    o =
                        region.getNoLRU(
                            this.key, false, true, true); // OFFHEAP: incrc, copy bytes, decrc
                    if (o != null
                        && !Token.isInvalid(o)
                        && !Token.isRemoved(o)
    ...
    private void replyWithNull(DistributionManager dm) {
      ResponseMessage.sendMessage(
          this.key, this.getSender(), processorId, null, 0, false, false, false, dm, null);
    }
  }
  ...
  public static final class ResponseMessage extends HighPriorityDistributionMessage {
    ...
    protected void process(DistributionManager dm) {
      ...
      processor.incomingResponse(
          this.result,
          lastModifiedSystemTime,
          this.isPresent,
          this.isSerialized,
  ...
  public static final class NetSearchRequestMessage extends PooledDistributionMessage {
    ...
    @Override
    public String toString() {
      return "SearchLoadAndWriteProcessor.NetSearchRequestMessage for \""
          + this.key
          + "\" in region \""
          + this.regionName
          + "\", processorId "
    ...
    private void doGet(DistributionManager dm) {
      ...
      try {
        ...
        NetSearchReplyMessage.sendMessage(
            NetSearchRequestMessage.this.getSender(),
            processorId,
            this.key,
            ebv,
            ebvObj,
        ...
        logger.warn(
            LocalizedMessage.create(
                LocalizedStrings.SearchLoadAndWriteProcessor_UNEXPECTED_EXCEPTION),
            t);
        replyWithNull(dm);
      } finally {
        LocalRegion.setThreadInitLevelRequirement(oldLevel);
  ...
  public static final class NetSearchReplyMessage extends HighPriorityDistributionMessage {
    ...
    protected void process(DistributionManager dm) {
      ...
      if (processor == null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "NetSearchReplyMessage() SearchLoadAndWriteProcessor {} no longer exists",
              processorId);
        }
        return;
      }
      ...
      processor.incomingNetSearchReply(
          this.value,
          lastModifiedSystemTime,
          this.isSerialized,
          this.requestorTimedOut,
  ...
  public static final class NetLoadRequestMessage extends PooledDistributionMessage {
    ...
    @Override
    public String toString() {
      return "SearchLoadAndWriteProcessor.NetLoadRequestMessage for \""
          + this.key
          + "\" in region \""
          + this.regionName
          + "\", processorId "
    ...
    private void doLoad(DistributionManager dm) {
      ...
      try {
        ...
        if (region != null
          ...
          if (loader != null) {
            LoaderHelper loaderHelper =
                region.loaderHelperFactory.createLoaderHelper(
                    this.key, this.aCallbackArgument, false, false, null);
            CachePerfStats stats = region.getCachePerfStats();
            long start = stats.startLoad();
            try {
              Object o = loader.load(loaderHelper);
              Assert.assertTrue(o != Token.INVALID && o != Token.LOCAL_INVALID);
              NetLoadReplyMessage.sendMessage(
                  NetLoadRequestMessage.this.getSender(),
                  processorId,
                  o,
                  dm,
  ...
  public static final class NetLoadReplyMessage extends HighPriorityDistributionMessage {
    ...
    protected void process(DistributionManager dm) {
      ...
      processor.incomingNetLoadReply(
          this.result,
          0,
          this.aCallbackArgument,
          this.e,
  ...
  public static final class NetWriteRequestMessage extends PooledDistributionMessage {
    ...
    protected void process(DistributionManager dm) {
      ...
      try {
        ...
        if (region != null
          ...
          if (writer != null) {
            try {
              ...
              NetWriteReplyMessage.sendMessage(
                  NetWriteRequestMessage.this.getSender(), processorId, dm, true, null, false);
            } catch (CacheWriterException cwe) {
              NetWriteReplyMessage.sendMessage(
                  NetWriteRequestMessage.this.getSender(), processorId, dm, false, cwe, true);
            } catch (Exception e) {
              NetWriteReplyMessage.sendMessage(
                  NetWriteRequestMessage.this.getSender(), processorId, dm, false, e, false);
            }

          } else {
            NetWriteReplyMessage.sendMessage(
                NetWriteRequestMessage.this.getSender(),
                processorId,
                dm,
                false,
                new TryAgainException(
        ...
        NetWriteReplyMessage.sendMessage(
            NetWriteRequestMessage.this.getSender(), processorId, dm, false, null, false);

      } catch (DistributedSystemDisconnectedException e) {
        // shutdown condition
        throw e;
      } catch (CancelException cce) {
        dm.getCancelCriterion()
            .checkCancelInProgress(cce); // TODO anyway to find the region or cache here?
        NetWriteReplyMessage.sendMessage(
            NetWriteRequestMessage.this.getSender(), processorId, dm, false, null, false);
      } catch (VirtualMachineError err) {
        SystemFailure.initiateFailure(err);
  ...
  public static final class NetWriteReplyMessage extends HighPriorityDistributionMessage {
    ...
    private void initialize(
        int procId, boolean netwriteSucceeded, Exception except, boolean cacheWriterExcept) {
      this.processorId = procId;
