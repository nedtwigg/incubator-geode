...
/**
 * A timer class that reports current or elapsed time in nanonseconds. The static method {@link
 * #getTime} reports the current time. The instance methods support basic stop-watch-style functions
 * that are convenient for simple performance measurements. For example:
 *
 * <pre>
 * class Example {
 * void example() {
...
public final class NanoTimer {
  ...
  private static final TimeService systemTimeService =
      new TimeService() {
        @Override
        public long getTime() {
   ...
   *
   * <p>The time rolls over to zero every 2^64 nanosecs (approx 584 years). Interval computations
