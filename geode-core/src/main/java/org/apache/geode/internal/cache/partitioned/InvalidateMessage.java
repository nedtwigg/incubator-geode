...
public final class InvalidateMessage extends DestroyMessage {
  ...
  public static InvalidateResponse send(
      DistributedMember recipient, PartitionedRegion r, EntryEventImpl event)
    ...
    try {
      ...
      if (!notificationOnly) {
        ...
        try {
          ...
          r.getDataView()
              .invalidateOnRemote(event, true /*invokeCallbacks*/, false /*forceNewEntry*/);
          this.versionTag = event.getVersionTag();
          if (logger.isTraceEnabled(LogMarker.DM)) {
            logger.trace(
                LogMarker.DM,
                "{} invalidateLocally in bucket: {}, key: {}",
                getClass().getName(),
                bucket,
                key);
          }
