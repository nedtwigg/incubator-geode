...
public class ConcurrentSerialGatewaySenderEventProcessor extends AbstractGatewaySenderEventProcessor {
  ...
  public void enqueueEvent(EnumListenerEvent operation, EntryEvent event, Object substituteValue, int index) throws CacheException, IOException {
    ...
    if (sender.getOrderPolicy() == OrderPolicy.KEY || sender.getOrderPolicy() == OrderPolicy.PARTITION) {
      ...
      try {
        ...
        // any.
        long newThreadId = ThreadIdentifier.createFakeThreadIDForParallelGateway(index, originalEventId.getThreadID(), 0 /*gateway sender event id index has already been applied in SerialGatewaySenderImpl.setModifiedEventId*/);
  ...
  private int getHashCode(EntryEventImpl event) {
    ...
    case THREAD:
      ...
      break;
    case PARTITION:
      eventHashCode = PartitionRegionHelper.isPartitionedRegion(event.getRegion()) ? PartitionedRegionHelper.getHashKey(event)
