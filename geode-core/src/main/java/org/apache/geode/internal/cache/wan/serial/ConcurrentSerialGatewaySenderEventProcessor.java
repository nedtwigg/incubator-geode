...
public class ConcurrentSerialGatewaySenderEventProcessor
  ...
  public void enqueueEvent(
      EnumListenerEvent operation, EntryEvent event, Object substituteValue, int index)
    ...
    if (sender.getOrderPolicy() == OrderPolicy.KEY
      ...
      try {
        ...
        // any.
        long newThreadId =
            ThreadIdentifier.createFakeThreadIDForParallelGateway(
                index,
                originalEventId.getThreadID(),
                0 /*gateway sender event id index has already been applied in SerialGatewaySenderImpl.setModifiedEventId*/);
        EventID newEventId =
            new EventID(
  ...
  private int getHashCode(EntryEventImpl event) {
    ...
    switch (this.sender.getOrderPolicy()) {
      ...
      case THREAD:
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "{}: Generated hashcode for event with key={}, memberId={}, threadId={}: {}",
              this,
              event.getKey(),
      ...
      case PARTITION:
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "{}: Generated partition hashcode for event with key={}: {}",
              this,
              event.getKey(),
              eventHashCode);
        }
  ...
  public void stopProcessing() {
    ...
    final LoggingThreadGroup loggingThreadGroup =
        LoggingThreadGroup.createThreadGroup(
            "ConcurrentSerialGatewaySenderEventProcessor Logger Group", logger);
