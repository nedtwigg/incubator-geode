 ...
 *
 * <p>It generates either a restore.sh for unix or a restore.bat for windows.
 */
public class RestoreScript {
  ...
  private void generateScript(File outputDir, File outputFile, ScriptGenerator osGenerator)
    ...
    try {
      ...
      writer.println();
      osGenerator.writeComment(
          writer, "Restore a backup of gemfire persistent data to the location it was backed up");
      osGenerator.writeComment(writer, "from.");
      osGenerator.writeComment(
          writer, "This script will refuse to restore if the original data still exists.");
      writer.println();
      osGenerator.writeComment(
          writer, "This script was automatically generated by the gemfire backup utility.");
      writer.println();
      osGenerator.writeComment(
          writer, "Test for existing originals. If they exist, do not restore the backup.");
      ...
      if (!this.baselineFiles.isEmpty()) {
        writer.println();
        osGenerator.writeComment(
            writer, "Incremental backup.  Restore baseline originals from previous backups.");
  ...
  private static interface ScriptGenerator {
    ...
    void writeCopyDirectoryContents(
        PrintWriter writer, File backup, File original, boolean backupHasFiles);
  ...
  private static class WindowsScriptGenerator implements ScriptGenerator {
    ...
    public void writeCopyDirectoryContents(
        PrintWriter writer, File backup, File original, boolean backupHasFiles) {
    ...
    public void writeExistenceTest(PrintWriter writer, File file) {
      writer.println(
          "IF EXIST \""
              + file
              + "\" echo \"Backup not restored. Refusing to overwrite "
              + file
              + "\" && exit /B 1 ");
    }
