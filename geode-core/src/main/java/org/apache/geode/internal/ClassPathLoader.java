 ...
 * <tt>ClassLoader</tt>.
 *
 * <p>The thread context class loaders can be excluded by setting the system property
 ...
 * </ul>
 *
 * <p>Class loading and resource loading order:
 *
 * <ul>
 ...
 *       class loaders results in either a {@link java.lang.SecurityException SecurityException} or
 *       a null, then that class loader is quietly skipped. Duplicate class loaders will be skipped.
 *
...
public final class ClassPathLoader {
  ...
  private static final ClassLoader TCCL_PLACEHOLDER =
      new ClassLoader() { // This is never used for class loading
      };
      ...
      if (file.exists()) {
        if (file.isDirectory()) {
          ...
          if (!JarClassLoader.hasValidJarContent(file)) {
            logger.warn(
                "Invalid JAR content when attempting to create ClassLoader for file: {}",
    ...
    try {
      ...
      if (EXT_LIB_DIR.exists()) {
        if (!EXT_LIB_DIR.isDirectory() || !EXT_LIB_DIR.canRead()) {
          logger.warn(
              "Cannot read from directory when attempting to load JAR files: {}",
   ...
   * context class for the resource. That failing, this method will invoke {@link
   * #getResource(String)} to find the resource.
   *
   ...
   *
   * <p>The search order is described in the documentation for {@link #getResource(String)}.
   *
   * @param name The resource name
   ...
   * Returns an input stream for reading the specified resource.
   *
   * <p>The search order is described in the documentation for {@link #getResource(Class, String)}.
   *
   * @param contextClass The class whose class loader will first be searched
   * @param name The resource name
   * @return An input stream for reading the resource, or <tt>null</tt> if the resource could not be
