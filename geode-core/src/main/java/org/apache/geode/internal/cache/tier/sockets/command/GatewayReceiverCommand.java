...
public class GatewayReceiverCommand extends BaseCommand {
  ...
  private void handleRegionNull(ServerConnection servConn, String regionName, int batchId) {
    ...
    if (gfc != null && gfc.isCacheAtShutdownAll()) {
      ...
      String reason =
          LocalizedStrings.ProcessBatch_WAS_NOT_FOUND_DURING_BATCH_CREATE_REQUEST_0
      ...
      try {
        ...
        try {
          ...
          logger.warn(
              LocalizedMessage.create(
                  ...
                  new Object[] {
                    servConn.getName(), Integer.valueOf(batchId), Integer.valueOf(numberOfEvents)
                  }),
              e);
          throw e;
        }
        ...
        switch (actionType) {
                ...
                if (result || clientEvent.isConcurrencyConflict()) {
                  ...
                  throw new Exception(
                      LocalizedStrings
                          ...
                          .toLocalizedString(
                              new Object[] {
                                servConn.getName(), regionName, key, valuePart, callbackArg
                              }));
                }
              } catch (Exception e) {
                logger.warn(
          ...
          case 1: // Update
            ...
            if (key == null || regionName == null) {
              ...
              if (key == null) {
                message =
                    LocalizedStrings
                        .ProcessBatch_0_THE_INPUT_KEY_FOR_THE_BATCH_UPDATE_REQUEST_1_IS_NULL;
              }
              if (regionName == null) {
                message =
                    LocalizedStrings
          ...
          case 2: // Destroy
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                  "{}: Processing batch destroy request {} on {} for region {} key {}",
                  servConn.getName(),
                  batchId,
                  servConn.getSocketString(),
                  regionName,
            ...
            if (region == null) {
              ...
              try {
                ...
                region.basicBridgeDestroy(
                    key, callbackArg, servConn.getProxyID(), false, clientEvent);
                servConn.setModificationInfo(true, regionName, key);
            ...
            if (key == null || regionName == null) {
              ...
              if (region == null) {
                ...
                try {
                  ...
                  region.basicBridgeUpdateVersionStamp(
                      key, callbackArg, servConn.getProxyID(), false, clientEvent);
          ...
          default:
            logger.fatal(
                LocalizedMessage.create(
                    ...
                    new Object[] {
                      servConn.getName(), Integer.valueOf(actionType), servConn.getSocketString()
                    }));
        ...
        if (e.getCause() instanceof PdxRegistryMismatchException) {
          ...
          logger.fatal(
              LocalizedMessage.create(
                  LocalizedStrings.GatewayReceiver_PDX_CONFIGURATION,
                  new Object[] {servConn.getMembershipID()}),
              e.getCause());
          break;
        }
  ...
  private static void writeBatchException(
      Message origMsg, List<BatchException70> exceptions, ServerConnection servConn, int batchId)
      throws IOException {
    ...
    for (Exception be : exceptions) {
      if (logger.isWarnEnabled()) {
        logger.warn(
            LocalizedMessage.create(
                LocalizedStrings.ProcessBatch_0_WROTE_BATCH_EXCEPTION, servConn.getName()),
            be);
      }
    }
  }

  private static void writeFatalException(
      Message origMsg, Throwable exception, ServerConnection servConn, int batchId)
      throws IOException {
    Message errorMsg = servConn.getErrorResponseMessage();
    errorMsg.setMessageType(MessageType.EXCEPTION);
    errorMsg.setNumberOfParts(2);
