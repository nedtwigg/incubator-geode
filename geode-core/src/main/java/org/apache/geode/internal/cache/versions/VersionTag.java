 ...
 *
 * <p>Note that on the receiving end the membership IDs in a version tag will not be references to
 * canonical IDs and should be made so before storing them for any length of time.
 *
 * <p>This class implements java.io.Serializable for dunit testing. It should not otherwise be
...
public abstract class VersionTag<T extends VersionSource>
  ...
  public boolean hasValidVersion() {
    return !(this.entryVersion == 0
        && this.regionVersionHighBytes == 0
  ...
  public String toString() {
    ...
    if (isGatewayTag()) {
      s.append("{ds=")
          .append(this.distributedSystemId)
          .append("; time=")
          .append(getVersionTimeStamp())
          .append("}");
    } else {
  ...
  public boolean equals(Object obj) {
    ...
    if (regionVersionHighBytes != other.regionVersionHighBytes) return false;
    if (regionVersionLowBytes != other.regionVersionLowBytes) return false;
