 ...
 *       that has not had retain called on it. The caller should call retain to ensure that the
 *       reference remains viable.
 *   <li>Use this annotation on a parameter to indicate that the parameter may be an off-heap value
 *       but it is neither retained nor released by the callee (or further sub-callees).
 *   <li>Use this annotation on a local variable to indicate that it references an off-heap value
 *       but it is neither retained nor released.
 *   <li>Use this annotation on a field member to indicate that the field is an off-heap reference
 *       that is neither retained or released by containing object.
 *   <li>Use this annotation on a constructor that does not retain off-heap field members.
 * </ul>
 *
