...
public interface EntriesMap extends ConcurrentMap {
   ...
   *
   * <p>SYNCHRONIZATION Rules: Any method that calls a method on an Entry that modifies fields in
   * the entry must be protected by the writeSynchronized method BEFORE the entry is obtained to
   * prevent it from being cloned in the map while you have a reference.
   */
  interface Entry {
