 ...
 *
 * <p>Code which wishes to observe events during Region clear should do so using the following
 * technique:
 *
 * <p>class MyCacheObserver extends CacheObserverAdapter { // ... override methods of interest ... }
 *
 * <p>CacheObserver old = CacheObserverHolder.setInstance(new MyCacheObserver());
 * org.apache.geode.internal.cache.LocalRegion.ISSUE_CALLBACKS_TO_CACHE_OBSERVER=true;
 *
 * <p>try { Call region methods here } finally { // reset to the original CacheObserver.
 * CacheObserverHolder.setInstance(old); }
 *
 * <p>The Region code will call methods on this static member using the following technique:
 *
 * <p>CacheObserver observer = CacheObserverHolder.getInstance(); try {
