...
/**
 * DistPeerTXStateStub lives on the transaction coordinator for a distributed transaction </br>1. It
 * forwards TX operations to primary or a selected replica (in case of RR) for each op </br>2.It
 * also records those transactional operations in order to send those to secondaries/replicas(in one
...
public final class DistPeerTXStateStub extends PeerTXStateStub
  ...
  public void precommit() throws CommitConflictException {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "DistPeerTXStateStub.precommit target="
              + target
  ...
  private final void addPrimaryTransactionalOperations(DistTxEntryEvent dtop) {
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
          "DistPeerTXStateStub.addPrimaryTransactionalOperations add "
              + dtop
              + " ,stub before="
              + this);
    }
    ...
    if (logger.isDebugEnabled()) {
      // [DISTTX] TODO Remove these
      logger.debug(
          "DistPeerTXStateStub.addPrimaryTransactionalOperations stub after add = " + this);
    }
  }
  ...
  public boolean putEntryOnRemote(
    ...
    if (logger.isDebugEnabled()) {
      // [DISTTX] TODO Remove throwable
      logger.debug(
          "DistPeerTXStateStub.putEntryOnRemote " + event.getKeyInfo().getKey(), new Throwable());
    }
    boolean returnValue =
        super.putEntryOnRemote(
            event,
            ifNew,
            ifOld,
            expectedOldValue,
  ...
  public void postPutAll(
    ...
    EntryEventImpl event =
        EntryEventImpl.createPutAllEvent(
            ...
            putallOp.getBaseEvent().getKey(),
            putallOp.getBaseEvent().getValue());
  ...
  public void postRemoveAll(
    ...
    // TODO DISTTX: event is never released
    EntryEventImpl event =
        EntryEventImpl.createRemoveAllEvent(
            removeAllOp, region, removeAllOp.getBaseEvent().getKey());
  ...
  public void gatherAffectedRegionsName(
      ...
      throws UnsupportedOperationInTransactionException {
    if (includePrimaryRegions) {
      DistTXStateOnCoordinator.gatherAffectedRegions(
          sortedRegionName, this.primaryTransactionalOperations);
    }
    if (includeRedundantRegions) {
      DistTXStateOnCoordinator.gatherAffectedRegions(
          sortedRegionName, this.secondaryTransactionalOperations);
    }
  }
