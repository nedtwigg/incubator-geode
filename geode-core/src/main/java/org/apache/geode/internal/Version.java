 ...
 *
 * <p>There are dependencies in versioning code that require newer versions to have ordinals higher
...
public final class Version implements Comparable<Version> {
  ...
  public static final Version TEST_VERSION =
      new Version(
          "TEST", "VERSION", (byte) 0, (byte) 0, (byte) 0, (byte) 0, validOrdinalForTesting);

  /** Creates a new instance of <code>Version</code> */
  private Version(
      String product, String name, byte major, byte minor, byte release, byte patch, byte ordinal) {
  ...
  public static Version fromOrdinal(short ordinal, boolean forGFEClients)
    ...
    if ((VALUES.length < ordinal + 1)
      ...
      throw new UnsupportedVersionException(
          LocalizedStrings.Version_REMOTE_VERSION_NOT_SUPPORTED.toLocalizedString(
   ...
   *     mode is useful for interoperatibility with previous versions while latter to use fixed size
   *     for writing version; typically former will be used for P2P/client-server communications
   *     while latter for persisting to disk; we use the token to ensure that {@link
   *     #readOrdinal(DataInput)} can deal with both compressed/uncompressed cases seemlessly
   */
  ...
  /**
   * Write this {@link Version}'s ordinal (result of {@link #ordinal()}) to given {@link
   * DataOutput}. This keeps the serialization of ordinal compatible with previous versions writing
   * a single byte to DataOutput when possible, and a token with 2 bytes if it is large.
   *
   * @param out the {@link DataOutput} to write the ordinal write to
   * @param compressed if true, then use single byte for ordinal < 128, and three bytes for beyond
   *     that, else always use three bytes where the first byte is {@link #TOKEN_ORDINAL}; former
  ...
  /**
   * Return the <code>Version</code> reading from given {@link DataInput} as serialized by {@link
   * #writeOrdinal(DataOutput, boolean)}.
   *
   * <p>If the incoming ordinal is greater than or equal to current ordinal then this will return
   * null or {@link #CURRENT} indicating that version is same as that of {@link #CURRENT} assuming
   * that peer will support this JVM.
   *
   * <p>This method is not meant to be used for client-server protocol since servers cannot support
