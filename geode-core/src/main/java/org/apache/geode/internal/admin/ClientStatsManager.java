...
public class ClientStatsManager {
  ...
  public static synchronized void publishClientStats(PoolImpl pool) {
    ...
    if (logger.fineEnabled()) logger.fine("Entering ClientStatsManager#publishClientStats...");
    ...
    if (logger.fineEnabled()) logger.fine("Exiting ClientStatsManager#publishClientStats.");
  }
  ...
  private static boolean initializeStatistics(GemFireCacheImpl currentCache) {
    ...
    if (cachePerfStats == null && restart) {
      logger.warning(
          LocalizedStrings.ClientStatsManager_CLIENTSTATSMANAGER_0_ARE_NOT_AVAILABLE,
          "CachePerfStats");
    }
    ...
    if (vmStats == null && restart) {
      logger.warning(
          LocalizedStrings.ClientStatsManager_CLIENTSTATSMANAGER_0_ARE_NOT_AVAILABLE, "VMStats");
    }
   ...
   */
  private static ClientHealthStats getClientHealthStats(
      GemFireCacheImpl currentCache, PoolImpl pool) {
    if (currentCache == null) {
      return null;
    }
      ...
      try {
        ...
        if (clientHealthMonitoringRegion != null) {
          ...
          if (oldStats != null) {
            ...
            logger.info(
                LocalizedStrings.DEBUG, "getClientHealthStats got oldPoolStats  " + oldPoolStats);
        ...
        logger.fine(
            "Exception in getting pool stats in  getClientHealthStats "
