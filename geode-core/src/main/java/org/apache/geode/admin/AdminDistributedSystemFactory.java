...
public class AdminDistributedSystemFactory {
   ...
   *
   * <p>Using a null or empty bindAddress will clear the usage of this option and connections to
   * distributed systems will return to using all available network interfaces.
   *
   * <p>This method always throws UnsupportedOperationException because it is now deprecated and is
   ...
   * content of <code>gemfire.properties</code>. The {@linkplain
   * DistributedSystemConfig#DEFAULT_REMOTE_COMMAND} default remote command is used.
   *
   ...
   *
   * <p>This method <em>must</em> be called before calling {@link AdminDistributedSystem#connect}.
   * It <em>must</em> also be called before {@link DistributedSystem#connect} is when creating a
   * colocated distributed system.
   *
   * <p>Once it has been enabled be careful to only use GemFire APIs from the <code>
   * org.apache.geode.admin</code> package. In particular do not create a {@link
   * org.apache.geode.cache.Cache} or a normal {@link DistributedSystem}.
   *
   * @param adminOnly <code>true</code> if this VM should be limited to administration APIs; <code>
   *     false</code> if this VM should allow all GemFire APIs.
   ...
   *     machines. If <code>null</code>, then the {@linkplain
   *     DistributedSystemConfig#DEFAULT_REMOTE_COMMAND default} will be used.
   * @since GemFire 4.0
   */
  public static DistributedSystemConfig defineDistributedSystem(
      DistributedSystem system, String remoteCommand) throws AdminException {
