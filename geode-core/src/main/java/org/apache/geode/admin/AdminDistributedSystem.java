...
public interface AdminDistributedSystem {
   ...
   * registered on this <code>AdminDistributedSystem</code>. The default level is {@link
   * AlertLevel#WARNING}.
   */
  ...
  /**
   * Creates a new <code>DistributionLocator</code> that is ready to {@linkplain
   * DistributionLocator#getConfig configure} and {@linkplain #start start}.
   *
   * <p>It is presumed that the newly-added locator is used to discover members of the distributed
  ...
  /**
   * Returns array of <code>DistributionLocator</code>s administered by this <code>
   ...
   * background thread that connects to the distributed system. As a result, a <code>
   * AdminDistributedSystem</code> can be "connected" to before any members of the system have been
   * started or have been seen. The {@link #waitToBeConnected} method will wait for the connection
   * to be made.
   *
   * @see #isConnected
   * @see #isRunning
  ...
  /**
   * Creates a new cache vm that is ready to {@linkplain CacheVmConfig configure} and {@linkplain
   * #start start}.
   *
   * @since GemFire 5.7
   */
  ...
  /**
   * Retrieve the set of persistent files that the existing members are waiting for. See {@link
   * AdminDistributedSystem#revokePersistentMember(InetAddress, String)}
   *
   ...
   *
   * <p>Killing individual members can lead to inconsistencies in the members persistent data, which
   ...
   *
   * <p>This is equivalent to calling shutDownAllMembers(0);
   *
   ...
   *     directory does not exist (on a member by member basis) a full backup will be performed for
   *     the member.
   * @return The status of the backup, which includes the set of members that were backed up and the
   *     set of members that were known to be offline at the time of backup.
   * @since GemFire 6.5
   ...
   * member connected to.
   *
   * <p>This is equivalent to calling {DiskStore#forceCompaction} on all members.
   *
