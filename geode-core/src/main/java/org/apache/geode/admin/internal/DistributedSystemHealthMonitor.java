...
/**
 * A thread that monitors the health of the distributed system. It is kind of like a {@link
 * org.apache.geode.distributed.internal.HealthMonitorImpl}. In order to get it to place nice with
 * the rest of the health monitoring APIs, this class pretends that it is a <code>GemFireVM</code>.
 * Kind of hokey, but it beats a bunch of special-case code.
 *
 * @since GemFire 3.5
 */
class DistributedSystemHealthMonitor implements Runnable, GemFireVM {
  ...
  DistributedSystemHealthMonitor(
      DistributedSystemHealthEvaluator eval, GemFireHealthImpl healthImpl, int interval) {
    this.eval = eval;
    ...
    ThreadGroup group =
        LoggingThreadGroup.createThreadGroup(
            ...
            logger);
    String name =
        LocalizedStrings.DistributedSystemHealthMonitor_HEALTH_MONITOR_FOR_0.toLocalizedString(
            eval.getDescription());
  ...
  public void run() {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Monitoring health of {} every {} seconds", this.eval.getDescription(), interval);
    }
