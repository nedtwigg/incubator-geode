...
public interface SystemMemberCacheServer {
   ...
   *
   * <p>The default value does not override the gemfire.properties. If you wish to override the
   * properties and want to have your server bind to all local addresses then use this string <code>
   * "0.0.0.0"</code>.
   *
   ...
   *
   * <p>If false, then an update to any key on the server causes an update to be sent to all
   ...
   *
   * <p>If true, then an update to any key on the server causes an update to be sent to only those
   ...
   *
   * <p>The load probe should implement DataSerializable if it is used with this interface, because
   * it will be sent to the remote VM.
   *
