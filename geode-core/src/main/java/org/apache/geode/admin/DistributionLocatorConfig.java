 ...
 * administration APIs.
 *
 * <p>A <code>DistributionLocatorConfig</code> can be modified using a number of mutator methods
 * until the <code>DistributionLocator</code> configured by this object is {@linkplain
 * ManagedEntity#start started}. After that, attempts to modify most attributes in the <code>
 * DistributionLocatorConfig</code> will result in an {@link IllegalStateException} being thrown. If
 * you wish to use the same <code>DistributionLocatorConfig</code> to configure another <code>
 * DistributionLocator</code>s, a copy of the <code>DistributionLocatorConfig</code> object can be
 * made by invoking the {@link Object#clone} method.
 *
