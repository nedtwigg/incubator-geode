 ...
 *       appear empty. It can be used to for zero footprint producers that only want to distribute
 *       their data to others and for zero footprint consumers that only want to see events.
 ...
 *   <li><code>PARTITION</code> causes data that this region holds to be spread across processes.
 *       The amount of data held in this cache is configured in {@link PartitionAttributes} with a
 ...
 *       memory. A distributed region will be initialized with the data from other caches. On
 *       distributed region operations that would cause the contents to differ with other caches are
 *       not allowed. This policy is allowed on local scope region but it behaves the same as <code>
 *       NORMAL</code>.
...
public class DataPolicy implements java.io.Serializable {
   ...
   *
   * <p>Data that this region is interested in is stored in local memory.
   */
   ...
   *
   * <p>Operations that would cause the contents to differ with other caches are not allowed.
   *
   * <p>Data that this region is interested in is stored in local memory.
   */
  ...
  private DataPolicy(int ordinal, String name) {
    if (ordinal >= VALUES.length) {
      throw new IllegalArgumentException(
          LocalizedStrings.DataPolicy_ONLY_0_DATAPOLICIES_MAY_BE_DEFINED.toLocalizedString(
              Integer.valueOf(VALUES.length + 1)));
    }
    if (VALUES[ordinal] != null) {
      throw new IllegalArgumentException(
          LocalizedStrings.DataPolicy_ORDINAL_0_IS_ALREADY_DEFINED_BY_1.toLocalizedString(
   ...
   *
   * <p>Although DataPolicy {@link #PARTITION} will return true to this query, it is possible to
   * turn off local storage with {@link PartitionAttributesFactory#setLocalMaxMemory(int)} by
   * setting localMaxMemory to zero.
   *
   * @return true if regions with this policy store data locally.
   * @see #NORMAL
   * @see #PRELOADED
