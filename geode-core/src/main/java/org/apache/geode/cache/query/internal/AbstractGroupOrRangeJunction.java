...
public abstract class AbstractGroupOrRangeJunction extends AbstractCompiledValue
  ...
  public SelectResults auxFilterEvaluate(
    ...
    while (i.hasNext()) {
      ...
      boolean isConditioningNeeded =
          filter.isConditioningNeededForIndex(
              this.indpndntItr.length == 1 ? this.indpndntItr[0] : null,
              context,
              this.completeExpansion);
      ...
      filterResults =
          filter.filterEvaluate(
              context,
              !isConditioningNeeded ? intermediateResults : null,
              this.completeExpansion,
              null /*
      ...
      if (_operator == LITERAL_and) {
        if (filterResults != null && filterResults.isEmpty()) {
          ...
          intermediateResults =
              (intermediateResults == null || !isConditioningNeeded)
                  ? filterResults
        ...
        boolean isDistinct =
            (DefaultQuery) context.getQuery() != null
  ...
  private SelectResults auxIterateEvaluate(
  ...
  OrganizedOperands createOrganizedOperandsObject(int indexCount, List evalOperands) {
    ...
    if (indexCount == 1) {
      ...
      filterOperands =
          new GroupJunction(
              getOperator(), getIndependentIteratorForGroup(), getExpansionFlag(), newOperands);
    }
   ...
   *     Object
   */
  abstract AbstractGroupOrRangeJunction createNewOfSameType(
      int operator,
      RuntimeIterator[] indpndntItr,
      boolean isCompleteExpansion,
      CompiledValue[] operands);
