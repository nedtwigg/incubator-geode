...
public interface GatewaySender {
  ...
  /**
   * The default buffer size for socket buffers from a sending GatewaySender to its receiving <code>
   * GatewayReceiver</code>.
   */
  public static final int DEFAULT_SOCKET_BUFFER_SIZE = 524288;

  /**
   * The default amount of time in milliseconds that a socket read between a sending <code>Gateway
   * </code> and its receiving <code>Gateway</code> will block.
   */
  public static final int DEFAULT_SOCKET_READ_TIMEOUT =
      Integer.getInteger(
              DistributionConfig.GEMFIRE_PREFIX + "cache.gatewaySender.default-socket-read-timeout",
              0)
          .intValue();
  ...
  public static final int QUEUE_OPLOG_SIZE =
      Integer.getInteger(
              DistributionConfig.GEMFIRE_PREFIX + "cache.gatewaySender.queueOpLogSize",
              1024 * 1024 * 100)
          .intValue();
  ...
  public static final long GATEWAY_SENDER_TIMEOUT =
      Integer.getInteger(DistributionConfig.GEMFIRE_PREFIX + "GATEWAY_SENDER_TIMEOUT", 30)
          .intValue();
  ...
  public static final int GATEWAY_CONNECTION_IDLE_TIMEOUT =
      Integer.getInteger(
              DistributionConfig.GEMFIRE_PREFIX + "GatewaySender.GATEWAY_CONNECTION_IDLE_TIMEOUT",
              -1)
          .intValue();
  ...
  public static final int CONNECTION_RETRY_INTERVAL =
      Integer.getInteger(
              DistributionConfig.GEMFIRE_PREFIX + "gateway-connection-retry-interval", 1000)
          .intValue();
   ...
   *
   * <p>The GatewaySender will wait for GatewaySender.MAXIMUM_SHUTDOWN_WAIT_TIME seconds before
   ...
   * dispatching events.
   *
   * <p>The batch of events that are in the process of being dispatched are dispatched irrespective
   * of the state of pause operation. We can expect maximum of one batch of events being received at
