 ...
 * running queries.
 *
 * <p>The queries to be monitored is added into the ordered queue, ordered based on its
 * start/arrival time. The first one in the Queue is the older query that will be canceled first.
 *
 * <p>The QueryMonitor cancels a query-execution thread if its taking more than the max time.
 *
 * @since GemFire 6.0
 */
public class QueryMonitor implements Runnable {
   ...
   *
   * <p>The max query execution time is set using the system property
      ...
      if (!queryCompleted[0]) { //cancel if query is not completed
        String reason =
            LocalizedStrings.QueryMonitor_LOW_MEMORY_CANCELED_QUERY.toLocalizedString(
                memoryThreshold);
        ((DefaultQuery) queryTask.query)
            .setCanceled(true, new QueryExecutionLowMemoryException(reason));
        queryTask.queryExecutionStatus.set(Boolean.TRUE);
      }
    }
  }
  ...
  private class QueryThreadTask {
    ...
    public String toString() {
      return new StringBuffer()
          .append("QueryThreadTask[StartTime:")
          .append(this.StartTime)
          .append(", queryThread:")
          .append(this.queryThread)
          .append(", threadId:")
          .append(this.queryThread.getId())
          .append(", query:")
          .append(this.query.getQueryString())
          .append(", queryExecutionStatus:")
          .append(this.queryExecutionStatus)
          .append("]")
          .toString();
    }
  }
