 ...
 * store.
 *
 * <p>Instances of this interface are created using {@link DiskStoreFactory#create}. So to create a
...
public interface DiskStore {
   ...
   *
   * <p>Note that calls to {@link #forceCompaction} will also be allowed if {@link #getAutoCompact
   ...
   * the queue. If this <code>DiskStore</code> configures synchronous writing, then <code>queueSize
   * </code> is ignored.
   *
   ...
   *
   * <p>If there are any currently open regions in the disk store this method will throw an
   * exception. If there are any closed regions that are persisted in this disk store, the data for
   * those regions will be destroyed.
   *
