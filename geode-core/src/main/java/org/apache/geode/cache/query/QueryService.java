 ...
 *
 * <p>Creating an index on an employee's age using QueryService in region "employeeRegion",
 ...
 *
 * <p>The CQs work on the server regions, the client can use the CQ methods supported in this class
 * to create/operate CQs on the server. The CQ obtains the Server connection from the corresponding
...
public interface QueryService {
   ...
   *     will correspond to the FROM clause in a query. Check following examples. The regionPath is
   *     restricted to only one expression
   *     <p>Example: Query1: "Select * from /portfolio p where p.mktValue = 25.00" For index on
   ...
   *     <p>Example: Query1: "Select * from /portfolio p where p.ID = 10" indexExpression: "p.ID"
   ...
   *     must include exactly one region, but may include multiple expressions as required to drill
   *     down into nested region contents.
   *     <p>Example: Query1: "Select * from /portfolio p where p.mktValue > 25.00" For index on
   *     mktValue field: indexExpression: "p.mktValue" regionPath: "/portfolio p"
   *     <p>Query2: "Select * from /portfolio p, p.positions.values pos where pos.secId ='VMWARE'"
   ...
   * @param regionPath that resolves to region values or nested collections of region values which
   *     will correspond to the FROM clause in a query. The regionPath must include exactly one
   *     region, but may include multiple expressions as required to drill down into nested region
   *     contents. Check following examples.
   * @param imports string containing imports (in the query language syntax, each import statement
   *     separated by a semicolon), provides packages and classes used in variable typing in the
   *     Indexed and FROM expressions. The use is the same as for the FROM clause in querying.
   *     <p>Example: Query1: "Select * from /portfolio p where p.mktValue > 25.00" For index on
  ...
  /**
   * @deprecated As of 6.6.2, use {@link #createIndex(String, String, String)} and {@link
   *     #createKeyIndex(String, String, String)} instead.
   *     <p>Create an index that can be used when executing queries.
   ...
   *     that are referenced in the fromClause. This expression is used to optimize the comparison
   *     of the same path found in a query's WHERE clause when used to compare against a constant
   *     expression. For example, an index with indexedExpression "mktValue" might be used for a
   *     query with a WHERE clause of "mktValue > 25.00". The exact use and specification of the
   *     indexedExpression varies depending on the indexType. Query parameters and region paths are
   *     not allowed in the indexedExpression (e.g. $1).
   ...
   *     region (otherwise the index could not be maintained on single entry updates). References to
   *     query parameters are not allowed. For primary key indexes, the fromClause must be just one
   *     collection which must be a region path only.
   * @return the newly created Index
   * @throws QueryInvalidException if the argument query language strings have invalid syntax
   * @throws IndexInvalidException if the arguments do not correctly specify an index
   * @throws IndexNameConflictException if an index with this name already exists
  ...
  /**
   * @deprecated As of 6.6.2, use {@link #createIndex(String, String, String, String)} and {@link
   *     #createKeyIndex(String, String, String)} instead.
   *     <p>Create an index that can be used when executing queries.
   * @param indexName the name of this index, used for statistics collection and to identify this
   *     index for later access
   ...
   *     <p>Get a collection of all the indexes on the specified Region of the specified index type.
   * @param region the region for the requested indexes
