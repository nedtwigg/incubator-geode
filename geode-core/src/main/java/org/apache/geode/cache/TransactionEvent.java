 ...
 *
 * <p>A <code>TransactionListener</code> receives an instance of this class allowing exploration of
 * the resultant operations. The resultant operation is the final result of, potentially, a sequence
 * of operations on a key such that earlier operations might be masked. For example, multiple put
 * operations using the same key will result in only one {@link EntryEvent} for that key.
 *
 * <p>An instance of TransactionEvent for the same transaction on the originating VM may differ from
 * a recipient VM. The amount of variation will depend on the variation of the state of Entries on
 ...
 *
 * <p>The event lists are ordered according to the chronological order of the indiviual operations.
 *
 * <p>The {@link EntryEvent} instances always return <code>null</code> as their {@link
 * CacheEvent#getCallbackArgument callback argument}.
 *
...
public interface TransactionEvent {
  ...
  /**
   * Gets all "create" EntryEvents for this transaction; <code>Region.create</code> and/or <code>
   * Region.put</code>
   *
  ...
  /**
   * Gets all "destroy" EntryEvents for this transaction; <code>Region.destroy</code> and <code>
   * Region.localDestroy</code>.
   *
   * @return <code>List</code> of <code>EntryEvents</code> or <code>Collections.EMPTY_LIST</code>
   * @deprecated as of GemFire 5.0, use {@link #getEvents} instead
   */
