 ...
 * controller. Eviction controllers are defined by an {@link
 * org.apache.geode.cache.EvictionAlgorithm} and a {@link org.apache.geode.cache.EvictionAction}.
 * Once a <code>Region</code> is created with an eviction controller, it can not be removed, however
 * it can be changed through an {@link org.apache.geode.cache.EvictionAttributesMutator}.
 *
...
public abstract class EvictionAttributes implements DataSerializable {
  /**
   * The default maximum for {@linkplain EvictionAlgorithm#LRU_ENTRY entry LRU}. Currently <code>900
   * </code> entries.
   */
  public static final int DEFAULT_ENTRIES_MAXIMUM = 900;
  /**
   * The default maximum for {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU}. Currently <code>
   * 10</code> megabytes.
   */
   ...
   * default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and default {@linkplain
   * #DEFAULT_ENTRIES_MAXIMUM maximum}.
   *
   * <p>{@link EvictionAttributes} cause regions to evict the least recently used (LRU) entry once
   * the region reaches a maximum capacity. The entry is either locally destroyed or its value
   * overflows to disk when evicted.
   *
   * <p>
   *
   * <p>This is not supported when replication is enabled.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, the EvictionAttribute <code>maximum</code>,
   ...
   * <p>
   *
   * <p>If you are using a <code>cache.xml</code> file to create a Cache Region declaratively, you
   * can include the following to configure a region for eviction
   *
   * <p>
   *
   * <pre>
   ...
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and default {@linkplain
   *     #DEFAULT_ENTRIES_MAXIMUM maximum}
   */
  ...
  /**
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_ENTRY entry LRU} eviction attributes with
   * default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and given <code>
   * maximumEntries</code>.
   *
   * @param maximumEntries the number of entries to keep in the Region
   * @return {@linkplain EvictionAlgorithm#LRU_ENTRY entry LRU} eviction attributes with default
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and <code>maximumEntries</code>
   ...
   */
  public static EvictionAttributes createLRUEntryAttributes(
      int maximumEntries, EvictionAction evictionAction) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_ENTRY)
        .setAction(evictionAction)
        .internalSetMaximum(maximumEntries);
   ...
   * default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and default {@linkplain
   * ObjectSizer#DEFAULT sizer}.
   *
   * <p>Heap LRU EvictionAttributes evict the least recently used {@link Region.Entry} when heap
   * usage exceeds the {@link ResourceManager} eviction heap threshold. If the eviction heap
   * threshold is exceeded the least recently used {@link Region.Entry}s are evicted.
   *
   * <p>
   *
   * <p>With other LRU-based eviction controllers, only cache actions (such as {@link
   * Region#put(Object, Object) puts} and {@link Region#get(Object) gets}) cause the LRU entry to be
   * evicted. However, with heap LRU, because the JVM's heap may be effected by more than just the
   * GemFire cache operations, a daemon thread will perform the eviction if no operations are being
   * done on the region.
   *
   * <p>The eviction attribute's {@linkplain ObjectSizer sizer} is used to estimate how much the
   * heap will be reduced by an eviction.
   *
   * <p>When using Heap LRU, the JVM must be launched with the <code>-Xmx</code> and <code>-Xms
   * </code> switches set to the same values. Many virtual machine implementations have additional
   * JVM switches to control the behavior of the garbage collector. We suggest that you investigate
   * tuning the garbage collector when using this type of eviction controller. A collector that
   * frequently collects is needed to keep our heap usage up to date. In particular, on the Sun <A
   * href="http://java.sun.com/docs/hotspot/gc/index.html">HotSpot</a> JVM, the <code>
   * -XX:+UseConcMarkSweepGC</code> flag needs to be set, and <code>
   * -XX:CMSInitiatingOccupancyFraction=N</code> should be set with N being a percentage that is
   * less than the {@link ResourceManager} eviction heap threshold.
   *
   * <p>The JRockit JVM has similar flags, <code>-Xgc:gencon</code> and <code>-XXgcTrigger:N</code>,
   ...
   *
   * <p>On the IBM JVM, the flag to get a similar collector is <code>-Xgcpolicy:gencon</code>, but
   ...
   *
   * <p>If you are using a <code>cache.xml</code> file to create a Cache Region declaratively, you
   * can include the following to create an LRU heap eviction controller:
   *
   * <p>
   *
   * <pre>
  ...
  /**
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_HEAP heap LRU} eviction attributes with
   * default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and the given <code>sizer
   * </code>.
   *
   * @param sizer the sizer implementation used to determine the size of each entry in this region
   * @return {@linkplain EvictionAlgorithm#LRU_HEAP heap LRU} eviction attributes with default
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action} and the given <code>sizer</code>
   ...
   */
  public static EvictionAttributes createLRUHeapAttributes(
      final ObjectSizer sizer, final EvictionAction evictionAction) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_HEAP)
        .setAction(evictionAction)
        .setObjectSizer(sizer);
   ...
   * with default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, default {@linkplain
   * ObjectSizer#DEFAULT sizer}, and default {@linkplain #DEFAULT_MEMORY_MAXIMUM maximum}.
   *
   * <p>Creates EvictionAttributes for an eviction controller that will remove the least recently
   * used (LRU) entry from a region once the region reaches a certain byte capacity. Capacity is
   ...
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, the EvictionAttribute <code>maximum</code>,
   * is always equal to {@link PartitionAttributesFactory#setLocalMaxMemory(int) " local max memory
   * "} specified for the {@link PartitionAttributes}. It signifies the amount of memory allowed in
   * the region, collectively for its primary buckets and redundant copies for this JVM. It can be
   * different for the same region in different JVMs.
   *
   * <p>If you are using a <code>cache.xml</code> file to create a Cache Region declaratively, you
   * can include the following to create an LRU memory eviction controller:
   *
   * <p>
   *
   * <pre>
   ...
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, default {@linkplain
   *     ObjectSizer#DEFAULT sizer}, and default {@linkplain #DEFAULT_MEMORY_MAXIMUM maximum}
   */
  public static EvictionAttributes createLRUMemoryAttributes() {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_MEMORY)
        .setAction(EvictionAction.DEFAULT_EVICTION_ACTION)
   ...
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes
   * with default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, default {@linkplain
   * ObjectSizer#DEFAULT sizer}, and given <code>maximumMegabytes</code>.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, even if maximumMegabytes are supplied, the
   * EvictionAttribute <code>maximum</code>, is always set to {@link
   * PartitionAttributesFactory#setLocalMaxMemory(int) " local max memory "} specified for the
  ...
  public static EvictionAttributes createLRUMemoryAttributes(int maximumMegabytes) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_MEMORY)
        .setAction(EvictionAction.DEFAULT_EVICTION_ACTION)
        .internalSetMaximum(maximumMegabytes)
        .setObjectSizer(null);
  }

  /**
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes
   * with default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, given <code>sizer
   * </code>, and given <code>maximumMegabytes</code>.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, even if maximumMegabytes are supplied, the
   ...
   * @return {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes with default
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, given <code>sizer</code>, and
   *     given <code>maximumMegabytes</code>
   * @see #createLRUMemoryAttributes()
   */
  public static EvictionAttributes createLRUMemoryAttributes(
      int maximumMegabytes, ObjectSizer sizer) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_MEMORY)
        .setAction(EvictionAction.DEFAULT_EVICTION_ACTION)
        .internalSetMaximum(maximumMegabytes)
  ...
  /**
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes
   * with the given <code>evictionAction</code>, given <code>sizer</code>, and given <code>
   * maximumMegabytes</code>.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, even if maximumMegabytes are supplied, the
   ...
   *     <code>evictionAction</code>, given <code>sizer</code>, and given <code>maximumMegabytes
   *     </code>
   * @see #createLRUMemoryAttributes()
   */
  public static EvictionAttributes createLRUMemoryAttributes(
      int maximumMegabytes, ObjectSizer sizer, EvictionAction evictionAction) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LRU_MEMORY)
        .setAction(evictionAction)
        .internalSetMaximum(maximumMegabytes)
   ...
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes
   * with default {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, given <code>sizer
   * </code>, and default {@linkplain #DEFAULT_MEMORY_MAXIMUM maximum}.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, even if maximumMegabytes are supplied, the
   ...
   * @return {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes with default
   *     {@linkplain EvictionAction#DEFAULT_EVICTION_ACTION action}, given <code>sizer</code>, and
   *     default {@linkplain #DEFAULT_MEMORY_MAXIMUM maximum}
   * @see #createLRUMemoryAttributes()
   * @since GemFire 6.0
   */
  ...
  /**
   * Creates and returns {@linkplain EvictionAlgorithm#LRU_MEMORY memory LRU} eviction attributes
   * with given <code>evictionAction</code>, given <code>sizer</code>, and default {@linkplain
   * #DEFAULT_MEMORY_MAXIMUM maximum}.
   *
   * <p>
   *
   * <p>For a region with {@link DataPolicy#PARTITION}, even if maximumMegabytes are supplied, the
   ...
   *     <code>evictionAction</code>, given <code>sizer</code>, and default {@linkplain
   *     #DEFAULT_MEMORY_MAXIMUM maximum}
   * @see #createLRUMemoryAttributes()
   * @since GemFire 6.0
   */
   ...
   * see {@link #createLRUMemoryAttributes()} for more. An {@link ObjectSizer} is used by {@link
   * EvictionAlgorithm#LRU_HEAP} to estimate how much heap will be saved when evicting a region
   * entry.
   *
   * @return the sizer used by {@link EvictionAlgorithm#LRU_MEMORY} or {@link
   *     EvictionAlgorithm#LRU_HEAP}, for all other algorithms null is returned.
   */
   ...
   * @since GemFire 5.7
   */
  public static EvictionAttributes createLIFOEntryAttributes(
      int maximumEntries, EvictionAction evictionAction) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LIFO_ENTRY)
        .setAction(evictionAction)
        .internalSetMaximum(maximumEntries);
  }
   ...
   * @since GemFire 5.7
   */
  public static EvictionAttributes createLIFOMemoryAttributes(
      int maximumMegabytes, EvictionAction evictionAction) {
    return new EvictionAttributesImpl()
        .setAlgorithm(EvictionAlgorithm.LIFO_MEMORY)
        .setAction(evictionAction)
        .internalSetMaximum(maximumMegabytes)
        .setObjectSizer(null);
  }
