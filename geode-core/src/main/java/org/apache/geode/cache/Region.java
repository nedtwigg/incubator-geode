 ...
 *
 * <p>The Region interface basically contains two set of APIs: Region management APIs; and
 ...
 *
 * <p>Each {@link org.apache.geode.cache.Cache} defines a single top region called the root region.
 * User applications can use the root region to create subregions for isolated name space and object
 * grouping.
 *
 * <p>A region's name can be any String except that it should not contain the region name separator,
 * a forward slash (/). <code>Region</code>s can be referenced by a relative path name from any
 * region higher in the hierarchy in {@link #getSubregion}. You can get the relative path from the
 * root region with {@link #getFullPath}. The name separator is used to concatenate all the region
 * names together from the root, starting with the root's subregions.
 *
 * <p>Relative region names can provide a convenient method to locate a subregion directly from some
 ...
 *  </pre>
 *
 * <p>Region entries are identified by their key. Any Object can be used as a key as long as the key
 ...
 *
 * <p>Regions and their entries can be locked. The <code>Lock</code> obtained from {@link
 * #getRegionDistributedLock} is a distributed lock on the entire Region, and the <code>Lock</code>
 * obtained from {@link Region#getDistributedLock} is a distributed lock on the individual entry.
 *
 * <p>If the scope is <code>Scope.GLOBAL</code>, the methods that modify, destroy, or invalidate the
 * entries in this region will also get a distributed lock. See the documentations for {@link
 * #getDistributedLock} and {@link #getRegionDistributedLock} for details on the implicit locking
 * that occurs for regions with <code>Scope.GLOBAL</code>.
 *
 * <p>Unless otherwise specified, all of these methods throw a <code>CacheClosedException</code> if
 * the Cache is closed at the time of invocation, or a <code>RegionDestroyedException</code> if this
 ...
 *
 * <p>Implementation of the java.util.concurrent.ConcurrentMap interface was added in version 6.5.
 ...
 *
 * <p>The semantics of the ConcurrentMap methods on a Partitioned Region are consistent with those
 ...
 *
 * <p>The same is true for a region with GLOBAL scope. All operations will be done atomically since
 * a distributed lock will be held while the operation is done.
 *
 * <p>The same is true for a region with LOCAL scope. All ops will be done atomically since the
 * underlying map is a concurrent hash map and no distribution is involved.
 *
 * <p>For peer REPLICATE and PRELOADED regions atomicity is limited to threads in the JVM the
 * operation starts in. There is no coordination with other members of the system unless the
 * operation is performed in a transaction.
 *
 * <p>For client server regions the atomicity is determined by the scope and data policy of the
 * server region as described above. The operation is actually performed on the server as described
 * above. Clients will always send the ConcurrentMap operation to the server and the result returned
 * by the ConcurrentMap method in client will reflect what was done on the server. Same goes for any
...
public interface Region<K, V> extends ConcurrentMap<K, V> {
   ...
   * "root".
   *
   * <p>Does not throw a <code>CacheClosedException</code> or a <code>RegionDestroyedException
   * </code>.
   *
  ...
  /**
   * Invalidates this region. Invalidation cascades to all entries and subregions. After the <code>
   * invalidateRegion</code>, this region and the entries in it still exist. To remove all the
   ...
   *
   * <p>Does not update any <code>CacheStatistics</code>.
   *
   ...
   * The region invalidate will be distributed to other caches if the scope is not <code>Scope.LOCAL
   * </code>.
   *
   * <p>Does not update any <code>CacheStatistics</code>.
   *
   ...
   * <p>Does not update any <code>CacheStatistics</code>.
   *
   * @throws IllegalStateException if this region is distributed and {@link
   ...
   * @param aCallbackArgument a user-defined parameter to pass to callback events triggered by this
   *     method. Can be null.
   * @throws IllegalStateException if the region is distributed and {@link
   *     DataPolicy#withReplication replicated}
   * @throws UnsupportedOperationException If the region is a partitioned region
   * @see CacheListener#afterRegionInvalidate
   */
  ...
  /**
   * Destroys the whole region and provides a user-defined parameter object to any <code>CacheWriter
   * </code> invoked in the process. Destroy cascades to all entries and subregions. After the
   * destroy, this region object can not be used any more. Any attempt to use this region object
   * will get a <code>RegionDestroyedException</code> exception. The region destroy is distributed
   ...
   * be used any more and any attempt to use this region object will get {@link
   * RegionDestroyedException} exception. This operation is not distributed to any other cache.
   *
   * <p>Does not update any <code>CacheStatistics</code>.
   *
   * @see CacheListener#afterRegionDestroy
   ...
   *
   * <p>No <code>CacheWriter</code> is invoked. Destroy will cascade to all the entries and
   * subregions. After the destroy, this region object can not be used any more. Any attempt to use
   * this region object will get {@link RegionDestroyedException} exception.
   *
   * <p>Does not update any <code>CacheStatistics</code>.
   *
   * @param aCallbackArgument a user-defined parameter to pass to callback events triggered by this
   ...
   * system to be reinitialized: remote regions are cleared of all data and distributed {@link
   * DataPolicy#withReplication replicated} remote regions will do a new getInitialImage operation
   * to get the data from this snapshot. Any existing references to this region or any region that
   * is reinitialized in this manner become unusable in that any subsequent methods invoked on those
   * references will throw a RegionReinitializedException (which is a subclass of
   * RegionDestroyedException).
   *
   * <p>In order to continue working with this region, a new reference needs to be acquired using
   * Cache#getRegion or Region#getSubregion (which will block until reinitialization is complete).
   *
   * <p>NOTE: SUBREGIONS ARE DESTROYED. Since loading a snapshot effectively destroys the region and
   ...
   *
   * <p>If any error occurs while loading the snapshot, this region is destroyed and threads in
   * remote caches that are attempting to get a reference to this region will get null instead of a
   * region reference.
   *
   * <p>The only callbacks that are invoked are called for the destroyRegion operation, i.e. the
   ...
   * this region, so it can be either a simple region name or a relative region path. If <code>
   * regionName</code> is the empty string, then this region itself is returned.
   *
   ...
   * not contain a region name separator. If the subregion is a distributed {@link
   * DataPolicy#withReplication replicated} region, it will be initialized with data from all other
   * caches in this distributed system that have the same region.
   *
   * <p>Updates the {@link CacheStatistics#getLastAccessedTime} and {@link
   ...
   * @throws IllegalStateException If the supplied RegionAttributes violate the <a
   *     href="AttributesFactory.html#creationConstraints">region creation constraints</a> with a
   *     region of the same name in another cache in the distributed system or with this (parent)
   *     region.
   * @throws org.apache.geode.distributed.LeaseExpiredException if lease expired on distributed lock
   *     for Scope.GLOBAL
   * @throws TimeoutException if timed out getting distributed lock for Scope.GLOBAL
   ...
   * @see AttributesFactory
   * @see Region#getFullPath
   * @deprecated as of 7.0 use {@link RegionFactory#createSubregion(Region, String)} or {@link
   *     ClientRegionFactory#createSubregion(Region, String)}.
   */
  public <SK, SV> Region<SK, SV> createSubregion(
      String subregionName, RegionAttributes<SK, SV> aRegionAttributes)
   ...
   *
   * <p>This <code>Set</code> is unmodifiable. It is backed by this region. Synchronization is not
   ...
   *
   * <p>For local scope, a local CacheLoader will be invoked if there is one. For global scope, the
   * order is netSearch, localLoad, netLoad. For any other distributed scope, the order is
   * localLoad, netSearch, netLoad.
   *
   * <p>netSearch and netLoad are never performed more than once, so if a loader attempts to do a
   * netSearch and one was already done, then another one will not be done.
   *
   * <p>The value returned by get is not copied, so multi-threaded applications should not modify
   * the value directly, but should use the update methods.
   *
   * <p>Updates the {@link CacheStatistics#getLastAccessedTime}, {@link
   * CacheStatistics#getHitCount}, {@link CacheStatistics#getMissCount}, and {@link
   * CacheStatistics#getLastModifiedTime} (if a new value is loaded) for this region and the entry.
   *
   * <p>If the <code>CacheWriter</code> throws a <code>CacheWriterException</code> when a new value
   * is retrieved from a loader, the value will not be put into the cache (a new entry will not be
   ...
   * @throws PartitionedRegionStorageException for a partitioned region fails to invoke a {@link
   *     CacheLoader}
   ...
   *
   * <p>If the <code>CacheWriter</code> throws a <code>CacheWriterException</code> when a new value
   * is retrieved from a loader, then the value will not be put into the cache (a new entry will not
   * be created) but the get will return the value and not propagate the exception.
   *
   * @param key whose associated value is to be returned. The key Object must implement the equals
   *     and hashCode methods. Note that even though key is typed as "Object" if a CacheLoader is
   *     used then make sure you only pass instances of "K" in this parameter.
   * @param aCallbackArgument an argument passed into the CacheLoader if loader is used. This same
   *     argument will also be subsequently passed to a CacheWriter if the loader returns a non-null
   *     value to be placed in the cache. Modifications to this argument made in the CacheLoader
   *     will be visible to the CacheWriter even if the loader and the writer are installed in
   *     different cache VMs. It will also be passed to any other callback events triggered by this
   *     method. May be null. Should be serializable.
   * @return the value with specified key, or null if the value is not found and can't be loaded
   * @throws NullPointerException if key is null
   * @throws IllegalArgumentException if aCallbackArgument is not serializable
   * @throws org.apache.geode.distributed.LeaseExpiredException if lease expired on distributed lock
   ...
   * @return the previous value stored locally for the key. If the entry did not exist then <code>
   *     null</code> is returned. If the entry was "invalid" then <code>null</code> is returned. In
   *     some cases <code>null</code> may be returned even if a previous value exists. If the region
   *     is a client proxy then <code>null</code> is returned. If the region is partitioned and the
   *     put is done on a non-primary then <code>null</code> is returned. If the value is not
   *     currently stored in memory but is on disk and if the region does not have cqs then <code>
   ...
   *     null</code> is returned. If the entry was "invalid" then <code>null</code> is returned. In
   *     some cases <code>null</code> may be returned even if a previous value exists. If the region
   *     is a client proxy then <code>null</code> is returned. If the region is off-heap and the old
   *     value was stored in off-heap memory then <code>null</code> is returned. If the region is
   *     partitioned and the put is done on a non-primary then <code>null</code> is returned. If the
   *     value is not currently stored in memory but is on disk and if the region does not have cqs
   ...
   * CacheStatistics#getLastModifiedTime} for this region and the entry.
   *
   * <p>If this region has a distributed scope, this operation may cause update events in caches
   * that already have this region with this entry, and it will cause create events in other caches
   * that have {@link InterestPolicy#ALL all events} configured.
   *
   * <p>This operation gets a distributed lock on the entry if the scope is <code>Scope.GLOBAL
   * </code>.
   *
   ...
   * @return the previous value stored locally for the key. If the entry was "invalid" then <code>
   *     null</code> is returned. In some cases <code>null</code> may be returned even if a previous
   *     value exists. If the region is a client proxy then <code>null</code> is returned. If the
   *     region is off-heap and the old value was stored in off-heap memory then <code>null</code>
   *     is returned. If the region is partitioned and the destroy is done on a non-primary then
   ...
   *
   * <p>This <code>Collection</code> is unmodifiable. It is backed by this region. Synchronization
   * is not necessary to access or iterate over this collection. No <code>
   * ConcurrentModificationException</code>s will be thrown, but values may be added or removed to
   * this collection while a thread is iterating. Iterators are intended to be used by one thread at
   * a time. If a stable "snapshot" view of the collection is required, then call one of the toArray
   * methods on the collection and iterate over the array. If you need to lock down the region so
   * this set is not modified while it is being accessed, use global scope with a distributed lock
   * on the region.
   *
   ...
   *
   * <p>Does not throw a <code>RegionDestroyedException</code> if this region has been destroyed.
   *
   ...
   *
   * <p>The operations invalidateRegion and destroyRegion do <em>not</em> automatically acquire a
   ...
   *
   * <p>See {@link #getDistributedLock} for the list of operations that automatically acquire
   * distributed entry locks for regions with global scope.
   *
   * <p>Note that Region locks are potentially very expensive to acquire.
   *
   ...
   *
   * <p>When a region has global scope, the following operations automatically acquire a distributed
   * lock on an entry: <code>create</code>, <code>put</code>, <code>destroy</code>, <code>invalidate
   * </code>, and <code>get</code> that causes a loader to be invoked.
   *
  ...
  /**
   * Determines whether there is a value in this <code>Region</code> that matches the given <code>
   * queryPredicate</code>. Filters the values of this region using the predicate given as a string
   * with the syntax of the <code>WHERE</code> clause of the query language. The predefined variable
   * <code>this</code> may be used inside the predicate to denote the current value being filtered.
   *
   ...
   *
   * <p>When executed from a client, this method always runs on the server. However application
   * should use QueryService to execute queries.
   *
   * <p>When executed from a client, this method always runs on the server. However application
   * should use QueryService to execute queries.
   *
  ...
  /**
   * Selects the single value in this <code>Region</code> that matches the given query <code>
   * predicate</code>. Filters the values of this region using the predicate given as a string with
   * the syntax of the where clause of the query language. The predefined variable <code>this</code>
   * may be used inside the predicate to denote the element currently being filtered.
   *
   * @param queryPredicate A query language boolean query predicate.
   * @return The single element that evaluates to true for the predicate. If no value in this <code>
   *     Region</code> matches the predicate, <code>null</code> is returned.
   * @throws QueryInvalidException If predicate does not correspond to valid query language syntax.
   ...
   *
   * <p>Calls to this method will block until grantor authority has been transferred to this member.
   *
   * <p>If another member later calls <code>becomeLockGrantor</code>, that member will transfer
   * grantor authority from this member to itself. Multiple calls to this operation will have no
   * effect unless another member has transferred grantor authority, in which case, this member will
   * transfer grantor authority back to itself.
   *
   * <p>This region's scope must be <code>Scope.GLOBAL</code> to become a lock grantor.
   *
   * <p>This operation should not be invoked repeatedly in an application. It is possible to create
   * a lock service and have two or more members endlessly calling becomeLockGrantor to transfer
   ...
   * global scope with a distributed lock.
   *
   * <p>A remove called on an entry via the iterator will result in an UnsupportedOperationException
   *
   * <p>The Region.Entry obtained via the iterator is backed by the region. If a setValue on that
   * entry is called, it will be similar in effect as calling a put on that key.
   *
   ...
   * not thrown. This operation will be distributed to other caches if the scope is not <code>
   * Scope.LOCAL</code>.
   *
   ...
   *
   * <p>Note that the keys collection should extend K since a load may be done and the key in that
   * case will be stored in the region. The keys parameter was not changed to extend K for backwards
   * compatibility.
   *
   ...
   *     that was stored locally for the removed entry is returned. If the entry was "invalid" then
   *     <code>null</code> is returned. In some cases <code>null</code> may be returned even if a
   *     previous value exists. If the region is a client proxy then <code>null</code> is returned.
   *     If the region is off-heap and the old value was stored in off-heap memory then <code>null
   *     </code> is returned. If the region is partitioned and the remove is done on a non-primary
   *     then <code>null</code> is returned. If the value is not currently stored in memory but is
   ...
   *
   * <p>For {@link DataPolicy#PARTITION}, this is a distributed operation that returns the number of
   * entries present in entire region.
   *
   * <p>For all other types of regions, it returns the number of entries present locally, and it is
   ...
   *     the keys in the <code>List</code> will be registered. The key can also be the special token
   *     'ALL_KEYS', which will register interest in all keys in the region. In effect, this will
   *     cause an update to any key in this region in the CacheServer to be pushed to the client.
   *     <p>This method uses the default <code>InterestResultPolicy</code>.
   *     <p><i>Using 'ALL_KEYS' is the same as calling {@link #registerInterestRegex(String)} with
   *     ".*" as the argument. This means that all keys any type are pushed to the client and
   *     inserted into the local cache.</i>
   *     <p>If you locally-destroy a key and your region has concurrency-checks-enabled turned off
   *     you will not receive invalidation events from your interest subscription for that key. When
   ...
   *
   * <p>If you locally-destroy a key and your region has concurrency-checks-enabled turned off you
   * will not receive invalidation events from your interest subscription for that key. When
   * concurrency-checks-enabled is turned on GemFire will accept invalidation and deliver these
   * events to your client cache.
   *
   * @param key The key on which to register interest. If the key is a <code>List</code>, then all
   ...
   *     <ul>
   *       <li>InterestResultPolicy.NONE - does not initialize the local cache
   ...
   *           current values satisfying the request
   *     </ul>
   *
   * @throws UnsupportedOperationException if the region is not configured with a pool name.
   ...
   *
   * <p>Note that if the <code>regex</code> is <code>".*"</code> then all keys of any type will be
   * pushed to the client.
   *
   * <p>This method uses the default <code>InterestResultPolicy</code>.
   *
   * <p>The regular expression string is compiled using the {@link java.util.regex.Pattern} class.
   *
   * <p>If you locally-destroy a key and your region has concurrency-checks-enabled turned off you
   * will not receive invalidation events from your interest subscription for that key. When
   * concurrency-checks-enabled is turned on GemFire will accept invalidation and deliver these
  ...
  public void registerInterest(
      K key, InterestResultPolicy policy, boolean isDurable, boolean receiveValues);
   ...
   *
   * <p>ConcurrentMap operations are supported on partitioned and replicated regions and in client
   * caches. They are also supported on non-empty local regions.
   *
   * <p>Please read the notes on ConcurrentMap operations in the javadoc for Region.
   *
   * <p>Region allows the value parameter to be null, which will create an invalid entry.
   *
   ...
   * except that the action is performed atomically.
   *
   * <p>As of 8.5, if value is an Array then Arrays.equals or Arrays.deepEquals is used instead of
   * Object.equals.
   *
   * <p>ConcurrentMap operations are supported on partitioned and replicated regions and in client
   * caches. They are also supported on non-empty local regions.
   ...
   * <p>Please read the notes on ConcurrentMap operations in the javadoc for Region.
   *
   * <p>Region allows the value parameter to be null, which will match an invalid entry.
   *
   ...
   * <p>Please read the notes on ConcurrentMap operations in the javadoc for Region.
   *
   * <p>Region allows the oldValue parameter to be null, which will match an invalid entry.
   *
   * @param key key with which the specified value is associated
   ...
   * for{Entry#setValue()}), are not distributed, do not acquire any locks, and do not affect <code>
   * CacheStatistics</code>.
   *
   * <p>Unless otherwise noted, all of these methods throw a <code>CacheClosedException</code> if
   * the Cache is closed at the time of invocation, or an <code>EntryDestroyedException</code> if
  ...
  public interface Entry<K, V> extends Map.Entry<K, V> {
    ...
    /**
     * Returns the value of this entry in the local cache. Does not invoke a <code>CacheLoader
     * </code>, does not do a netSearch, netLoad, etc.
     *
     ...
     *
     * <p>Does not throw a <code>EntryDestroyedException</code> if this entry has been destroyed.
     *
     ...
     *     then <code>null</code> is returned. If the entry was "invalid" then <code>null</code> is
     *     returned. In some cases <code>null</code> may be returned even if a previous value
     *     exists. If the region is a client proxy then <code>null</code> is returned. If the region
     *     is off-heap and the old value was stored in off-heap memory then <code>null</code> is
     *     returned. If the region is partitioned and the setValue is done on a non-primary then
     *     <code>null</code> is returned. If the value is not currently stored in memory but is on
     *     disk and if the region does not have cqs then <code>null</code> is returned.
     * @since GemFire 5.0
