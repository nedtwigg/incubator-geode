 ...
 *
 * <p>Re-balancing the GemFire Cache resources can be accomplished using a {@link
 * RebalanceOperation}:
 *
 * <pre>
 ...
 * </pre>
 *
 * Monitoring of heap utilization is enabled by setting the critical heap percentage using {@link
...
public interface ResourceManager {
   ...
   *
   * <p>Changing this value can cause {@link LowMemoryException} to be thrown from the following
   ...
   *   <li>{@linkplain QueryService#createIndex(String, org.apache.geode.cache.query.IndexType,
   *       String, String) index creation}
   *   <li>Execution of {@link Function}s whose {@link Function#optimizeForWrite()} returns true.
   * </ul>
   *
   * <p>Only one change to this attribute or the eviction heap percentage will be allowed at any
   * given time and its effect will be fully realized before the next change is allowed.
   *
   * <p>When using this threshold, the VM must be launched with the <code>-Xmx</code> and <code>-Xms
   * </code> switches set to the same values. Many virtual machine implementations have additional
   * VM switches to control the behavior of the garbage collector. We suggest that you investigate
   ...
   * href="http://java.sun.com/docs/hotspot/gc/index.html">HotSpot</a> VM, the <code>
   ...
   * less than the {@link ResourceManager} critical and eviction heap thresholds.
   *
   * <p>The JRockit VM has similar flags, <code>-Xgc:gencon</code> and <code>-XXgcTrigger:N</code>,
   ...
   * 70 percent.
   *
   * <p>On the IBM VM, the flag to get a similar collector is <code>-Xgcpolicy:gencon</code>, but
   ...
   * </ul>
   *
   * <p>Only one change to this attribute or the eviction off-heap percentage will be allowed at any
   * given time and its effect will be fully realized before the next change is allowed.
   *
   ...
   *
   * <p>Changing this value may cause eviction to begin immediately.
   *
   * <p>Only one change to this attribute or critical heap percentage will be allowed at any given
   * time and its effect will be fully realized before the next change is allowed.
   *
   * <p>This feature requires additional VM flags to perform properly. See {@linkplain
   ...
   * <p>Changing this value may cause eviction to begin immediately.
   *
   * <p>Only one change to this attribute or critical off-heap percentage will be allowed at any
   * given time and its effect will be fully realized before the next change is allowed.
   *
   * @param offHeapPercentage a percentage of the maximum off-heap memory available
