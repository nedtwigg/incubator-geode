...
public class ServerRegionProxy extends ServerProxy implements ServerRegionDataAccess {
  ...
  public Object get(Object key, Object callbackArg, EntryEventImpl clientEvent) {
    ...
    return GetOp.execute(
        this.pool, this.region, key, callbackArg, this.pool.getPRSingleHopEnabled(), clientEvent);
  }
  ...
  /**
   * Do not call this method if the value is Delta instance. Exclicitly passing <code>
   * Operation.CREATE</code> to the <code>PutOp.execute()</code> method as the caller of this method
   * does not put Delta instances as value.
   *
   * @param key
   * @param value
   * @param event
   * @param callbackArg
   */
  public Object putForMetaRegion(
      Object key,
      Object value,
      byte[] deltaBytes,
  ...
  public Object put(
      ...
      boolean isCreate) {
    recordTXOperation(
        ServerRegionOperation.PUT,
        key,
        value,
        deltaBytes,
        event.getEventId(),
        op,
        Boolean.valueOf(requireOldValue),
        expectedOldValue,
        callbackArg,
        Boolean.valueOf(isCreate));
    Operation operation = op;
    if (!isCreate
        && this.region.getDataPolicy() == DataPolicy.EMPTY
        && op.isCreate()
   ...
   */
  public void putOnForTestsOnly(
      Connection con, Object key, Object value, EventID eventId, Object callbackArg) {
  ...
  public Object destroy(
    ...
    recordTXOperation(
        ServerRegionOperation.DESTROY,
        key,
        expectedOldValue,
        operation,
        event.getEventId(),
        callbackArg);
    return DestroyOp.execute(
        this.pool,
        this.region,
        key,
        expectedOldValue,
   ...
   * @param callbackArg an optional callback arg to pass to any cache callbacks
   */
  public void destroyOnForTestsOnly(
      Connection con,
      Object key,
      Object expectedOldValue,
      Operation operation,
      ...
      Object callbackArg) {
    DestroyOp.execute(
        con, this.pool, this.regionName, key, expectedOldValue, operation, event, callbackArg);
  }
  ...
  public List registerInterest(
    ...
    if (interestType == InterestType.KEY && key instanceof List) {
      ...
      try {
        ...
        rit.addSingleInterest(
            this.region, key, interestType, policy, isDurable, receiveUpdatesAsInvalidates);
        result =
            RegisterInterestOp.execute(
                this.pool,
                this.regionName,
                key,
                interestType,
        ...
        if (!finished) {
          rit.removeSingleInterest(
              this.region, key, interestType, isDurable, receiveUpdatesAsInvalidates);
        }
      }
    }
  }
  ...
  public void addSingleInterest(
    ...
    try {
      rit.addSingleInterest(
          this.region, key, interestType, pol, isDurable, receiveUpdatesAsInvalidates);
      finished = true;
    } finally {
      if (!finished) {
        rit.removeSingleInterest(
  ...
  public void addListInterest(
      List keys, InterestResultPolicy pol, boolean isDurable, boolean receiveUpdatesAsInvalidates) {
    RegisterInterestTracker rit = this.pool.getRITracker();
    boolean finished = false;
    try {
  ...
  /** Support for server-side interest registration */
  public void removeSingleInterest(
      Object key, int interestType, boolean isDurable, boolean receiveUpdatesAsInvalidates) {
    this.pool
        .getRITracker()
  ...
  public void removeListInterest(
      List keys, boolean isDurable, boolean receiveUpdatesAsInvalidates) {
    this.pool
        .getRITracker()
        .removeInterestList(this.region, keys, isDurable, receiveUpdatesAsInvalidates);
  }
   ...
   * @return list of keys
   */
  public List registerInterestOn(
      ServerLocation sl,
      final Object key,
      final int interestType,
      final InterestResultPolicy policy,
  ...
  public List registerInterestOn(
      ...
      final byte regionDataPolicy) {
    if (interestType == InterestType.KEY && key instanceof List) {
      return RegisterInterestListOp.executeOn(
          sl,
          this.pool,
          this.regionName,
          (List) key,
      ...
      return RegisterInterestOp.executeOn(
          sl,
          this.pool,
          this.regionName,
          key,
   ...
   * @return list of keys
   */
  public List registerInterestList(
      List keys,
    ...
    try {
      ...
      result =
          RegisterInterestListOp.execute(
              this.pool,
              this.regionName,
              keys,
              policy,
  ...
  public void unregisterInterest(
      Object key, int interestType, boolean isClosing, boolean keepAlive) {
    if (interestType == InterestType.KEY && key instanceof List) {
      ...
      if (removed) {
        UnregisterInterestOp.execute(
            this.pool, this.regionName, key, interestType, isClosing, keepAlive);
      }
    }
  }
  ...
  @Override
  public VersionedObjectList putAll(
      Map map, EventID eventId, boolean skipCallbacks, Object callbackArg) {
    ...
    if (this.pool.getPRSingleHopEnabled() && (txID == TXManagerImpl.NOTX)) {
      return PutAllOp.execute(
          this.pool,
          this.region,
          map,
          eventId,
          skipCallbacks,
          this.pool.getRetryAttempts(),
          callbackArg);
    } else {
      return PutAllOp.execute(
          this.pool, this.region, map, eventId, skipCallbacks, false, callbackArg);
    }
  }

  @Override
  public VersionedObjectList removeAll(
      Collection<Object> keys, EventID eventId, Object callbackArg) {
    recordTXOperation(ServerRegionOperation.REMOVE_ALL, null, keys, eventId);
    int txID = TXManagerImpl.getCurrentTXUniqueId();
    if (this.pool.getPRSingleHopEnabled() && (txID == TXManagerImpl.NOTX)) {
      return RemoveAllOp.execute(
          this.pool, this.region, keys, eventId, this.pool.getRetryAttempts(), callbackArg);
    } else {
  ...
  public void executeFunction(
    ...
    recordTXOperation(
        ServerRegionOperation.EXECUTE_FUNCTION,
        null,
        Integer.valueOf(1),
        function,
      ...
      if (cms.isMetadataStable()) {
        if (serverRegionExecutor.getFilter().isEmpty()) {
          ...
          Map<ServerLocation, HashSet> serverToFilterMap =
              cms.getServerToFilterMap(
                  serverRegionExecutor.getFilter(),
                  region,
  ...
  public void executeFunctionNoAck(
      String rgnName,
      Function function,
      ServerRegionFunctionExecutor serverRegionExecutor,
      byte hasResult,
    ...
    recordTXOperation(
        ...
        serverRegionExecutor,
        Byte.valueOf(hasResult));
    ExecuteRegionFunctionNoAckOp.execute(
        this.pool, rgnName, function, serverRegionExecutor, hasResult);
  }
  ...
  public void executeFunctionNoAck(
    ...
    recordTXOperation(
        ...
        Byte.valueOf(hasResult));
    ExecuteRegionFunctionNoAckOp.execute(
        this.pool, rgnName, functionId, serverRegionExecutor, hasResult, isHA, optimizeForWrite);
  }
  ...
  public void beforeCompletion(int txId) {
    TXSynchronizationOp.execute(
        pool, 0, txId, TXSynchronizationOp.CompletionType.BEFORE_COMPLETION);
  }
  ...
  public TXCommitMessage afterCompletion(int status, int txId) {
    return TXSynchronizationOp.execute(
        pool, status, txId, TXSynchronizationOp.CompletionType.AFTER_COMPLETION);
  }
