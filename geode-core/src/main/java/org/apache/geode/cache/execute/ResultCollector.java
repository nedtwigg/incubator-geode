 ...
 * to the result using a custom ResultCollector. Results arrive as they are sent using the {@link
 * ResultSender#sendResult(Object)} and can be used as they arrive. To indicate that all results
 * have been received {@link #endResults()} is called. Example: <br>
 *
 * <pre>
 *  Region region ;
...
public interface ResultCollector<T, S> {
   ...
   * ResultCollector} Unless the {@link ResultCollector} has received {@link
   * ResultSender#lastResult(Object) last result} from all the executing nodes, it keeps waiting for
