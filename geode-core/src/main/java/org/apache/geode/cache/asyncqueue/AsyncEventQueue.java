...
public interface AsyncEventQueue {
  ...
  /**
   * Represents the maximum time interval that can elapse before a batch is sent from <code>
   * AsyncEventQueue</code>. Default batchTimeInterval is 5 ms.
   *
   * @return int
   */
  ...
  /**
   * Represents whether batch conflation is enabled for batches sent from <code>AsyncEventQueue
  ...
  /**
   * Represents if expiration destroy operations are forwarded (passed) to <code>AsyncEventListener
   * </code>.
   *
