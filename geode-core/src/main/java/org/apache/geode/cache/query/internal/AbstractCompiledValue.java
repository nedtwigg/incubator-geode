...
public abstract class AbstractCompiledValue implements CompiledValue, Filter, OQLLexerTokenTypes {
  ...
  public SelectResults filterEvaluate(ExecutionContext context, SelectResults intermediateResults)
    ...
    if (result == null || result == QueryService.UNDEFINED)
      return new ResultsBag(
          intermediateResults.getCollectionType().getElementType(), 0, context.getCachePerfStats());
    if (!(result instanceof Boolean))
      throw new TypeMismatchException(
          LocalizedStrings.AbstractCompiledValue_BOOLEAN_VALUE_EXPECTED_NOT_TYPE_0
              .toLocalizedString(result.getClass().getName()));
    ...
    if (b) {
      ...
      if (len == 1) {
        ...
        emptySet =
            context.isDistinct()
                ? new StructSet(new StructTypeImpl(fieldNames, fieldTypes))
                : new StructBag(
    ...
    for (Iterator itr = getChildren().iterator(); itr.hasNext(); ) {
      ...
      if (v == null) {
        throw new NullPointerException(
            LocalizedStrings.AbstractCompiledValue_GOT_NULL_AS_A_CHILD_FROM_0.toLocalizedString(
                this));
      }
