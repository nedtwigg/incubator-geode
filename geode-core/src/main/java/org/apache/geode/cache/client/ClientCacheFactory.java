 ...
 *
 * <p>Once the factory has been configured using its set* methods you produce a {@link ClientCache}
 * by calling the {@link #create} method. The {@link
 * org.apache.geode.distributed.ConfigurationProperties#CACHE_XML_FILE} property can be used to
 * specify a cache.xml file to initialize the cache with. The contents of this file must comply with
 * the <code>"doc-files/cache8_0.dtd"</code> file and the top level element must be a <code>
 * client-cache</code> element.
 *
 * <p>Client connections are managed through connection {@link Pool pools}. ClientCacheFactory
 * creates a single pool to use by default on the cache it creates. ClientCacheFactory can also be
 * used to configure the default connection pool using its <code>setPool*</code> and <code>addPool*
 * </code> methods. In most cases, the defaults used by this implementation will suffice. For the
 * default pool attributes see {@link PoolFactory}. If no pool is configured and a pool was not
 * declared in cache.xml or created using {@link PoolManager} then a default one will be created
 * that connects to a server on the default cache server port and local host. If multiple pools are
 * declared in cache.xml or created by the PoolFactory then no default pool will exist and <code>
 * ClientRegionFactory.setPoolName</code> will need to be called on each region created.
 *
 * <p>To get the existing unclosed singleton client cache instance call {@link #getAnyInstance}.
 *
 * <p>The following examples illustrate bootstrapping the client cache using region shortcuts:
 *
 * <p>Example 1: Connect to a CacheServer on the default host and port and access a region
 * "customers"
 *
 * <PRE>
 ...
 * Applications that need to explicitly control the individual region attributes can do this
 * declaratively in XML or using API.
 *
 * <p>Example 5: Define custom region attributes for persistence in XML and create region using API.
...
public class ClientCacheFactory {
   ...
   *     using the {@link #set} method For a full list of valid gemfire properties see {@link
   ...
   *
   * <p>While creating the cache instance any declarative cache configuration (cache.xml) is
   * processed and used to initialize the created cache.
   *
   * <p>Note that the cache that is produced is a singleton. Before a different instance can be
   ...
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>connectionLifetime</code> is less than <code>-1
   *     </code>.
   */
   ...
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>bufferSize</code> is less than or equal to <code>0
   *     </code>.
   */
   ...
   *
   * <p>If <code>false</code> then connections are returned to the pool as soon as the operation
   * being done with the connection completes. This allows connections to be shared amonst multiple
   ...
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>maxConnections</code> is less than <code>
   *     minConnections</code>.
   ...
   *
   * <p>These pings are used by the server to monitor the health of the client. Make sure that the
   ...
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>pingInterval</code> is less than or equal to <code>0
   *     </code>.
   ...
   * monitor clients.
   *
   * <p>A value of <code>-1</code> disables the sending of client statistics to the server.
   *
   * @param statisticInterval The amount of time in milliseconds between sends of client statistics
   *     to the server.
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>statisticInterval</code> is less than <code>-1</code>
   *     .
   */
   ...
   *
   * @param group the server group that this pool will connect to. If <code>null</code> or <code>""
   *     </code> then all servers will be connected to.
   * @return a reference to <code>this</code>
   */
   ...
   * @return a reference to <code>this</code>
   * @throws IllegalArgumentException if <code>ackInterval</code> is less than or equal to <code>0
   *     </code>.
   */
   ...
   * location of partitions on servers hosting {@link Region regions} with {@link
   * org.apache.geode.cache.DataPolicy#PARTITION}. Using this information, the client routes the
   * client cache operations directly to the server which is hosting the required partition for the
   * cache operation using a single network hop. This mode works best when {@link
   * #setPoolMaxConnections(int)} is set to <code>-1</code> which is the default. This mode causes
   * the client to have more connections to the servers.
   *
   * <p>If setPRSingleHopEnabled is <code>false</code> the client may need to do an extra network
   * hop on servers to go to the required partition for that cache operation. The client will use
   * fewer network connections to the servers.
   *
   * <p>Caution: for {@link org.apache.geode.cache.DataPolicy#PARTITION partition} regions with
  ...
  /**
   * Gets an arbitrary open instance of {@link ClientCache} produced by an earlier call to {@link
   * #create}.
   *
   * @throws CacheClosedException if a cache has not been created or the only created one is {@link
   *     ClientCache#isClosed closed}
   ...
   *
   * <p>Note that a PdxInstance is only returned if a serialized PDX is found in the cache. If the
   ...
   * that persistence.
   *
   * <p>If not set, the metadata will go in the default disk store.
   *
   ...
   *
   * <p>You should only set this attribute to <code>true</code> if you know this member will only be
