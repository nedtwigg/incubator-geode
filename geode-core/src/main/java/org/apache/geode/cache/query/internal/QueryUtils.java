...
public class QueryUtils {
   ...
   * c1 or c2.
   */
  public static SelectResults intersection(
      SelectResults c1, SelectResults c2, ExecutionContext contextOrNull) {
   ...
   * c2.
   */
  public static SelectResults union(
      SelectResults c1, SelectResults c2, ExecutionContext contextOrNull) {
  ...
  public static SelectResults createResultCollection(
      ExecutionContext context, ObjectType elementType) {
    return context.isDistinct()
        ? new ResultsSet(elementType)
        : new ResultsBag(elementType, context.getCachePerfStats());
  }

  public static SelectResults createStructCollection(
      ExecutionContext context, StructType elementType) {
    return context.isDistinct()
        ? new StructSet(elementType)
        : new StructBag(elementType, context.getCachePerfStats());
  }

  public static SelectResults createResultCollection(
      boolean distinct, ObjectType elementType, ExecutionContext context) {
    return distinct
        ? new ResultsSet(elementType)
        : new ResultsBag(elementType, context.getCachePerfStats());
  }
  ...
  public static SelectResults createStructCollection(
      boolean distinct, StructType elementType, ExecutionContext context) {
    return distinct
        ? new StructSet(elementType)
        : new StructBag(elementType, context.getCachePerfStats());
  }
   ...
   */
  public static SelectResults getEmptySelectResults(
      ObjectType objectType, CachePerfStats statsOrNull) {
   ...
   */
  public static SelectResults getEmptySelectResults(
      CollectionType collectionType, CachePerfStats statsOrNull) {
    SelectResults emptyResults = null;
  ...
  private static SelectResults sizeSortedIntersection(
      SelectResults small, SelectResults large, ExecutionContext contextOrNull) {
    ...
    if (smallModifiable) {
      try {
        for (Iterator itr = small.iterator(); itr.hasNext(); ) {
          ...
          if (small.occurrences(element)
              > count) { // bag intersection: only retain smaller number of dups
            itr.remove();
          }
        }
        return small;
    ...
    if (contextOrNull != null) {
      rs =
          contextOrNull.isDistinct()
              ? new ResultsSet(small)
  ...
  private static SelectResults sizeSortedUnion(
      SelectResults small, SelectResults large, ExecutionContext contextOrNull) {
    // if one is a set and one is a bag,
    // then treat the set like a bag (and return a bag)
    boolean smallModifiable = small.isModifiable() && (isBag(small) || !isBag(large));
   ...
   *     RuntimeIterator should be equal to the size of array of SelectResults object passed and the
   *     number of RuntimeIterators in each row should be equal to the number of fields in the
   *     SelectResults object . The SelectResults object itself may be a ResultBag object or a
   *     StructBag object.
   ...
   *     present in the resultant SelectResults and their relative positions. The Runtime Iterators
   *     present in the List should be either available in the expansion List or should be present
   *     in each row of the two dimensional RuntimeIterator array.
   ...
   */
  public static SelectResults cartesian(
      SelectResults[] results,
      RuntimeIterator[][] itrsForResultFields,
      List expansionList,
      List finalList,
    ...
    doNestedIterations(
        ...
        finalList,
        expnItr,
        (results.length + expansionList.size()),
        context,
        operand);
    return returnSet;
  }
  ...
  private static void doNestedIterations(
    ...
    if (level == finalLevel) {
      ...
      if (len > 1) {
        ...
        if (select) returnSet.add(((RuntimeIterator) itr.next()).evaluate(context));
      }
  ...
  private static void mergeRelationshipIndexResultsWithIntermediateResults(
    ...
    for (int j = 0; j < resultSize; ++j) {
      if (setIndexFieldValuesInRespectiveIterators(
          indexResults[level][j], indexFieldToItrsMapping[level], icdeh[level])) {
        if (level == (indexResults.length - 1)) {
          ...
          doNestedIterations(
              ...
              returnSet,
              intermediateResults,
              itrsForIntermediateResults,
              finalItrs,
              expansionListItr,
              maxExpnCartesianDepth,
              context,
              iterOps);
        } else {
          mergeRelationshipIndexResultsWithIntermediateResults(
              returnSet,
              intermediateResults,
              itrsForIntermediateResults,
              indexResults,
  ...
  private static void mergeAndExpandCutDownRelationshipIndexResults(
      ...
      RuntimeIterator[][] indexFieldToItrsMapping,
      ListIterator expansionListIterator,
      List finalItrs,
      ExecutionContext context,
      List[] checkList,
      CompiledValue iterOps,
    ...
    for (int j = 0; j < resultSize; ++j) {
      ...
      if (setIndexFieldValuesInRespectiveIterators(
          values[level][j], indexFieldToItrsMapping[level], icdeh[level])) {
        if (level == (values.length - 1)) {
          doNestedIterationsForIndex(
              expansionListIterator.hasNext(),
              result,
              finalItrs,
              expansionListIterator,
              context,
              iterOps,
              limit,
              null);
          ...
          mergeAndExpandCutDownRelationshipIndexResults(
              values,
              result,
              indexFieldToItrsMapping,
              expansionListIterator,
  ...
  private static boolean setIndexFieldValuesInRespectiveIterators(
      Object value, RuntimeIterator[] indexFieldToItrsMapping, IndexCutDownExpansionHelper icdeh) {
    Object[] checkFields = null;
    boolean select = true;
  ...
  private static SelectResults cutDownAndExpandIndexResults(
    ...
    if (finalItrs.size() == 1) {
      ...
      if (useLinkedDataStructure) {
        returnSet =
            context.isDistinct()
                ? new LinkedResultSet(resultType)
      ...
      if (useLinkedDataStructure) {
        returnSet =
            context.isDistinct()
                ? new LinkedStructSet(resultType)
  ...
  private static void cutDownAndExpandIndexResults(
    ...
    while (itr.hasNext()) {
      ...
      if (setIndexFieldValuesInRespectiveIterators(
          value,
          indexFieldToItrsMapping,
          icdeh)) { //does that mean we don't get dupes even if they exist in the index?
        //         DO NESTED LOOPING
        if (IndexManager.JOIN_OPTIMIZATION) {
  ...
  private static int getLimitValue(ExecutionContext context) {
    ...
    if (context.cacheGet(CompiledValue.ORDERBY_ATTRIB) == null) {
      limit =
          ((Integer) context.cacheGet(CompiledValue.RESULT_LIMIT)) != null
              ? ((Integer) context.cacheGet(CompiledValue.RESULT_LIMIT)).intValue()
              : -1;
    }
    return limit;
  }
  ...
  //Add comments
  private static void doNestedIterationsForIndex(
      ...
      CompiledValue iterOps,
      int limit,
      Map<String, SelectResults> derivedResults)
      throws FunctionDomainException, TypeMismatchException, NameResolutionException,
          QueryInvocationTargetException {
    ...
    if (!continueRecursion) {
      ...
      if (len > 1) {
        ...
        if (select) resultSet.add(((RuntimeIterator) itr.next()).evaluate(context));
      }
    } else {
      ...
      if (currentLevel.getCmpIteratorDefn().getCollectionExpr().getType()
          == OQLLexerTokenTypes.RegionPath) {
        key = currentLevel.getCmpIteratorDefn().getName() + ":" + currentLevel.getDefinition();
      } else if (currentLevel.getCmpIteratorDefn().getCollectionExpr().getType()
          == OQLLexerTokenTypes.LITERAL_select) {
        useDerivedResults = false;
      } else {
        key =
            getCompiledIdFromPath(currentLevel.getCmpIteratorDefn().getCollectionExpr()).getId()
                + ":"
   ...
   * @throws TypeMismatchException
   */
  static IndexData[] getRelationshipIndexIfAny(
      CompiledValue lhs, CompiledValue rhs, ExecutionContext context, int operator)
      throws AmbiguousNameException, TypeMismatchException, NameResolutionException {
    if (operator != OQLLexerTokenTypes.TOK_EQ) return null; // Operator must be
    // '='
    IndexData lhsIndxData =
        QueryUtils.getAvailableIndexIfAny(
            lhs, context, false /* Do not use PrimaryKey Index */); // findOnlyFunctionalIndex.
    if (lhsIndxData == null) return null;
    IndexData rhsIndxData =
        QueryUtils.getAvailableIndexIfAny(
            rhs, context, false /* Do not use PrimaryKey Index */); // findOnlyFunctionalIndex.
    if (rhsIndxData == null) {
  ...
  private static IndexData getAvailableIndexIfAny(
      CompiledValue cv, ExecutionContext context, boolean usePrimaryIndex)
      throws AmbiguousNameException, TypeMismatchException, NameResolutionException {
    Set set = new HashSet();
    ...
    IndexData indexData =
        IndexUtils.findIndex(
            regionPath, definitions, cv, "*", context.getCache(), usePrimaryIndex, context);
    if (indexData != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Indexed expression for indexed data : {}  for region : {}",
   ...
   *     The conditioned Index Resultset will be created as per the chain of dependent iterators for
   *     each group.
   ...
   * @throws QueryInvocationTargetException
   */
  static SelectResults getconditionedIndexResults(
      SelectResults indexResults,
      IndexInfo indexInfo,
      ExecutionContext context,
      int indexFieldsSize,
    ...
    if (!completeExpansion && grpIndpndntItr != null && grpIndpndntItr.length > 1) {
      ...
      // expansion is false
      IndexConditioningHelper ich =
          new IndexConditioningHelper(
              indexInfo, context, indexFieldsSize, completeExpansion, iterOperands, null);
      ...
      try {
        ...
        indexResults =
            QueryUtils.cutDownAndExpandIndexResults(
                indexResults,
                ich.indexFieldToItrsMapping,
                ich.expansionList,
                ich.finalList,
                context,
                ich.checkList,
   ...
   *     This can exist only if instead of AllGroupJunction we have a single CompositeGroupJunction
  ...
  static SelectResults getRelationshipIndexResultsMergedWithIntermediateResults(
      ...
      IndexInfo[] indxInfo,
      ExecutionContext context,
      boolean completeExpansionNeeded,
      CompiledValue iterOperands,
      RuntimeIterator[] indpdntItrs)
      throws FunctionDomainException, TypeMismatchException, NameResolutionException,
          QueryInvocationTargetException {
    ObjectType resultType1 = indxInfo[0]._index.getResultSetType();
    int indexFieldsSize1 =
        resultType1 instanceof StructType
            ? ((StructTypeImpl) resultType1).getFieldNames().length
            : 1;
    ObjectType resultType2 = indxInfo[1]._index.getResultSetType();
    int indexFieldsSize2 =
        resultType2 instanceof StructType
            ? ((StructTypeImpl) resultType2).getFieldNames().length
            : 1;
    /*
    ...
    if (noOfIndexesToUse == 2) {
      ...
      IndexCutDownExpansionHelper[] icdeh =
          new IndexCutDownExpansionHelper[] {
      ...
      if (dataItr.hasNext()) {
        ...
        try {
          observer.beforeMergeJoinOfDoubleIndexResults(
              indxInfo[0]._index, indxInfo[1]._index, data);
          ...
          while (dataItr.hasNext()) {
            ...
            if (doMergeWithIntermediateResults) {
              mergeRelationshipIndexResultsWithIntermediateResults(
                  returnSet,
                  new SelectResults[] {intermediateResults},
                  resultMappings,
                  values,
                  mappings,
                  expansionListIterator,
      ...
      try {
        ...
        while (intrmdtRsItr.hasNext()) {
          ...
          cutDownAndExpandIndexResults(
              returnSet,
              singlUsblIndxRes,
              singleUsableICH.indexFieldToItrsMapping,
              totalExpList,
              finalList,
              context,
              singleUsableICH.checkList,
              iterOperands,
              singleUsableICH.indxInfo);
      ...
      if (logger.isDebugEnabled()) {
        ...
        logger.debug(
            "For better performance indexes are not used for the condition {} = {}",
            tempBuffLhs,
            tempBuffRhs);
      }
      CompiledValue reconstructedVal =
          new CompiledComparison(
      ...
      if (iterOperands != null) {
        ...
        finalVal =
            new CompiledJunction(
                new CompiledValue[] {iterOperands, reconstructedVal},
   ...
   *     give us the set of tuples satisfying the join criteria. Each element of the row of Object
  ...
  static SelectResults getconditionedRelationshipIndexResultsExpandedToTopOrCGJLevel(
    ...
    if (dataItr.hasNext()) {
      ...
      try {
        observer.beforeMergeJoinOfDoubleIndexResults(
            ich1.indxInfo._index, ich2.indxInfo._index, data);
        while (dataItr.hasNext()) {
          //TODO:Asif Change the code in range Index so that while collecting
          // data
          // instead of creating
  ...
  static SelectResults testCutDownAndExpandIndexResults(List dataList)
      throws FunctionDomainException, TypeMismatchException, NameResolutionException,
          QueryInvocationTargetException {
    return cutDownAndExpandIndexResults(
        (SelectResults) dataList.get(0),
        (RuntimeIterator[]) dataList.get(1),
        (List) dataList.get(2),
        (List) dataList.get(3),
        (ExecutionContext) dataList.get(4),
        (List) dataList.get(5),
        null,
        null);
  }
  ...
  static List queryEquijoinConditionBucketIndexes(IndexInfo[] indxInfo, ExecutionContext context)
    ...
    for (Object b : context.getBucketList()) {
      i0 =
          (pr0 != null)
              ? PartitionedIndex.getBucketIndex(pr0, index0.getName(), (Integer) b)
              : indxInfo[0]._index;
      i1 =
          (pr1 != null)
              ? PartitionedIndex.getBucketIndex(pr1, index1.getName(), (Integer) b)
              : indxInfo[1]._index;
...
class IndexConditioningHelper {
   ...
   *
   * <p>consider a index result containing 3 fields field1 field2 & field3 . Assume that field3 is
   * for cutdown. Since the expansion iterators can either be independent of all the fields in the
   * index result or at the max be dependent on field1 & field2, we should expand for a given
   * combination of field1 & field2 , only once ( as we have resulst as Set, we can only have unique
   * entries) ie. suppose a index result tuple has values ( 1,2 , 3 ) & ( 1,2,4) , we should expand
   * only once ( as field with value 3 & 4 are to be discarded).
   */
  /*
  ...
  public IndexConditioningHelper(
      IndexInfo indexInfo,
      ExecutionContext context,
      int indexFieldsSize,
      boolean completeExpansion,
    ...
    if (this.indpndntItr == null) {
      ...
      Support.Assert(
          this.indpndntItr.getScopeID()
              == context.currentScope().getScopeID() /*context.getScopeCount()*/,
    ...
    if (cutDownNeeded) {
      ...
      if (checkSize > 1) {
        ...
        if (useLinkedDataStructure) {
          checkSet =
              context.isDistinct()
                  ? new LinkedStructSet((StructTypeImpl) checkType)
