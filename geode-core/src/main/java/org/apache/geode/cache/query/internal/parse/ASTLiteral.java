...
public class ASTLiteral extends GemFireAST {
  ...
  public Long getLong(String s) throws QueryInvalidException {
    ...
    if (strip) s = s.substring(0, s.length() - 1);
    try {
  ...
  private String getString(String s, char delim) {
    ...
    for (int i = 0; i < s.length(); i++) {
      char c = s.charAt(i);
      if (c == delim && i < (s.length() - 1) && s.charAt(i + 1) == delim) i++;
      buf.append(c);
    }
    return buf.toString();
  }
  ...
  private java.sql.Timestamp getTimestamp(String s) {
    try {
      ...
      int count = 0;
      int index = -1;
      while ((index = s.indexOf('-', index + 1)) >= 0) count++;
      ...
      int second;
      if (period >= 0) second = Integer.parseInt(sec_s.substring(0, period));
      else second = Integer.parseInt(sec_s);
      if (second < 0 || second > 59)
        throw new QueryInvalidException(
