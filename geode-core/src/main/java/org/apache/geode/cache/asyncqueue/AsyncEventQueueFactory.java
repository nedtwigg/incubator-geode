...
public interface AsyncEventQueueFactory {
   ...
   * instance of <code>AsyncEventListener</code> should be thread safe in that case. The <code>
   * AsyncEventListener</code> will start receiving events when the <code>AsyncEventQueue</code> is
   * created.
   *
