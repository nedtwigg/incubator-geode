...
/**
 * This class was derived from <tt>HashMap<tt> implementation of the <tt>Map</tt> interface. This
 * implementation provides all of the optional map operations, and supports ONLY int primitive type
 * values, stored in primitive field of int type instead of promoting them to Integer and the
 * <tt>null</tt> key. The default for a value is 0. (The <tt>ObjectIntHashMap</tt> class is roughly
 * equivalent to <tt>HashMap</tt>, except that it is unsynchronized and permits nulls.) This class
 * makes no guarantees as to the order of the map; in particular, it does not guarantee that the
...
public class ObjectIntHashMap implements Cloneable, Serializable {
  ...
  public int get(Object key) {
    ...
    for (Entry e = table[indexFor(hash, table.length)]; e != null; e = e.next) {
      Object k;
      if (e.hash == hash && ((k = e.key) == key || hashingStrategy.equals(k, key))) return e.value;
    }
    return 0;
  }
  ...
  public boolean containsValue(int value) {
    ...
    for (int i = 0; i < tab.length; i++)
      for (Entry e = tab[i]; e != null; e = e.next) if (value == e.value) return true;
    return false;
  }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ObjectIntHashMap)) return false;
   ...
   *
   * <p>This implementation iterates over <tt>entrySet()</tt>, calling {@link
   * java.util.Map.Entry#hashCode() hashCode()} on each element (entry) in the set, and adding up
   * the results.
   *
   ...
   *
   * <p>Subclass overrides this to alter the behavior of IntHashMap(Map), clone, and readObject.
   */
