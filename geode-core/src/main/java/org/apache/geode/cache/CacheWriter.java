 ...
 *
 * <p>Before the region is updated via a put, create, or destroy operation, GemFire will call a
 ...
 * <code>CacheWriter</code> in the distributed system. If there are multiple <code>CacheWriter
 * </code>s available in the distributed system, the GemFire implementation always prefers one that
 * is stored locally, or else picks one arbitrarily; in any case only one <code>CacheWriter</code>
 * will be invoked.
 *
 * <p>The <code>CacheWriter</code> is capable of aborting the update to the cache by throwing a
 * <code>CacheWriterException</code>. This exception or any runtime exception thrown by the <code>
 * CacheWriter</code> will abort the operation and the exception will be propagated to the initiator
 * of the operation, regardless of whether the initiator is in the same VM as the <code>CacheWriter
 * </code>.
 *
...
public interface CacheWriter<K, V> extends CacheCallback {
   ...
   * this value comes from a <code>get</code> or not by evaluating the {@link
   * CacheEvent#getOperation() Operation}'s {@link Operation#isLoad()} method. The entry being
   * created may already exist in the local cache where this <code>CacheWriter</code> is installed,
   ...
   * cascading subregion destroys. This method is <em>not</em> called as a result of {@link
   * Region#close}, {@link Cache#close}, or {@link Region#localDestroyRegion()}. However, the {@link
   ...
   *
   * <p>WARNING: This method should not destroy or create any regions itself or a deadlock will
   * occur.
   *
   ...
   *
   * <p>WARNING: This method should not clear/destroy any regions
   *
   * @param event a RegionEvent that provides information about the operation
   * @throws CacheWriterException if thrown, will abort the operation in progress, and the exception
   *     will be propagated back to the caller that initiated the operation
