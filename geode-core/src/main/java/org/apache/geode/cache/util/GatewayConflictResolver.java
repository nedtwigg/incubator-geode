...
public interface GatewayConflictResolver {
   ...
   *
   * <p>The given GatewayConflictHelper can be used to allow the change to be made to the cache,
   * disallow the modification or make a change to the value to be stored in the cache.
   *
   * <p>This method is invoked under synchronization on the cache entry in order to prevent it from
   * concurrent modification
   *
   * <p>For any two events, all GatewayConflictResolvers must make the same decision on the
   * resolution of the conflict in order to maintain consistency. They must do so regardless of the
   * order of the events.
   *
