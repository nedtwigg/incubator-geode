 ...
 *
 * <p>Prior to release <code>6.0</code> the <code>NOT_AVAILABLE</code> constant was used to indicate
 * an object value was not available. However in <code>6.0</code> generic typing was added to {@link
 * Region} and since this constant's type will not be an instance of the generic type <code>V</code>
 * returning it would cause a ClassCastException. So in <code>6.0</code> and later <code>null</code>
 * is now used in place of <code>NOT_AVAILABLE</code>.
 *
 * @see CacheListener
 * @since GemFire 2.0
 */
public interface CacheEvent<K, V> {
   ...
   *     callback argument is not propagated to the event. This happens for events given to {@link
   *     TransactionListener} and to {@link CacheListener} on the remote side of a transaction
   *     commit.
   */
   ...
   * callback argument may have existed but it could not be obtained. Note that {@link
   * #getCallbackArgument} will return <code>null</code> when this method returns <code>false</code>
   * .
   *
   * @since GemFire 6.0
   */
   ...
   *
   * <p>This is useful to distinguish between invalidate and localInvalidate, and destroy and
   * localDestroy.
   *
