...
public interface GatewaySenderFactory {
  ...
  /**
   * Sets a time interval in milliseconds to wait to form a batch to be dispatched from a <code>
   * GatewaySender</code>'s queue. Default is 1000.
   *
   * @param interval The maximum time interval (in milliseconds) that can elapse before a partial
   *     batch is sent from a <code>GatewaySender</code> to its corresponding <code>GatewayReceiver
   *     </code>.
   */
   ...
   * false.
   *
   * @param isConflation Whether or not to enable batch conflation for batches sent from a <code>
   *     GatewaySender</code> to its corresponding <code>GatewayReceiver</code>.
   */
