...
public class ExecutionContext {
   ...
   * resolved.
   *
   * <p>SPECIAL CASE: If we are unable to resolve the name on any iterator, but there is only one
  ...
  public void addToIndependentRuntimeItrMap(CompiledIteratorDef itrDef)
    ...
    currItr.setIndexInternalID(
        (mgr == null
                || (tempIndexID = mgr.getCanonicalizedIteratorName(itrDef.genFromClause(this)))
                    == null)
            ? currItr.getInternalId()
            : tempIndexID);
  }
