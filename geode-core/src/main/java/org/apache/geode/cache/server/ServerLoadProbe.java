 ...
 *
 * <p>The getLoad method will be called once per poll interval see {@link
 * CacheServer#setLoadPollInterval(long)} The {@link ServerLoad} object returned by the getLoad
 * method will be sent to the locator every time the load for this server changes. To conserve
 * bandwidth, it's a good idea to round the load calculations so that the load will not be sent too
 * frequently.
 *
 * <p>The {@link ServerLoad} object contains two floating point numbers indicating the load on the
 ...
 *
 * <p>It is generally a good idea to pick a load function where 0 connections corresponds to 0 load.
 ...
 *
 * <p>Because cache servers can be stopped, reconfigured, and restarted, the open and close methods
 * on this callback can be called several times. If the same callback object is installed on
 * multiple cache servers, open and close will be called once for each cache server.
 *
 * @since GemFire 5.7
 */
