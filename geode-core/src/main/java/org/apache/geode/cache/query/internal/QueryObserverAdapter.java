...
public class QueryObserverAdapter implements QueryObserver {
   ...
   *     represents an equi join satisfying value ( i.e something like 1 = 1, 2 = 2 , 3 = 3 ) etc.
   *     Since multiple objects of a region can have same value , so there exits rows of Object
   *     Array for each Index.
   */
  public void beforeMergeJoinOfDoubleIndexResults(
      Index index1, Index index2, Collection initialResult) {}
   ...
   *     results where one operand uses Index Results & other operand though has an Index available
   *     but cannot be used. The value of that operand is calculated by iteration of intermediate
   *     resultset for each tuple. This function gets invoked only for an AND junction. The
   *     resultset may or may not be up to the level of top level query iterators or
   *     CompositeGroupJunction iterators level.
   */
  public void afterIterJoinOfSingleIndexResults(Collection finalResult) {}

  /**
   * @see
   *     org.apache.geode.cache.query.internal.QueryObserver#beforeIndexLookup(org.apache.geode.cache.query.Index,
