...
public class ExecuteRegionFunctionSingleHopOp {
  ...
  public static void execute(
    ...
    if (isDebugEnabled) {
      logger.debug(
          "ExecuteRegionFunctionSingleHopOp#execute : The serverToFilterMap is : {}",
          serverToFilterMap);
    }
    List<SingleHopOperationCallable> callableTasks =
        constructAndGetExecuteFunctionTasks(
            region.getFullPath(),
    ...
    reexecute =
        SingleHopClientExecutor.submitAllHA(
            callableTasks, (LocalRegion) region, function.isHA(), resultCollector, failedNodes);

    if (isDebugEnabled) {
      logger.debug(
          "ExecuteRegionFunctionSingleHopOp#execute : The size of callableTask is : {}",
          callableTasks.size());
    }
    ...
    if (reexecute) {
      ...
      if (function.isHA()) {
        ExecuteRegionFunctionOp.reexecute(
            pool,
            region.getFullPath(),
            function,
            serverRegionExecutor,
  ...
  public static void execute(
    ...
    reexecute =
        SingleHopClientExecutor.submitAllHA(
            callableTasks, (LocalRegion) region, isHA, resultCollector, failedNodes);
    ...
    for (ServerLocation server : servers) {
      ServerRegionFunctionExecutor executor =
          (ServerRegionFunctionExecutor)
              serverRegionExecutor.withFilter(serverToFilterMap.get(server));

      AbstractOp op =
          new ExecuteRegionFunctionSingleHopOpImpl(
              region, function, executor, rc, hasResult, new HashSet<String>(), allBucket);
  ...
  static class ExecuteRegionFunctionSingleHopOpImpl extends AbstractOp {
    ...
    public ExecuteRegionFunctionSingleHopOpImpl(
        String region,
        Function function,
        ServerRegionFunctionExecutor serverRegionExecutor,
        ResultCollector rc,
    ...
    private void addBytes(byte functionState) {
      if (GemFireCacheImpl.getClientFunctionTimeout()
          == GemFireCacheImpl.DEFAULT_CLIENT_FUNCTION_TIMEOUT) {
        ...
        switch (executeFunctionResponseMsg.getMessageType()) {
              ...
              if (result instanceof FunctionException) {
                ...
                if (ex instanceof InternalFunctionException) {
                  ...
                  FunctionStats.getFunctionStats(
                          this.functionId, this.executor.getRegion().getSystem())
                      .incResultsReceived();
                  continue;
                } else if (((FunctionException) result).getCause()
                ...
                if (!ex.getMessage().equals("Buckets are null")) exception = ex;
          ...
          case MessageType.EXCEPTION:
            ...
            if (obj instanceof FunctionException) {
              ...
              if (((FunctionException) obj).getCause()
                  instanceof InternalFunctionInvocationTargetException) {
                InternalFunctionInvocationTargetException ifite =
                    (InternalFunctionInvocationTargetException) ex.getCause();
                this.failedNodes.addAll(ifite.getFailedNodeSet());
