 ...
 * instead.
 *
 * <p>Once the factory has been configured using its {@link #set(String, String)} method you produce
 * a {@link Cache} by calling the {@link #create()} method.
 *
 * <p>To get the existing unclosed singleton cache instance call {@link #getAnyInstance}.
 *
 * <p>If an instance of {@link DistributedSystem} already exists when this factory creates a cache,
 * that instance will be used if it is compatible with this factory.
 *
 * <p>The following examples illustrate bootstrapping the cache using region shortcuts:
 *
 * <p>Example 1: Create a cache and a replicate region named customers.
 *
 * <PRE>
 ...
 * </PRE>
 *
 * <p>For a complete list of all region shortcuts see {@link RegionShortcut}. Applications that need
 * to explicitly control the individual region attributes can do this declaratively in XML or using
 * APIs.
 *
 * @since GemFire 3.0
 */
public class CacheFactory {
   ...
   *
   * <p>The <code>system</code> can specify a <A
   * href="../distributed/DistributedSystem.html#cache-xml-file">"cache-xml-file"</a> property which
   * will cause this creation to also create the regions, objects, and attributes declared in the
   * file. The contents of the file must comply with the <code>"doc-files/cache8_0.dtd"></code>
   ...
   *
   * @param system a <code>DistributedSystem</code> obtained by calling {@link
   ...
   * @throws CacheClosedException if a cache has not been created or the created one is {@link
