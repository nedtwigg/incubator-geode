...
/**
 * Indicates that an attempt to send a distributed cache event to one or more {@link
 * MembershipAttributes#getRequiredRoles required roles} may have failed. Failure may be caused by
 * departure of one or more required roles while sending the message to them. If the region scope is
 * {@linkplain org.apache.geode.cache.Scope#DISTRIBUTED_NO_ACK DISTRIBUTED_NO_ACK} or {@linkplain
 * org.apache.geode.cache.Scope#GLOBAL GLOBAL} then failure may be caused by one or more required
 * roles not acknowledging receipt of the message.
 *
...
public class RegionDistributionException extends RegionRoleException {
   ...
   */
  public RegionDistributionException(
      String s, String regionFullPath, Set failedRoles, Throwable ex) {
