...
public interface Filter {
   ...
   *     mostly passed as null or the value is ignored except when evaluating a filter evaluatable
   *     composite condition ( equi join condition across the regions) for an AND junction inside a
   *     CompositeGroupJunction
   * @param completeExpansionNeeded A boolean which indicates whether a GroupJunction or a
   *     CompositeGroupJunction needs to be expanded to the query from clause iterator level ( i.e
   *     to the top level)
   ...
   *     present in the Select Results ( StructBag or ResultBag) so that their is consistency in the
   *     Object Type of the SelectResults which enables union or intersection to occur correctly. It
   *     also helps in identifying the mapping of SelectResults fields and their RuntimeIterators
   *     during the cartesian/expansion of results in the AllGroupJunction. The Independent For
   *     GroupJunction , it is normally a single Iterator representing the independent iterator for
   *     the Group. But when a GroupJunction is part of a CompositeGroupJunction & there exists only
   *     a single GroupJunction in it , the Array will contain more than one independent iterators
   *     for the group, with the independent iterators being that of CompositeGroupJunction. If the
   *     completeExpansion flag is true , then this will be null as in that case , the position of
   *     iterators in the Results is decided by the actual order of iterators in the Query from
   *     clause .
