...
/**
 * Class <code>ClientSession</code> represents a client connection on the server. <code>
 * ClientSessions</code> can be used from the cache server to perform interest registrations and
 * unregistrations on behalf of clients. <code>ClientSessions</code> are only available on the cache
 * server.
 *
 * <p>The ClientSession is often used in conjunction with a callback <code>EntryEvent</code> as
 * shown below.
 *
 * <pre>
 ...
 * @see
 *     org.apache.geode.cache.server.CacheServer#getClientSession(org.apache.geode.distributed.DistributedMember)
 *     getClientSession
 */
