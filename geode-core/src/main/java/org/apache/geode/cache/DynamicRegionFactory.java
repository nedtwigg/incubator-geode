 ...
 *
 * <ul>
 *   <li> If your application is a client in a client/server installation, either specify the pool
 *       name in the {@link DynamicRegionFactory.Config} you'll use to create a DynamicRegionFactory
 *       <i>or</i> specify it in a dynamic-region-factory element in your cache.xml.
 *   <li> Before you've created a GemFire Cache in your application, add a line of code as follows:
 *       <br>
 *       <pre><code>  DynamicRegionFactory factory = DynamicRegionFactory.get();
 *  factory.open(config);</code></pre>
 *       <pre><code>
 *         DynamicRegionFactory myFactoryHandle = DynamicRegionFactory.get().open(config);</code>
 *       </pre>
 *       or just use a dynamic-region-factory element in the cache.xml.
 *   <li> Create the GemFire Cache. During cache creation, the list of dynamic Regions will either
 *       be discovered by recovering their names from disk (see {@link
 ...
 *       RegionAttributes from their parent Region, though you can override callbacks when you
 *       configure the factory.
 *       <p>All other instances of GemFire across the distributed system that instantiate and open a
 *       DynamicRegionFactory will also get the dynamic distributed Regions.
 *   <li>Non-dynamic parent Regions should be declared in cache.xml so that they can be created
 *       before the dynamic Region factory goes active and starts creating Regions. You will have
 *       cache creation problems if this isn't done.
 ...
 * </ul>
 *
 * <p>Saving the factory on disk: If {@link DynamicRegionFactory.Config#persistBackup} is configured
 ...
 *
 * <p>Registering interest in cache server information: The {@link
 * DynamicRegionFactory.Config#registerInterest} setting determines whether clients will register
 * interest in server keys or not. You will generally want this to be turned on so that clients will
 * see updates made to servers. In server processes, DynamicRegionFactory forces use of
 ...
 *       dynamic-region-factory element then the cache.xml will override the open call's
 *       configuration.
 *   <li>Since the RegionAttributes of a dynamically created Region are copied from the parent
 *       Region, any callbacks, ({@link CacheListener}, {@link CacheWriter}, and {@link CacheLoader}
 *       are shared by the parent and all its dynamic children so make sure the callback is
 *       thread-safe and that its {@link CacheCallback#close} implementation does not stop it from
 *       functioning. However the products LRUAlgorithm instances will be cloned so that each
 *       dynamic Region has its own callback.
 *   <li>The root Region name "DynamicRegions" is reserved. The factory creates a root Region of
 *       that name and uses it to keep track of what dynamic Regions exist. Applications should not
 *       directly access this Region; instead use the methods on this factory.
 * </ul>
 *
 * @since GemFire 4.3
 */
...
public abstract class DynamicRegionFactory {
  ...
  public Config getConfig() {
    ...
    else return new Config(this.config);
  }
    ...
    try {
      ...
      if (this.dynamicRegionList == null) {
        ...
        if (isClient) {
          ...
          if (!this.config
              .getPersistBackup()) { // if persistBackup, the data policy has already been set
  ...
  private void doBeforeRegionCreated(
      String parentRegion, String regionName, DistributedMember mbr) {
  ...
  private void doAfterRegionCreated(
      Region region, boolean distributed, boolean isOriginRemote, DistributedMember mbr) {
    RegionEvent event =
  ...
  private void doBeforeRegionDestroyed(
      Region region,
      boolean distributed,
  ...
  private void doAfterRegionDestroyed(
      Region region,
      boolean distributed,
      boolean isOriginRemote,
      boolean expiration,
  ...
  private Region createDynamicRegionImpl(
      String parentRegionName, String newRegionName, boolean addEntry) throws CacheException {
  ...
  private void destroyDynamicRegionImpl(String fullRegionName) throws CacheException {
    ...
    try {
      ...
      c.getLoggerI18n()
          .warning(
              LocalizedStrings.DynamicRegionFactory_ERROR_DESTROYING_DYNAMIC_REGION__0,
              fullRegionName,
              e);
      throw e;
    }
  ...
  public static class Config {
    ...
    public boolean equals(Object obj) {
      ...
      if (diskDir == null) {
        if (other.diskDir != null) return false;
      } else if (!diskDir.equals(other.diskDir)) return false;
      if (persistBackup != other.persistBackup) return false;
      if (poolName == null) {
        if (other.poolName != null) return false;
      } else if (!poolName.equals(other.poolName)) return false;
      if (registerInterest != other.registerInterest) return false;
      return true;
    }
     ...
     * null if no directory has been specified. The diskDir is only used if <code>persistBackup
     * </code> is true.
     */
  ...
  protected void buildDynamicRegion(EntryEvent event) {
    if (!DynamicRegionFactory.this.isOpen()) return;
  ...
  private class LocalMetaRegion extends LocalRegion {
    ...
    public void invokeDestroyCallbacks(
        EnumListenerEvent eventType,
        EntryEventImpl event,
        boolean callDispatchEventsCallback,
