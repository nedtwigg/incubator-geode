 ...
 *
 * <p>When a cache is created a {@link DistributedSystem} is also created. This system tells the
 * cache where to find other caches on the network and how to communicate with them. The system can
 * also specify a <a
 * href="../distribution/DistributedSystem.html#cache-xml-file">"cache-xml-file"</a> property which
 * will cause this cache to be initialized with the contents of that file. The contents must comply
 * with the <code>"doc-files/cache8_0.dtd"</code> file and the top level element must be a <code>
 * cache</code> element.
 *
 * <p>When a cache will no longer be used it should be {@link #close() closed}. Once it {@link
 * #isClosed is closed} any attempt to use it or any {@link Region} obtained from it will cause a
 * {@link CacheClosedException} to be thrown.
 *
...
public interface Cache extends GemFireCache {
   ...
   *     region of the same name in another cache in the distributed system
  ...
  /**
   * Creates a {@link RegionFactory} which can be used to specify additional attributes for {@link
   * Region} creation.
   *
   ...
   * this cache.
   *
   * <p>For the default configuration see the constants in {@link
   ...
   * cache server may be an application VM or may be a stand-along VM launched using {@linkplain
   * org.apache.geode.admin.AdminDistributedSystem#addCacheServer administration API} or the <code>
   * cacheserver</code> command line utility.
   *
   * @since GemFire 4.0
   */
   ...
   *
   * <p>Durable clients must call this method after they are done creating regions. If it is called
   ...
   *
   * <p>This will also return true if the Cache has finished reconnecting. When reconnect has
   * completed you can use {@link Cache#getReconnectedCache} to retrieve the new cache instance.
   *
