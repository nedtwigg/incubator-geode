 ...
 *
 * <p>Function execution provides a means to route application behaviour to {@linkplain Region data}
 * or more generically to peers in a {@link DistributedSystem} or servers in a {@link Pool}. While
 * {@link FunctionService} is a customer facing interface to this functionality, all of the work is
 * done here. In addition, internal only functionality is exposed in this class.
 *
 * @since GemFire 7.0
 */
public class FunctionServiceManager {
   ...
   * GemFire members that define the data region, or a subset of members. {@link
   * Execution#withFilter(Set)}).
   *
   * <p>For DistributedRegions with DataPolicy.NORMAL, it throws UnsupportedOperationException. For
  ...
  public final Execution onServer(RegionService regionService, String... groups) {
    if (regionService == null) {
      throw new FunctionException(
          LocalizedStrings.FunctionService_0_PASSED_IS_NULL.toLocalizedString(
  ...
  public final Execution onMembers(DistributedSystem system, String... groups) {
    ...
    if (members.isEmpty()) {
      throw new FunctionException(
          LocalizedStrings.FunctionService_NO_MEMBERS_FOUND_IN_GROUPS.toLocalizedString(
   ...
   * @since GemFire 6.0
   */
  public final Execution onMembers(
      DistributedSystem system, Set<DistributedMember> distributedMembers) {
    if (system == null) {
      throw new FunctionException(
          LocalizedStrings.FunctionService_0_PASSED_IS_NULL.toLocalizedString(
              "DistributedSystem instance "));
  ...
  /**
   * Registers the given {@link Function} with the {@link FunctionService} using {@link
   * Function#getId()}.
   *
   * <p>Registering a function allows execution of the function using {@link
   * Execution#execute(String)}. Every member that could execute a function using its {@link
   * Function#getId()} should register the function.
   *
  ...
  /**
   * Unregisters the given {@link Function} with the {@link FunctionService} using {@link
   * Function#getId()}.
   *
   * <p>
   *
