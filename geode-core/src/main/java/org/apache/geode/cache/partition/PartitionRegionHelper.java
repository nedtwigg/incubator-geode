...
public final class PartitionRegionHelper {
  ...
  /**
   * Given a partitioned Region, return a map of {@linkplain
   * PartitionAttributesFactory#setColocatedWith(String) colocated Regions}. Given a local data
   * reference to a partitioned region, return a map of local {@linkplain
   * PartitionAttributesFactory#setColocatedWith(String) colocated Regions}. If there are no
   * colocated regions, return an empty map.
   *
  ...
  public static Map<String, Region<?, ?>> getColocatedRegions(final Region<?, ?> r) {
    ...
    if (isPartitionedRegion(r)) {
      ...
      if (fc != null) {
        ...
        ret =
            ColocationHelper.getColocatedLocalDataSetsForBuckets(
                lds.getProxy(), lds.getBucketSet());
      }
    } else {
      throw new IllegalArgumentException(
          LocalizedStrings.PartitionManager_REGION_0_IS_NOT_A_PARTITIONED_REGION.toLocalizedString(
              r.getFullPath()));
    }
  ...
  public static PartitionRegionInfo getPartitionRegionInfo(final Region<?, ?> region) {
    try {
      ...
      return pr.getRedundancyProvider()
          .buildPartitionedRegionInfo(
  ...
  private static void fillInPartitionedRegionInfo(
      GemFireCacheImpl cache, final Set prDetailsSet, final boolean internal) {
   ...
   *
   * <p>This method will block until all buckets are assigned.
   *
   * @param region The region which should have it's buckets assigned.
   * @throws IllegalStateException if the provided region is something other than a {@linkplain
  ...
  private static int getNumberOfBuckets(PartitionedRegion pr) {
    if (pr.isFixedPartitionedRegion()) {
      int numBuckets = 0;
      Set<FixedPartitionAttributesImpl> fpaSet =
          new HashSet<FixedPartitionAttributesImpl>(
   ...
   * @since GemFire 6.0
   */
  public static <K, V> Set<DistributedMember> getRedundantMembersForKey(
      final Region<K, V> r, final K key) {
    DistributedMember primary = getPrimaryMemberForKey(r, key);
   ...
   * @since GemFire 6.0
   */
  public static <K, V> Set<DistributedMember> getAllMembersForKey(
      final Region<K, V> r, final K key) {
    return Collections.unmodifiableSet(getAllForKey(r, key));
  }

  private static <K, V> Set<? extends DistributedMember> getAllForKey(
      final Region<K, V> r, final K key) {
    PartitionedRegion pr = isPartitionedCheck(r);
    int bucketId = PartitionedRegionHelper.getHashKey(pr, null, key, null, null);
   ...
   *
   * <p>Writes using these Region have no constraints and behave the same as a partitioned Region.
   *
   * <p>If there are no colocated regions, return an empty map.
   *
   ...
   * Returned Region provides only one copy of the data although {@link
   * PartitionAttributes#getRedundantCopies() redundantCopies} configured is more than 0. If the
   * invoking Function is configured to have {@link Function#optimizeForWrite() optimizeForWrite} as
   * true,the returned Region will only contain primary copy of the data.
   *
   * <p>Writes using this Region have no constraints and behave the same as a partitioned Region.
   *
   ...
   * an exception.
   *
   * <p>All keys which exist in the same bucket will also be moved to the new node.
   *
   * <p>Any data in colocated regions that are colocated with this key will also be moved.
   *
   * <p>This method allows direct control of what data to move. To automatically balance buckets,
   * see {@link ResourceManager#createRebalanceFactory()}
   *
   ...
   *     the region, but if using a {@link PartitionResolver} the resolver should be able to get the
   ...
   *     destination are not valid members of the system, if the destination already hosts a copy of
   *     the bucket, or if the bucket does not exist.
   * @since GemFire 7.1
   */
  public static <K> void moveBucketByKey(
      Region<K, ?> region, DistributedMember source, DistributedMember destination, K key) {
    PartitionedRegion pr = isPartitionedCheck(region);
    if (pr.isFixedPartitionedRegion()) {
      throw new IllegalStateException("Cannot move data in a fixed partitioned region");
    }
    int bucketId = pr.getKeyInfo(key).getBucketId();
    ExplicitMoveDirector director =
        new ExplicitMoveDirector(
            key, bucketId, source, destination, region.getCache().getDistributedSystem());
   ...
   *
   * <p>If this region has colocated regions, the colocated data will also be moved. The total
   * amount of data in all colocated regions will be taken into consideration when determining what
   * percentage of data will be moved.
   *
   * <p>It may not be possible to move data to the destination member, if the destination member has
   * no available space, no bucket smaller than the given percentage exists, or if moving data would
   ...
   * @since GemFire 7.1
   */
  public static RebalanceResults moveData(
      Region<?, ?> region,
