...
public interface CacheServer {
  ...
  /**
   * The default maximum amount of time between client pings. This value is used by the <code>
   * ClientHealthMonitor</code> to determine the health of this <code>CacheServer</code>'s clients.
   */
   ...
   *
   * <p>Setting a specific bind address will cause the cache server to always use this address and
   * ignore any address specified by "server-bind-address" or "bind-address" in the <code>
   * gemfire.properties</code> file (see {@link org.apache.geode.distributed.DistributedSystem} for
   * a description of these properties).
   *
   * <p>The value <code>""</code> does not override the <code>gemfire.properties</code>. It will
   * cause the local machine's default address to be listened on if the properties file does not
   * specify and address. If you wish to override the properties and want to have your cache server
   ...
   * Sets the ip address or host name that this cache server is to listen on for client connections.
   *
   * <p>Setting a specific hostname-for-clients will cause server locators to use this value when
   ...
   *
   * <p>If false, then an update to any key on the cache server causes an update to be sent to all
   ...
   *
   * <p>If true, then an update to any key on the cache server causes an update to be sent to only
  ...
  /**
   * Sets the maximum amount of time between client pings. This value is used by the <code>
   * ClientHealthMonitor</code> to determine the health of this <code>CacheServer</code>'s clients.
  ...
  /**
   * Returns the maximum amount of time between client pings. This value is used by the <code>
   * ClientHealthMonitor</code> to determine the health of this <code>CacheServer</code>'s clients.
   * The default is 60000 ms.
   *
   ...
   *     Modifying this array will not change the server groups that this cache server belongs to.
   * @since GemFire 5.7
   * @deprecated as of 7.0 use the groups gemfire property
   */
