 ...
 *
 * <p>A ClientCache is created using {@link ClientCacheFactory#create}. See {@link
 * ClientCacheFactory} for common usage patterns for creating the client cache instance.
 *
 * <p>ClientCache provides access to functionality when a member connects as a client to GemFire
 ...
 *   <li> Access the query service (see {@link #getQueryService} and {@link #getLocalQueryService}).
 *   <li> Access the GemFire logger (see {@link #getLogger}).
 *   <li> Access the GemFire distributed system (see {@link #getDistributedSystem}).
 *   <li> Access the GemFire resource manager (see {@link #getResourceManager}).
 *   <li> Manages local disk stores for this cache instance (see {@link #createDiskStoreFactory}).
 *   <li> Creation of authenticated cache views that support multiple users (see {@link
 *       #createAuthenticatedView}).
 * </ul>
 *
 * <p>A ClientCache connects to a server using a {@link Pool}. This pool can be configured in the
...
public interface ClientCache extends GemFireCache {
   ...
   *
   * <p>Named region attributes are defined in cache.xml by setting the name as the value of the
   ...
   *
   * <p>Durable clients must call this method after they are done creating regions and issuing
   * interest registration requests.If it is called before then events will be lost.Any time a new
   ...
   * client cache.
   *
   * <p>Requires that {@link ClientCacheFactory#setPoolMultiuserAuthentication(boolean)
   * multiuser-authentication} to be set to true on the default pool.
   *
   * <p>Applications must use this instance to do operations, when multiuser-authentication is set
   * to true.
   *
   * <p>Authenticated cache views are only allows to access {@link ClientRegionShortcut#PROXY proxy}
   ...
   * pool to connect to servers. Requires that {@link
   * PoolFactory#setMultiuserAuthentication(boolean) multiuser-authentication} to be set to true on
   * the given pool.
   *
   * <p>See {@link #createAuthenticatedView(Properties)} for more information on the returned cache
   * view.
   *
   * @param userSecurityProperties the security properties of a user.
