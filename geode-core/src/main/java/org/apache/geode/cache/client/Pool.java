 ...
 * instance is created by calling {@link PoolManager#createFactory}. So to create a default <code>
 * Pool</code> do this:
 *
 * <PRE>
 ...
 *
 * <p>Existing Pool instances can be found using {@link PoolManager#find(String)} and {@link
 * PoolManager#getAll}.
 *
 * <p>The pool name must be configured on the client regions that will use this pool by calling
...
public interface Pool {
   ...
   * other threads.
   *
   * <p>If this pool is not using thread local connections, this method will have no effect.
   */
   ...
   * from it.
   *
   * <p>Durable clients can call this method on reconnect to assess the amount of 'stale' data i.e.
   * events accumulated at server while this client was away and, importantly, before calling {@link
   * ClientCache#readyForEvents()}.
   *
   * <p>Any number of invocations of this method during a single session will return the same value.
   *
   * <p>It may return a zero value if there are no events pending at server for this client pool. A
   * negative value returned tells us that no queue was available at server for this client pool.
   *
   * <p>A value -1 indicates that this client pool reconnected to server after its
   * 'durable-client-timeout' period elapsed and hence its subscription queue at server was removed,
   * possibly causing data loss.
   *
   * <p>A value -2 indicates that this client pool connected to server for the first time.
   *
   * @return int The number of subscription events maintained at server for this durable client pool
   *     at the time this pool (re)connected. A negative value indicates no queue was found for this
   *     client pool.
