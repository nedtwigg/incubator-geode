...
public interface CacheStatistics {
   ...
   *
   * <p>The number is expressed as the number of milliseconds since January 1, 1970. The granularity
   * may be as course as 100ms, so the accuracy may be off by up to 50ms.
   *
   * <p>Entry and subregion creation will update the modification time on a region, but <code>
   * destroy</code>, <code>destroyRegion</code>, <code>invalidate</code>, and <code>invalidateRegion
   * </code> do not update the modification time.
   *
   ...
   * the last time any of its entries or the entries of its subregions were accessed with <code>
   * Region.get</code>. Any modifications will also update the lastAccessedTime, so <code>
   * lastAccessedTime</code> is always <code>>= lastModifiedTime</code>. The <code>lastAccessedTime
   * </code> on a region is propagated upward to parent regions, transitively, to the the root
   * region.
   *
   * <p>The number is expressed as the number of milliseconds since January 1, 1970. The granularity
   * may be as course as 100ms, so the accuracy may be off by up to 50ms.
   ...
   * to {@link Region#get(Object) Region.get}. If there have been zero calls to <code>Region.get
   * </code>, then zero is returned.
   *
   * <p>The hit ratio is equivalent to:
   *
   * <pre>
