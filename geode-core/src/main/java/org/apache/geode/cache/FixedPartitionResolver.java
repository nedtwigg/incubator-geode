 ...
 *
 * <p>GemFire uses the partition name returned by {@link
 * FixedPartitionResolver#getPartitionName(EntryOperation, Set)} to determine on which member the
 * data is being managed. Say, for example, you want to partition all Trades according to quarters.
 * You can implement FixedPartitionResolver to get the name of the quarter based on the date given
 * as part of {@link org.apache.geode.cache.EntryOperation}. public class QuarterPartitionResolver
 * implements FixedPartitionResolver{<br>
...
public interface FixedPartitionResolver<K, V> extends PartitionResolver<K, V> {
   ...
   *     will be removed in future release. Represents all the available primary partitions on the
   *     nodes.
   * @return partition-name associated with node which allows mapping of given data to user defined
   *     partition
   */
  public String getPartitionName(
      EntryOperation<K, V> opDetails, @Deprecated Set<String> targetPartitions);
}
