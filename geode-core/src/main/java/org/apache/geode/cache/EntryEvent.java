 ...
 * circumstances of the event. It is passed in to <code>CacheListener</code>, <code>
 * CapacityController</code>, and <code>CacheWriter</code>.
 *
 * <p>If this event originated from a region stored off heap then this event can only be used as
 * long as the notification method that obtained it has not returned. For example in your
 * implementation of {@link CacheListener#afterUpdate(EntryEvent)} the event parameter is only valid
...
public interface EntryEvent<K, V> extends CacheEvent<K, V> {
   ...
   *
   * <p>If this event is for a Partitioned Region, then true will be returned if the loader ran in
   * the same VM as where the data is hosted. If true is returned, and {@link
   * CacheEvent#isOriginRemote} is true, it means the data is not hosted locally, but the loader was
  ...
  /**
   * Returns true if this event resulted from a <code>netSearch</code>. If the <code>netSearch
   * </code> was invoked by a loader however, this will return false and <code>isLocalLoad()</code>
