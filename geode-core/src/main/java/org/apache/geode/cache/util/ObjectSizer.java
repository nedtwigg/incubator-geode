 ...
 *
 * <p>You should use a sizer with a {@link EvictionAttributes#createLRUHeapAttributes(ObjectSizer)}
 * or {@link EvictionAttributes#createLRUMemoryAttributes(ObjectSizer)} if you want to use a faster
 * or more accurate method of sizing than provided by the default object sizer, which is {#link
...
public interface ObjectSizer {
   ...
   *
   * <p>This sizer is a compromise between generating accurate sizes for every object and
   * performance. It should work well if the keys and values in the region don't vary greatly in
   * size. For accurate sizing of every instance use {@link #REFLECTION_SIZE} instead.
   *
   * <p>This sizer does generate an accurate size for strings and byte arrays every time, because
   * there is very little performance impact from sizing these objects.
   *
   * @since GemFire 6.5
   */
   ...
   * it sizes.
   *
   * <p>This sizer will add up the sizes of all objects that are reachable from the keys and values
   * in your region by non-static fields.
   *
   * <p>For objects that are all approximately the same size, consider using {@link
   * #SIZE_CLASS_ONCE}. It will have much better performance.
   *
   * @since GemFire 6.5
   */
