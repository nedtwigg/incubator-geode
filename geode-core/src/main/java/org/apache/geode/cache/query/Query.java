...
public interface Query {
   ...
   * returned.
   *
   * <p>Query execution can potentially take a long time depending on data size and query
   * complexity. The system property "gemfire.Cache.MAX_QUERY_EXECUTION_TIME" can be set to define
   * the maximum time allowed for a query to complete its execution. If query execution time exceeds
   ...
   *     select statement then it will return a result that is an instance of {@link SelectResults}.
   *     However, since a query is not necessarily just a select statement, the return type of this
   *     method is <code>Object</code>. For example, the query <code>
   *     <b>(select distinct * from /rgn).size</b></code> returns an instance of <code>
   *     java.lang.Integer</code>.
   * @throws FunctionDomainException A function was applied to a parameter that is improper for that
   *     function. For example, the ELEMENT function was applied to a collection of more than one
   *     element
   ...
   *     variable "gemfire.Cache.MAX_QUERY_EXECUTION_TIME". This is a thrown when query is executed
   *     on the local regions (embedded mode).
   ...
   * the local node.
   *
   * <p>The RegionFunctionContext defines the execution context of a data dependent {@link
   * Function}. And is obtained when the function is executed using {@link
   * FunctionService#onRegion(Region)}, the {@link Function#execute(FunctionContext)
   * FunctionContext}
   *
   * <p>E.g.: // Function service execute() method:
   *
   * <p>public void execute(FunctionContext context) { RegionFunctionContext regionContext =
   ...
   *
   * <p>Using this method a join query query can be executed between two co-located PartitionRegion
   * data-set referenced through {@link RegionFunctionContext}.
   *
   * <p>This method should NOT be used for multiple partitioned regions based join queries. We
   * support equi-join queries only on co-located PartitionedRegions if the the co-located columns
   * ACTUALLY EXIST IN WHERE CLAUSE and in case of multi-column partitioning , should have "AND"
   * clause.
   *
   * <p>E.g.: // Equi-join query:
   *
   * <p>Select * from /partitionRegion1 p1, /PartitionRegion2 p2 where p1.field = p2.field [AND
   * .....]
   *
   * @since GemFire 6.2.2
