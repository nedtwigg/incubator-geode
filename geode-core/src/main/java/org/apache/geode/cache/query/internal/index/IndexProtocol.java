...
public interface IndexProtocol extends Index {
   ...
   *     both should not be present in the index results being fetched ( Fetched results should not
   *     contain index results corresponding to key + keys of keysToRemove set )
   * @param context ExecutionContext object
   * @throws TypeMismatchException
   */
  void query(
   ...
   *     List represents a value (key of Range Index) which satisfies the equi join condition. The
   *     Object array will have two rows. Each row ( one dimensional Object Array ) will contain
   *     objects or type Struct or Object.
   * @throws TypeMismatchException
   * @throws QueryInvocationTargetException
   * @throws NameResolutionException
   * @throws FunctionDomainException
  ...
  public boolean isMatchingWithIndexExpression(
      CompiledValue condnExpr, String condnExprStr, ExecutionContext context)
