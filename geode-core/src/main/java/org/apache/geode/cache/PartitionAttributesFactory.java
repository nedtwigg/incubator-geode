 ...
 * </pre>
 *
 * <p>{@link PartitionAttributes} can also be defined in a declarative fashion using a <a
 * href="package-summary.html#declarative">cache.xml</a> file. Here is an example of how to
 ...
 * {@link org.apache.geode.cache.Region.Entry}, configures a {@link
 * org.apache.geode.cache.CacheLoader} implementation, and sets {@link
 * #setGlobalProperties(Properties) global properties} as well as {@link #setLocalMaxMemory(int)
...
public class PartitionAttributesFactory<K, V> {
   ...
   *     <p>The {@link #setLocalProperties(Properties) local property} name that sets the maximum
   *     heap storage a VM contributes to a partitioned Region. When set to zero, the resulting
   *     Region reference allows access to the partitioned Region without any consuming any heap
   *     storage.
   */
   ...
   */
  @Deprecated public static final int LOCAL_MAX_MEMORY_DEFAULT = computeMaxMem();

  /**
   * @deprecated - use {@link #setTotalMaxMemory(long)} instead.
   *     <p>The {@link #setGlobalProperties(Properties) global property} name that defines the total
   *     maximum size for the partitioned Region.
   *     <p><em>This setting must be the same in all processes using the Region.</em>
   */
  ...
  public static final String GLOBAL_MAX_BUCKETS_PROPERTY =
      "GLOBAL_MAX_BUCKETS"; // "GLOBAL_MAX_BUCKETS";
  ...
  /**
   * Creates a new instance of PartitionAttributesFactory ready to create a <code>
   * PartitionAttributes</code> with default settings.
   */
  public PartitionAttributesFactory() {}

  /**
   * Creates a new instance of PartitionAttributesFactory ready to create a {@link
   * PartitionAttributes} with the same settings as those in the specified {@link
   * PartitionAttributes}
   *
   ...
   * operations.
   *
   * <p><em>This setting must be the same in all processes using the Region.</em> Default number of
   ...
   *
   * <p><em>This setting must be the same in all processes using the Region.</em> The default value
   ...
   * <p><em>This setting must be the same in all processes using the Region.</em>
   *
   * <p>A bucket is the smallest unit of data management in a partitioned region. {@link
   * org.apache.geode.cache.Region.Entry Entries} are stored in buckets and buckets may move from
   * one VM to another. Buckets may also have copies, depending on {@link #setRedundantCopies(int)
   * redundancy} to provide high availability in the face of VM failure.
   *
   * <p>The number of buckets should be prime and as a rough guide at the least four times the
   * number of partition VMs. However, there is significant overhead to managing a bucket,
   * particularly for higher values of {@link #setRedundantCopies(int) redundancy}. The default
   * number of buckets for a PartitionedRegion is 113.
   */
   ...
   *
   * <p>Note that global settings must be the same in all processes using the Region.
   *
   ...
   * PartitionAttributes</code> created.
   *
  ...
  */
  ...
  /**
   * Enables/Disables local Caching for this node for region entries for the next <code>
   * PartitionAttributes</code> created. Default is true.
   *
   * @param isLocalCache
