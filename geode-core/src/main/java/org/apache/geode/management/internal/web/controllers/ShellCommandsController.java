...
public class ShellCommandsController extends AbstractCommandsController {
  ...
  public ResponseEntity<?> invoke(
    ...
    try {
      final Object result =
          getMBeanServer()
              .invoke(
                  ObjectName.getInstance(decode(resourceName)),
                  decode(operationName),
                  parameters,
                  signature);
  ...
  public LinkIndex index(
      @RequestParam(value = "scheme", required = false, defaultValue = "http")
          final String scheme) {
    ...
    return new LinkIndex()
        ...
        .add(
            new Link(
                ...
                toUri("/members/{member}/regions/{region}/data", scheme),
                HttpMethod.GET))
        .add(
            new Link(
        ...
        .add(
            new Link(
                ...
                toUri("/regions/{region}/data/location", scheme),
                HttpMethod.GET))
        .add(new Link(CliStrings.QUERY, toUri("/regions/data/query", scheme), HttpMethod.GET))
        .add(
            new Link(
                CliStrings.REBALANCE, toUri("/regions/data?op=rebalance", scheme), HttpMethod.POST))
        // Function Commands
        .add(new Link(CliStrings.LIST_FUNCTION, toUri("/functions", scheme)))
        .add(
            new Link(
                CliStrings.DESTROY_FUNCTION, toUri("/functions/{id}", scheme), HttpMethod.DELETE))
        .add(
            new Link(
                CliStrings.EXECUTE_FUNCTION, toUri("/functions/{id}", scheme), HttpMethod.POST))
        // Client Commands
        ...
        .add(
            new Link(
                CliStrings.IMPORT_SHARED_CONFIG, toUri("/config/cluster", scheme), HttpMethod.POST))
        // Deploy Commands
        ...
        .add(
            new Link(
                ...
                toUri("/diskstores?op=backup", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.COMPACT_DISK_STORE,
                toUri("/diskstores/{name}?op=compact", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                ...
                toUri("/diskstores/{name}", scheme),
                HttpMethod.DELETE))
        .add(
            new Link(
                CliStrings.REVOKE_MISSING_DISK_STORE,
                toUri("/diskstores/{id}?op=revoke", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                ...
                toUri("/durable-clients/{durable-client-id}?op=close", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.CLOSE_DURABLE_CQS,
                toUri(
                    "/durable-clients/{durable-client-id}/cqs/{durable-cq-name}?op=close", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                ...
                toUri("/groups/{groups}/loglevel", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.CHANGE_LOGLEVEL,
                toUri("/members/{members}/loglevel", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.CHANGE_LOGLEVEL,
                toUri("/members/{members}/groups/{groups}/loglevel", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                CliStrings.CREATE_ASYNC_EVENT_QUEUE,
                toUri("/async-event-queues", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                MBEAN_OPERATION_LINK_RELATION, toUri("/mbean/operation", scheme), HttpMethod.POST))
        ...
        .add(
            new Link(
                ...
                toUri("/gateways/receivers", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.CREATE_GATEWAYSENDER,
                toUri("/gateways/senders", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.LOAD_BALANCE_GATEWAYSENDER,
                toUri("/gateways/senders/{id}?op=load-balance", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.PAUSE_GATEWAYSENDER,
                toUri("/gateways/senders/{id}?op=pause", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.RESUME_GATEWAYSENDER,
                toUri("/gateways/senders/{id}?op=resume", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.START_GATEWAYRECEIVER,
                toUri("/gateways/receivers?op=start", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.START_GATEWAYSENDER,
                toUri("/gateways/senders?op=start", scheme),
                HttpMethod.POST))
        ...
        .add(
            new Link(
                ...
                toUri("/gateways/receivers?op=stop", scheme),
                HttpMethod.POST))
        .add(
            new Link(
                CliStrings.STOP_GATEWAYSENDER,
                toUri("/gateways/senders/{id}?op=stop", scheme),
                HttpMethod.POST));
  }
  ...
  public ResponseEntity<String> ping() {
    return new ResponseEntity<String>(
        "<html><body><h1>Mischief Managed!</h1></body></html>", HttpStatus.OK);
  }
