...
public class ShellCommands implements CommandMarker {
  ...
  @CliCommand(
    value = {CliStrings.EXIT, "quit"},
    help = CliStrings.EXIT__HELP
  )
  @CliMetaData(
    shellOnly = true,
  ...
  private static final int CONNECT_LOCATOR_TIMEOUT_MS = 60000; // see bug 45971
  ...
  @CliCommand(
    value = {CliStrings.CONNECT},
    help = CliStrings.CONNECT__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {
      CliStrings.TOPIC_GFSH,
  ...
  public Result connect(
      @CliOption(
            key = {CliStrings.CONNECT__LOCATOR},
            ...
            help = CliStrings.CONNECT__LOCATOR__HELP
          )
          ConnectionEndpoint locatorTcpHostPort,
      @CliOption(
            key = {CliStrings.CONNECT__JMX_MANAGER},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            optionContext = ConnectionEndpoint.JMXMANAGER_OPTION_CONTEXT,
            help = CliStrings.CONNECT__JMX_MANAGER__HELP
          )
          ConnectionEndpoint memberRmiHostPort,
      @CliOption(
            key = {CliStrings.CONNECT__USE_HTTP},
            mandatory = false,
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
            help = CliStrings.CONNECT__USE_HTTP__HELP
          )
          boolean useHttp,
      @CliOption(
            key = {CliStrings.CONNECT__URL},
            mandatory = false,
            unspecifiedDefaultValue = CliStrings.CONNECT__DEFAULT_BASE_URL,
            help = CliStrings.CONNECT__URL__HELP
          )
          String url,
      @CliOption(
            key = {CliStrings.CONNECT__USERNAME},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__USERNAME__HELP
          )
          String userName,
      @CliOption(
            key = {CliStrings.CONNECT__PASSWORD},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__PASSWORD__HELP
          )
          String password,
      @CliOption(
            key = {CliStrings.CONNECT__KEY_STORE},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__KEY_STORE__HELP
          )
          String keystore,
      @CliOption(
            key = {CliStrings.CONNECT__KEY_STORE_PASSWORD},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__KEY_STORE_PASSWORD__HELP
          )
          String keystorePassword,
      @CliOption(
            key = {CliStrings.CONNECT__TRUST_STORE},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__TRUST_STORE__HELP
          )
          String truststore,
      @CliOption(
            key = {CliStrings.CONNECT__TRUST_STORE_PASSWORD},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__TRUST_STORE_PASSWORD__HELP
          )
          String truststorePassword,
      @CliOption(
            key = {CliStrings.CONNECT__SSL_CIPHERS},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__SSL_CIPHERS__HELP
          )
          String sslCiphers,
      @CliOption(
            key = {CliStrings.CONNECT__SSL_PROTOCOLS},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__SSL_PROTOCOLS__HELP
          )
          String sslProtocols,
      @CliOption(
            key = CliStrings.CONNECT__SECURITY_PROPERTIES,
            optionContext = ConverterHint.FILE_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.CONNECT__SECURITY_PROPERTIES__HELP
          )
          final String gfSecurityPropertiesPath,
      @CliOption(
            key = {CliStrings.CONNECT__USE_SSL},
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
            help = CliStrings.CONNECT__USE_SSL__HELP
          )
    ...
    if (gfsh != null && gfsh.isConnectedAndReady()) {
      return ResultBuilder.createInfoResult(
          "Already connected to: " + getGfsh().getOperationInvoker().toString());
    }
      ...
      if (userName != null && userName.length() > 0) {
        ...
        if (passwordToUse == null || passwordToUse.length() == 0) {
          return ResultBuilder.createConnectionErrorResult(
              CliStrings.CONNECT__MSG__JMX_PASSWORD_MUST_BE_SPECIFIED);
        }
      }

      sslConfigProps =
          this.readSSLConfiguration(
              useSsl,
              keystoreToUse,
  ...
  private Result httpConnect(
    ...
    try {
      ...
      LogWrapper.getInstance()
          .warning(
              String.format(
                  "Sending HTTP request for Link Index at (%1$s)...",
                  url.concat("/index").concat(query)));
  ...
  private Result jmxConnect(
    ...
    try {
      ...
      if (memberRmiHostPort != null) {
        ...
        Gfsh.println(
            CliStrings.format(
                CliStrings.CONNECT__MSG__CONNECTING_TO_LOCATOR_AT_0,
                new Object[] {locatorTcpHostPort.toString(false)}));
        ConnectToLocatorResult connectToLocatorResult =
            connectToLocator(
                locatorTcpHostPort.getHost(),
                locatorTcpHostPort.getPort(),
      ...
      if (!retry) {
        Gfsh.println(
            CliStrings.format(
                CliStrings.CONNECT__MSG__CONNECTING_TO_MANAGER_AT_0,
      ...
      JmxOperationInvoker operationInvoker =
          new JmxOperationInvoker(
              hostPortToConnect.getHost(),
              hostPortToConnect.getPort(),
              userName,
              passwordToUse,
              sslConfigProps,
              gfSecurityPropertiesPath);
      ...
      LogWrapper.getInstance()
          .info(
              CliStrings.format(
                  CliStrings.CONNECT__MSG__SUCCESS, hostPortToConnect.toString(false)));
  ...
  private Result handleExcpetion(Exception e, ConnectionEndpoint hostPortToConnect) {
    ...
    if (hostPortToConnect != null) {
      errorMessage =
          CliStrings.format(
              CliStrings.CONNECT__MSG__ERROR, hostPortToConnect.toString(false), e.getMessage());
    }
  ...
  private Map<String, String> readSSLConfiguration(
      boolean useSsl,
      String keystoreToUse,
      String keystorePasswordToUse,
      String truststoreToUse,
      String truststorePasswordToUse,
      String sslCiphersToUse,
      String sslProtocolsToUse,
      String gfSecurityPropertiesPath)
      throws IOException {
    ...
    if (useSsl || gfSecurityPropertiesPath != null) {
      ...
      if (!StringUtils.isBlank(gfSecurityPropertiesPathToUse)) {
        ...
        if (!IOUtils.isExistingPathname(gfSecurityPropertiesPathToUse)) {
          gfshInstance.printAsSevere(
              CliStrings.format(
                  CliStrings.GEODE_0_PROPERTIES_1_NOT_FOUND_MESSAGE,
                  "Security ",
                  gfSecurityPropertiesPathToUse));
        } else {
      ...
      if (gfSecurityPropertiesUrl != null) {
        gfshInstance.logToFile(
            "Using security properties file : "
      ...
      if (keystoreToUse != null && keystoreToUse.length() > 0) {
        if (keystorePasswordToUse == null || keystorePasswordToUse.length() == 0) {
          ...
          if (keystorePasswordToUse == null || keystorePasswordToUse.length() == 0) {
            ...
            keystorePasswordToUse =
                readPassword(gfshInstance, CliStrings.CONNECT__KEY_STORE_PASSWORD + ": ");
      ...
      if (truststoreToUse != null && truststoreToUse.length() > 0) {
        if (truststorePasswordToUse == null || truststorePasswordToUse.length() == 0) {
          ...
          if (truststorePasswordToUse == null || truststorePasswordToUse.length() == 0) {
            // not even in properties file, prompt user for it
            truststorePasswordToUse =
                readPassword(gfshInstance, CliStrings.CONNECT__TRUST_STORE_PASSWORD + ": ");
      ...
      try {
        ...
        if (!props.isEmpty()) {
          ...
          for (Entry<Object, Object> entry : entrySet) {
            ...
            } else if (!jmxSpecificProps.contains(
                key)) { // Prefer properties ending with "-jmx" over default SSL props.
              propsMap.put(key, (String) entry.getValue());
            }
          }
          props.clear();
  ...
  public static ConnectToLocatorResult connectToLocator(
      String host, int port, int timeout, Map<String, String> props) throws IOException {
    ...
    if (StringUtils.isBlank(locatorResponse.getHost()) || locatorResponse.getPort() == 0) {
      ...
      if (locatorResponseException != null) {
        ...
        exceptionMessage =
            "Exception caused JMX Manager startup to fail because: '"
                .concat(locatorResponseExceptionMessage)
                .concat("'");
      }
    ...
    String resultMessage =
        CliStrings.format(
            CliStrings.CONNECT__MSG__CONNECTING_TO_MANAGER_AT_0, memberEndpoint.toString(false));

    return new ConnectToLocatorResult(
        memberEndpoint, resultMessage, locatorResponse.isJmxManagerSslEnabled());
  }

  @CliCommand(
    value = {CliStrings.DISCONNECT},
    help = CliStrings.DISCONNECT__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {
    ...
    if (getGfsh() != null && !getGfsh().isConnectedAndReady()) {
      ...
      try {
        ...
        if (gfshInstance.isConnectedAndReady()) {
          ...
          infoResultData.addLine(
              CliStrings.format(
                  CliStrings.DISCONNECT__MSG__DISCONNECTED, operationInvoker.toString()));
          LogWrapper.getInstance()
              .info(
                  CliStrings.format(
                      CliStrings.DISCONNECT__MSG__DISCONNECTED, operationInvoker.toString()));
        ...
        result =
            ResultBuilder.createConnectionErrorResult(
  ...
  @CliCommand(
    value = {CliStrings.DESCRIBE_CONNECTION},
    help = CliStrings.DESCRIBE_CONNECTION__HELP
  )
  @CliMetaData(
    shellOnly = true,
  ...
  @CliCommand(
    value = {CliStrings.ECHO},
    help = CliStrings.ECHO__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {CliStrings.TOPIC_GFSH}
  ...
  public Result echo(
      @CliOption(
            key = {CliStrings.ECHO__STR, ""},
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            specifiedDefaultValue = "",
            mandatory = true,
            help = CliStrings.ECHO__STR__HELP
          )
          String stringToEcho) {
    Result result = null;
  ...
  @CliCommand(
    value = {CliStrings.SET_VARIABLE},
    help = CliStrings.SET_VARIABLE__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {CliStrings.TOPIC_GFSH}
  ...
  public Result setVariable(
      @CliOption(
            key = CliStrings.SET_VARIABLE__VAR,
            mandatory = true,
            help = CliStrings.SET_VARIABLE__VAR__HELP
          )
          String var,
      @CliOption(
            key = CliStrings.SET_VARIABLE__VALUE,
            mandatory = true,
  ...
  @CliCommand(
    value = {CliStrings.DEBUG},
    help = CliStrings.DEBUG__HELP
  )
  @CliMetaData(
    shellOnly = true,
  ...
  public Result history(
      @CliOption(
            ...
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.HISTORY__FILE__HELP
          )
          String saveHistoryTo,
      @CliOption(
            key = {CliStrings.HISTORY__CLEAR},
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
            help = CliStrings.HISTORY__CLEAR__HELP
          )
    ...
    if (clearHistory) {
      ...
      try {
        ...
        return ResultBuilder.createInfoResult(
            "File error " + ex.getMessage() + " for file " + saveHistoryTo);
      } finally {
        try {
          if (output != null) {
            output.close();
  ...
  Result executeClearHistory() {
    try {
      ...
      return ResultBuilder.createGemFireErrorResult(
          "Exception occured while clearing history " + e.getMessage());
    }
    return ResultBuilder.createInfoResult(CliStrings.HISTORY__MSG__CLEARED_HISTORY);
  }

  @CliCommand(
    value = {CliStrings.RUN},
    help = CliStrings.RUN__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {CliStrings.TOPIC_GFSH}
  ...
  public Result executeScript(
      ...
      @CliOption(
          ...
          boolean quiet,
      @CliOption(
            key = {CliStrings.RUN__CONTINUEONERROR},
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
  ...
  @CliCommand(
    value = {CliStrings.VERSION},
    help = CliStrings.VERSION__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {CliStrings.TOPIC_GFSH}
  ...
  @CliCommand(
    value = {CliStrings.SLEEP},
    help = CliStrings.SLEEP__HELP
  )
  @CliMetaData(
    shellOnly = true,
    relatedTopic = {CliStrings.TOPIC_GFSH}
  ...
  public Result sh(
      @CliArgument(
            name = CliStrings.SH__COMMAND,
            mandatory = true,
            help = CliStrings.SH__COMMAND__HELP
          )
          String command,
      @CliOption(
            key = CliStrings.SH__USE_CONSOLE,
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
