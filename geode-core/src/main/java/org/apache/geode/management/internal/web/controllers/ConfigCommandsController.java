...
public class ConfigCommandsController extends AbstractMultiPartCommandsController {
  ...
  public String alterRuntime(
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__GROUP, required = false)
          final String group,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__MEMBER, required = false)
          final String memberNameId,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__ARCHIVE__DISK__SPACE__LIMIT,
            required = false
          )
          final Integer archiveDiskSpaceLimit,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__ARCHIVE__FILE__SIZE__LIMIT,
            required = false
          )
          final Integer archiveFileSizeLimit,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__LOG__DISK__SPACE__LIMIT,
            required = false
          )
          final Integer logDiskSpaceLimit,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__LOG__FILE__SIZE__LIMIT,
            required = false
          )
          final Integer logFileSizeLimit,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__LOG__LEVEL, required = false)
          final String logLevel,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__STATISTIC__ARCHIVE__FILE,
            required = false
          )
          final String statisticsArchiveFile,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__STATISTIC__SAMPLE__RATE,
            required = false
          )
          final Integer statisticsSampleRate,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__STATISTIC__SAMPLING__ENABLED,
            required = false
          )
          final Boolean enableStatistics,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__COPY__ON__READ, required = false)
          final Boolean copyOnRead,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__LOCK__LEASE, required = false)
          final Integer lockLease,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__LOCK__TIMEOUT, required = false)
          final Integer lockTimeout,
      @RequestParam(
            value = CliStrings.ALTER_RUNTIME_CONFIG__MESSAGE__SYNC__INTERVAL,
            required = false
          )
          final Integer messageSyncInterval,
      @RequestParam(value = CliStrings.ALTER_RUNTIME_CONFIG__SEARCH__TIMEOUT, required = false)
    ...
    if (hasValue(logDiskSpaceLimit)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__LOG__DISK__SPACE__LIMIT,
          Integer.toString(logDiskSpaceLimit));
    }

    if (hasValue(logFileSizeLimit)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__LOG__FILE__SIZE__LIMIT,
    ...
    if (hasValue(statisticsArchiveFile)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__STATISTIC__ARCHIVE__FILE, statisticsArchiveFile);
    }
    ...
    if (hasValue(enableStatistics)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__STATISTIC__SAMPLING__ENABLED,
          Boolean.toString(enableStatistics));
    }

    if (hasValue(copyOnRead)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__COPY__ON__READ, Boolean.toString(copyOnRead));
    }
    ...
    if (hasValue(lockTimeout)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__LOCK__TIMEOUT, Integer.toString(lockTimeout));
    }
    ...
    if (hasValue(searchTimeout)) {
      command.addOption(
          CliStrings.ALTER_RUNTIME_CONFIG__SEARCH__TIMEOUT, Integer.toString(searchTimeout));
    }
  ...
  public String describeConfig(
      ...
      @RequestParam(value = CliStrings.DESCRIBE_CONFIG__HIDE__DEFAULTS, defaultValue = "true")
  ...
  public Callable<ResponseEntity<String>> exportConfig(
      @RequestParam(value = CliStrings.EXPORT_CONFIG__GROUP, required = false)
          final String[] groups,
      @RequestParam(value = CliStrings.EXPORT_CONFIG__MEMBER, required = false)
          final String[] members,
      @RequestParam(value = CliStrings.EXPORT_CONFIG__DIR, required = false)
          final String directory) {
    final CommandStringBuilder command = new CommandStringBuilder(CliStrings.EXPORT_CONFIG);

    if (hasValue(groups)) {
      command.addOption(
          CliStrings.EXPORT_CONFIG__GROUP, StringUtils.concat(groups, StringUtils.COMMA_DELIMITER));
    }

    if (hasValue(members)) {
      command.addOption(
          CliStrings.EXPORT_CONFIG__MEMBER,
  ...
  public Callable<ResponseEntity<String>> exportClusterConfig(
      ...
      @RequestParam(value = CliStrings.EXPORT_SHARED_CONFIG__DIR, required = false)
          final String directory) {
  ...
  public Callable<ResponseEntity<String>> importClusterConfig(
    ...
    return getProcessCommandCallable(
        command.toString(), getEnvironment(), ConvertUtils.convert(zipFileResources));
  }
}
