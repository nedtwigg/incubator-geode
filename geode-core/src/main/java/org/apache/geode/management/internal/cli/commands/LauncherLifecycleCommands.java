...
public class LauncherLifecycleCommands extends AbstractCommandsSupport {
  ...
  public Result startLocator(
      @CliOption(
            key = CliStrings.START_LOCATOR__MEMBER_NAME,
            mandatory = true,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__BIND_ADDRESS,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_LOCATOR__BIND_ADDRESS__HELP
          )
          final String bindAddress,
      @CliOption(
            key = CliStrings.START_LOCATOR__CLASSPATH,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_LOCATOR__CLASSPATH__HELP
          )
          final String classpath,
      @CliOption(
            key = CliStrings.START_LOCATOR__FORCE,
          ...
          final Boolean force,
      @CliOption(
            key = CliStrings.START_LOCATOR__GROUP,
          ...
          final String group,
      @CliOption(
            key = CliStrings.START_LOCATOR__HOSTNAME_FOR_CLIENTS,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_LOCATOR__HOSTNAME_FOR_CLIENTS__HELP
          )
          final String hostnameForClients,
      @CliOption(
            key = CliStrings.START_LOCATOR__INCLUDE_SYSTEM_CLASSPATH,
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__LOCATORS,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__LOG_LEVEL,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__PROPERTIES,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__SECURITY_PROPERTIES,
            optionContext = ConverterHint.FILE_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__CONNECT,
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__ENABLE__SHARED__CONFIGURATION,
            unspecifiedDefaultValue = "true",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__LOAD__SHARED_CONFIGURATION__FROM__FILESYSTEM,
            unspecifiedDefaultValue = "false",
      ...
      @CliOption(
            key = CliStrings.START_LOCATOR__CLUSTER__CONFIG__DIR,
    ...
    try {
      ...
      if (!StringUtils.isBlank(gemfirePropertiesPathname)
        ...
        return ResultBuilder.createUserErrorResult(
            CliStrings.format(
                CliStrings.GEODE_0_PROPERTIES_1_NOT_FOUND_MESSAGE,
      ...
      gemfireProperties.setProperty(
          LOCATORS, StringUtils.valueOf(locators, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          LOG_LEVEL, StringUtils.valueOf(logLevel, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MCAST_ADDRESS, StringUtils.valueOf(mcastBindAddress, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MCAST_PORT, StringUtils.valueOf(mcastPort, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
      ...
      ProcessStreamReader stderrReader =
          new ProcessStreamReader.Builder(locatorProcess)
      ...
      try {
        getGfsh()
            .logInfo(
                String.format(
                    CliStrings.START_LOCATOR__RUN_MESSAGE,
          ...
          try {
            ...
            return ResultBuilder.createShellClientErrorResult(
                String.format(
                    ...
                    exitValue,
                    locatorLauncher.getWorkingDirectory(),
                    message.toString()));
            ...
            locatorState =
                (ProcessUtils.isAttachApiAvailable()
            ...
            if (isStartingOrNotResponding(locatorState.getStatus())
                && !(StringUtils.isBlank(currentLocatorStatusMessage)
                    || currentLocatorStatusMessage.equalsIgnoreCase(
                        previousLocatorStatusMessage))) {
              Gfsh.println();
        ...
        stderrReader.stopAsync(
            PROCESS_STREAM_READER_ASYNC_STOP_TIMEOUT_MILLIS); // stop will close ErrorStream
      ...
      if (asyncStart) {
        infoResultData.addLine(
            String.format(CliStrings.ASYNC_PROCESS_LAUNCH_MESSAGE, LOCATOR_TERM_NAME));
      } else {
        ...
        if (enableSharedConfiguration) {
          infoResultData.addLine(
              getSharedConfigurationStatusFromLocator(locatorHostName, locatorPort));
        }
      }
      ...
      if (message != null
        ...
        message =
            CliStrings.format(
                CliStrings.LAUNCHERLIFECYCLECOMMANDS__MSG__FAILED_TO_START_0_REASON_1,
                LOCATOR_TERM_NAME,
                message);
      }
      ...
      String errorMessage =
          String.format(
              CliStrings.START_LOCATOR__GENERAL_ERROR_MESSAGE,
              StringUtils.defaultIfBlank(workingDirectory, memberName),
              getLocatorId(bindAddress, port),
  ...
  protected String[] createStartLocatorCommandLine(
      final LocatorLauncher launcher,
    ...
    commandLine.add(
        getLocatorClasspath(Boolean.TRUE.equals(includeSystemClasspath), userClasspath));
    ...
    for (int attempts = 0; (attempts < 10 && !connectSuccess); attempts++) {
      try {
        ConnectToLocatorResult connectToLocatorResult =
            ShellCommands.connectToLocator(
                locatorHostname,
                locatorPort,
        ...
        getGfsh()
            .setOperationInvoker(
                new JmxOperationInvoker(
                    memberEndpoint.getHost(),
        ...
        String shellAndLogMessage =
            CliStrings.format(
                CliStrings.CONNECT__MSG__SUCCESS, "JMX Manager " + memberEndpoint.toString(false));
  ...
  private void doOnConnectionFailure(
      final String locatorHostName,
      final int locatorPort,
    ...
    infoResultData.addLine(
        CliStrings.format(
            CliStrings.START_LOCATOR__USE__0__TO__CONNECT,
    ...
    if (jmxManagerSslEnabled) {
      message
          .append(jmxManagerAuthEnabled ? " and " : StringUtils.EMPTY_STRING)
    ...
    if (IOUtils.isExistingPathname(configurationPropertiesPathname)) {
      try {
        configurationProperties.putAll(
            ShellCommands.loadPropertiesFromURL(
        ...
        LogWrapper.getInstance()
            .warning(
                String.format(
                    ...
                    configurationPropertiesPathname),
                ignore);
      }
    }
  ...
  private String getSharedConfigurationStatusFromLocatorState(LocatorState locatorState)
    ...
    return getSharedConfigurationStatusFromLocator(
        locatorState.getHost(), Integer.parseInt(locatorState.getPort()));
  }
      ...
      SharedConfigurationStatusResponse statusResponse =
          (SharedConfigurationStatusResponse)
              client.requestToServer(
                  networkAddress, locatorPort, new SharedConfigurationStatusRequest(), 10000, true);
          ...
          statusResponse =
              (SharedConfigurationStatusResponse)
                  client.requestToServer(
                      networkAddress,
                      locatorPort,
                      new SharedConfigurationStatusRequest(),
                      10000,
                      true);
          try {
            Thread.sleep(5000);
  ...
  public Result statusLocator(
      @CliOption(
            key = CliStrings.STATUS_LOCATOR__MEMBER,
    ...
    try {
      ...
      return ResultBuilder.createShellClientErrorResult(
          String.format(
              CliStrings.STATUS_LOCATOR__GENERAL_ERROR_MESSAGE,
  ...
  public Result stopLocator(
      @CliOption(
            key = CliStrings.STOP_LOCATOR__MEMBER,
            optionContext = ConverterHint.LOCATOR_MEMBER_IDNAME,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          final String member,
      @CliOption(
            key = CliStrings.STOP_LOCATOR__PID,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.STOP_LOCATOR__PID__HELP
          )
          final Integer pid,
      @CliOption(
            key = CliStrings.STOP_LOCATOR__DIR,
            optionContext = ConverterHint.DIR_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
    ...
    try {
        ...
        if (isConnectedAndReady()) {
          ...
          if (locatorProxy != null) {
            ...
            if (locatorProxy.isServer()) {
              throw new IllegalStateException(
                  CliStrings.format(
                      CliStrings.STOP_LOCATOR__LOCATOR_IS_CACHE_SERVER_ERROR_MESSAGE, member));
            }
      ...
      if (Status.ONLINE.equals(locatorState.getStatus())) {
        getGfsh()
            .logInfo(
                String.format(
                    CliStrings.STOP_LOCATOR__STOPPING_LOCATOR_MESSAGE,
                    locatorState.getWorkingDirectory(),
                    locatorState.getServiceLocation(),
                    locatorState.getMemberName(),
                    locatorState.getPid(),
                    locatorState.getLogFile()),
                null);
  ...
  // throw an Exception!
  protected void addCurrentLocators(
      final List<String> commandLine, final Properties gemfireProperties)
      throws MalformedObjectNameException {
    if (StringUtils.isBlank(gemfireProperties.getProperty(LOCATORS))) {
      ...
      if (!StringUtils.isBlank(currentLocators)) {
        commandLine.add(
            "-D"
                .concat(ProcessLauncherContext.OVERRIDDEN_DEFAULTS_PREFIX)
                .concat(LOCATORS)
                .concat("=")
                .concat(currentLocators));
      }
    }
  }
  ...
  protected void addGemFirePropertyFile(
      final List<String> commandLine, final String gemfirePropertiesPathname) {
    if (!StringUtils.isBlank(gemfirePropertiesPathname)) {
      commandLine.add("-DgemfirePropertyFile=" + gemfirePropertiesPathname);
    }
  }

  protected void addGemFireSecurityPropertyFile(
      final List<String> commandLine, final String gemfireSecurityPropertiesPathname) {
    if (!StringUtils.isBlank(gemfireSecurityPropertiesPathname)) {
      commandLine.add("-DgemfireSecurityPropertyFile=" + gemfireSecurityPropertiesPathname);
    }
  }

  protected void addGemFireSystemProperties(
      final List<String> commandLine, final Properties gemfireProperties) {
  ...
  protected void addJvmArgumentsAndOptions(
  ...
  protected LocatorState locatorStatus(
      final File locatorPidFile, final int oldPid, final String memberName) {
  ...
  protected ServerState serverStatus(
      final File serverPidFile, final int oldPid, final String memberName) {
  ...
  protected String getLocatorClasspath(
      final boolean includeSystemClasspath, final String userClasspath) {
    return toClasspath(
        includeSystemClasspath, new String[] {CORE_DEPENDENCIES_JAR_PATHNAME}, userClasspath);
  }

  protected String getServerClasspath(
      final boolean includeSystemClasspath, final String userClasspath) {
  ...
  String toClasspath(
      final boolean includeSystemClasspath, String[] jarFilePathnames, String... userClasspaths) {
  ...
  protected String getAttachAPINotFoundMessage() {
    return CliStrings.format(
        CliStrings.ATTACH_API_IN_0_NOT_FOUND_ERROR_MESSAGE,
  ...
  public Result startServer(
      @CliOption(
            key = CliStrings.START_SERVER__ASSIGN_BUCKETS,
            unspecifiedDefaultValue = "false",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__BIND_ADDRESS,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_SERVER__BIND_ADDRESS__HELP
          )
          final String bindAddress,
      @CliOption(
            key = CliStrings.START_SERVER__CACHE_XML_FILE,
            optionContext = ConverterHint.FILE_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
      ...
      @CliOption(
            key = CliStrings.START_SERVER__CLASSPATH,
      ...
      @CliOption(
          ...
          String workingDirectory,
      @CliOption(
            key = CliStrings.START_SERVER__DISABLE_DEFAULT_SERVER,
            unspecifiedDefaultValue = "false",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__DISABLE_EXIT_WHEN_OUT_OF_MEMORY,
            unspecifiedDefaultValue = "false",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__ENABLE_TIME_STATISTICS,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            specifiedDefaultValue = "true",
      ...
      @CliOption(
          ...
          final Boolean force,
      @CliOption(
            key = CliStrings.START_SERVER__GROUP,
            optionContext = ConverterHint.MEMBERGROUP,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          final String group,
      @CliOption(
            key = CliStrings.START_SERVER__HOSTNAME__FOR__CLIENTS,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
      ...
      @CliOption(
          ...
          final Boolean includeSystemClasspath,
      @CliOption(
            key = CliStrings.START_SERVER__INITIAL_HEAP,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_SERVER__INITIAL_HEAP__HELP
          )
          final String initialHeap,
      @CliOption(
            key = CliStrings.START_SERVER__J,
            optionContext = ConverterHint.STRING_LIST,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          final String[] jvmArgsOpts,
      @CliOption(
            key = CliStrings.START_SERVER__LOCATORS,
            optionContext = ConverterHint.LOCATOR_DISCOVERY_CONFIG,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          final String locators,
      @CliOption(
            key = CliStrings.START_SERVER__LOCATOR_WAIT_TIME,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_SERVER__LOCATOR_WAIT_TIME_HELP
          )
          final Integer locatorWaitTime,
      @CliOption(
            key = CliStrings.START_SERVER__LOCK_MEMORY,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__LOG_LEVEL,
            optionContext = ConverterHint.LOG_LEVEL,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
      ...
      @CliOption(
          ...
          final String memberName,
      @CliOption(
            key = CliStrings.START_SERVER__OFF_HEAP_MEMORY_SIZE,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_SERVER__OFF_HEAP_MEMORY_SIZE__HELP
          )
          final String offHeapMemorySize,
      @CliOption(
            key = CliStrings.START_SERVER__PROPERTIES,
            optionContext = ConverterHint.FILE_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          String gemfirePropertiesPathname,
      @CliOption(
            key = CliStrings.START_SERVER__REBALANCE,
            unspecifiedDefaultValue = "false",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__SECURITY_PROPERTIES,
            optionContext = ConverterHint.FILE_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          String gemfireSecurityPropertiesPathname,
      @CliOption(
            key = CliStrings.START_SERVER__SERVER_BIND_ADDRESS,
      ...
      @CliOption(
            key = CliStrings.START_SERVER__REST_API,
            unspecifiedDefaultValue = "false",
            specifiedDefaultValue = "true",
      ...
      @CliOption(
            key = CliStrings.START_SERVER__HTTP_SERVICE_PORT,
      ...
      @CliOption(
            key = CliStrings.START_SERVER__HTTP_SERVICE_BIND_ADDRESS,
  ...
  // NOTICE: keep the parameters in alphabetical order based on their CliStrings.START_SERVER_* text
  {
    ...
    try {
      ...
      gemfireProperties.setProperty(
          BIND_ADDRESS, StringUtils.valueOf(bindAddress, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          CACHE_XML_FILE, StringUtils.valueOf(cacheXmlPathname, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
      ...
      gemfireProperties.setProperty(
          LOCATORS, StringUtils.valueOf(locators, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          LOCATOR_WAIT_TIME, StringUtils.valueOf(locatorWaitTime, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          LOG_LEVEL, StringUtils.valueOf(logLevel, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MCAST_ADDRESS, StringUtils.valueOf(mcastBindAddress, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MCAST_PORT, StringUtils.valueOf(mcastPort, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MEMCACHED_PORT, StringUtils.valueOf(memcachedPort, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MEMCACHED_PROTOCOL, StringUtils.valueOf(memcachedProtocol, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          MEMCACHED_BIND_ADDRESS,
          StringUtils.valueOf(memcachedBindAddress, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          REDIS_PORT, StringUtils.valueOf(redisPort, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          REDIS_BIND_ADDRESS, StringUtils.valueOf(redisBindAddress, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          REDIS_PASSWORD, StringUtils.valueOf(redisPassword, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
      ...
      gemfireProperties.setProperty(
          LOCK_MEMORY, StringUtils.valueOf(lockMemory, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          OFF_HEAP_MEMORY_SIZE, StringUtils.valueOf(offHeapMemorySize, StringUtils.EMPTY_STRING));

      gemfireProperties.setProperty(
          START_DEV_REST_API, StringUtils.valueOf(startRestApi, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
          HTTP_SERVICE_PORT, StringUtils.valueOf(httpServicePort, StringUtils.EMPTY_STRING));
      gemfireProperties.setProperty(
      ...
      Process serverProcess =
          new ProcessBuilder(serverCommandLine)
      ...
      ProcessStreamReader stderrReader =
          new ProcessStreamReader.Builder(serverProcess)
      ...
      try {
        getGfsh()
            .logInfo(
                String.format(
                    CliStrings.START_SERVER__RUN_MESSAGE,
                    IOUtils.tryGetCanonicalPathElseGetAbsolutePath(
          ...
          try {
            ...
            return ResultBuilder.createShellClientErrorResult(
                String.format(
                    ...
                    exitValue,
                    serverLauncher.getWorkingDirectory(),
                    message.toString()));
          } catch (IllegalThreadStateException ignore) {
  ...
  protected String[] createStartServerCommandLine(
    ...
    if (launcher.getServerBindAddress() != null) {
      commandLine.add(
          "--server-bind-address=" + launcher.getServerBindAddress().getCanonicalHostName());
    }
    ...
    if (launcher.getMaxThreads() != null) {
      commandLine.add(
          "--" + CliStrings.START_SERVER__MAX__THREADS + "=" + launcher.getMaxThreads());
    }
    ...
    if (launcher.getHostNameForClients() != null) {
      commandLine.add(
          "--"
              + CliStrings.START_SERVER__HOSTNAME__FOR__CLIENTS
              + "="
  ...
  public Result statusServer(
      @CliOption(
            key = CliStrings.STATUS_SERVER__MEMBER,
          ...
          final String member,
      @CliOption(
            key = CliStrings.STATUS_SERVER__PID,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.STATUS_SERVER__PID__HELP
          )
          final Integer pid,
      @CliOption(
            key = CliStrings.STATUS_SERVER__DIR,
            optionContext = ConverterHint.DIR_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
    ...
    try {
      if (!StringUtils.isBlank(member)) {
        if (isConnectedAndReady()) {
          ...
          if (serverProxy != null) {
            return ResultBuilder.createInfoResult(
                ServerState.fromJson(serverProxy.status()).toString());
          } else {
            return ResultBuilder.createUserErrorResult(
                CliStrings.format(
  ...
  public Result stopServer(
      @CliOption(
            key = CliStrings.STOP_SERVER__MEMBER,
            optionContext = ConverterHint.MEMBERIDNAME,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
          ...
          final String member,
      @CliOption(
            key = CliStrings.STOP_SERVER__PID,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.STOP_SERVER__PID__HELP
          )
          final Integer pid,
      @CliOption(
            key = CliStrings.STOP_SERVER__DIR,
            optionContext = ConverterHint.DIR_PATHSTRING,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
    ...
    try {
        ...
        if (isConnectedAndReady()) {
          ...
          if (serverProxy != null) {
            if (!serverProxy.isServer()) {
              throw new IllegalStateException(
                  CliStrings.format(
                      CliStrings.STOP_SERVER__MEMBER_IS_NOT_SERVER_ERROR_MESSAGE, member));
            }
      ...
      if (Status.ONLINE.equals(serverState.getStatus())) {
        getGfsh()
            .logInfo(
                String.format(
                    CliStrings.STOP_SERVER__STOPPING_SERVER_MESSAGE,
                    serverState.getWorkingDirectory(),
                    serverState.getServiceLocation(),
                    serverState.getMemberName(),
                    serverState.getPid(),
                    serverState.getLogFile()),
                null);
  ...
  public Result startJConsole(
      @CliOption(
            key = CliStrings.START_JCONSOLE__INTERVAL,
            unspecifiedDefaultValue = "4",
            help = CliStrings.START_JCONSOLE__INTERVAL__HELP
          )
          final int interval,
      @CliOption(
            key = CliStrings.START_JCONSOLE__NOTILE,
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
      ...
      @CliOption(
            key = CliStrings.START_JCONSOLE__PLUGINPATH,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
            help = CliStrings.START_JCONSOLE__PLUGINPATH__HELP
          )
          final String pluginpath,
      @CliOption(
            key = CliStrings.START_JCONSOLE__VERSION,
            specifiedDefaultValue = "true",
            unspecifiedDefaultValue = "false",
      ...
      @CliOption(
            key = CliStrings.START_JCONSOLE__J,
            optionContext = ConverterHint.STRING_LIST,
            unspecifiedDefaultValue = CliMetaData.ANNOTATION_NULL_VALUE,
    ...
    try {
      ...
      if (isDebugging()) {
        getGfsh()
            .printAsInfo(
                String.format(
                    "JConsole command-line ($1%s)", Arrays.toString(jconsoleCommandLine)));
      }
      ...
      return ResultBuilder.createShellClientErrorResult(
          CliStrings.START_JCONSOLE__IO_EXCEPTION_MESSAGE);
    } catch (VirtualMachineError e) {
      SystemFailure.initiateFailure(e);
      throw e;
    } catch (Throwable t) {
  ...
  protected String[] createJConsoleCommandLine(
      final String member,
      final int interval,
      final boolean notile,
      final String pluginpath,
  ...
  protected String getJConsolePathname() {
    return getJdkToolPathname(
        "jconsole" + getExecutableSuffix(),
        new JConsoleNotFoundException(CliStrings.START_JCONSOLE__NOT_FOUND_ERROR_MESSAGE));
  }

  protected String getJdkToolPathname(
      final String jdkToolExecutableName, final GemFireException throwable) {
    ...
    pathnames.push(
        IOUtils.appendToPath(System.getenv("JAVA_HOME"), "..", "bin", jdkToolExecutableName));
  ...
  protected String getJdkToolPathname(
      final Stack<String> pathnames, final GemFireException throwable) {
    ...
    try {
      ...
      if (isDebugging()) {
        getGfsh()
            .printAsInfo(
                String.format(
                    "JVisualVM command-line (%1$s)", Arrays.toString(jvisualvmCommandLine)));
      }
  ...
  public Result startPulse(
      @CliOption(
            key = CliStrings.START_PULSE__URL,
      ...
      if (!StringUtils.isBlank(url)) {
        ...
        if (isConnectedAndReady()) {
          ...
          if (!StringUtils.isBlank(pulseURL)) {
            ...
            return ResultBuilder.createInfoResult(
                CliStrings.START_PULSE__RUN + " with URL: " + pulseURL);
          } else {
            String pulseMessage =
                (String)
                    operationInvoker.getAttribute(managerObjectName.toString(), "StatusMessage");
          ...
          return ResultBuilder.createUserErrorResult(
              CliStrings.format(
                  CliStrings.GFSH_MUST_BE_CONNECTED_FOR_LAUNCHING_0, "GemFire Pulse"));
        }
      }
    } catch (GemFireException e) {
      return ResultBuilder.createShellClientErrorResult(e.getMessage());
  ...
  protected File readIntoTempFile(final String classpathResourceLocation) throws IOException {
    String resourceName =
        classpathResourceLocation.substring(
    ...
    if (!resourceFile.exists() && resourceFile.createNewFile()) {
      ...
      try {
        for (String line = resourceReader.readLine();
            line != null;
  ...
  public Result startVsd(
      @CliOption(key = CliStrings.START_VSD__FILE, help = CliStrings.START_VSD__FILE__HELP)
    ...
    try {
      ...
      assertState(
          IOUtils.isExistingPathname(getPathToVsd()),
  ...
  protected void processStatisticsArchiveFiles(
      final File path, final Set<String> statisticsArchiveFiles) {
  ...
  public Result startDataBrowser() {
    try {
      ...
      if (isConnectedAndReady()
        ...
        assertState(
            IOUtils.isExistingPathname(dataBrowserPath),
            String.format(CliStrings.START_DATABROWSER__NOT_FOUND_ERROR_MESSAGE, gemfireHome));

        JmxOperationInvoker operationInvoker =
            (JmxOperationInvoker) getGfsh().getOperationInvoker();

        String dataBrowserCommandLine =
            String.format(
                "%1$s %2$s %3$d",
                getPathToDataBrowser(),
                operationInvoker.getManagerHost(),
                operationInvoker.getManagerPort());
  ...
  protected String waitAndCaptureProcessStandardOutputStream(final Process process) {
    return waitAndCaptureProcessStandardOutputStream(
        process, DEFAULT_PROCESS_OUTPUT_WAIT_TIME_MILLISECONDS);
  }

  protected String waitAndCaptureProcessStandardOutputStream(
      final Process process, final long waitTimeMilliseconds) {
  ...
  protected String waitAndCaptureProcessStandardErrorStream(final Process process) {
    return waitAndCaptureProcessStandardErrorStream(
        process, DEFAULT_PROCESS_OUTPUT_WAIT_TIME_MILLISECONDS);
  }

  protected String waitAndCaptureProcessStandardErrorStream(
      final Process process, final long waitTimeMilliseconds) {
    return waitAndCaptureProcessStream(process, process.getErrorStream(), waitTimeMilliseconds);
  }

  private String waitAndCaptureProcessStream(
      final Process process, final InputStream processInputStream, long waitTimeMilliseconds) {
    ...
    ProcessStreamReader reader =
        new ProcessStreamReader.Builder(process)
  ...
  @CliAvailabilityIndicator({
    CliStrings.START_LOCATOR,
    CliStrings.STOP_LOCATOR,
    CliStrings.STATUS_LOCATOR,
    CliStrings.START_SERVER,
    CliStrings.STOP_SERVER,
    CliStrings.STATUS_SERVER,
    CliStrings.START_MANAGER,
    CliStrings.START_PULSE,
    CliStrings.START_VSD,
    CliStrings.START_DATABROWSER
  })
