 ...
 *
 * <p>Even though this class have a lot of utility functions it interacts with the state of the
 * system and contains some state itself.
 */
public class ManagementAdapter {
  ...
  public void handleManagerStop() throws ManagementException {
    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        logger.warn(
            "Failed to invoke aggregator for {} with exception {}", objectName, e.getMessage(), e);
      } catch (ClassNotFoundException e) {
        logger.warn(
            "Failed to invoke aggregator for {} with exception {}", objectName, e.getMessage(), e);
      }
  ...
  public <K, V> void handleRegionCreation(Region<K, V> region) throws ManagementException {
    ...
    synchronized (regionOpLock) {
      ...
      ObjectName regionMBeanName =
          MBeanJMXAdapter.getRegionMBeanName(
  ...
  public void handleDiskCreation(DiskStore disk) throws ManagementException {
    ...
    ObjectName diskStoreMBeanName =
        MBeanJMXAdapter.getDiskStoreMBeanName(
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.DISK_STORE_CREATED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleLockServiceCreation(DLockService lockService) throws ManagementException {
    ...
    ObjectName lockServiceMBeanName =
        MBeanJMXAdapter.getLockServiceMBeanName(
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.LOCK_SERVICE_CREATED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewaySenderCreation(GatewaySender sender) throws ManagementException {
    ...
    ObjectName senderObjectName =
        MBeanJMXAdapter.getGatewaySenderMBeanName(
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_SENDER_CREATED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  private void createGatewayReceiverMBean(GatewayReceiver recv) {
    ...
    ObjectName recvObjectName =
        MBeanJMXAdapter.getGatewayReceiverMBeanName(
            cacheImpl.getDistributedSystem().getDistributedMember());
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_RECEIVER_CREATED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewayReceiverStart(GatewayReceiver recv) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_RECEIVER_STARTED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewayReceiverStop(GatewayReceiver recv) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_RECEIVER_STOPPED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleAsyncEventQueueCreation(AsyncEventQueue queue) throws ManagementException {
    ...
    ObjectName senderObjectName =
        MBeanJMXAdapter.getAsycnEventQueueMBeanName(
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.ASYNC_EVENT_QUEUE_CREATED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleSystemNotification(AlertDetails details) {
    ...
    if (service.isManager()) {
      ...
      Notification notification =
          new Notification(
              JMXNotificationType.SYSTEM_ALERT,
              systemSource,
              SequenceNumber.next(),
              details.getMsgTime().getTime(),
              details.getMsg());
  ...
  private Map<String, String> prepareUserData(AlertDetails details) {
    ...
    userData.put(
        JMXNotificationUserData.ALERT_LEVEL,
  ...
  public void handleCacheServerStart(CacheServer cacheServer) {
    ...
    ClientMembershipListener managementClientListener =
        new CacheServerMembershipListenerAdapter(
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.CACHE_SERVER_STARTED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleCacheServerStop(CacheServer server) {
    ...
    ObjectName cacheServerMBeanName =
        MBeanJMXAdapter.getClientServiceMBeanName(
            server.getPort(), cacheImpl.getDistributedSystem().getDistributedMember());
    service.unregisterMBean(cacheServerMBeanName);

    Notification notification =
        new Notification(
            JMXNotificationType.CACHE_SERVER_STOPPED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleRegionRemoval(Region region) throws ManagementException {
    ...
    synchronized (regionOpLock) {
      ...
      Notification notification =
          new Notification(
              JMXNotificationType.REGION_CLOSED,
              memberSource,
              SequenceNumber.next(),
              System.currentTimeMillis(),
  ...
  public void handleDiskRemoval(DiskStore disk) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.DISK_STORE_CLOSED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleLockServiceRemoval(DLockService lockService) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.LOCK_SERVICE_CLOSED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
   ...
   *
   * <p>There is no corresponding handleStopLocator() method. Locator will close the cache whenever
   * its stopped and it should also shutdown all the management services by closing the cache.
   *
  ...
  public void handleLocatorStart(Locator locator) throws ManagementException {
    ...
    ObjectName locatorMBeanName =
        MBeanJMXAdapter.getLocatorMBeanName(
            cacheImpl.getDistributedSystem().getDistributedMember());
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.LOCATOR_STARTED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewaySenderStart(GatewaySender sender) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_SENDER_STARTED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewaySenderStop(GatewaySender sender) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_SENDER_STOPPED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewaySenderPaused(GatewaySender sender) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_SENDER_PAUSED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleGatewaySenderResumed(GatewaySender sender) throws ManagementException {
    ...
    Notification notification =
        new Notification(
            JMXNotificationType.GATEWAY_SENDER_RESUMED,
            memberSource,
            SequenceNumber.next(),
            System.currentTimeMillis(),
  ...
  public void handleCacheServiceCreation(CacheService cacheService) throws ManagementException {
    ...
    if (mbean != null) {
      ...
      Notification notification =
          new Notification(
              JMXNotificationType.CACHE_SERVICE_CREATED,
              memberSource,
              SequenceNumber.next(),
              System.currentTimeMillis(),
  ...
  private static class CacheServerMembershipListenerAdapter
    ...
    public void memberJoined(ClientMembershipEvent event) {
      Notification notification =
          new Notification(
              JMXNotificationType.CLIENT_JOINED,
              serverSource,
              SequenceNumber.next(),
              System.currentTimeMillis(),
    ...
    public void memberLeft(ClientMembershipEvent event) {
      Notification notification =
          new Notification(
              JMXNotificationType.CLIENT_LEFT,
              serverSource,
              SequenceNumber.next(),
              System.currentTimeMillis(),
    ...
    public void memberCrashed(ClientMembershipEvent event) {
      Notification notification =
          new Notification(
              JMXNotificationType.CLIENT_CRASHED,
              serverSource,
              SequenceNumber.next(),
              System.currentTimeMillis(),
