...
public class ResultBuilder {
   ...
   *
   * <p>Note: To build your own error result, use {@link #createErrorResultData()} to build {@link
   * ErrorResultData} & then use {@link #buildResult(ResultData)}
   *
  ...
  public static Result createGemFireErrorResult(String message) {
    return createErrorResult(
        ERRORCODE_GEMFIRE_ERROR, "Could not process command due to GemFire error. " + message);
  }
  ...
  public static Result createBadResponseErrorResult(String message) {
    return createErrorResult(
        ERRORCODE_BADRESPONSE_ERROR, "Could not read command response. " + message);
  }
  ...
  static ResultData getReadOnlyResultData(ResultData resultData) {
    ...
    if (ResultData.TYPE_TABULAR.equals(contentType)) {
      wrapperResultData =
          new TabularResultData(resultData.getGfJsonObject()) {
            @Override
      ...
      wrapperResultData =
          new InfoResultData(resultData.getGfJsonObject()) {
            @Override
            public ResultData addAsFile(
                String fileName,
                byte[] data,
      ...
      wrapperResultData =
          new ErrorResultData(resultData.getGfJsonObject()) {
            @Override
            public ResultData addAsFile(
                String fileName,
                byte[] data,
      ...
      wrapperResultData =
          new CompositeResultData(resultData.getGfJsonObject()) {
            @Override
            public ResultData addAsFile(
                String fileName,
                byte[] data,
      ...
      wrapperResultData =
          new ObjectResultData<CliJsonSerializable>() {
            @Override
            public ResultData addAsFile(
                String fileName,
                byte[] data,
      ...
      errorResultData.addLine(
          "Can not detect result type, unknown result data format for: " + resultData);
