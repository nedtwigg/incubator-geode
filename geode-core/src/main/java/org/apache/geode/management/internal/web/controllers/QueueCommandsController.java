...
public class QueueCommandsController extends AbstractCommandsController {
  ...
  public String createAsyncEventQueue(
      ...
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__LISTENER_PARAM_AND_VALUE,
            required = false
          )
          final String[] listenerParametersValues,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__GROUP, required = false)
          final String[] groups,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__PARALLEL, defaultValue = "false")
          final Boolean parallel,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__ENABLEBATCHCONFLATION,
            defaultValue = "false"
          )
          final Boolean enableBatchConflation,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__BATCH_SIZE, defaultValue = "100")
          final Integer batchSize,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__BATCHTIMEINTERVAL,
            defaultValue = "1000"
          )
          final Integer batchTimeInterval,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__PERSISTENT, defaultValue = "false")
          final Boolean persistent,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__DISK_STORE, required = false)
          final String diskStore,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__DISKSYNCHRONOUS,
            defaultValue = "true"
          )
          final Boolean diskSynchronous,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__FORWARD_EXPIRATION_DESTROY,
            defaultValue = "false"
          )
      ...
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__DISPATCHERTHREADS,
            defaultValue = "1"
          )
          final Integer dispatcherThreads,
      @RequestParam(value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__ORDERPOLICY, defaultValue = "KEY")
          final String orderPolicy,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__GATEWAYEVENTFILTER,
            required = false
          )
          final String[] gatewayEventFilters,
      @RequestParam(
            value = CliStrings.CREATE_ASYNC_EVENT_QUEUE__SUBSTITUTION_FILTER,
            required = false
          )
    ...
    if (hasValue(groups)) {
      command.addOption(
          CliStrings.CREATE_ASYNC_EVENT_QUEUE__GROUP,
          StringUtils.concat(groups, StringUtils.COMMA_DELIMITER));
    }

    command.addOption(
        CliStrings.CREATE_ASYNC_EVENT_QUEUE__PARALLEL,
    ...
    if (hasValue(batchTimeInterval)) {
      command.addOption(
          CliStrings.CREATE_ASYNC_EVENT_QUEUE__BATCHTIMEINTERVAL,
          String.valueOf(batchTimeInterval));
    }

    command.addOption(
        CliStrings.CREATE_ASYNC_EVENT_QUEUE__PERSISTENT,
    ...
    command.addOption(
        CliStrings.CREATE_ASYNC_EVENT_QUEUE__DISKSYNCHRONOUS,
        String.valueOf(Boolean.TRUE.equals(diskSynchronous)));

    if (hasValue(maxQueueMemory)) {
      command.addOption(
          CliStrings.CREATE_ASYNC_EVENT_QUEUE__MAXIMUM_QUEUE_MEMORY,
          String.valueOf(maxQueueMemory));
    }

    if (hasValue(dispatcherThreads)) {
      command.addOption(
          CliStrings.CREATE_ASYNC_EVENT_QUEUE__DISPATCHERTHREADS,
    ...
    if (hasValue(gatewaySubstitutionFilter)) {
      command.addOption(
          CliStrings.CREATE_ASYNC_EVENT_QUEUE__SUBSTITUTION_FILTER, gatewaySubstitutionFilter);
    }
