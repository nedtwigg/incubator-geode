 ...
 *
 * <p>Any custom ProxyListener should implement this interface. For the time being its not exposed
 * to the user. But is being kept as an extension point for future releases if we decide to support
 * custom ProxyListener.
 *
 * <p>TODO Along side we need to make FederationCompoent public as well.
 *
 * <p>A simple usage of a ProxyListener could be custom aggregator of Statistics exposed by MBeans.
 *
 * <p>ManagementService service = ManagementService .getManagementService(cache);
 *
 * <p>ProxyListener listener = new MyListener(); //implements ProxyListener.
 * service.addProxyListener(listener)
 *
 * <p>Adding an ProxyListener in a non Manager node wont be useful as it wont get any proxy related
 * callbacks
 */
public interface ProxyListener extends MembershipListener {
   ...
   *
   * <p>e.g. When manager starts and not a single ManagementTask cycle has been run. This will
   * result in lost updates. To capture all updates we can feed the newly created data to existing
   * aggregate.
   *
   * @param objectName name of the proxy object
   * @param interfaceClass interface class of the proxy object.
