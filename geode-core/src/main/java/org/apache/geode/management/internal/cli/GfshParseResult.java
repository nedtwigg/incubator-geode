...
/**
 * Immutable representation of the outcome of parsing a given shell line. * Extends {@link
 * ParseResult} to add a field to specify the command string that was input by the user.
 *
 * <p>Some commands are required to be executed on a remote GemFire managing member. These should be
 * marked with the annotation {@link CliMetaData#shellOnly()} set to <code>false</code>. {@link
 * GfshExecutionStrategy} will detect whether the command is a remote command and send it to
