...
/**
 * MBean that provides access to information and management operations for a {@link
 * DistributedSystem}.
 *
 * <p>It also provides an API for navigating the other MBeans exposed by the GemFire distributed
 * system.
 *
 * <p>There will be one DistributedSystemMBean per GemFire cluster.
 *
...
public interface DistributedSystemMXBean {
  ...
  public String[]
      listLocators(); //TODO - Abhishek Should be renamed to listLocatorDiscoveryConfigurations? Do we need something for mcast too?
   ...
   *
   * <p>e.g. {"result":[["java.lang.String","v"],["java.lang.String","b"]]}
   *
   * <p>GemFire PDXInstances are also supported. The type of PDXInstance is PDXInstance and value
   * will be key value pair. There is no marker to know the "IdentityField" of the PDXInstance.
   *
   * <p>If the query is executed on the cluster and no member list is given in input first key of
   * the JSON string will be "result" followed by the result set in JSON format.
   *
   * <p>If the query is executed on one or more specific members then returned string will have an
   * array of "member" and "result" keys.
   *
   * <p>For query on replicated region data from a random node which have the region is shown, if no
   ...
   *
   * <p>Join queries on PR mandates that user provide one member as input. If the member does not
   * host the regions or the regions are not co-located error string will be returned.
   *
   ...
   * <p>e.g.
   *
   * <p>GZIPInputStream gis = new GZIPInputStream(new ByteArrayInputStream(bytes));
   *
   * <p>BufferedReader bf = new BufferedReader(new InputStreamReader(gis,"UTF-8"));
   *
   * <p>String outStr = ""; String line; while ((line = bf.readLine()) != null) { outStr += line; }
   *
   * @param queryString GemFire supported OQL query
   * @param members comma separated list of members on which the query is to be executed. It is not
   *     mandatory to give this input barring join queries on PR. If member list is not provided
