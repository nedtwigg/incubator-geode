...
public class LogFileFunction implements Function, InternalEntity {
  ...
  public void execute(final FunctionContext context) {
                  ...
                  if (ds != null && ds.isConnected()) {
                    ...
                    final String filterStr =
                        filterString.substring(
                            0,
                            filterString.lastIndexOf(".") > 0
                                ? filterString.lastIndexOf(".")
                    ...
                    if (dir == null) {
                      context
                          ...
                          .lastResult(
                              CliStrings.format(
                                  CliStrings
                                      .EXPORT_LOGS__MSG__FAILED_TO_EXPORT_LOG_FILES_FOR_MEMBER_0,
                                  ds.getMemberId()));
                      return;
                    }
                    try {
                    ...
                    if (isDebugEnabled) {
                      logger.debug(
                          "For member={}: Exporting logs LogFileFunction targetDirName={}",
                          ds.getMemberId(),
                          targetDirName);
                    }
    ...
    try {
      ...
      if (isDebugEnabled) {
        logger.debug(
            "For member={}: started copying log files",
      ...
      if (isDebugEnabled) {
        logger.debug(
            "For member={}: done with Exporting all log files",
            ((InternalDistributedSystem) CacheFactory.getAnyInstance().getDistributedSystem())
                .getMemberId());
      }
    } catch (Exception e) {
  ...
  public String processLogs(
    ...
    try {
      ...
      if (isDebugEnabled) {
        logger.debug(
            "LogFileFunction  processLogs logsInDir={} sample={}", logsInDir.length, logsInDir[0]);
      }
      ...
      if (logsInDir.length > 0) {
        ...
        if (isDebugEnabled) {
          logger.debug(
              "LogFileFunction  before copy  numOfLogFilesForTesting={}", numOfLogFilesForTesting);
        }

        for (int i = 0, j = 0;
            i < logsInDir.length && (j < numOfLogFilesForTesting || numOfLogFilesForTesting == 0);
            i++, j++) {
          ...
          if (isDebugEnabled) {
            logger.debug(
                "LogFileFunction  processLogs fileName={} logToBeWritten={}",
                fileName,
                logToBeWritten);
          }
          ...
          if (resultString.contains("Sucessfully written file")) {
            ...
            if (isDebugEnabled) {
              logger.debug(
                  "LogFileFunction wrote file logToBeWritten={} resultString={}",
                  logToBeWritten,
                  resultString);
            }
          }
        ...
        if (isDebugEnabled) {
          logger.debug(
              CliStrings.format("No file was found for exporting in export logs function"));
        }
      }
      if (isDebugEnabled) {
