      ...
      public void run() {
        ...
        try {
          ...
          if (ds != null && ds.isConnected()) {
            ...
            final String filterStr = filterString.substring(0, filterString.lastIndexOf(".") > 0 ? filterString.lastIndexOf(".") : filterString.length() - 1);

            File dir = ds.getConfig().getLogFile(); // get log file object
            if (dir == null) {
              context.getResultSender().lastResult(CliStrings.format(CliStrings.EXPORT_LOGS__MSG__FAILED_TO_EXPORT_LOG_FILES_FOR_MEMBER_0, ds.getMemberId()));
              return;
            }
            try {
            ...
            if (targetDir.exists() == false) {
              if (targetDir.mkdirs() == true) {
                ...
                if (isDebugEnabled) {
                  logger.debug("For member={}{} {}", ds.getMemberId(), CliStrings.EXPORT_LOGS__MSG__TARGET_DIR_CANNOT_BE_CREATED, targetDirName);
                }
    ...
    try {
      ...
      if (logsInDir.length > 0) {
        ...
        for (int i = 0, j = 0; i < logsInDir.length && (j < numOfLogFilesForTesting || numOfLogFilesForTesting == 0); i++, j++) {
          ...
          String logToBeWritten = targetDirName + File.separator + MBeanJMXAdapter.getMemberNameOrId(cache.getDistributedSystem().getDistributedMember()) + "_" + logsInDir[i].substring(0, logsInDir[i].length() - 4) + "_" + sdf.format(new java.util.Date()) + logsInDir[i].substring(logsInDir[i].length() - 4, logsInDir[i].length());
