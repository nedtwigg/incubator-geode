 ...
 *
 * <p>Ordinarily, when a <code>DataSerializable</code> object is written using {@link
 * DataSerializer#writeObject(Object, java.io.DataOutput)}, a special marker class id is written to
 * the stream followed by the class name of the <code>DataSerializable</code> object. After the
 * marker class id is read by {@link DataSerializer#readObject} it performs the following
 * operations,
 *
 * <OL>
 *   <LI>The class name is read
 *   <LI>The class is loaded using {@link Class#forName(java.lang.String)}
 *   <LI>An instance of the class is created using reflection
 *   <LI>{@link DataSerializable#fromData} is invoked on the newly-created object
 * </OL>
 *
 ...
 * <code>DataSerializable</code> class. When the object is written using {@link
 * DataSerializer#writeObject(Object, java.io.DataOutput)}, the object's registered class id is
 * written to the stream. Consequently, when the data is read from the stream, the {@link
 * #newInstance} method of the appropriate <code>Instantiator</code> instance is invoked to create
 * an "empty" instance of the <code>DataSerializable</code> instead of using reflection to create
 * the new instance.
 *
 * <p>Commonly, a <code>DataSerializable</code> class will register itself with the <code>
 * Instantiator</code> in a static initializer as shown in the below example code.
 * <!--
 ...
 *
 * <p>Note that the framework does not require that an <code>Instantiator</code> be {@link
 * java.io.Serializable}, but it does require that it provide a {@linkplain #Instantiator(Class,
 * int) two-argument constructor}.
 *
...
public abstract class Instantiator {
   ...
   * method is usually invoked from the static initializer of a class that implements <code>
   * DataSerializable</code>.
   *
   ...
   * @throws IllegalArgumentException If <code>c</code> does not implement <code>DataSerializable
   *     </code>, <code>classId</code> is less than or equal to zero.
  ...
  public Instantiator(Class<? extends DataSerializable> c, int classId) {
    ...
    if (classId == 0) {
      throw new IllegalArgumentException(
          LocalizedStrings.Instantiator_CLASS_ID_0_MUST_NOT_BE_0.toLocalizedString(
  ...
  /**
   * Returns the <code>DataSerializable</code> class that is instantiated by this <code>Instantiator
   * </code>.
   */
