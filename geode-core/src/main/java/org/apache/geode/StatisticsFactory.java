 ...
 * because it implements {@link StatisticsTypeFactory}. {@link
 * org.apache.geode.distributed.DistributedSystem} is the only instance of this interface.
 *
 * <p>A <code>StatisticsFactory</code> can create a {@link StatisticDescriptor statistic} of three
 * numeric types: <code>int</code>, <code>long</code>, and <code>double</code>. A statistic (<code>
 * StatisticDescriptor</code>) can either be a <I>gauge</I> meaning that its value can increase and
 * decrease or a <I>counter</I> meaning that its value is strictly increasing. Marking a statistic
 * as a counter allows statistic display tools to properly display a statistics whose value "wraps
 * around" (that is, exceeds its maximum value).
 *
 * <p>The following code is an example of how to create a type using the api. In this example the
 * type has two stats whose values always increase:
 *
 * <pre>
 * StatisticsFactory f = ...;
...
public interface StatisticsFactory extends StatisticsTypeFactory {
   ...
   * default ids.
   *
   * <p>The created instance may not be {@link Statistics#isAtomic atomic}.
   */
   ...
   * default ids.
   *
   * <p>The created instance will be {@link Statistics#isAtomic atomic}.
   */
