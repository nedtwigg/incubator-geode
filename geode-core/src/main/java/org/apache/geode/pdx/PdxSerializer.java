 ...
 *
 * <p>GemFire allows a single PdxSerializer to be configured on a cache using {@link
 * org.apache.geode.cache.CacheFactory#setPdxSerializer(PdxSerializer) setPdxSerializer} or {@link
 * org.apache.geode.cache.client.ClientCacheFactory#setPdxSerializer(PdxSerializer) client
 * setPdxSerializer}. It can also be configured in <code>cache.xml</code> using the <code>
 * pdx-serializer</code> element. The same PdxSerializer should be configured on each member of a
 * distributed system that can serialize or deserialize PDX data.
 *
 * <p>The {@link #toData toData} method is used for serialization; {@link #fromData fromData} is
 * used for deserialization. The order in which fields are serialized and deserialized in these
 * methods for the same class must match. For the same class toData and fromData must write the same
 * fields each time they are called.
 *
...
public interface PdxSerializer {
   ...
   * do the serialization then it must return <code>true</code>; otherwise it must return <code>
   * false</code> in which case it will be serialized using standard serialization.
   *
