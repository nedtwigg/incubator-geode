 ...
 *
 * <p>PdxInstances are immutable. If you want to change one call {@link #createWriter}.
 *
 * <p>A PdxInstance's fields will always be those of the version it represents. So if you add a
 * field to your domain class you can end up with a PdxInstance for version 1 (that does not have
 * the field) and a PdxInstance for version 2. The PdxInstance for version 1 will not have the added
...
public interface PdxInstance extends java.io.Serializable {
   ...
   * an int field named "ordinal". It is ok to cast a PdxInstance that represents an enum to {@link
   * java.lang.Comparable}. PdxInstances representing enums are not writable.
   *
   ...
   *
   * <p>This can be useful when writing code that handles more than one version of a PDX class.
   *
   ...
   * field.
   *
   * <p>Note that if no fields have been marked then all the fields are used as identity fields even
   ...
   * returned.
   *
   * <p>A <code>null</code> result indicates that the field does not exist or that it exists and its
   ...
   *
   * <p>If an Object[] is deserialized by this call then that array's component type will be <code>
   * Object.class</code> instead of the original class that the array had when it was serialized.
   * This is done so that PdxInstance objects can be added to the array.
   *
   ...
   *
   * <p>If <code>other</code> is not a PdxInstance then it is not equal to this instance. NOTE: Even
   * if <code>other</code> is the result of calling {@link #getObject()} it will not be equal to
   * this instance.
   *
   ...
   * it has the field with a default value. If a PdxInstance has marked identity fields using {@link
   * PdxWriter#markIdentityField(String) markIdentityField} then only the marked identity fields are
   * its identity fields. Otherwise all its fields are identity fields.
   *
   ...
   * </ol>
   *
   * <p>If a field's type is <code>OBJECT</code> then its value must be deserialized to determine if
   * it is equals. If the deserialized object is an array then {@link
   * java.util.Arrays#deepEquals(Object[], Object[]) deepEquals} is used to determine equality.
   * Otherwise {@link Object#equals(Object) equals} is used.
   *
   * <p>If a field's type is <code>OBJECT[]</code> then its value must be deserialized and {@link
   * java.util.Arrays#deepEquals(Object[], Object[]) deepEquals} is used to determine equality.
   *
   * <p>For all other field types then the value does not need to be deserialized. Instead the
   * serialized raw bytes are compared and used to determine equality.
   *
   * <p>Note that any fields that have objects that do not override {@link Object#equals(Object)
   * equals} will cause equals to return false when you might have expected it to return true. The
   * only exceptions to this are those that call {@link java.util.Arrays#deepEquals(Object[],
   * Object[]) deepEquals} as noted above. You should either override equals and hashCode in these
   * cases or mark other fields as your identity fields.
   *
   ...
   *
   * <p>If a PdxInstance has marked identity fields using {@link PdxWriter#markIdentityField(String)
   ...
   *
   * <p>If an identity field is of type <code>OBJECT</code> then it is deserialized. If the
   ...
   *
   * <p>If an identity field is of type <code>OBJECT[]</code> this it is deserialized and {@link
   * java.util.Arrays#deepHashCode(Object[]) deepHashCode} is used.
   *
   * <p>Otherwise the field is not deserialized and the raw bytes of its value are used to compute
   * the hash code.
   *
   * <p>The algorithm used to compute the hashCode is: hashCode = 1; foreach (field:
