 ...
 *
 * <p>Using these methods an applications may convert a JSON document into a PdxInstance for storing
 * in the cache. Indexes can then be defined on the PdxInstances so that queries can be performed
 * using OQL. Queries will return PdxInstances and these can be turned back back into JSON documents
 * using JSONFormatter.
 *
 * <p>JSONFormatter treats values in a json document as number(byte, short, int, long..), string,
...
public class JSONFormatter {
  ...
  public static PdxInstance fromJSON(String jsonString) {
    ...
    try {
      ...
      throw new JSONFormatterException(
          "Could not parse JSON document: " + jp.getCurrentLocation(), e);
    } catch (Exception e) {
      throw new JSONFormatterException(
          "Could not parse JSON document: " + jp.getCurrentLocation(), e);
    }
  ...
  private PdxInstanceHelper getPdxInstance(
      JsonParser jp, states currentState, PdxInstanceHelper currentPdxInstance)
        ...
        case END_ARRAY:
          {
            ...
            currentState = states.LIST_ENDS;
            if (currentPdxList.getParent() == null) return currentPdxList;
