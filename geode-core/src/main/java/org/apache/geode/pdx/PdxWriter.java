...
/**
 * A PdxWriter will be passed to {@link PdxSerializable#toData(PdxWriter) toData} or {@link
 * PdxSerializer#toData(Object, PdxWriter) PdxSerializer toData} by GemFire when it is serializing
 * the domain class. The domain class needs to serialize instance fields using this interface. This
 * interface is implemented by GemFire.
 *
 * <p>The order in which the fields are written must match the order in which they are read by
 * {@link PdxReader}.
 *
 * <p>Field names are case sensitive.
 *
 * <p>All methods on this interface return itself to allow method calls to be chained together.
 *
 * @since GemFire 6.6
 */
public interface PdxWriter {
  ...
  /**
   * Writes the named field with the given value to the serialized form. The fields type is <code>
   * char</code>.
   *
   ...
   * Object</code>.
   *
   * <p>It is best to use one of the other writeXXX methods if your field type will always be XXX.
   * This method allows the field value to be anything that is an instance of Object. This gives you
   * more flexibility but more space is used to store the serialized field.
   *
   * <p>Note that some Java objects serialized with this method may not be compatible with non-java
   * languages. To ensure that only portable objects are serialized use {@link #writeObject(String,
   * Object, boolean)}.
   *
   * @param fieldName the name of the field to write
   * @param value the value of the field to write
   ...
   * </code> parameter to true. The following is a list of the Java classes that are portable and
   * the .NET class they are mapped to:
   *
   * <ul>
   ...
   * Object[]</code>.
   *
   * <p>Java Object[] is mapped to .NET System.Collections.Generic.List<Object>. For how each
   ...
   * languages. To ensure that only portable objects are serialized use {@link
   ...
   * ensure that only objects that are portable to non-java languages are serialized use {@link
   * #writeField(String, Object, Class, boolean)} instead.
   *
   ...
   * @throws PdxSerializationException if serialization of the field fails.
   */
  public <CT, VT extends CT> PdxWriter writeField(
      String fieldName, VT fieldValue, Class<CT> fieldType);

  /**
   * Writes the named field with the given value and type to the serialized form. This method uses
   * the <code>fieldType</code> to determine which writeXXX method it should call. If it can not
   * find a specific match to a writeXXX method it will call {@link #writeObject(String, Object,
   * boolean) writeObject}. To ensure that only objects that are portable to non-java languages are
   * serialized set the <code>checkPortability</code> parameter to true.
   *
   * <p>The fieldTypes that map to a specific method are:
   *
   * <ul>
   ...
   */
  public <CT, VT extends CT> PdxWriter writeField(
      String fieldName, VT fieldValue, Class<CT> fieldType, boolean checkPortability);
   ...
   * server.
   *
   * <p>The fields that are marked as identity fields are used to generate the hashCode and equals
   ...
   *
   * <p>If no fields are set as identity fields, then all fields will be used in hashCode and equals
   * checks.
   *
   * <p>The identity fields should be marked after they are written using a write* method.
   *
