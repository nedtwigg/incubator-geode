 ...
 *
 * <p>This implementation will serialize all relevant fields
 *
 * <p>For example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>In this example <code>DomainObject</code> would not need to implement <code>PdxSerializable
 * </code> to be serialized.
 *
 * <p>The equivalent <code>cache.xml</code> entries might be as follows:
 *
 * <pre>
 * &lt;pdx&gt;
 *   &lt;pdx-serializer&gt;
...
public class ReflectionBasedAutoSerializer implements PdxSerializer, Declarable {
   ...
   * Instances created with this constructor will not match any classes so use {@link
   * #ReflectionBasedAutoSerializer(String...)} instead.
   */
   ...
   *
   * <p>Each string in the list represents a definition in the following form:
   *
   * <pre>
   ...
   * </pre>
   *
   * This would select all classes with a class name beginning with <code>com.company.DomainObject
   * </code> and would select as PDX identity fields any fields beginning with <code>id</code> and
   * would not serialize the field called <code>creationDate</code>.
   *
   * <p>There is no association between the the <i>identity</i> and <i>exclude</i> options, so the
   ...
   *       select classes for serialization, patterns to select identity fields and patterns to
   *       exclude fields. See {@link ReflectionBasedAutoSerializer#reconfigure(String...)
   *       reconfigure} for specifics.
   ...
   * </ul>
   *
   * <p>This method is only called the first time it sees a new class. The result will be remembered
   ...
   *
   * <p>The default implementation returns the name obtained from <code>f</code>.
   *
   * <p>This method is only called the first time it sees a new class. The result will be remembered
   * and used the next time the same class is seen.
   *
   ...
   *
   * <p>The default implementation only marks fields that match an "#identity=" pattern as identity
   * fields.
   *
   * <p>This method is only called the first time it sees a new class. The result will be remembered
   * and used the next time the same class is seen.
   ...
   *
   * <p>The default implementation uses {@link FieldType#get(Class)} by passing it {@link
   * Field#getType()}.
   *
   * <p>This method is only called the first time it sees a new class. The result will be remembered
   * and used the next time the same class is seen.
