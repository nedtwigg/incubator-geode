 ...
 *
 * <p>For improved performance, each statistic may be referred to by its {@link #nameToDescriptor
 * descriptor}.
 *
 * <p>For optimal performance, each statistic may be referred to by its {@link #nameToId id} in the
 ...
 *
 * <p>Note that as of the 5.1 release the <code>incInt</code>, <code>incLong</code>, and <code>
 * incDouble</code> methods no longer return the new value of the statistic. They now return <code>
 * void</code>. This incompatible change was made to allow for a more efficient concurrent increment
 * implementation.
 *
 * <p>
 *
...
public interface Statistics {
   ...
   *
   * <p>Returns false if modifications are not atomic. This means that modifications to this
   * instance are cheaper but not thread safe.
   */
   ...
   *
   * @throws IllegalArgumentException If no statistic exists with the specified <code>descriptor
   *     </code> or if the described statistic is not of type <code>int</code>.
   */
   ...
   *
   * @throws IllegalArgumentException If no statistic exists with the specified <code>descriptor
   *     </code> or if the described statistic is not of type <code>long</code>.
   */
   ...
   *
   * @throws IllegalArgumentException If no statistic exists with the specified <code>descriptor
   *     </code> or if the described statistic is not of type <code>double</code>.
   */
   ...
   *
   * @param descriptor a statistic descriptor obtained with {@link #nameToDescriptor} or {@link
   ...
   *
   * <p>The callback should return quickly because it is invoked on a shared thread. It should not
   * do any expensive computations, network calls, or access any resources under locks that may be
   * locked by long running processes.
   *
   * <p>This callback will only be invoked if the distributed system property
   * statistic-sampling-enabled is set to true, and it will be invoked at intervals determined by
   * the statistic-sampling-rate.
   *
   * <p>Get methods are not guaranteed to recompute a new value, they may return the last sampled
   * value
   *
   * @param id a statistic id obtained with {@link #nameToId} or {@link StatisticsType#nameToId}.
