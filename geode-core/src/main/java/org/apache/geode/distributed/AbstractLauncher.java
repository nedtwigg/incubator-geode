...
public abstract class AbstractLauncher<T extends Comparable<T>> implements Runnable {
  ...
  private static final List<String> ATTACH_API_PACKAGES =
      Arrays.asList(
  ...
  protected Logger logger =
      Logger.getLogger(getClass().getName()); // TODO:KIRK: does this need log4j2?

  public AbstractLauncher() {
    try {
      if (Boolean.getBoolean(SIGNAL_HANDLER_REGISTRATION_SYSTEM_PROPERTY)) {
        ClassUtils.forName(
            SUN_SIGNAL_API_CLASS_NAME,
  ...
  protected static void assertPortAvailable(final InetAddress bindAddress, final int port)
    ...
    if (!AvailablePort.isPortAvailable(port, AvailablePort.SOCKET, bindAddress)) {
      throw new BindException(
          String.format(
              "Network is unreachable; port (%1$d) is not available on %2$s.",
   ...
   *     returned if this call is made in-process, or finally, null is returned if neither name name
   *     was specified or the call is out-of-process.
   * @see #getMemberName()
   * @see #getMemberId()
   */
  ...
  protected void redirectOutput(final DistributedSystem distributedSystem) throws IOException {
    if (distributedSystem instanceof InternalDistributedSystem) {
      OSProcess.redirectOutput(
          ((InternalDistributedSystem) distributedSystem).getConfig().getLogFile());
    }
  }
    ...
    protected ServiceState(
      ...
      this.jvmArguments =
          ObjectUtils.defaultIfNull(
              Collections.unmodifiableList(jvmArguments), Collections.<String>emptyList());
      ...
      switch (getStatus()) {
        ...
        default: // NOT_RESPONDING
          return LocalizedStrings.Launcher_ServiceStatus_MESSAGE.toLocalizedString(
              getServiceName(), getWorkingDirectory(), getServiceLocation(), getStatus());
      }
    }
