...
public final class LocatorLauncher extends AbstractLauncher<String> {
  ...
  static {
    helpMap.put(
        "launcher", LocalizedStrings.LocatorLauncher_LOCATOR_LAUNCHER_HELP.toLocalizedString());
    helpMap.put(
        Command.START.getName(),
        LocalizedStrings.LocatorLauncher_START_LOCATOR_HELP.toLocalizedString(
            String.valueOf(getDefaultLocatorPort())));
    helpMap.put(
        Command.STATUS.getName(),
        LocalizedStrings.LocatorLauncher_STATUS_LOCATOR_HELP.toLocalizedString());
    helpMap.put(
        Command.STOP.getName(),
        LocalizedStrings.LocatorLauncher_STOP_LOCATOR_HELP.toLocalizedString());
    helpMap.put(
        Command.VERSION.getName(),
    ...
    helpMap.put(
        "port",
        LocalizedStrings.LocatorLauncher_LOCATOR_PORT_HELP.toLocalizedString(
            String.valueOf(getDefaultLocatorPort())));
    helpMap.put(
        "redirect-output",
  ...
  private static Integer getDefaultLocatorPort() {
    return Integer.getInteger(
        DistributionLocator.TEST_OVERRIDE_DEFAULT_PORT_PROPERTY,
  ...
  private LocatorLauncher(final Builder builder) {
    ...
    this.controlHandler =
        new ControlNotificationHandler() {
          @Override
  ...
  public static LocatorStatusResponse statusLocator(int port, InetAddress bindAddress)
    ...
    try {
      ...
      return (LocatorStatusResponse)
          client.requestToServer(bindAddress, port, new LocatorStatusRequest(), timeout, true);
  ...
  public final String getId() {
    return LocatorState.getBindAddressAsString(this)
        .concat("[")
        .concat(LocatorState.getPortAsString(this))
        .concat("]");
  }
   ...
   *
   * <p>Note, this information is purely information and should not be used to re-construct state or
   * for other purposes.
   *
  ...
  public void help(final Command command) {
    if (Command.isUnspecified(command)) {
      ...
      info(
          StringUtils.wrap(
              "> java ... " + getClass().getName() + " " + usageMap.get(command), 80, "\t\t"));
      info("\n\noptions: \n\n");
   ...
   *     LocatorLauncher, which is true if the LocatorLauncher has not already started a Locator or
   *     a Locator is not already running.
   * @see #start()
   */
   ...
   *
   * <p>'start' is an asynchronous invocation of the Locator. As such, this method makes no
   * guarantees whether the Locator's location services (peer and server) are actually running
   * before it returns. The Locator's location-based services are initiated in separate, daemon
   * Threads and depends on the relative timing and scheduling of those Threads by the JVM. If the
   * application using this API wishes for the Locator to continue running after normal application
   * processing completes, then one must call <code>waitOnLocator</code>.
   *
   * <p>Given the nature of start, the Locator's status will be in either 1 of 2 possible states. If
  ...
  public LocatorState start() {
    if (isStartable()) {
      ...
      try {
        ...
        debug(
            "Running Locator on (%1$s) in (%2$s) as (%2$s)...",
            getId(), getWorkingDirectory(), getMember());
        running.set(true);
        ...
        throw new RuntimeException(
            LocalizedStrings.Launcher_Command_START_IO_ERROR_MESSAGE.toLocalizedString(
   ...
   *
   * <p>However, this does not necessarily imply the Locator start was unsuccessful, only that a
   * response was not received in the given time period.
   *
   * <p>Note, this method does not block or cause the Locator's location-based services (daemon
   ...
   *     status request within the given time period. It should not be taken as the Locator failed
   *     to start.
   * @see #waitOnLocator()
   */
  public LocatorState waitOnStatusResponse(
      final long timeout, final long interval, final TimeUnit timeUnit) {
   ...
   * or ID.
   *
   * <p>However, if 'dir' or 'pid' were not specified, then determining the Locator's status
   * defaults to using the configured bind address and port. If the bind address or port was not
   * specified when using the Builder to construct a LocatorLauncher instance, then the defaults for
   * both bind address and port are used. In either case, an actual TCP/IP request is made to the
   * Locator's ServerSocket to ensure it is listening for client requests. This is true even when
   * the LocatorLauncher is used in-process by calling the API.
   *
   * <p>If the conditions above hold, then the Locator is deemed to be 'ONLINE', otherwise, the
   * Locator is considered 'OFFLINE'.
   *
  ...
  public LocatorState status() {
    ...
    // if this instance is running then return local status
      ...
      debug(
          "Getting Locator status using host (%1$s) and port (%2$s)%n",
  ...
  private LocatorState statusWithPid() {
    try {
      ...
      return createNoResponseState(
          e, "Failed to communicate with locator with process id " + getPid());
      ...
      Thread.currentThread().interrupt();
      return createNoResponseState(
          e, "Interrupted while trying to communicate with locator with process id " + getPid());
  ...
  private LocatorState statusWithWorkingDirectory() {
    ...
    try {
      ...
      // failed to open or read file or dir
      return createNoResponseState(
          e, "Failed to communicate with locator with process id " + parsedPid);
    } catch (MBeanInvocationFailedException e) {
      // MBean either doesn't exist or method or attribute don't exist
      return createNoResponseState(
          e, "Failed to communicate with locator with process id " + parsedPid);
      ...
      return createNoResponseState(
          e,
          "Failed to find usable process id within file "
              + ProcessType.LOCATOR.getPidFileName()
              + " in "
              + getWorkingDirectory());
    } catch (UnableToControlProcessException e) {
      ...
      Thread.currentThread().interrupt();
      return createNoResponseState(
          e, "Interrupted while trying to communicate with locator with process id " + parsedPid);
    } catch (TimeoutException e) {
      return createNoResponseState(
          e, "Failed to communicate with locator with process id " + parsedPid);
    }
   ...
   *
   * <p>If the "shutdown" request is successful, then the Locator will be 'STOPPED'. Otherwise, the
  ...
  private Properties getOverriddenDefaults() {
    ...
    overriddenDefaults.put(
        ProcessLauncherContext.OVERRIDDEN_DEFAULTS_PREFIX.concat(LOG_FILE), getLogFileName());
  ...
  public static class Builder {
     ...
     *     as when the --hostname-for-clients command-line option may have been specified without
     *     any argument.
    ...
    public Builder setMemberName(final String memberName) {
      if (StringUtils.isEmpty(StringUtils.trim(memberName))) {
        throw new IllegalArgumentException(
            LocalizedStrings.Launcher_Builder_MEMBER_NAME_ERROR_MESSAGE.toLocalizedString(
                "Locator"));
      }
    ...
    public Builder setPort(final Integer port) {
      ...
      if (port != null && (port < 0 || port > 65535)) {
        throw new IllegalArgumentException(
            LocalizedStrings.Launcher_Builder_INVALID_PORT_ERROR_MESSAGE.toLocalizedString(
                "Locator"));
      }
      this.port = port;
      return this;
  ...
  public static enum Command {
    ...
    Command(final String name, final String... options) {
      assert !StringUtils.isBlank(name)
          : "The name of the locator launcher command must be specified!";
      this.name = name;
      this.options =
          (options != null
  ...
  public static final class LocatorState extends ServiceState<String> {
    ...
    public LocatorState(
        final LocatorLauncher launcher, final Status status, final String errorMessage) {
      this(
          status, // status
      ...
      if (InternalLocator.hasLocator()) {
        ...
        if (logFile != null && logFile.isFile()) {
          ...
          if (!StringUtils.isBlank(
              logFileCanonicalPath)) { // this is probably not need but a safe check none-the-less.
    ...
    private LocatorState(
        final LocatorLauncher launcher, final Status status, final LocatorStatusResponse response) {
      this(
          status,
          launcher.statusMessage,
          System.currentTimeMillis(),
