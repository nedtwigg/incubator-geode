...
public class InternalLocator extends Locator implements ConnectListener {
   ...
   *
   * <p>This is for internal use only as it does not create a distributed system unless told to do
   * so.
   *
   * @param port the tcp/ip port to listen on
   * @param logFile the file that log messages should be written to
    ...
    try {
      ...
      try {
        ...
        if (startDistributedSystem) {
          try {
            slocator
                .startDistributedSystem(); // TODO:GEODE-1243: throws Exception if TcpServer still has zero for its locator port
          ...
          if (ids != null) {
            ids.getDistributionManager()
                .addHostedLocators(
                    ...
                    getLocatorStrings(),
                    slocator.isSharedConfigurationEnabled());
          }
        }
  ...
  private InternalLocator(
    ...
    final boolean hasLogFileButConfigDoesNot =
        this.logFile != null
            && this.config
                .getLogFile()
                .toString()
    ...
    if (logWriter == null && hasLogFileButConfigDoesNot) {
      this.config.unsafeSetLogFile(
          this.logFile); // LOG: this is(was) a hack for when logFile and config don't match -- if config specifies a different log-file things will break!
    }
    ...
    if (hasLogFile || hasSecurityLogFile) {
      ...
      if (hasLogFile) {
        ...
        LogWriterAppenders.getOrCreateAppender(
            LogWriterAppenders.Identifier.MAIN, true, false, this.config, !startDistributedSystem);
      }

      if (hasSecurityLogFile) {
        // if security-log-file then create securityLogWriterAppender
        LogWriterAppenders.getOrCreateAppender(
            LogWriterAppenders.Identifier.SECURITY, true, false, this.config, false);
    ...
    if (logWriter == null) {
      logWriter =
          LogWriterFactory.createLogWriterLogger(
              false, false, this.config, !startDistributedSystem);
  ...
  public void startPeerLocation(boolean withDS) throws IOException {
    if (isPeerLocator()) {
      throw new IllegalStateException(
          LocalizedStrings.InternalLocator_PEER_LOCATION_IS_ALREADY_RUNNING_FOR_0.toLocalizedString(
              this));
    }
    logger.info(
        LocalizedMessage.create(
            LocalizedStrings.InternalLocator_STARTING_PEER_LOCATION_FOR_0, this));
  ...
  public void startDistributedSystem() throws UnknownHostException {
    ...
    if (existing != null) {
      ...
      logger.info(
          LocalizedMessage.create(
              LocalizedStrings.InternalLocator_USING_EXISTING_DISTRIBUTED_SYSTEM__0, existing));
      startCache(existing);
    } else {
      String thisLocator;
      {
      ...
      logger.info(
          LocalizedMessage.create(
              LocalizedStrings.InternalLocator_LOCATOR_STARTED_ON__0, thisLocator));
  ...
  public void startServerLocation(InternalDistributedSystem distributedSystem) throws IOException {
    ...
    logger.info(
        LocalizedMessage.create(
            LocalizedStrings.InternalLocator_STARTING_SERVER_LOCATION_FOR_0, this));
  ...
  public void stop(boolean forcedDisconnect, boolean stopForReconnect, boolean waitForDisconnect) {
    ...
    if (this.server.isAlive()) {
      ...
      try {
        ...
        logger.warn(
            LocalizedMessage.create(
                LocalizedStrings.InternalLocator_INTERRUPTED_WHILE_STOPPING__0, this),
            ex);
      ...
      if (this.server.isAlive()) {
        logger.fatal(
            LocalizedMessage.create(
                LocalizedStrings.InternalLocator_COULD_NOT_STOP__0__IN_60_SECONDS, this));
      }
    }
  ...
  private void handleShutdown() {
    ...
    if (myDs != null && !this.forcedDisconnect) {
      if (myDs.isConnected()) {
        logger.info(
            LocalizedMessage.create(
                LocalizedStrings.InternalLocator_DISCONNECTING_DISTRIBUTED_SYSTEM_FOR_0, this));
        myDs.disconnect();
      }
    }
  }
  ...
  private void launchRestartThread() {
    ...
    this.restartThread =
        new Thread(group, "Location services restart thread") {
          public void run() {
  ...
  public void installSharedConfigDistribution() {
    if (!this.handler.isHandled(ConfigurationRequest.class)) {
      this.handler.addHandler(
          ConfigurationRequest.class, new ConfigurationRequestHandler(this.sharedConfig));
  ...
  public void installSharedConfigStatus() {
    if (!this.handler.isHandled(SharedConfigurationStatusRequest.class)) {
      this.handler.addHandler(
          SharedConfigurationStatusRequest.class, new SharedConfigurationStatusRequestHandler());
