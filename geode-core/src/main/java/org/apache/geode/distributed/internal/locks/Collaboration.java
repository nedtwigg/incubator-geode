 ...
 *
 * <p>Collaborating threads may jointly synchronize on this structure if they all agree on the same
 * topic of collaboration.
 *
 * <p>Threads that want to change the topic will wait until the current topic has been released.
 */
public class Collaboration {
  ...
  public void acquireUninterruptibly(final Object topic) {
    ...
    synchronized (this.topicsQueue) {
      ...
      if (this.currentTopic == null) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Collaboration.acquireUninterruptibly: {}: already current topic: {}",
              this.toString(),
              topicObject);
        }
        this.currentTopic.addThread(Thread.currentThread());
    ...
    synchronized (this.topicsQueue) {
      ...
      if (topic != null) {
        synchronized (topic) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Collaboration.setCurrentTopic: {}: new topic is {}", this.getIdentity(), topic);
          }
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Collaboration.setCurrentTopic: {} setting current topic to null", this.toString());
        }
    ...
    synchronized (topic) {
      while (!topic.isCurrentTopic()) {
        if (topic.isOldTopic()) {
          ...
          Assert.assertTrue(
              false, "[" + getIdentity() + ".awaitTopic] attempting to wait on old topic");
        }
        boolean interrupted = Thread.interrupted();
        try {
   ...
   *
   * <p>Nothing happens if the calling thread is not participating in the current topic.
   */
  public void release() {
