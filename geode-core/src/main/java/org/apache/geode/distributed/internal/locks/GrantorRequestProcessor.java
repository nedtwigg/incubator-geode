...
public class GrantorRequestProcessor extends ReplyProcessor21 {
  ...
  private static final boolean basicStartElderCall(
  ...
  static void readyForElderRecovery(
      InternalDistributedSystem sys, InternalDistributedMember elderId, DLockService dls) {
    GrantorRequestContext grc = sys.getGrantorRequestContext();
    if (elderId != null) {
      grc.elderLock.lock();
      try {
  ...
  private static void elderSyncWait(
      InternalDistributedSystem sys, InternalDistributedMember newElder, DLockService dls) {
    GrantorRequestContext grc = sys.getGrantorRequestContext();
   ...
   */
  private static final ElderState startElderCall(
      InternalDistributedSystem sys, DLockService dls, boolean usesElderCollaborationLock) {
    InternalDistributedMember elder;
    ElderState es = null;
  ...
  private static final void finishElderCall(GrantorRequestContext grc, ElderState es) {
    if (es == null) {
      grc.elderLock.lock();
      try {
   ...
   */
  public static GrantorInfo getGrantor(
      DLockService service, int dlsSerialNumber, InternalDistributedSystem sys) {
   ...
   *     recovery
   */
  public static GrantorInfo becomeGrantor(
   ...
   * @param sys the distributed system
   */
  public static void clearGrantor(
      long grantorVersion,
      DLockService service,
      int dlsSerialNumber,
      InternalDistributedSystem sys,
  ...
  private static GrantorInfo basicOp(
      long grantorVersion,
      DLockService service,
      int dlsSerialNumber,
      InternalDistributedSystem sys,
      ...
      byte opCode) {
    return basicOp(
        grantorVersion, service.getName(), service, dlsSerialNumber, sys, oldTurk, opCode);
  }
          ...
          if (es != null) {
            ...
            if (!sent) {
              if (logger.isTraceEnabled(LogMarker.DLS)) {
                logger.trace(
                    LogMarker.DLS, "Unable to communicate with elder {}", grc.currentElder);
              }
            }
            try {
  ...
  private GrantorRequestProcessor(
      InternalDistributedSystem system, InternalDistributedMember elder) {
    super(system, elder);
  }
  ...
  public static final class GrantorRequestMessage extends PooledDistributionMessage
    ...
    protected void basicProcess(final DM dm) {
      ...
      switch (this.opCode) {
        ...
        case CLEAR_OP:
          es.clearGrantor(
              this.grantorVersion, this.serviceName, this.dlsSerialNumber, getSender(), false);
          replyClear(dm);
          break;
        case CLEAR_WITH_LOCKS_OP:
          es.clearGrantor(
              this.grantorVersion, this.serviceName, this.dlsSerialNumber, getSender(), true);
          replyClear(dm);
          break;
        default:
  ...
  public static final class GrantorInfoReplyMessage extends ReplyMessage {
    ...
    public GrantorInfo getGrantorInfo() {
      return new GrantorInfo(
          this.grantor, this.elderVersionId, this.grantorSerialNumber, this.needsRecovery);
    }
