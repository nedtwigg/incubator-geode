 ...
 *
 * <p>Note that, unlike the reply processors in versions previous to GemFire 2.1, this reply
 * processor is kept entirely in JOM.
 *
 * <p>Recommended usage pattern in subclass...
 *
 * <pre>
 *
...
public class ReplyProcessor21 implements MembershipListener {
   ...
   *
   * <p>Concurrency: protected by synchronization of itself
   */
   ...
   *
   * <p>Since this is set by the executor and read by the initiating thread, this is a volatile.
   *
  ...
  static {
    String str =
        System.getProperty(
            DistributionConfig.GEMFIRE_PREFIX + "ack-severe-alert-reduction-ratio", ".80");
    double ratio;
    try {
   ...
   *
   * <p>NOTE: the communication layer does not reliably support sending a message to oneself, so
   * other means must be used to execute the message in this VM. Typically you would set the sender
   * of the message and then invoke its process() method in another thread. If you have questions,
   * talk to Bruce Schuchardt.
   */
  ...
  protected void process(DistributionMessage msg, boolean warn) {
    ...
    if (!removeMember(sender, false) && warn) {
      ...
      if (ids == null || ids.contains(sender)) {
        ...
        if (system.getConfig().getMcastPort()
                == 0 // could be using multicast & will get responses from everyone
   ...
   *
   * <p>Rationale for default handling: New operations can have caused changes to other newer
   * versioned GFE JVMs that cannot be reverted. So ignoring exceptions is a conservative way
   * considering such scenarios. It will be upto individual messages to handle differently by
  ...
  protected synchronized void processException(DistributionMessage msg, DSFIDNotFoundException ex) {
    ...
    logger.fatal(
        LocalizedMessage.create(
            LocalizedStrings.ReplyProcessor21_UNKNOWN_DSFID_ERROR,
            new Object[] {ex.getUnknownDSFID(), msg.getSender(), versionStr}),
        ex);
  }
      ...
      if (msecs == 0) {
        ...
        if (timedOut || !latch.await(timeout - timeSoFar - 1)) {
          ...
          if (isSevereAlertProcessingEnabled() && severeAlertTimeout > 0) {
            boolean timedout;
            do {
              this.severeAlertTimerReset =
                  false; // retry if this gets set by suspect processing (splitbrain requirement)
   ...
   * milliseconds pass before all acknowlegdements are received, <code>false</code> is returned.
   *
   * <p>Thread interruptions will be ignored while waiting. If interruption occurred while in this
  ...
  public final boolean waitForRepliesUninterruptibly(
      long p_msecs, StoppableCountDownLatch latch, boolean doCleanUp) throws ReplyException {
    if (this.keeperCleanedUp) {
      throw new IllegalStateException(
          LocalizedStrings
              .ReplyProcessor21_THIS_REPLY_PROCESSOR_HAS_ALREADY_BEEN_REMOVED_FROM_THE_PROCESSOR_KEEPER
  ...
  private void timeout(boolean suspectThem, boolean severeAlert) {
    ...
    final Object[] msgArgs =
        new Object[] {
          Long.valueOf(timeout + (severeAlert ? getSevereAlertThreshold() : 0)),
          this,
