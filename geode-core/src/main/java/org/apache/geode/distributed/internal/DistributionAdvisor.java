 ...
 * exists.
 *
 * <p>Provides <code>advise</code> methods for each type of operation that requires specialized
 ...
 *
 * <p>A primary design goal of this class is scalability: the footprint must be kept to a minimum as
...
public class DistributionAdvisor {
  ...
  public static final int START_VERSION_NUMBER =
      Integer.getInteger(
              DistributionConfig.GEMFIRE_PREFIX + "DistributionAdvisor.startVersionNumber", 1)
          .intValue();
  ...
  /**
   * Used to compare profile versioning numbers against {@link Integer#MAX_VALUE} and {@link
   * Integer#MIN_VALUE} to determine if a rollover has occurred.
   */
                ...
                while (!dr.isInitialized()) {
                  if (dr.isDestroyed()) {
                    ...
                    try {
                      if (isDebugEnabled) {
                        logger.debug(
                            "da.syncForCrashedMember waiting for region to finish initializing: {}",
                            dr);
                      }
                      Thread.sleep(100);
  ...
  /**
   * Update the Advisor with profiles describing remote instances of the {@link
   * DistributionAdvisor#getAdvisee()}. Profile information is versioned via {@link
   * Profile#getVersion()} and may be ignored if an older version is received after newer versions.
   *
  ...
  private synchronized boolean doPutProfile(Profile newProfile, boolean forceProfile) {
    ...
    if (isDebugEnabled_DA) {
      logger.trace(
          LogMarker.DA,
          "putProfile: Updating existing profile: {} with new profile: {} for {}",
          oldProfile,
          newProfile,
          getAdvisee().getFullPath());
    }
    if (oldProfile != null && !isNewerProfile(newProfile, oldProfile)) {
      ...
      if (isDebugEnabled_DA) {
        logger.trace(
            LogMarker.DA,
            "putProfile: Ignoring {} because it's older than or same as {} for {}",
            newProfile,
            oldProfile,
            getAdvisee().getFullPath());
      }
  ...
  /**
   * Returns true if newProfile is newer than oldProfile. This is determined by comparing {@link
   * Profile#getSerialNumber()} and {@link Profile#getVersion()}. If the old versioning number being
   * compared is above {@link #ROLLOVER_THRESHOLD_UPPER} and the new versioning number is below
   * {@link #ROLLOVER_THRESHOLD_LOWER} then a rollover is assumed to have occurred, which means the
   * new versioning number is newer.
   *
  ...
  public synchronized void forceNewMembershipVersion() {
    if (!membershipClosed) {
      ...
      if (logger.isTraceEnabled(LogMarker.STATE_FLUSH_OP)) {
        logger.trace(
            LogMarker.STATE_FLUSH_OP,
            "StateFlush forced new membership version: {}",
            membershipVersion);
      }
      synchronized (this.opCountLock) {
        previousVersionOpCount += currentVersionOpCount;
        ...
        if (logger.isTraceEnabled(LogMarker.DISTRIBUTION_STATE_FLUSH_OP)) {
          logger.trace(
              LogMarker.DISTRIBUTION_STATE_FLUSH_OP,
              "advisor for {} forced new membership version to {} previousOpCount={}",
              getAdvisee(),
              membershipVersion,
  ...
  public final synchronized long startOperation() {
    ...
    synchronized (this.opCountLock) {
      ...
      if (logger.isTraceEnabled(LogMarker.STATE_FLUSH_OP)) {
        logger.trace(
            LogMarker.STATE_FLUSH_OP,
            "StateFlush current opcount incremented: {}",
            currentVersionOpCount);
      }
    }
  ...
  public void waitForCurrentOperations(long timeout) {
    ...
    while (true) {
      ...
      if (isDebugEnabled_STATE_FLUSH_OP) {
        logger.trace(
            LogMarker.STATE_FLUSH_OP, "Waiting for current operations to finish({})", opCount);
      }
      try {
        Thread.sleep(50);
      } catch (InterruptedException e) {
   ...
   * still in the view.
   *
   * <p>We need this because we're asking membership questions from within listeners, and we don't
   * know whether the DM's membership listener fires before or after our own.
   *
   ...
   *
   * <p>This is often overridden in subclasses, but they need to defer to their superclass at some
  ...
  private boolean basicRemoveId(ProfileId memberId, boolean crashed, boolean destroyed) {
    ...
    if (profileRemoved == null) {
      if (isDebugEnabled) {
        logger.trace(
            LogMarker.DA, "DistributionAdvisor.removeId: no profile to remove for {}", memberId);
      }
      return false;
    }
    if (isDebugEnabled) {
  ...
  private boolean doRemoveId(
      ProfileId memberId, boolean crashed, boolean destroyed, boolean fromMembershipListener) {
    final boolean isDebugEnabled_DA = logger.isTraceEnabled(LogMarker.DA);
    if (isDebugEnabled_DA) {
      logger.trace(
          LogMarker.DA,
          "removeId: removing member {} from resource {}",
          memberId,
          getAdvisee().getFullPath());
    }
    synchronized (this) {
      ...
      if (!fromMembershipListener) {
        ...
        while (profileToRemove != null) {
          ...
          this.removedProfiles.put(
              profileToRemove.getDistributedMember(),
   ...
   */
  public boolean removeIdWithSerial(
      InternalDistributedMember memberId, int serialNum, boolean regionDestroyed) {
    if (logger.isTraceEnabled(LogMarker.DA)) {
      logger.trace(
          LogMarker.DA,
          "removeIdWithSerial: removing member {} with serial {} from resource {}",
          memberId,
          serialNum,
          getAdvisee().getName());
    }
    ...
    if (stillInView(memberId)) {
      ...
      if (isNews) {
        if (isDebugEnabled_DA) {
          logger.trace(
              LogMarker.DA,
              "updateRemovedProfiles: adding serial {} for member {} to removedProfiles",
              serialNum,
              memberId);
        }
  ...
  public Set adviseProfileExchange() {
    ...
    do {
      ...
      if (advisee == null) return getDefaultDistributionMembers();
  ...
  /**
   * A visitor interface for all the available profiles used by {@link
   * DistributionAdvisor#accept(ProfileVisitor, Object)}. Unlike the {@link Filter} class this does
   * not assume of two state visit of inclusion or exclusion rather allows manipulation of an
   * arbitrary aggregator that has been passed to the {@link #visit} method. In addition this is
  ...
  /**
   * Invoke the given {@link ProfileVisitor} on all the {@link Profile}s exiting when the {@link
   * ProfileVisitor#visit} method returns false. Unlike the {@link #adviseFilter(Filter)} method
   * this does assume the return type to be a Set of qualifying members rather allows for population
   * of an arbitrary aggregator passed as the argument to this method.
   *
   ...
   *     {@link ProfileVisitor#visit} method; this allows the {@link ProfileVisitor} to not maintain
   *     any state so that in many situations a global static object encapsulating the required
   *     behaviour will work
  ...
  public static class Profile implements DataSerializableFixedID {
    ...
    public void processIncoming(
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "While processing UpdateAttributes message ignored incoming profile: {}", this);
      }
    }
     ...
     */
    protected final void handleDistributionAdvisee(
