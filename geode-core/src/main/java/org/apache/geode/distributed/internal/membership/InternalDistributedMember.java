...
public class InternalDistributedMember
   ...
   *
   * <p>This, and the following constructor are the only valid ways to create an ID for a
   * distributed member for use in the P2P cache. Use of other constructors can break
  ...
  public InternalDistributedMember(
    ...
    this.netMbr =
        MemberFactory.newNetMember(
            i, p, splitBrainEnabled, canBeCoordinator, Version.CURRENT_ORDINAL, attr);
   ...
   *
   * <p>This is not the preferred way of creating an instance since the NetMember may not have all
   ...
   * string).
   *
   * <p><b> [bruce]THIS METHOD IS FOR TESTING ONLY. DO NOT USE IT TO CREATE IDs FOR USE IN THE
   * PRODUCT. IT DOES NOT PROPERLY INITIALIZE ATTRIBUTES NEEDED FOR P2P FUNCTIONALITY. </b>
   *
   ...
   * string).
   *
   * <p><b> THIS METHOD IS FOR TESTING ONLY. DO NOT USE IT TO CREATE IDs FOR USE IN THE PRODUCT. IT
   ...
   *
   * <p>< b> [bruce]DO NOT USE THIS METHOD TO CREATE ANYTHING OTHER THAN A LONER ID WITHOUT TALKING
   * TO ME FIRST. IT DOES NOT PROPERLY INITIALIZE THE ID. </b>
   *
  ...
  public InternalDistributedMember(
      ...
      throws UnknownHostException {
    MemberAttributes mattr =
        new MemberAttributes(
            p, org.apache.geode.internal.OSProcess.getId(), vmKind, -1, n, groups, attr);
  ...
  private void synchPayload() {
    netMbr.setAttributes(
        new MemberAttributes(
            dcPort, vmPid, vmKind, vmViewId, name, groups, durableClientAttributes));
  }
  ...
  public int compareTo(DistributedMember o, boolean checkNetMembersIfEqual, boolean verifyViewId) {
    ...
    if (myAddr == null && otherAddr == null) {
      if (myPort < otherPort) return -1;
      else if (myPort > otherPort) return 1;
      else return 0;
    } else if (myAddr == null) {
      return -1;
    } else if (otherAddr == null) return 1;
    ...
    if (myBytes != otherBytes) {
      for (int i = 0; i < myBytes.length; i++) {
        if (i >= otherBytes.length) return -1; // same as far as they go, but shorter...
        if (myBytes[i] < otherBytes[i]) return -1;
        if (myBytes[i] > otherBytes[i]) return 1;
      }
      if (myBytes.length > otherBytes.length) return 1; // same as far as they go, but longer...
    }
  ...
  private String shortName(String hostname) {
    if (hostname == null) return "<null inet_addr hostname>";
  ...
  public String toString() {
    ...
    if (result == null) {
      ...
      if (add.isMulticastAddress()) host = add.getHostAddress();
      else {
      ...
      if (vmPid > 0 || vmKind != DistributionManager.NORMAL_DM_TYPE || !"".equals(myName)) {
        ...
        if (vmPid > 0) sb.append(Integer.toString(vmPid));
  ...
  public void fromDataPre_GFE_7_1_0_0(DataInput in) throws IOException, ClassNotFoundException {
    ...
    netMbr =
        MemberFactory.newNetMember(
            ...
            sbEnabled,
            elCoord,
            InternalDataSerializer.getVersionForDataStream(in).ordinal(),
            attr);
