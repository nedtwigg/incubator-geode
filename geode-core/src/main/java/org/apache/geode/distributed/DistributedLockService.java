...
public abstract class DistributedLockService {
   ...
   * @throws IllegalStateException if this process is in the middle of disconnecting from the <code>
   *     DistributedSystem</code>
   */
   ...
   * thrown.
   *
   * @param serviceName the name of the instance to destroy, previously supplied in the <code>
   *     create(String, DistributedSystem)</code> invocation.
   ...
   * and returning <code>false</code>. If the lock is acquired, it is held until <code>
   * unlock(Object name)</code> is invoked, or until <code>leaseTimeMillis</code> milliseconds have
   * passed since the lock was granted - whichever comes first.
   *
   * <p>Locks are reentrant. If a thread invokes this method n times on the same instance,
   * specifying the same <code>name</code>, without an intervening release or lease expiration
   * expiration on the lock, the thread must invoke <code>unlock(name)</code> the same number of
   * times before the lock is released (unless the lease expires). When this method is invoked for a
   * lock that is already acquired, the lease time will be set to the maximum of the remaining least
   * time from the previous invocation, or <code>leaseTimeMillis</code>
   *
   ...
   *     A value of 0 causes this method to return false without waiting for the lock if the lock is
   ...
   *     and returning false. A value of -1 causes this method to block until suspension is granted.
   ...
   *
   * <p>If another member calls <code>becomeLockGrantor</code> after this member, that member will
   ...
   * this service. This can be explicitly requested by calling {@link
   * org.apache.geode.distributed.DistributedLockService#becomeLockGrantor()}. If no member has
   * explicitly requested grantor authority, then one member participating in the service will be
   * implicitly selected. In either case, this method returns true if the calling member is the
   * grantor.
   *
