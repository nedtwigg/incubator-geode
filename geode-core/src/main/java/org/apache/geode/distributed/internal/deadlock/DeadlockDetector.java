...
/**
 * A class used for detecting deadlocks. The static method {@link
 * #collectAllDependencies(Serializable)} will find all dependencies between threads and locks in
 * the current VM.
 *
 * <p>To use this class, collect dependencies in each VM you want to analyze, and add them to an
 * instance of a {@link DeadlockDetector} using the {@link #addDependencies(Set)} method. The {@link
 * #findDeadlock()} method will analyze the dependencies to find any deadlocks.
 *
 * <p>This class uses Java 1.6 management APIs on {@link ThreadMXBean}, so it will not find any
 ...
 *
 * <p>This class also has a main() method that can read serialized DependencyGraphs from multiple
 * JVMs, merge them and perform various analysis on them.
 */
