...
public class GMSJoinLeave implements JoinLeave, MessageHandler {
   ...
   *
   * <p>If the response indicates there's no coordinator it will contain a set of members that have
  ...
  public boolean join() {
    ...
    try {
      ...
      for (int tries = 0; !this.isJoined && !this.isStopping; tries++) {
        ...
        try {
          logger.debug(
              "sleeping for {} before making another attempt to find the coordinator", retrySleep);
      ...
      if (!this.isJoined) {
        logger.debug(
            "giving up attempting to join the distributed system after "
      ...
      if (!this.isJoined && state.hasContactedAJoinedLocator) {
        throw new SystemConnectException(
            "Unable to join the distributed system in "
                + (System.currentTimeMillis() - startTime)
                + "ms");
      }
  ...
  private JoinResponseMessage waitForJoinResponse() throws InterruptedException {
    ...
    synchronized (joinResponse) {
      if (joinResponse[0] == null && !isJoined) {
        ...
        long timeout =
            Math.max(
                services.getConfig().getMemberTimeout(), services.getConfig().getJoinTimeout() / 5);
  ...
  private void processJoinRequest(JoinRequestMessage incomingRequest) {
    ...
    if (incomingRequest.getMemberID().getVersionObject().compareTo(Version.CURRENT) < 0) {
      logger.warn(
          "detected an attempt to start a peer using an older version of the product {}",
          incomingRequest.getMemberID());
      JoinResponseMessage m =
          new JoinResponseMessage(
              "Rejecting the attempt of a member using an older version",
    ...
    try {
      rejection =
          services
              .getAuthenticator()
              .authenticate(incomingRequest.getMemberID(), (Properties) creds);
    } catch (Exception e) {
  ...
  private void processLeaveRequest(LeaveRequestMessage incomingRequest) {
    ...
    logger.info(
        "received leave request from {} for {}",
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "JoinLeave.processLeaveRequest invoked.  isCoordinator="
              + isCoordinator
              + "; isStopping="
              + isStopping
              + "; cancelInProgress="
    ...
    if (incomingRequest.getMemberID().equals(this.localAddress)) {
      logger.info(
          "I am being told to leave the distributed system by {}", incomingRequest.getSender());
  ...
  private void processRemoveRequest(RemoveMemberMessage incomingRequest) {
    ...
    if (v != null && !v.contains(incomingRequest.getSender())) {
      logger.info(
          "Membership ignoring removal request for "
              + mbr
              + " from non-member "
              + incomingRequest.getSender());
      return;
    ...
    if (!fromMe) {
      logger.info(
          "Membership received a request to remove "
              + mbr
              + " from "
              + incomingRequest.getSender()
              + " reason="
  ...
  private void becomeCoordinator(InternalDistributedMember oldCoordinator) {
    ...
    if (currentView == null) {
      ...
      newView.setFailureDetectionPort(
          localAddress, services.getHealthMonitor().getFailureDetectionPort());
  ...
  private void createAndStartViewCreator(NetView newView) {
    if (viewCreator == null || viewCreator.isShutdown()) {
      ...
      if (newView != null) {
        viewCreator.setInitialView(
            newView,
            newView.getNewMembers(),
  ...
  private NetView addMemberToNetView(InternalDistributedMember oldCoordinator) {
    ...
    synchronized (viewInstallationLock) {
      ...
      newView.setFailureDetectionPort(
          this.localAddress, services.getHealthMonitor().getFailureDetectionPort());
    }
    return newView;
  }
  ...
  private void sendRemoveMessages(
      List<InternalDistributedMember> removals,
      List<String> reasons,
  ...
  private boolean sendView(NetView view, boolean preparing, ViewReplyProcessor viewReplyProcessor)
    ...
    InstallViewMessage msg =
        new InstallViewMessage(
            view, services.getAuthenticator().getCredentials(this.localAddress), preparing);
    ...
    if (preparing) {
      ...
      if (isCoordinator
          && !localAddress.equals(view.getCoordinator())
          && getViewCreator() != null) {
    ...
    if (preparing) {
      ...
      if (conflictingView != null) {
        logger.warn(
            "received a conflicting membership view from "
                + conflictingViewSender
                + " during preparation: "
                + conflictingView);
        return false;
      }
  ...
  private void processViewMessage(final InstallViewMessage m) {
    ...
    if (currentView != null && !currentView.contains(m.getSender())) {
      ...
      if (this.preparedView == null || !this.preparedView.contains(m.getSender())) {
        logger.info(
            "Ignoring the view {} from member {}, which is not in my current view {} ",
            view,
            m.getSender(),
            currentView);
        return;
  ...
  /**
   * * testing purpose. Sets the TcpClient that is used by GMSJoinLeave to communicate with
   * Locators.
   *
  ...
  private boolean findCoordinator() {
    ...
    FindCoordinatorRequest request =
        new FindCoordinatorRequest(
            ...
            state.alreadyTried,
            state.viewId,
            services.getMessenger().getPublicKey(localAddress),
            services.getMessenger().getRequestId(),
            dhalgo);
        ...
        try {
          ...
          if (response != null) {
            ...
            if (!state.hasContactedAJoinedLocator
              ...
              logger.debug(
                  "Locator's address indicates it is part of a distributed system "
  ...
  protected class TcpClientWrapper {
    protected Object sendCoordinatorFindRequest(
        InetSocketAddress addr, FindCoordinatorRequest request, int connectTimeout)
        throws ClassNotFoundException, IOException {
      TcpClient client = new TcpClient();
      return client.requestToServer(
          addr.getAddress(), addr.getPort(), request, connectTimeout, true);
    }
  }
    ...
    if (this.isJoined) {
      ...
      resp =
          new FindCoordinatorResponse(
              v.getCoordinator(),
              localAddress,
              services.getMessenger().getPublicKey(v.getCoordinator()),
              req.getRequestId());
    } else {
      resp =
          new FindCoordinatorResponse(
  ...
  private void setCoordinatorPublicKey(FindCoordinatorResponse response) {
    if (response.getCoordinator() != null && response.getCoordinatorPublicKey() != null)
      services
          .getMessenger()
          .setPublicKey(response.getCoordinatorPublicKey(), response.getCoordinator());
  }
    ...
    synchronized (viewInstallationLock) {
      ...
      if (isJoined && isNetworkPartition(newView, true)) {
        if (quorumRequired) {
          ...
          forceDisconnect(
              LocalizedStrings.Network_partition_detected.toLocalizedString(
  ...
  private void removeMembersFromCollectionIfNotInView(
      Collection<InternalDistributedMember> members, NetView currentView) {
  ...
  private boolean isNetworkPartition(NetView newView, boolean logWeights) {
    ...
    if (failedWeight > 0 && logWeights) {
      ...
      if (failedWeight > failurePoint && quorumLostView != newView) {
        ...
        logger.warn(
            "total weight lost in this view change is {} of {}.  Quorum has been lost!",
            failedWeight,
            oldWeight);
  ...
  private void startViewBroadcaster() {
    services
        .getTimer()
        .schedule(new ViewBroadcaster(), VIEW_BROADCAST_INTERVAL, VIEW_BROADCAST_INTERVAL);
  }
  ...
  public void memberShutdown(DistributedMember mbr, String reason) {
    LeaveRequestMessage msg =
        new LeaveRequestMessage(
            Collections.singleton(this.localAddress), (InternalDistributedMember) mbr, reason);
  ...
  class ViewReplyProcessor {
    ...
    synchronized void processPendingRequests(
        Set<InternalDistributedMember> pendingLeaves,
    ...
    synchronized void processViewResponse(
        int viewId, InternalDistributedMember sender, NetView conflictingView) {
      if (!waiting) {
        return;
      }
  ...
  class ViewBroadcaster extends TimerTask {
    ...
    void sendCurrentView() {
      ...
      if (v != null) {
        InstallViewMessage msg =
            new InstallViewMessage(
                v, services.getAuthenticator().getCredentials(localAddress), false);
  ...
  class ViewCreator extends Thread {
     ...
     */
    synchronized void setInitialView(
        NetView newView,
      ...
      try {
        for (; ; ) {
          ...
          if (requests != null && !requests.isEmpty()) {
            logger.info(
                "View Creator is processing {} requests for the next membership view",
                requests.size());
            try {
    ...
    synchronized boolean informToPendingJoinRequests() {
      ...
      for (DistributionMessage msg : requests) {
        switch (msg.getDSFID()) {
          case JOIN_REQUEST:
            logger.debug(
                "Informing to pending join requests {} myid {} coord {}",
                msg,
                localAddress,
                v.getCoordinator());
            if (!v.getCoordinator().equals(localAddress)) {
              ...
              JoinResponseMessage jrm =
                  new JoinResponseMessage(
                      ((JoinRequestMessage) msg).getMemberID(),
                      v,
      ...
      do {
        ...
        if (conflictingView != null
          ...
          if (!newMembers.isEmpty()) {
            logger.info(
                "adding these new members from a conflicting view to the new view: {}", newMembers);
        ...
        if (!prepared) {
          ...
          NetView tempView =
              new NetView(
                  localAddress, newView.getViewId() + 1, newMembers, leaveReqs, removalReqs);
          for (InternalDistributedMember mbr : newView.getMembers()) {
    ...
    private void removeHealthyMembers(final Set<InternalDistributedMember> suspects)
      ...
      for (InternalDistributedMember mbr : suspects) {
        ...
        checkers.add(
            new Callable<InternalDistributedMember>() {
              @Override
    ...
    protected void filterMembers(
        Collection<InternalDistributedMember> mbrs,
        Set<InternalDistributedMember> matchingMembers,
        short requestType) {
    ...
    private <T> List<Future<T>> submitAll(
        ExecutorService executor, Collection<? extends Callable<T>> tasks) {
  ...
  boolean checkIfAvailable(InternalDistributedMember fmbr) {
    ...
    if (services
        .getHealthMonitor()
        .checkIfAvailable(fmbr, "Member failed to acknowledge a membership view", false)) {
  ...
  private InternalDistributedMember getMemId(
      NetMember jgId, List<InternalDistributedMember> members) {
