...
public class DLockToken {
  ...
  /**
   * Tracks expired leases so that the leasing thread can report a {@link
   * org.apache.geode.distributed.LeaseExpiredException}. Keys are threads that have had their lease
   * expire on this lock, but may not yet have noticed. Would use weak set if available. Entry is
   * removed upon throwing LeaseExpiredException. Protected by synchronization on this lock token.
   */
   ...
   *
   * <p>Public because {@link org.apache.geode.internal.admin.remote.RemoteDLockInfo} is a caller.
   *
    ...
    if (this.leaseId > -1 && this.leaseExpireTime < Long.MAX_VALUE) {
      ...
      if (currentTime > this.leaseExpireTime) {
        if (logger.isTraceEnabled(LogMarker.DLS)) {
          logger.trace(
              LogMarker.DLS, "[checkForExpiration] Expiring token at {}: {}", currentTime, this);
        }
        noteExpiredLease();
        basicReleaseLock();
        expired = true;
   ...
   */
  synchronized boolean grantLock(
      long newLeaseExpireTime, int newLeaseId, int newRecursion, RemoteThread remoteThread) {
   ...
   * @return true if lock was successfully released
   */
  synchronized boolean releaseLock(
      int leaseIdToRelease, RemoteThread remoteThread, boolean decRecursion) {
    ...
    if (this.destroyed) {
      IllegalStateException e =
          new IllegalStateException(
              LocalizedStrings.DLockToken_ATTEMPTING_TO_USE_DESTROYED_TOKEN_0.toLocalizedString(
                  this));
      throw e;
    }
  }
