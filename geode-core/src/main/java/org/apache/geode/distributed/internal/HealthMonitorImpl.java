...
public class HealthMonitorImpl implements HealthMonitor, Runnable {
  ...
  public HealthMonitorImpl(
      InternalDistributedMember owner, GemFireHealthConfig config, DistributionManager dm) {
    this.owner = owner;
    this.id = getNewId();
    this.dm = dm;
  ...
  public void run() {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Starting health monitor.  Health will be evaluated every {} seconds.",
          (sleepTime / 1000));
    }
    try {
      ...
      if (!this.stopRequested) {
        logger.warn(
            LocalizedMessage.create(
                LocalizedStrings.HealthMonitorImpl_UNEXPECTED_STOP_OF_HEALTH_MONITOR),
            ex);
      }
    } finally {
      this.eval.close();
