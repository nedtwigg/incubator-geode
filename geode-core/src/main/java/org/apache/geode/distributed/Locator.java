 ...
 *
 * <p>The stand-alone locator configuration provides high-availability of membership information.
 *
 * <p>This class allows a GemFire application VM to host a distribution locator. Such a
 * configuration minimizes the number of processes that are required to run GemFire. However,
 * hosting distribution locators is not generally recommended because if the application VM exits it
 * would not be possible for new applications to connect to the distributed system until it is
 * restarted.
 *
 * <p>Locators persist membership information in a locatorXXXview.dat file. This file is used to
 ...
 *
 * <p><b>NOTE:</b> In this release of the product locators log membership views and cache server
 * status in a locatorXXXviews.log file, where XXX is the locator's port. This is a rolling log
 * capped in size at 5mb. In order to log cache server status the locator will enable
 * server-location, so the locator must be started with a DistributedSystem or be started so that it
 * creates a DistributedSystem. This means that it is not possible in this release to use APIs to
 * start a locator and <i>then</i> start a DistributedSystem.
 *
 * <p>
 *
 * @since GemFire 4.0
...
public abstract class Locator {
   ...
   * will use.
   *
   * <p>The locator will not start a distributed system. The locator will provide peer location
   * services only.
   *
   ...
   *     <code>outputDir</code>
   * @throws org.apache.geode.GemFireIOException If the directory containing the <code>logFile
   *     </code> does not exist or cannot be written to
  ...
  public static Locator startLocator(int port, File logFile) throws IOException {
    ...
    return startLocator(
        port, logFile, false, (InetAddress) null, (Properties) null, true, false, null);
  }
   ...
   *
   * <p>The locator starts a AdminDistributedSystem configured with the given properties to provide
   * the system with a long-running process that can be relied on for stable membership information.
   * The locator will provide provide peer and cache server location services.
   *
   * @since GemFire 5.0
   * @param port The port on which the locator will listen for membership information requests from
   *     new members
   ...
   *     "locators" setting. If The distributed system is using multicast, the "mcast-port" should
   *     also be set.
   * @throws IllegalArgumentException If <code>port</code> is not in the range 0 to 65536
   * @throws org.apache.geode.SystemIsRunningException If another locator is already running in
   *     <code>outputDir</code>
  ...
  public static Locator startLocatorAndDS(
      int port, File logFile, Properties distributedSystemProperties) throws IOException {

    return startLocator(
        port, logFile, (InetAddress) null, distributedSystemProperties, true, true, null);
  }
   ...
   * configuration.
   *
   * <p>The locator will not start a distributed system. The locator will provice peer location
   * services only.
   *
   * @param port The port on which the locator will listen for membership information requests from
   *     new members
   ...
   *     are multiple locators, the "locators" property should be set. If multicast is being used,
   *     the "mcast-port" property should be set.
   * @throws IllegalArgumentException If <code>port</code> is not in the range 0 to 65536
   * @throws org.apache.geode.SystemIsRunningException If another locator is already running in
   *     <code>outputDir</code>
   * @throws org.apache.geode.GemFireIOException If the directory containing the <code>logFile
   ...
   */
  public static Locator startLocatorAndDS(
      int port, File logFile, InetAddress bindAddress, java.util.Properties dsProperties)
      throws IOException {
   ...
   * @throws IllegalArgumentException If <code>port</code> is not in the range 0 to 65536 or <code>
   *     peerLocator</code> and <code> serverLocator</code> are both false.
   * @throws org.apache.geode.SystemIsRunningException If another locator is already running in
   *     <code>outputDir</code>
   * @throws org.apache.geode.GemFireIOException If the directory containing the <code>logFile
   *     </code> does not exist or cannot be written to
  ...
  public static Locator startLocatorAndDS(
      ...
      throws IOException {
    return startLocator(
        port, logFile, bindAddress, dsProperties, peerLocator, serverLocator, hostnameForClients);
  }
  ...
  public String toString() {
    return LocalizedStrings.DistributionLocator_DISTRIBUTION_LOCATOR_ON_0.toLocalizedString(
        asString());
  }
   ...
   *
   * <p>This method of starting the locator is provided as an alternative to the <i>gemfire
   * start-locator</i> command to give you complete control over the java virtual machine's
   * configuration.
   *
   * <p>The <i>gemfire stop-locator</i> command can be used to stop a locator that is started with
   * this class.
   *
   * <p>java org.apache.geode.distributed.Locator port [bind-address] [gemfire-properties-file]
   * [peer] [server]
   *
   * <p>port - the tcp/ip port that the locator should listen on. This is the port number that
   * applications will refer to in their <i>locators</i> property in gemfire.properties
   *
   * <p>bind-address - the tcp/ip address that the locator should bind to. This can be missing or be
   * an empty string, which causes the locator to listen on all host addresses.
   *
   * <p>gemfire-properties-file - the location of a gemfire.properties file to be used in
   * configuring the locator's distributed system. This can be missing or be an empty string, which
   * will cause the locator to use the default search for gemfire.properties.
   *
   * <p>peer - true to start the peer locator service, false to disable it. If unspecified, default
   * to true.
   *
   * <p>server - true to start the cache server locator service, false to disable it. If
   * unspecified, defaults to true.
   *
   * <p>hostname-for-clients - the ip address or host name that clients will be told to use to
   * connect to this locator. If unspecified, defaults to the bind-address.
   *
   * @since GemFire 5.0
   */
