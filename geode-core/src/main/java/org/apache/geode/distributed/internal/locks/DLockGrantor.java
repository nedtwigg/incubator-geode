 ...
 * grantor.
 *
 * <p>ReadWriteLocks are not currently handled by grantor recovery or transfer.
 */
...
public class DLockGrantor {
   ...
   *
   * <p>State starts out as INITIALIZING and moves to READY at which point the grantor will start
   * servicing requests.
   *
   * <p>If a newcomer requests transfer of grantorship, then the state will change to HALTED and
   * then finally become DESTROYED when transfer is complete. During HALTED, the elder recognizes
   * the newcomer as the current grantor.
   *
   * <p>If this service is shutting down, it will seek out a successor to become the new grantor.
   ...
   *
   * <p>Key: Object batchId, Value: DLockBatch batch
   *
   * <p>Handling of batch locks synchronizes on this to assure serial processing.
   *
   * <p>guarded.By batchLocks
   */
   ...
   *
   * <p>Concurrency: protected by synchronization of {@link #suspendLock}
   */
   ...
   *
   * <p>Key=RemoteThread, Value=ReadLockCount
   *
   * <p>guarded.By {@link #suspendLock}
   */
   ...
   * service.
   *
   * <p>Returns null if unable to get ready grantor or if this process is not the grantor for the
   * service.
   *
   ...
   *
   * <p>Synchronizes on {@link #batchLocks}.
   *
   * @throws LockGrantorDestroyedException if grantor is destroyed
   */
  void handleLockBatch(DLockRequestMessage request) throws InterruptedException {
    synchronized (this.batchLocks) { // assures serial processing
      ...
      try {
        ...
        if (isDebugEnabled_DLS) {
          logger.trace(
              LogMarker.DLS, "[DLockGrantor.handleLockBatch] granting {}", batch.getBatchId());
        }
   ...
   *
   * <p>Acquires acquireDestroyReadLock. Synchronizes on batchLocks.
   *
   * <p>see org.apache.geode.internal.cache.TXCommitMessage#updateLockMembers()
   *
   ...
   * <p>Acquires acquireDestroyReadLock. Synchronizes on batchLocks.
   *
   * <p>see org.apache.geode.internal.cache.locks.TXCommitMessage#updateLockMembers()
   *
   ...
   *
   * <p>Synchronizes on grantTokens and the grant token if one exists.
   *
   ...
   *
   * <p>Acquires destroyReadLock. Synchronizes on grantTokens.
   *
   ...
   *
   * <p>Acquires destroyReadLock. Synchronizes on grantTokens, suspendLock and the grant token.
   *
  ...
  void handleLockRequest(DLockRequestMessage request) throws InterruptedException {
    ...
    if (!acquireDestroyReadLock(0)) {
      if (isLocalRequest(request) && this.dlock.isDestroyed()) {
        ...
        if (isDebugEnabled_DLS) {
          logger.trace(
              LogMarker.DLS, "[DLockGrantor.handleLockRequest] about to waitUntilDestroyed");
        }
   ...
   *
   * <p>Calling thread must hold destroyReadLock. Synchronizes on grantTokens, suspendLock and the
   * grant token.
   *
   * @param request the lock request to be processed by this grantor guarded.By {@link
   *     #acquireDestroyReadLock(long)}
   */
   ...
   *
   * <p>Acquires destroyReadLock. Synchronizes on this grantor, grantTokens, suspendLock, the grant
   * token.
   *
        ...
        for (Iterator iter = tokens.iterator(); iter.hasNext(); ) {
          ...
          try {
            ...
            synchronized (grantToken) {
              ...
              grantToken.grantLock(
                  owner, token.getLeaseExpireTime(), token.getLeaseId(), token.getLesseeThread());
   ...
   *
   * <p>Acquires destroyReadLock. Synchronizes on grantTokens and the grant token.
   *
    ...
    try {
      ...
      if (grant == null) {
        if (logger.isTraceEnabled(LogMarker.DLS)) {
          logger.trace(
              LogMarker.DLS,
              "[DLockGrantor.reenterLock] no grantToken found for {}",
      ...
      synchronized (grant) { // synchronize against grant.expireAndGrantLock
        ...
        grant.grantLock(
            request.getSender(), leaseExpireTime, request.getLockId(), grant.getRemoteThread());
   ...
   *
   * <p>Caller must hold destroyReadLock. Synchronizes on grantTokens and the grant token.
   *
   * @param name the name of the lock to release
   * @param owner the member attempting to release the granted lock
   * @param lockId the id of the lease used by the owner guarded.By {@link
   *     #acquireDestroyReadLock(long)}
   */
  private void getAndReleaseGrantIfLockedBy(
      Object name, InternalDistributedMember owner, int lockId) {
    synchronized (this.grantTokens) {
      ...
      try {
        if (isDestroyed()) {
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS,
                "[DLockGrantor.handleDepartureOf] grantor is destroyed; ignoring {}",
                owner);
          }
          return;
        }
        try {
          ...
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS, "[DlockGrantor.handleDepartureOf] ignored cancellation (1)");
          }
        } finally {
          synchronized (this.suspendLock) {
            ...
            for (Iterator it = removals.iterator(); it.hasNext(); ) {
              try {
                ...
                if (isDebugEnabled_DLS) {
                  logger.trace(
                      LogMarker.DLS, "[DlockGrantor.handleDepartureOf] ignored cancellation (2)");
                }
              }
            }
          } // synchronized
          synchronized (this.grantTokens) {
            ...
            for (Iterator iter = grants.iterator(); iter.hasNext(); ) {
              ...
              try {
                ...
                if (isDebugEnabled_DLS) {
                  logger.trace(
                      LogMarker.DLS, "[DlockGrantor.handleDepartureOf] ignored cancellation (3)");
                }
              }
            } // for
            ...
            for (Iterator iter = grantsReferencingMember.iterator(); iter.hasNext(); ) {
              ...
              try {
                ...
                if (isDebugEnabled_DLS) {
                  logger.trace(
                      LogMarker.DLS, "[DlockGrantor.handleDepartureOf] ignored cancellation (4)");
                }
              }
            } // for
            ...
            for (Iterator iter = grantsToRemoveIfUnused.iterator(); iter.hasNext(); ) {
              ...
              try {
                ...
                if (isDebugEnabled_DLS) {
                  logger.trace(
                      LogMarker.DLS, "[DlockGrantor.handleDepartureOf] ignored cancellation (5)");
                }
              }
            } // for
          } // synchronized this.grantTokens
   ...
   *
   * <p>Acquires destroyWriteLock. Synchronizes on this grantor, grantTokens, and each grant token.
   */
  void destroy() {
    synchronized (this) {
      if (isDestroyed()) return;
      ...
      try {
        ...
        try {
          ...
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS, "[simpleDestroy] {} locks held", (locksHeld ? "with" : "without"));
          }
        } finally {
   ...
   *
   * <p>Caller must acquire destroyWriteLock. Synchronizes on suspendLock, grantTokens, and each
   * grant token.
   *
   * <p>guarded.By {@link #acquireDestroyWriteLock(long)}
   */
   ...
   *
   * <p>Caller must acquire destroyWriteLock.
   *
  ...
  void debug() {
    logger.info(
        ...
        LocalizedMessage.create(
            ...
            "[DLockGrantor.debug] svc="
                + this.dlock.getName()
                + "; state="
                + this.state
                + "; initLatch.ct="
                + this.whileInitializing.getCount()));
  }

  /**
   * Make this grantor ready for handling lock requests.
   *
   * <p>Synchronizes on this grantor.
   *
   ...
   *
   * <p>Acquires destroyReadLock. Synchronizes on suspendLock, grantTokens, and the grant token.
   *
   ...
   *
   * <p>Synchronizes on grantTokens and the grant token.
   *
   ...
   *
   * <p>Caller must synchronize on grantTokens
   *
   ...
   *
   * <p>Caller must synchronize on grantTokens and then the grantToken.
   *
  ...
  private void basicRemoveGrantToken(DLockGrantToken grantToken) {
    ...
    if (removed != null) {
      ...
      if (logger.isTraceEnabled(LogMarker.DLS)) {
        logger.trace(
            LogMarker.DLS,
            "[DLockGrantor.basicRemoveGrantToken] removed {}; removed={}",
            grantToken,
            removed);
      }
    }
   ...
   *
   * <p>Synchronizes on each grant token.
   *
  ...
  private String stateToString(int stateInt) {
    ...
    if (stateDesc == null) {
      throw new IllegalArgumentException(
          LocalizedStrings.DLockGrantor_UNKNOWN_STATE_FOR_GRANTOR_0.toLocalizedString(
   ...
   *
   * <p>Caller must synchronize on suspendLock.
   *
   * <p>Concurrency: protected by synchronization of {@link #suspendLock}
   *
  ...
  protected void suspendLocking(final RemoteThread myRThread, final int lockId) {
    ...
    if (logger.isTraceEnabled(LogMarker.DLS)) {
      logger.trace(
          LogMarker.DLS,
          "Suspend locking of {} by {} with lockId of {}",
          this.dlock,
          myRThread,
          lockId);
    }
    ...
    Assert.assertTrue(
        this.lockingSuspendedBy == null || this.lockingSuspendedBy.equals(myRThread),
        "Attempted to suspend locking for "
            + myRThread
            + " but locking is already suspended by "
  ...
  private void postReleaseSuspendLock(RemoteThread rThread, Object lock) {
    if (!isLockingSuspendedBy(rThread)) {
      ...
      if (logger.isTraceEnabled(LogMarker.DLS)) {
        logger.trace(
            LogMarker.DLS,
            "[postReleaseSuspendLock] locking is no longer suspended by {}",
            rThread);
      }
      return;
    }
    ...
    if (resume) {
      ...
      while (!suspendQueue.isEmpty()) {
        ...
        if (nextRequest.isSuspendLockingRequest()) {
          Assert.assertTrue(
              writeLockWaiters > 0, "SuspendLocking request is waiting but writeLockWaiters is 0");
          break;
        }
    ...
    if (logger.isTraceEnabled(LogMarker.DLS)) {
      logger.trace(
          LogMarker.DLS, "[postReleaseSuspendLock] new status {}", displayStatus(rThread, null));
    }
  }
  ...
  private void postReleaseReadLock(RemoteThread rThread, Object lock) {
    ...
    if (readLockCount < 1) {
      ...
      if (isDebugEnabled_DLS) {
        logger.trace(
            LogMarker.DLS, "[postReleaseReadLock] no locks are currently held by {}", rThread);
      }
      return;
    }
    readLockCount--;
    ...
    if (totalReadLockCount < 0) {
      if (isDebugEnabled_DLS) {
        logger.trace(
            LogMarker.DLS,
            "Total readlock count has dropped to {} for {}",
            totalReadLockCount,
            this);
      }
    }
    if (totalReadLockCount == 0 && !suspendQueue.isEmpty()) {
      ...
      if (nextRequest.isSuspendLockingRequest()) {
        ...
        String s =
            new StringBuilder("\n (readLockCount=")
                .append(readLockCount)
                .append(", totalReadLockCount=")
                .append(totalReadLockCount)
                .append(", writeLockWaiters=")
                .append(writeLockWaiters)
                .append(",\nsuspendQueue=")
                .append(suspendQueue)
                .append(",\npermittedRequests=")
                .append(permittedRequests)
                .toString();
        logger.warn(
            LocalizedMessage.create(
        ...
        Assert.assertTrue(
            false,
            LocalizedStrings.DLockGrantor_RELEASED_REGULAR_LOCK_WITH_WAITING_READ_LOCK_0.toString(
                s));
      }
    }
    if (isDebugEnabled_DLS) {
      logger.trace(
          LogMarker.DLS, "[postReleaseReadLock] new status {}", displayStatus(rThread, null));
    }
    checkTotalReadLockCount();
  }
  ...
  protected void postReleaseLock(RemoteThread rThread, Object lock) {
    ...
    synchronized (suspendLock) {
      ...
      if (logger.isTraceEnabled(LogMarker.DLS)) {
        logger.trace(
            LogMarker.DLS,
            "[postReleaseLock] rThread={} lock={} permittedRequests={} suspendQueue={}",
            rThread,
            lock,
            permittedRequests,
            suspendQueue);
   ...
   *
   * <p>Caller must acquire destroyReadLock. Synchronizes on suspendLock, grantTokens and each grant
   * token.
   *
   * <p>Concurrency: protected by {@link #destroyLock} via invoking {@link
   * #acquireDestroyReadLock(long)}
   */
      ...
      try {
        ...
        try {
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS,
                "LockGrantorDestroyedException respondWithNotGrantor to {}",
                request);
          }
          request.respondWithNotGrantor();
        } finally {
        ...
        try {
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS,
                "LockServiceDestroyedException respondWithNotGrantor to {}",
                request);
          }
          request.respondWithNotGrantor();
        } finally {
    ...
    synchronized (suspendLock) {
      ...
      if (isLockingSuspended() || writeLockWaiters > 0 || othersHaveReadLocks) {
        ...
        if (isDebugEnabled_DLS) {
          logger.trace(
              LogMarker.DLS,
              "[DLockGrantor.acquireSuspend] added '{}' to end of suspendQueue.",
              request);
        }
      } else {
    ...
    synchronized (suspendLock) {
      ...
      if (!threadHoldsLock && (isLockingSuspended() || writeLockWaiters > 0)) {
        ...
        if (isDebugEnabled_DLS) {
          logger.trace(
              LogMarker.DLS,
              "[DLockGrantor.acquireReadLockPermission] added {} to end of suspendQueue.",
              request);
        }
      } else {
        readLockCount++;
        ...
        if (isDebugEnabled_DLS) {
          logger.trace(
              LogMarker.DLS, "[DLockGrantor.acquireReadLockPermission] permitted {}", request);
        }
      }
      if (isDebugEnabled_DLS) {
        logger.trace(
  ...
  public static class DLockGrantToken {
     ...
     * queue.
     *
     * <p>Synchronizes on this grant token.
     *
     ...
     * <p>Synchronizes on this grant token.
     *
     * <p>NOTE: expiration is only as accurate as clock synchronization on the hardware that the
     ...
     *
     * <p>Caller must synchronize on this grant token.
     *
     * <p>Concurrency: protected by synchronization of *this* DLockGrantToken
     *
    ...
    protected synchronized boolean grantLockToRequest(DLockRequestMessage request) {
      Assert.assertTrue(
          request.getRemoteThread() != null); // KIRK search for these assertions and remove
     ...
     * <p>Caller must synchronize on this grant token.
     *
     * <p>Call stack: DLockReleaseMessage -> releaseIfLocked -> getAndReleaseGrantIfLockedBy ->
     ...
     *
     * <p>Synchronizes on this grant token, suspendLock, and grantTokens.
     *
     * @param member the departed member
     */
    protected void handleDepartureOf(
        final InternalDistributedMember member, final ArrayList grantsToRemoveIfUnused) {
      boolean released = false;
      RemoteThread rThread = null;
      final boolean isDebugEnabled_DLS = logger.isTraceEnabled(LogMarker.DLS);
      try {
        ...
        if (released) {
          try {
            ...
            if (isDebugEnabled_DLS) {
              logger.trace(
                  LogMarker.DLS, "[DLockGrantToken.handleDepartureOf] ignored cancellation (2)");
            }
          }
    ...
    protected synchronized void checkDepartureOf(
        final InternalDistributedMember member, final List grantsReferencingMember) {
    ...
    protected boolean grantLockToNextRequest() {
      ...
      if (isDebugEnabled_DLS) {
        logger.trace(
            LogMarker.DLS,
            "[DLockGrantToken.grantLock] {} isGranted={} hasWaitingRequests={}",
            getName(),
            isLeaseHeld(),
            hasWaitingRequests());
      }
      ...
      while (!isGranted(true) && hasWaitingRequests()) {
        try {
          ...
          if (isDebugEnabled_DLS) {
            logger.trace(
                LogMarker.DLS,
                "[DLockGrantToken.grantLock] granting {} to {}",
                getName(),
                request.getSender());
          }
      ...
      synchronized (request) {
        ...
        if (isSuspendLockingToken()) {
          synchronized (this.grantor.suspendLock) {
            Assert.assertTrue(
                ...
                "Locking is suspended by "
                    + this.grantor.lockingSuspendedBy
                    + " with lockId of "
        ...
        if (isSuspendLockingToken()) {
          synchronized (this.grantor.suspendLock) {
            ...
            Assert.assertTrue(
                this.grantor.isLockingSuspendedBy(request.getRemoteThread()),
                "Locking should now be suspended by "
                    + request.getRemoteThread()
                    + " with lockId of "
                    + request.getLockId()
                    + " instead of "
      ...
      if (this.lessee != null && this.leaseId > -1) {
        ...
        if (currentTime > this.leaseExpireTime) {
          ...
          if (logger.isTraceEnabled(LogMarker.DLS)) {
            logger.trace(
                LogMarker.DLS,
                "[checkForExpiration] Expired token at {}: {}",
                currentTime,
                toString(true));
          }
    ...
    private void basicGrantLock(
      ...
      if (logger.isTraceEnabled(LogMarker.DLS)) {
        logger.trace(
            LogMarker.DLS, "[DLockGrantToken.grantLock.grantor] Granting {}", toString(false));
      }
    }
     ...
     * use.
     *
     * <p>Caller must synchronize on this grant token. guarded.By this
     */
    void destroy() {
      if (!this.destroyed) {
        this.destroyed = true;
    ...
    private boolean releaseLock(InternalDistributedMember member, int lockId) {
      ...
      if (isLeaseHeldBy(member, lockId)) {
        if (logger.isTraceEnabled(LogMarker.DLS)) {
          logger.trace(
              LogMarker.DLS, "[DLockGrantToken.releaseLock] releasing ownership: {}", this);
        }
      ...
      if (logger.isTraceEnabled(LogMarker.DLS)) {
        logger.trace(
            LogMarker.DLS,
            "[DLockGrantToken.releaseLock] {} attempted to release: {}",
            member,
            this);
      }
      return false;
          ...
          synchronized (this.lock) { // synchronized
            ...
            if (this.timeToWait > 0) {
              if (isDebugEnabled_DLS) {
                logger.trace(
                    LogMarker.DLS,
                    "DLockGrantorThread is about to wait for {} ms.",
                    this.timeToWait);
              }
        ...
        public void memberDeparted(final InternalDistributedMember id, final boolean crashed) {
          ...
          try {
            if (isDebugEnabled_DLS) {
              logger.trace(
                  LogMarker.DLS,
                  "[DLockGrantor.memberDeparted] waiting thread pool will process id={}",
                  id);
            }
            distMgr
                    ...
                    new Runnable() {
                      public void run() {
                        try {
                          ...
                          if (isDebugEnabled_DLS) {
                            logger.trace(
                                LogMarker.DLS, "Ignored interrupt processing departed member");
                          }
                        }
                      }
                    });
            ...
            if (isDebugEnabled_DLS) {
              logger.trace(
                  LogMarker.DLS, "[DLockGrantor.memberDeparted] rejected handling of id={}", id);
            }
          }
        }

        protected void processMemberDeparted(
            InternalDistributedMember id, boolean crashed, DLockGrantor me)
            throws InterruptedException {
          if (logger.isTraceEnabled(LogMarker.DLS)) {
