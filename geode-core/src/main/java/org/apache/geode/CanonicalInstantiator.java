 ...
 * instead of needing to implement <code>newInstance()</code> you now must implement <code>
 * newInstance(DataInput)</code>. The addition of the <code>DataInput</code> parameter allows the
 * instance creator to optionally read data from the data input stream and use it to decide the
 ...
 *
 * <p>Note that {@link DataSerializer#fromData} is always called on the instance returned from
...
public abstract class CanonicalInstantiator extends Instantiator {
  ...
  /**
   * This method is not supported and if called will throw UnsupportedOperationException. Use {@link
