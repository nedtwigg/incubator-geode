...
public interface AuthInitialize extends CacheCallback {
   ...
   *
   * <p>This method can modify the given set of properties. For example it may invoke external
   * agents or even interact with the user.
   *
   * <p>Normally it is expected that implementations will filter out <i>security-*</i> properties
   * that are needed for credentials and return only those.
   *
   * @param securityProps the security properties obtained using a call to {@link
   *     DistributedSystem#getSecurityProperties} that will be used for obtaining the credentials
  ...
  public Properties getCredentials(
      Properties securityProps, DistributedMember server, boolean isPeer)
      throws AuthenticationFailedException;
