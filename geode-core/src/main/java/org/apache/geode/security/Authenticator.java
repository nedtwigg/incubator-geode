 ...
 * initialized on the group coordinator for each member during the {@link
 * DistributedSystem#connect(Properties)} call of a new member. For client-server, an object of this
 * class is created for each connection during the client-server handshake.
 *
 * <p>The static creation function should have the following signature: <code>
 * public static Authenticator [method-name]();</code> i.e. it should be a zero argument function.
 *
 * @since GemFire 5.5
