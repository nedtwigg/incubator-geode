...
public class SearchAndLoadDUnitTest extends JUnit4CacheTestCase {
  ...
  public final void preTearDownCacheTestCase() throws Exception {
    for (int h = 0; h < Host.getHostCount(); h++) {
      ...
      for (int v = 0; v < host.getVMCount(); v++) {
        host.getVM(v)
            .invoke(
                new SerializableRunnable("Clean up") {
                  public void run() {
                    cleanup();
                  }
                });
  ...
  public void testNetSearch() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Create ACK Region") {
          public void run() {
            try {
    ...
    vm0.invoke(
        new SerializableRunnable("Get a value") {
          public void run() {
            try {
              Object result = null;
   ...
   *
   * <p>VM0 is used to create a cache and a region having a loader that simulates the conditions
   * that caused the bug. One async thread then does a get() which invokes the loader. Another async
  ...
  public void testConcurrentLoad() throws Throwable {
    ...
    vm0.invoke(
        new CacheSerializableRunnable("create region " + name + " in vm0") {
          public void run2() {
            remoteLoaderInvoked = false;
            loaderInvoked = false;
            AttributesFactory factory = new AttributesFactory();
                  ...
                  public synchronized Object load(LoaderHelper helper) {
                    ...
                    if (firstInvocation) {
                      ...
                      try {
                        ...
                        System.out.println(
                            "waiting for vm0t2 to be ready before throwing exception");
                ...
                public void run2() {
                  ...
                  try {
                    LogWriterUtils.getLogWriter()
                        .info("t1 retrieved value " + region.get(objectName));
  ...
  public void testNetLoadNoLoaders() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Get with No Loaders defined") {
          public void run() {
            try {
  ...
  public void testNetLoad() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Get a value from remote loader") {
          public void run() {
            for (int i = 0; i < 1; i++) {
              try {
                Object result = getRootRegion().getSubregion(name).get(objectName);
  ...
  public void testEmptyNetLoad() throws CacheException, InterruptedException {
    ...
    SerializableRunnable installLoader =
        new SerializableRunnable("Create ACK Region") {
          public void run() {
            loaderInvoked = false;
            remoteLoaderInvoked = false;
    ...
    assertEquals(
        "vm1="
            + vmRemoteLoaderInvoked(vm1)
            + " vm2="
            + vmRemoteLoaderInvoked(vm2)
            + " vm1Count="
            + vmRemoteLoaderInvokedCount(vm1)
            + " vm2Count="
            + vmRemoteLoaderInvokedCount(vm2),
        true,
        xor);
  ...
  public void testLocalLoad() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Get a value from local loader") {
          public void run() {
            try {
              Object result = getRootRegion().getSubregion(name).get(objectName);
              assertEquals(value, result);
  ...
  public void testNetWrite() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Create ACK Region with cacheWriter") {
          public void run() {
    ...
    vm0.invoke(
        new SerializableRunnable("ensure that cache writer was invoked") {
          public void run() {
  ...
  public void testOneHopNetWrite() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Create replicated region with cacheWriter") {
          public void run() {
            netWriteInvoked = false;
    ...
    vm1.invoke(
        new SerializableRunnable("Create empty Region") {
          public void run() {
            try {
              AttributesFactory factory = new AttributesFactory();
              factory.setScope(Scope.DISTRIBUTED_ACK);
  ...
  public void testOneHopNetWriteRemoteWriter() throws CacheException, InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Create replicate Region") {
          public void run() {
            try {
              AttributesFactory factory = new AttributesFactory();
              factory.setScope(Scope.DISTRIBUTED_ACK);
