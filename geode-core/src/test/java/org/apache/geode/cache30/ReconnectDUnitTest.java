...
public class ReconnectDUnitTest extends JUnit4CacheTestCase {
  ...
  public final void postSetUp() throws Exception {
    ...
    Host.getHost(0)
        .getVM(locatorVMNumber)
        .invoke(
            new SerializableRunnable("start locator") {
              public void run() {
                try {
  ...
  public final void postTearDownCacheTestCase() throws Exception {
    ...
    SerializableRunnable disconnect =
        new SerializableRunnable("disconnect and clean up") {
          public void run() {
  ...
  public void testReconnectWithQuorum() throws Exception {
    ...
    host.getVM(locatorVMNumber)
        .invoke(
            new SerializableRunnable("disable force-disconnect") {
              public void run() {
                GMSMembershipManager mgr =
                    (GMSMembershipManager)
                        MembershipManagerHelper.getMembershipManager(
  ...
  public void doTestReconnectOnForcedDisconnect(final boolean createInAppToo) throws Exception {
    ...
    SerializableCallable create2 =
        new SerializableCallable("Create Cache and Regions from cache.xml") {
          public Object call() throws CacheException {
            ...
            if (createInAppToo) {
              Thread recreateCacheThread =
                  new Thread("ReconnectDUnitTest.createInAppThread") {
                    public void run() {
    ...
    DistributedMember newdm =
        (DistributedMember)
            vm1.invoke(
                new SerializableCallable("wait for reconnect(1)") {
                  public Object call() {
                    ...
                    LogWriterUtils.getLogWriter()
                        .info("ds.isReconnecting() = " + ds.isReconnecting());
                    ...
                    try {
                      ...
                      assertTrue(
                          "Expected system to be restarted", ds.getReconnectedSystem() != null);
                      assertTrue(
                          "Expected system to be running", ds.getReconnectedSystem().isConnected());
                      ...
                      LogWriterUtils.getLogWriter()
                          .warning("interrupted while waiting for reconnect");
                      return null;
                    } finally {
                      if (failure) {
                        ds.disconnect();
    ...
    boolean stopped =
        (Boolean)
            vm1.invoke(
                new SerializableCallable("wait for reconnect and stop") {
                  public Object call() {
                    final DistributedSystem ds = ReconnectDUnitTest.savedSystem;
                    ReconnectDUnitTest.savedSystem = null;
                    Wait.waitForCriterion(
  ...
  private void ensureLocationServiceRunning(VM vm) {
    vm.invoke(
        new SerializableRunnable("ensureLocationServiceRunning") {
          public void run() {
  ...
  private DistributedMember waitForReconnect(VM vm) {
    return (DistributedMember)
        vm.invoke(
            new SerializableCallable("wait for Reconnect and return ID") {
              public Object call() {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "VM"
                            ...
                            + " waiting up to "
                            + waitTime
                            + " seconds for reconnect to complete");
                try {
                ...
                if (!(newViewId > oldViewId)) {
                  fail(
                      "expected a new ID to be assigned.  oldViewId="
                          + oldViewId
                          + "; newViewId="
                          + newViewId);
                }
  ...
  public void testReconnectALocator() throws Exception {
    ...
    vm0.invoke(
        new SerializableRunnable("Create a second locator") {
    ...
    try {
      ...
      vm0.invoke(
          new SerializableRunnable("check for running locator") {
            public void run() {
              WaitCriterion wc =
                  new WaitCriterion() {
                    public boolean done() {
      ...
      assertTrue(
          "expected " + locator2ViewLog.getPath() + " to grow in size",
          locator2ViewLog.length() > log2Size);
      // the other locator should have logged a new view
      assertTrue(
          "expected " + locatorViewLog.getPath() + " to grow in size",
  ...
  private void createGfshWaitingThread(VM vm) {
    vm.invoke(
        new SerializableRunnable("create Gfsh-like waiting thread") {
          public void run() {
            final Locator loc = Locator.getLocator();
            assertNotNull(loc);
            gfshThread =
                new Thread("ReconnectDUnitTest_Gfsh_thread") {
                  public void run() {
                    try {
                      ((InternalLocator) loc).waitToStop();
                    } catch (InterruptedException e) {
                      System.out.println(
                          Thread.currentThread().getName() + " interrupted - exiting");
                    }
                  }
                };
  ...
  private void assertGfshWaitingThreadAlive(VM vm) {
    vm.invoke(
        new SerializableRunnable("assert gfshThread is still waiting") {
          public void run() {
  ...
  public void testReconnectWithRoleLoss() throws TimeoutException, RegionExistsException {
    ...
    SerializableRunnable roleLoss =
        new CacheSerializableRunnable("ROLERECONNECTTESTS") {
          public void run2() throws RuntimeException {
            ...
            WaitCriterion ev =
                new WaitCriterion() {
                  ...
                  public String description() {
                    return "Waiting for reconnect count "
                        + timeReconnect
                        + " currently "
                        + reconnectTries;
                  }
  ...
  public void testReconnectWithRequiredRoleRegained() throws Throwable {
    ...
    vm0.invoke(
        new CacheSerializableRunnable("reset reconnect count") {
          @Override
    ...
    SerializableRunnable roleAPlayerForCacheInitialization =
        getRoleAPlayerForCacheInitializationRunnable(
            vm0,
            locPort,
            regionName,
  ...
  private CacheSerializableRunnable getRoleLossRunnable(
      final VM otherVM,
      final int locPort,
        ...
        try {
          ...
          WaitCriterion ev =
              new WaitCriterion() {
                public boolean done() {
                  return otherVM
                      .invoke(() -> ReconnectDUnitTest.isInitialRolePlayerStarted())
                      .booleanValue();
                }
              ...
              new WaitCriterion() {
                ...
                public boolean done() {
                  ...
                  try {
                    ...
                    LogWriterUtils.getLogWriter()
                        .info("MyKey : " + key + " and myvalue : " + value);
                    return true;
  ...
  public void testReconnectFailsInCacheCreation() throws Exception {
    ...
    SerializableRunnable createCache =
        new SerializableRunnable("Create Cache and Regions") {
          public void run() {
            locatorPort = locPort;
            final Properties props = getDistributedSystemProperties();
            props.put(MAX_WAIT_TIME_RECONNECT, "1000");
    ...
    vm1.invoke(
        new SerializableRunnable("wait for reconnect") {
          public void run() {
  ...
  private CacheSerializableRunnable getRoleAPlayerForCacheInitializationRunnable(
      final VM otherVM, final int locPort, final String regionName, final String startupMessage) {
    ...
    ReconnectListener reconlis =
        new ReconnectListener() {
          ...
          public void onReconnect(
              InternalDistributedSystem system1, InternalDistributedSystem system2) {}
        };
  ...
  public boolean forceDisconnect(VM vm) {
    return (Boolean)
        vm.invoke(
            new SerializableCallable("crash distributed system") {
  ...
  public static class CacheKillingListener extends CacheListenerAdapter implements Declarable {
    ...
    public void afterRegionCreate(final RegionEvent event) {
      if (crashCount == 0) {
        ...
        Thread t =
            new Thread("crash reconnecting system (ReconnectDUnitTest)") {
              public void run() {
