 ...
 *
 * <p>Note: See LocalRegionTest and MultiVMRegionTestCase for more expiration tests.
 *
 * @since GemFire 3.0
 */
...
public class RegionExpirationDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testRegionTTLAfterMutating() throws InterruptedException, CacheException {
    ...
    createWithExpiration(
        regionName, new ExpirationAttributes(firstTimeout, ExpirationAction.DESTROY), null);
  ...
  private void _testRegionTTL(final String regionName, final ExpirationAction action)
    ...
    vm0.invoke(
        new CacheSerializableRunnable("Create Region & Key") {
        ...
        new CacheSerializableRunnable("Get") {
          public void run2() throws CacheException {
            ...
            if (!originalValue.equals(newVal)) {
              assertTrue(
                  "expected original value but got " + newVal,
  ...
  protected void createWithExpiration(
      String regionName, ExpirationAttributes ttl, ExpirationAttributes idle)
