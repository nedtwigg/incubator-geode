...
public class TXDistributedDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testRemoteGrantor() throws Exception {
    ...
    SerializableRunnable remoteComm = new SerializableRunnable("testRemoteGrantor: remote grantor commit") {
      public void run() {
        try {
          Cache c = getCache();
  ...
  public void testInternalCallbacks() throws Exception {
...

    {
      ...
      for (int i = cbSensors.length - 3; i >= 0; --i) {
        assertEquals("Internal callback " + i + " was not called the expected number of times!", (byte) 1, cbSensors[i]);
      }
      for (int i = cbSensors.length - 1; i > cbSensors.length - 3; --i) {
        assertEquals("Internal \"during\" callback " + i + " invoked an unexpected number of times!", (byte) 2, cbSensors[i]);
      }
    }
