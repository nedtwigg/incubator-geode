...
public class TXDistributedDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testRemoteGrantor() throws Exception {
    ...
    Invoke.invokeInEveryVM(
        new SerializableRunnable("testRemoteGrantor: initial configuration") {
          public void run() {
            try {
    ...
    vm0.invoke(
        new SerializableRunnable("testRemoteGrantor: remote grantor init") {
          public void run() {
            try {
    ...
    vm0.invoke(
        new SerializableRunnable("Disconnect from DS, remote grantor death") {
          public void run() {
            try {
  ...
  public void testInternalCallbacks() throws Exception {
...

    {
      ...
      for (int i = cbSensors.length - 3; i >= 0; --i) {
        assertEquals(
            "Internal callback " + i + " was not called the expected number of times!",
            (byte) 1,
            cbSensors[i]);
      }
  ...
  static final void setInternalCallbacks(TXStateInterface txp, final byte[] cbSensors) {
    ...
    tx.setAfterReservation(
        new Runnable() {
          public void run() {
            cbSensors[0]++;
          }
        });
    tx.setAfterConflictCheck(
        new Runnable() {
          public void run() {
            cbSensors[1]++;
          }
        });
    tx.setAfterApplyChanges(
        new Runnable() {
          public void run() {
            cbSensors[2]++;
          }
        });
    tx.setAfterReleaseLocalLocks(
        new Runnable() {
          public void run() {
            cbSensors[3]++;
          }
        });
    tx.setAfterIndividualSend(
        new Runnable() {
          public void run() {
            cbSensors[4]++;
          }
        });
    tx.setAfterIndividualCommitProcess(
        new Runnable() {
          public void run() {
            cbSensors[5]++;
          }
    ...
    tx.setDuringIndividualSend(
        new Runnable() {
          public void run() {
            cbSensors[7]++;
          }
        });
    tx.setDuringIndividualCommitProcess(
        new Runnable() {
          public void run() {
            cbSensors[8]++;
          }
  ...
  public void testDACKLoadedMessage() throws Exception {
    ...
    Invoke.invokeInEveryVM(
        new SerializableRunnable("testDACKLoadedMessage: intial configuration") {
          public void run() {
            try {
    ...
    Invoke.invokeInEveryVM(
        new SerializableRunnable("testDACKLoadedMessage: confirm standard case") {
          public void run() {
            Region rgn1 = getCache().getRegion(rgnName);
  ...
  public void testHighAvailabilityFeatures() throws Exception {
    ...
    originVM.invoke(
        new SerializableRunnable("Flakey DuringIndividualSend Transaction") {
          public void run() {
            ...
            try {
              ...
              TXState txState =
                  (TXState)
                      ((TXStateProxyImpl) ((TXManagerImpl) txMgrImpl).getTXState())
                          .getRealDeal(null, null);
              txState.setDuringIndividualSend(
                  new Runnable() {
    ...
    originVM.invoke(
        new SerializableRunnable("Flakey AfterIndividualSend Transaction") {
          public void run() {
            final Region rgn1 = getCache().getRegion(rgnName);
            assertNotNull(rgn1);
            try {
              ...
              TXState txState =
                  (TXState)
                      ((TXStateProxyImpl) ((TXManagerImpl) txMgrImpl).getTXState())
                          .getRealDeal(null, null);
              txState.setAfterIndividualSend(
                  new Runnable() {
                    public synchronized void run() {
                      MembershipManagerHelper.crashDistributedSystem(getSystem());
                    }
    ...
    originVM.invoke(
        new SerializableRunnable("Flakey DuringIndividualCommitProcess Transaction") {
          public void run() {
            final Region rgn1 = getCache().getRegion(rgnName);
            assertNotNull(rgn1);
            try {
              ...
              // call.
              txState.setDuringIndividualCommitProcess(
                  new Runnable() {
                    private int numCalled = 0;
                    ...
                    public synchronized void run() {
                      ...
                      rgn1.getCache()
                          .getLogger()
                          .info(
                              "setDuringIndividualCommitProcess Runnable called "
                                  + numCalled
                                  + " times");
                      if (numCalled > 1) {
                        MembershipManagerHelper.crashDistributedSystem(getSystem());
    ...
    originVM.invoke(
        new SerializableRunnable("Flakey solo region DuringIndividualSend Transaction") {
          public void run() {
  ...
  public static class PausibleTX implements Runnable {
    ...
    public void run() {
      ...
      TXState txState =
          (TXState)
              ((TXStateProxyImpl) ((TXManagerImpl) txMgr).getTXState()).getRealDeal(null, null);
  ...
  public void testLockBatchParticipantsUpdate() throws Exception {
    ...
    // is turned on
    vm2.invoke(
        new SerializableRunnable("connect vm2 if not connected") {
          public void run() {
            getCache();
          }
        });
    ...
    vm0.invoke(
        new SerializableRunnable("testLockBatchParticipantsUpdate: remote grantor init") {
          public void run() {
            try {
              Region rgn1 = getCache().getRegion(rgnName);
              final CacheTransactionManager txMgr2 = getCache().getCacheTransactionManager();
    ...
    vm0.invoke(
        new SerializableRunnable(
          ...
          public void run() {
            ...
            assertTrue(
                "Members in lock batch "
                    + txLockBatch.getParticipants()
                    + " not the same as "
    ...
    vm2.invoke(
        new SerializableRunnable("testLockBatchParticipantsUpdate: start GII") {
          public void run() {
            try {
              AttributesFactory factory = new AttributesFactory();
              factory.setScope(Scope.DISTRIBUTED_ACK);
  ...
  public void testRemoteCommitFailure() throws Exception {
    try {
      ...
      try {
        ee =
            IgnoredException.addIgnoredException(
                DiskAccessException.class.getName()
                    + "|"
      ...
      SerializableCallable allowCacheToShutdown =
