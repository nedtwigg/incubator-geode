 ...
 *
 * <p>Used by: DiskDistributedNoAckRegionTestCase TODO: move this content into
...
public class DiskRegionTestImpl implements Serializable {
   ...
   *
   * <p>Note: The regions in the following description all have the same unique name. 1) Create
   * backup region in VM0 and add some values so they get backed up 2) Close that region 3) Create
  ...
  public void testBackupFillValues() throws CacheException {
    ...
    vm0.invoke(
        new CacheSerializableRunnable("Create backup Region in VM0") {
          public void run2() throws CacheException {
    ...
    vm1.invoke(
        new CacheSerializableRunnable("Create & Populate non-mirrored in VM1") {
          public void run2() throws CacheException {
            ...
            DataPolicy dataPolicy =
                DiskRegionTestImpl.this.rtc.getRegionAttributes().getDataPolicy();
    ...
    vm0.invoke(
        new CacheSerializableRunnable("Close Cache in VM0") {
          public void run2() throws CacheException {
            CacheTestCase.closeCache();
          }
        });
    ...
    vm0.invoke(
        new CacheSerializableRunnable(runnableName) {
          public void run2() throws CacheException {
