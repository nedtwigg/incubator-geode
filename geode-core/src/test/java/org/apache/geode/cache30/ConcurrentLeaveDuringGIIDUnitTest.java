...
public class ConcurrentLeaveDuringGIIDUnitTest extends JUnit4CacheTestCase {
   ...
   *
   * <p>The test installs a GII hook in A that causes it to pause after announcing creation of the
   * region.
   *
   * <p>X then creates its region and does an operation and closes its cache.
   *
   * <p>B then starts and creates its region, not doing a GII from A since A is still initializing.
   *
   * <p>A is then allowed to start its GII and pulls an image from B.
   */
  @Ignore
  @Test
  public void testBug48962() throws Exception {
    ...
    SerializableCallable createRegionXB =
        new SerializableCallable("create region in X and B") {
          public Object call() {
    ...
    SerializableCallable createRegionA =
        new SerializableCallable("create region in A") {
          public Object call() {
            final GiiCallback cb =
                new GiiCallback(
                    InitialImageOperation.GIITestHookType.BeforeGetInitialImage, regionName);
            InitialImageOperation.setGIITestHook(cb);
            Thread t =
                new Thread("create region in a thread that will block before GII") {
                  public void run() {
                    Region r =
    ...
    A.invoke(
        new SerializableRunnable("make sure A got keyFromX from X") {
          public void run() {
    ...
    A.invoke(
        new SerializableRunnable("allow A to continue GII from B") {
          public void run() {
            GiiCallback cb =
                (GiiCallback)
                    InitialImageOperation.getGIITestHookForCheckingPurpose(
    ...
    B.invoke(
        new SerializableRunnable("ensure B is now consistent") {
          public void run() {
