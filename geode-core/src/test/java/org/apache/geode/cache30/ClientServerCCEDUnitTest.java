...
public class ClientServerCCEDUnitTest extends JUnit4CacheTestCase {
  ...
  private void registerInterest(VM vm) {
    vm.invoke(
        new SerializableRunnable("register interest in all keys") {
          public void run() {
  ...
  private void unregisterInterest(VM vm) {
    vm.invoke(
        new SerializableRunnable("unregister interest in all keys") {
          public void run() {
  ...
  private void registerInterest(VM vm, final List keys) {
    vm.invoke(
        new SerializableRunnable("register interest in key list") {
          public void run() {
  ...
  private void registerInterestOneKey(VM vm, final String key) {
    vm.invoke(
        new SerializableRunnable("register interest in " + key) {
          public void run() {
  ...
  private void ensureAllTombstonesPresent(VM vm) {
    vm.invoke(
        new SerializableCallable("check all are tombstones") {
          public Object call() {
            for (int i = 0; i < 10; i++) {
              assertTrue(
                  "expected a tombstone for Object" + i,
  ...
  private void ensureAllTombstonesPresent(VM vm, final List keys) {
    vm.invoke(
        new SerializableCallable("check tombstones in list") {
          public Object call() {
            for (Object key : keys) {
              assertTrue(
                  "expected to find a tombstone for " + key, TestRegion.containsTombstone(key));
            }
            return null;
          }
        });
    ...
    vm.invoke(
        new SerializableCallable("check all are tombstones") {
          public Object call() {
            for (int i = 0; i < 10; i++) {
              assertTrue(
                  "expected to find an entry for Object" + i, TestRegion.containsKey("Object" + i));
              assertTrue(
                  "expected to find entry invalid for Object" + i,
        ...
        new SerializableCallable("check tombstones in list") {
          public Object call() {
            for (Object key : keys) {
              ...
              assertTrue(
                  "expected to find entry invalid for " + key, !TestRegion.containsValue(key));
            }
            return null;
          }
        });
  ...
  private void getAll(VM vm) {
    vm.invoke(
        new SerializableRunnable("getAll for all keys") {
          public void run() {
  ...
  private void clearLocalCache(VM vm) {
    vm.invoke(
        new SerializableRunnable("clear local cache") {
          public void run() {
  ...
  private void createEntries(VM vm) {
    vm.invoke(
        new SerializableCallable("create entries") {
          public Object call() {
            for (int i = 0; i < 10; i++) {
  ...
  private void destroyEntries(VM vm) {
    vm.invoke(
        new SerializableCallable("destroy entries") {
          public Object call() {
            for (int i = 0; i < 10; i++) {
  ...
  private void doPutAllInClient(VM vm) {
    vm.invoke(
        new SerializableRunnable("do putAll") {
          public void run() {
            Map map = new HashMap();
            ...
            try {
              ...
              for (int i = 1000; i < 1100; i++) {
                assertTrue(
                    "expected key object_" + i + " to be in the cache but it isn't",
  ...
  private void invalidateEntries(VM vm) {
    vm.invoke(
        new SerializableCallable("invalidate entries") {
          public Object call() {
            for (int i = 0; i < 10; i++) {
  ...
  private void checkClientReceivedGC(VM vm) {
    vm.invoke(
        new SerializableCallable("check that GC happened") {
          public Object call() throws Exception {
            WaitCriterion wc =
                new WaitCriterion() {
                ...
                new WaitCriterion() {
                  ...
                  public boolean done() {
                    ...
                    if (regionEntryCount > 0) {
                      LogWriterUtils.getLogWriter()
                          .info(
                              "TestRegion has unexpected entries - all should have been GC'd but we have "
                                  + regionEntryCount);
                      TestRegion.dumpBackingMap();
                      return false;
                    }
  ...
  private void checkClientDoesNotReceiveGC(VM vm) {
    vm.invoke(
        new SerializableCallable("check that GC did not happen") {
          public Object call() throws Exception {
  ...
  private void createClientRegion(
      ...
      final int port,
      final boolean ccEnabled,
      final ClientRegionShortcut clientRegionShortcut) {
    SerializableCallable createRegion =
        new SerializableCallable() {
          public Object call() throws Exception {
  ...
  private void createDurableClientRegion(
      final VM vm,
      final String regionName,
      final int port1,
      final int port2,
