 ...
 * Scope#DISTRIBUTED_NO_ACK distributed no ACK}.
 *
 * @since GemFire 3.0
 */
...
public class DistributedNoAckRegionDUnitTest extends MultiVMRegionTestCase {
  ...
  public void testIncompatibleSubregions() throws Exception {
    ...
    vm0.invoke(
        new SerializableRunnable("Create NO ACK Region") {
          public void run() {
            try {
              Region region = createRegion(name, "INCOMPATIBLE_ROOT", getRegionAttributes());
              assertTrue(
                  getRootRegion("INCOMPATIBLE_ROOT")
                      .getAttributes()
                      .getScope()
                      .isDistributedNoAck());
    ...
    vm1.invoke(
        new SerializableRunnable("Create GLOBAL Region") {
          public void run() {
            try {
  ...
  public void testBug30705() throws Exception {
    ...
    SerializableRunnable put =
        new CacheSerializableRunnable("Distributed NoAck Puts") {
          public void run2() throws CacheException {
    ...
    try {
      ...
      vm0.invoke(
          new SerializableRunnable("Interrupt Puts") {
            public void run() {
      ...
      vm2.invoke(
          new SerializableRunnable("Wait for Overflow Queue") {
            public void run() {
              WaitCriterion ev =
                  new WaitCriterion() {
                    public boolean done() {
                      return getSystem().getDistributionManager().getStats().getOverflowQueueSize()
                          == 0;
                    }
