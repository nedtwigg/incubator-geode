 ...
 *       <tt>assertTrue</tt>, etc.) It is OK (but not particularly recommended) for other code to
 *       use these forms too. Only the most typically used JUnit assertion methods are defined this
 *       way, but enough to live with.
 *   <li> If you override {@link #setUp} or {@link #tearDown}, make sure to invoke
 *       <tt>super.setUp</tt> and <tt>super.tearDown</tt> within them. These methods are used to
 *       clear and check for thread assertion failures.
 *   <li>All delays and timeouts must use one of the constants <tt> SHORT_DELAY_MS</tt>, <tt>
 *       SMALL_DELAY_MS</tt>, <tt> MEDIUM_DELAY_MS</tt>, <tt> LONG_DELAY_MS</tt>. The idea here is
 *       that a SHORT is always discriminable from zero time, and always allows enough time for the
 *       small amounts of computation (creating a thread, calling a few methods, etc) needed to
 *       reach a timeout point. Similarly, a SMALL is always discriminable as larger than SHORT and
 *       smaller than MEDIUM. And so on. These constants are set to conservative values, but even
 *       so, if there is ever any doubt, they can all be increased in one spot to rerun tests on
 *       slower platforms
 *   <li> All threads generated must be joined inside each test case method (or <tt>fail</tt> to do
 *       so) before returning from the method. The <tt> joinPool</tt> method can be used to do this
