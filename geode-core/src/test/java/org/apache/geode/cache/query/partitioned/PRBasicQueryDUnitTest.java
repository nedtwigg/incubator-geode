...
public class PRBasicQueryDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void testPRCountStarQuery() throws Exception {
    ...
    vm0.invoke(
        PRQHelp.getCacheSerializableRunnableForPRDuplicatePuts(
            name, portfolio, cnt, cntDest + 100));
    ...
    vm0.invoke(
        PRQHelp.getCacheSerializableRunnableForPRDuplicatePuts(
            localName, portfolio, cnt, cntDest + 100));
    vm2.invoke(
        PRQHelp.getCacheSerializableRunnableForPRPuts(localName, portfolio, cnt, cntDest + 100));
    vm2.invoke(
        PRQHelp.getCacheSerializableRunnableForPRDuplicatePuts(
  ...
  public void testPROrderBy() throws Exception {
    ...
    vm1.invoke(
        new CacheSerializableRunnable("Adding portfolios") {
          public void run2() {
            Cache cache = getCache();
            Region region = cache.getRegion(name);
            for (int j = 0; j < 100; j++) region.put(new Integer(j), new Portfolio(j));
          }
        });

    vm1.invoke(
        new CacheSerializableRunnable("Running Query") {
          public void run2() {
            try {
