...
public class SelectStarQueryDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testSelectStarQueryForPartitionedRegion() throws Exception {
    ...
    server1.invoke(
        new SerializableCallable("Set observer") {
          @Override
    ...
    client.invoke(
        new SerializableCallable("Put objects") {
          @Override
          public Object call() throws Exception {
    ...
    client.invoke(
        new SerializableCallable("Query") {
          @Override
          public Object call() throws Exception {
                  ...
                  if (rs instanceof StructImpl) {
                    for (Object obj : ((StructImpl) rs).getFieldValues()) {
                      if (obj instanceof PortfolioPdx || obj instanceof PositionPdx) {
                        ...
                        fail(
                            "Result objects for remote client query: "
                                + queries[i]
    ...
    server1.invoke(
        new SerializableCallable("Get observer") {
          @Override
          public Object call() throws Exception {
        ...
        new SerializableCallable("Query") {
          ...
          public Object call() throws Exception {
            QueryObserver observer =
                QueryObserverHolder.setInstance(new QueryResultTrackingObserver());
              ...
              if (i == 3) {
                ...
                for (Object o : res) {
                  if (o instanceof byte[]) {
                    ...
                    fail(
                        "Result objects for server local query: "
                            + queries[i]
        ...
        (Integer)
            vm.invoke(
                new SerializableCallable("Create Server1") {
                  ...
                  public Object call() throws Exception {
                    Region r1 =
                        getCache().createRegionFactory(RegionShortcut.PARTITION).create(regName);
                    // put domain objects
                    for (int i = 0; i < objs.length; i++) {
                      r1.put("key-" + i, objs[i]);
                    }
                ...
                new SerializableCallable("Create Server1") {
                  ...
                  public Object call() throws Exception {
                    Region r1 =
                        getCache().createRegionFactory(RegionShortcut.REPLICATE).create(regName);
                    Region r2 =
                        getCache().createRegionFactory(RegionShortcut.REPLICATE).create(regName2);
                    // put domain objects
                    for (int i = 0; i < 10; i++) {
                      r1.put("key-" + i, new PortfolioPdx(i));
                      r2.put("key-" + i, new PortfolioPdx(i));
