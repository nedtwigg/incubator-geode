...
public class PRQueryDUnitHelper implements Serializable {
          ...
          public void run2() throws CacheException {
            ...
            try {
              cache
                  ...
                  .setDiskDirs(JUnit4CacheTestCase.getDiskDirs())
                  .create("diskstore");
              AttributesFactory attr = new AttributesFactory();
              attr.setValueConstraint(constraint);
            ...
            try {
              ...
              PartitionAttributes prAttr =
                  paf.setRedundantCopies(redundancy)
                      .setColocatedWith(colocatedRegion.getFullPath())
                      .create();
              attr.setPartitionAttributes(prAttr);
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRPuts(
    ...
    SerializableRunnable prPuts =
        new CacheSerializableRunnable("PRPuts") {
          @Override
          public void run2() throws CacheException {
            Cache cache = getCache();
   ...
   * like, invalidate, destroy and create are performed in random manner based on {@link
   * Random#nextInt(int)}.
   *
   * @param regionName
   * @param to
    ...
    SerializableRunnable prPuts =
        new CacheSerializableRunnable("PRPuts") {
          public void run2() throws CacheException {
            Cache cache = getCache();
            Region region = cache.getRegion(regionName);
            for (int j = from; j < to; j++) region.put(portfolio[j], portfolio[j]);
          }
        };
    return (CacheSerializableRunnable) prPuts;
  }
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRQueryAndCompareResults(
    ...
    SerializableRunnable PrRegion =
        new CacheSerializableRunnable("PRQuery") {
          @Override
          public void run2() throws CacheException {
                ...
                for (int j = 0; j < queries.length; j++) {
                  ...
                  synchronized (region) {
                    ...
                    if (sr.asList().size() > l * l) {
                      fail(
                          "The resultset size exceeds limit size. Limit size="
                              + l * l
                              + ", result size ="
                              + sr.asList().size());
                    }
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRCountStarQueries(
      final String regionName, final String localRegion) {
    SerializableRunnable PrRegion =
        new CacheSerializableRunnable("PRCountStarQuery") {
          public void run2() throws CacheException {
            ...
            try {
              ...
              StructSetOrResultsSet ssORrs = new StructSetOrResultsSet();
              ssORrs.CompareCountStarQueryResultsWithoutAndWithIndexes(
                  r, queries.length, true, queries);
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForIndexUsageCheck() {
    SerializableRunnable PrIndexCheck =
        new CacheSerializableRunnable("PrIndexCheck") {
          @Override
          public void run2() {
            Cache cache = getCache();
            ...
            while (it.hasNext()) {
              ...
              logger.info(
                  "index uses for "
                      + ind.getNumberOfIndexedBuckets()
                      + " index "
                      + ind.getName()
                      + ": "
                      + ind.getStatistics().getTotalUses());
  ...
  public void compareTwoQueryResults(Object[][] r, int len) {
    ...
    for (int j = 0; j < len; j++) {
      if ((r[j][0] != null) && (r[j][1] != null)) {
        ...
        assertEquals(
            "PRQueryDUnitHelper#compareTwoQueryResults: FAILED: "
                + "result contents are not equal, ",
            set1,
            set2);
      }
    }
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForRegionClose(
      final String regionName, final int redundancy, final Class constraint) {
    SerializableRunnable PrRegion =
        new CacheSerializableRunnable("regionClose") {
          @Override
          public void run2() throws CacheException {
            Cache cache = getCache();
   ...
   *
   * @return cacheSerializable object
   *     <p>NOTE: Closing of the cache must be done from the test case rather than in
   *     PRQueryDUintHelper
   */
  public CacheSerializableRunnable getCacheSerializableRunnableForCacheClose(
      final String regionName, final int redundancy, final Class constraint) {
    SerializableRunnable PrRegion =
        new CacheSerializableRunnable("cacheClose") {
          @Override
          public void run2() throws CacheException {
            ...
            ResourceObserverAdapter observer =
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRIndexCreate(
    ...
    SerializableRunnable prIndexCreator =
        new CacheSerializableRunnable("PartitionedIndexCreator") {
          @Override
          public void run2() {
            try {
              Cache cache = getCache();
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForDefineIndex(
      final String prRegionName,
      final ArrayList<String> indexName,
      final ArrayList<String> indexedExpression) {
    return getCacheSerializableRunnableForDefineIndex(
        prRegionName, indexName, indexedExpression, null);
  }
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForDefineIndex(
      ...
      final ArrayList<String> indexName,
      final ArrayList<String> indexedExpression,
      final ArrayList<String> fromClause) {
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRCreate(
      final String regionName) {
    SerializableRunnable prIndexCreator =
        new CacheSerializableRunnable("PrRegionCreator") {
          @Override
          public void run2() {
            try {
              Cache cache = getCache();
  ...
  public SerializableRunnableIF
      getCacheSerializableRunnableForPRColocatedDataSetQueryAndCompareResults(
          final String name,
          ...
          public void run2() throws CacheException {
            ...
            try {
              for (int j = 0; j < queries.length; j++) {
                ...
                Object funcResult =
                    FunctionService.onRegion(
                            (getCache().getRegion(name) instanceof PartitionedRegion)
                                ? getCache().getRegion(name)
                                : getCache().getRegion(coloName))
                        .withArgs(
                            "<trace> Select "
              ...
              ssORrs.CompareQueryResultsAsListWithoutAndWithIndexes(
                  r, queries.length, false, false, queries);
                    ...
                    FunctionService.onRegion(
                        ...
                        .withArgs(
                            "<trace> Select "
                                ...
                                + " r1, /"
                                + coloName
                                + " r2, r2.positions.values pos2 where "
                                + queries[j])
                        .execute(func)
                        .getResult();
  ...
  public SerializableRunnableIF
      getCacheSerializableRunnableForPRAndRRQueryWithCompactAndRangeIndexAndCompareResults(
          final String name,
          final String coloName,
          final String localName,
          final String coloLocalName) {
