 ...
 *
 * <p>TODO: javadoc this test properly and cleanup the helper methods to be more flexible and
 * understandable
 */
...
public class ResourceManagerDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testGetPartitionedRegionDetails() {
    ...
    createRegion(
        ...
        redundantCopies[0]);
    createRegion(
        Host.getHost(0).getVM(1),
        regionPath[0],
        localMaxMemory[1],
        numBuckets[0],
        redundantCopies[0]);
    createRegion(
        Host.getHost(0).getVM(2),
        regionPath[0],
        localMaxMemory[2],
        numBuckets[0],
        redundantCopies[0]);
    ...
    fillValidationArrays(
        members, memberSizes, memberBucketCounts, memberPrimaryCounts, regionPath[0]);
              ...
              new SerializableRunnable() {
                public void run() {
                  ...
                  for (Iterator<PartitionRegionInfo> prIter = detailsSet.iterator();
                    ...
                    assertTrue(
                        "Unknown regionPath=" + details.getRegionPath(),
                    ...
                    if (details.getRegionPath().contains(regionPath[0])) {
                      ...
                      for (Iterator<PartitionMemberInfo> mbrIter = memberDetails.iterator();
                          mbrIter.hasNext();
                          ) {
                        ...
                        assertEquals(
                            localMaxMemory[membersIdx] * (1024L * 1024L),
                        ...
                        if (mbr.equals(getSystem().getDistributedMember())) {
                          ...
                          assertEquals(
                              ds.getNumberOfPrimaryBucketsManaged(), mbrDetails.getPrimaryCount());
                        }
                      }
                      ...
                      assertEquals(
                          getSystem().getDistributedMember(), mbrDetails.getDistributedMember());
                  ...
                  Host.getHost(0)
                      ...
                      .invoke(
                          new SerializableCallable() {
                            public Object call() {
                              PartitionedRegion pr =
                                  (PartitionedRegion) getCache().getRegion(regionPath[0]);
                              PartitionedRegionDataStore ds = pr.getDataStore();
                              if (ds == null) {
                ...
                public void run() {
                    ...
                    InternalPRInfo info =
                        pr.getRedundancyProvider()
                            .buildPartitionedRegionInfo(
                                true, cache.getResourceManager().getLoadProbe());
                    detailsSet.add(info);
                  }
                  if (vm == 0) {
                    assertEquals(2, detailsSet.size());
                  ...
                  for (Iterator<InternalPRInfo> prIter = detailsSet.iterator();
                    ...
                    if (details.getRegionPath().contains(regionPath[0])) {
                      ...
                      for (Iterator<InternalPartitionDetails> mbrIter = memberDetails.iterator();
                          mbrIter.hasNext();
                          ) {
                        ...
                        assertEquals(
                            localMaxMemory[membersIdx] * 1024 * 1024,
                            mbrDetails.getConfiguredMaxMemory());
                        assertEquals(memberSizes[membersIdx], mbrDetails.getSize());
                        assertEquals(memberBucketCounts[membersIdx], mbrDetails.getBucketCount());
                        assertEquals(memberPrimaryCounts[membersIdx], mbrDetails.getPrimaryCount());
  ...
  private void createBucket(
      final int vm, final String regionPath, final Integer bucketKey, final byte[] value) {
    createBuckets(vm, regionPath, new Integer[] {bucketKey}, value);
  }

  private void createBuckets(
      final int vm, final String regionPath, final Integer[] bucketKeys, final Object value) {
  ...
  private void fillValidationArrays(
                  ...
                  Host.getHost(0)
                      ...
                      .invoke(
                          new SerializableCallable() {
                            public Object call() {
                              PartitionedRegion pr =
                                  (PartitionedRegion) getCache().getRegion(regionPath);
                              PartitionedRegionDataStore ds = pr.getDataStore();
                              if (ds == null) {
                                return Long.valueOf(0);
                              } else {
                          ...
                          public Object call() {
                            ...
                            BecomePrimaryBucketResponse response =
                                BecomePrimaryBucketMessage.send(
                                    members[finalOtherVM], pr, 0, false);
                            if (response != null) {
        ...
        .invoke(
            new SerializableRunnable() {
              public void run() {
                ...
                assertNotNull(
                    "BucketRegion is null on target member",
        ...
        .invoke(
            new SerializableRunnable() {
              public void run() {
                ...
                assertFalse(
                    "Target member is still hosting removed bucket. Bucket:"
                        + bucket
                        + " Advisor state:"
  ...
  @Category(
      FlakyTest
          .class) // GEODE-755: thread unsafe test hook (bucketReadHook), remove bucket fails, possible product bug in rebalancing
  @Test
  ...
  @Category(
      FlakyTest
          .class) // GEODE-673: thread unsafe test hook (bucketReadHook), remove bucket fails, possible product bug in rebalancing
  @Test
  ...
  @Category(
      FlakyTest
          .class) // GEODE-1768: thread unsafe test hook (bucketReadHook), remove bucket fails, possible product bug in rebalancing
  @Test
    ...
    doOpDuringBucketRemove(
        new OpDuringBucketRemove() {
          public void runit(PartitionedRegion pr, Object key, Object value) {
            try {
              SelectResults sr =
                  pr.query("toString()='doOpDuringBucketRemove.VALUE'"); // fetch everything
  ...
  public void testRemoveColocatedBuckets() {
    final String[] regionPath =
        new String[] {
          getUniqueName() + "-PR-0", getUniqueName() + "-PR-1", getUniqueName() + "-PR-2"
        };
    ...
    createRegion(
        Host.getHost(0).getVM(0), regionPath[0], localMaxMemory, numBuckets, redundantCopies);
    createRegion(
        Host.getHost(0).getVM(1), regionPath[0], localMaxMemory, numBuckets, redundantCopies);
            ...
            new SerializableRunnable() {
              public void run() {
                for (int i = 0; i < regionPath.length; i++) {
                  ...
                  Bucket bucket = pr.getRegionAdvisor().getBucket(0);
                  assertTrue(
                      "Target member is not hosting bucket to remove for " + regionPath[i],
                      bucket.isHosting());
                  assertNotNull("Bucket is null on target member for " + regionPath[i], bucket);
                  assertNotNull(
                      "BucketRegion is null on target member for " + regionPath[i],
                      bucket.getBucketAdvisor().getProxyBucketRegion().getHostedBucketRegion());
                }
              }
            });
            ...
            new SerializableRunnable() {
              public void run() {
                for (int i = 0; i < regionPath.length; i++) {
                  ...
                  assertFalse(
                      "Target member is still hosting removed bucket for " + regionPath[i],
                      bucket.isHosting());

                  assertNull(
                      "BucketRegion is not null on target member for " + regionPath[i],
                      bucketRegion);
                }
              }
            });
  ...
  public void testCreateRedundantBucket() {
        ...
        .invoke(
            new SerializableRunnable() {
              public void run() {
                ...
                assertEquals(
                    "SRC member reports redundancy " + redundancy, redundantCopies, redundancy);
              }
            });
    ...
    createRegion(
        Host.getHost(0).getVM(finalNewVM),
        regionPath[0],
        localMaxMemory,
        numBuckets,
        redundantCopies);
            ...
            new SerializableRunnable() {
              public void run() {
                ...
                assertEquals(
                    ...
                    pr.getDataStore()
                        .createRedundantBucket(0, false, new InternalDistributedMember()));
            ...
            new SerializableRunnable() {
              public void run() {
                for (int i = 0; i < regionPath.length; i++) {
                  ...
                  assertTrue(
                      "SRC member is not hosting bucket for " + regionPath[i], bucket.isHosting());
                  assertNotNull(
                      "BucketRegion is null on SRC member for " + regionPath[i], bucketRegion);

                  int redundancy = bucket.getBucketAdvisor().getBucketRedundancy();
                  assertEquals(
                      "SRC member reports redundancy " + redundancy + " for " + regionPath[i],
                      redundantCopies,
                      redundancy);
                }
              }
            ...
            new SerializableRunnable() {
              public void run() {
                for (int i = 0; i < regionPath.length; i++) {
                  ...
                  assertNotNull("BucketRegion is null on DST member", bucketRegion);
                  assertEquals(
                      redundantCopies + 1, bucket.getBucketAdvisor().getBucketRedundancy());
                }
              }
            });
  }
