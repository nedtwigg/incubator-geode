...
public class DistinctAndNonDistinctQueryJUnitTest {
  ...
  static List data = Arrays.asList(new String[] { "abcd", "bcdd", "cde", "de", "abcd", "bcdd", "cde", "de" });
  ...
  public void testDistinctNonDistinctWithIndexes() throws Exception {
    ...
    Region rgn = CacheUtils.createRegion("testDistinctNonDistinctWithIndexes", String.class, Scope.LOCAL);
    ...
    String queryString = "select distinct * from /testDistinctNonDistinctWithIndexes s " + " where 3 >= s.length";
    Query q = CacheUtils.getQueryService().newQuery(queryString);
    ...
    queryString = "select distinct * from /testDistinctNonDistinctWithIndexes " + "where 3 >= length";
    q = CacheUtils.getQueryService().newQuery(queryString);
