...
public class PRBasicIndexCreationDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void testCreatePartitionedIndexThroughXML() throws Exception {
    ...
    AsyncInvocation asyInvk0 = vm0.invokeAsync(PRQHelp.getCacheSerializableRunnableForPRCreate(name));
    AsyncInvocation asyInvk1 = vm1.invokeAsync(PRQHelp.getCacheSerializableRunnableForPRCreate(name));
  ...
  public void testCreatePartitionedIndexWithNoAliasAfterPuts() throws Exception {
    ...
    VM vm3 = host.getVM(3);
    setCacheInVMs(vm0, vm1, vm3);
    LogWriterUtils.getLogWriter().info("PRBasicIndexCreationDUnitTest.testCreatePartitionedIndexWithNoAliasBeforePuts started ");
    // creating all the prs
    vm1.invoke(PRQHelp.getCacheSerializableRunnableForPRCreate(name, redundancy, PortfolioData.class));
    vm3.invoke(PRQHelp.getCacheSerializableRunnableForPRCreate(name, redundancy, PortfolioData.class));
    vm0.invoke(PRQHelp.getCacheSerializableRunnableForPRCreate(name, redundancy, PortfolioData.class));
  ...
  public void testIndexQueryingWithOrderBy() throws Exception {
    ...
    vm1.invoke(PRQHelp.getCacheSerializableRunnableForPRIndexCreate(name, "PrIndexOnKeyID", "key.ID", "/" + name + ".keys key", null));
  ...
  */
  ...
  public void testIndexQueryingWithOrderByLimit() throws Exception {
    ...
    vm1.invoke(PRQHelp.getCacheSerializableRunnableForPRIndexCreate(name, "PrIndexOnKeyStatus", "key.status", "/" + name + ".keys key", null));

    vm1.invoke(PRQHelp.getCacheSerializableRunnableForPRIndexCreate(name, "PrIndexOnsecID", "p.position1.secId", "/" + name + " p", null));

    vm0.invoke(PRQHelp.getCacheSerializableRunnableForRRIndexCreate(localName, "rrIndexOnStatus", "p.status", null, "p"));

    vm0.invoke(PRQHelp.getCacheSerializableRunnableForRRIndexCreate(localName, "rrIndexOnID", "p.ID", null, "p"));

    vm0.invoke(PRQHelp.getCacheSerializableRunnableForRRIndexCreate(localName, "rrIndexOnKeyID", "key.ID", "/" + localName + ".keys key", null));

    vm0.invoke(PRQHelp.getCacheSerializableRunnableForRRIndexCreate(localName, "rrIndexOnsecID", "p.position1.secId", "/" + localName + " p", null));

    vm0.invoke(PRQHelp.getCacheSerializableRunnableForRRIndexCreate(localName, "rrIndexOnKeyStatus", "key.status", "/" + localName + ".keys key", null));
