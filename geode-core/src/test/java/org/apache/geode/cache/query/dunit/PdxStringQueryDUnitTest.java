...
public class PdxStringQueryDUnitTest extends JUnit4CacheTestCase {
  ...
  private final String[] queryString =
      new String[] {
        ...
        "SELECT pos.secId, p.status FROM "
            + regName
  ...
  private final String[] queryString2 =
      new String[] {
        ...
        "SELECT pos.secIdIndexed, p.status FROM "
            + regName
        ...
        "select pos.secIdIndexed , count(pos.id) from "
            + regName
            + " p, p.positions.values pos where  pos.secIdIndexed > 'APPL' group by pos.secIdIndexed ", //7
        "select pos.secIdIndexed , sum(pos.id) from "
            + regName
        ...
        "select  count(distinct pos.secIdIndexed) from "
            + regName
  ...
  public void testReplicatedRegionNoIndex() throws CacheException {
    ...
    server0.invoke(
        new CacheSerializableRunnable("Create Server1") {
          public void run2() throws CacheException {
            ...
            try {
              index =
                  localQueryService.createIndex(
                      "secIdIndex2", "pos.secIdIndexed", regName + " p, p.positions.values pos");
    ...
    // Start server2
    server1.invoke(
        new CacheSerializableRunnable("Create Server2") {
          public void run2() throws CacheException {
            configAndStartBridgeServer(false, false, false);
    ...
    // Start server3
    server2.invoke(
        new CacheSerializableRunnable("Create Server3") {
          public void run2() throws CacheException {
            configAndStartBridgeServer(false, false, false);
            Region region = getRootRegion().getSubregion(regionName);
          }
        ...
        new CacheSerializableRunnable("Create client") {
          public void run2() throws CacheException {
            ...
            ClientServerTestCase.configureConnectionPool(
                factory, host0, port1, -1, true, -1, -1, null);
    ...
    SerializableRunnable executeQueries =
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR remote indexType: no index  size of resultset: "
                            + rs[0][0].size()
                            + " for query: "
                            + queryString[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  client local indexType:no index size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
              ...
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  remote region2 size of resultset: "
                            + resWithIndexRemote[i].size()
                            + " for query: "
                            + queryString2[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  local region2 size of resultset: "
                            + resWithIndexLocal[i].size()
                            + " for query: "
                            + queryString2[i]);
                ;
              ...
              if (i < orderByQueryIndex) {
                ...
                if (!compareResultsOfWithAndWithoutIndex(rs)) {
                  fail(
                      "Local and Remote Query Results are not matching for query :"
                          + queryString[i]);
                }
              } else {
            ...
            for (int i = 0; i < queryString.length; i++) {
              ...
              if (i < orderByQueryIndex) {
                if (!compareResultsOfWithAndWithoutIndex(rs)) {
                  fail(
                      "Results with and without index are not matching for query :"
                          + queryString2[i]);
                }
              } else {
                //compare the order of results returned
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                SelectResults rs =
                    (SelectResults) localQueryService.newQuery(queryString[i]).execute();
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR server local indexType: no  size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR server remote readSerializedTrue: indexType: false size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
          ...
          public void run2() throws CacheException {
            ...
            while (iter.hasNext()) {
              ...
              if (!(key instanceof PdxString)) {
                fail(
                    "All keys of the CompactRangeIndex should be PdxStrings and not "
                        + key.getClass());
              }
            }
          }
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR remote indexType: CompactRange size of resultset: "
                            + rs[0][0].size()
                            + " for query: "
                            + queryString[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  client local indexType: CompactRange size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR server local indexType:Range  size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
        ...
        new CacheSerializableRunnable("Create Server1") {
          public void run2() throws CacheException {
            ...
            try {
              index =
                  localQueryService.createIndex(
                      "secIdIndex", "pos.secId", regName + " p, p.positions.values pos");
              if (!(index instanceof RangeIndex)) {
                fail("Range Index should have been created instead of " + index.getClass());
              }
            } catch (Exception ex) {
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR remote indexType: Range size of resultset: "
                            + rs[0][0].size()
                            + " for query: "
                            + queryString[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  client local indexType: Range size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR  server local readSerializedTrue: indexType: Range size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR server remote readSerializedTrue: indexType: Range size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
        ...
        new CacheSerializableRunnable("Create Server1") {
          public void run2() throws CacheException {
            ...
            try {
              index =
                  localQueryService.createIndex(
                      "secIdIndex", "pos.secIdIndexed", regName + " p, p.positions.values pos");
              if (index instanceof PartitionedIndex) {
                for (Object o : ((PartitionedIndex) index).getBucketIndexes()) {
                  if (!(o instanceof RangeIndex)) {
                    fail(
                        "RangeIndex Index should have been created instead of " + index.getClass());
                  }
                }
              } else {
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR remote no index size of resultset: "
                            + rs[0][0].size()
                            + " for query: "
                            + queryString[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "isPR: "
                            + isPr
                            + "  client local indexType:no index size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
              ...
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "isPR: "
                            + isPr
                            + "  remote region2 size of resultset: "
                            + resWithIndexRemote[i].size()
                            + " for query: "
                            + queryString2[i]);
                ;
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "isPR: "
                            + isPr
                            + "  local region2 size of resultset: "
                            + resWithIndexLocal[i].size()
                            + " for query: "
                            + queryString2[i]);
                ;
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "PR server local indexType:no  size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queryString.length; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "RR server remote readSerializedTrue: indexType:no index size of resultset: "
                            + rs.size()
                            + " for query: "
                            + queryString[i]);
                // The results should not be PdxString
              ...
              if (index instanceof PartitionedIndex) {
                for (Object o : ((PartitionedIndex) index).getBucketIndexes()) {
                  if (!(o instanceof CompactRangeIndex)) {
                    fail(
                        "CompactRangeIndex Index should have been created instead of "
                            + index.getClass());
                  }
                }
              } else {
              ...
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "isPR: "
                            + isPr
                            + "  client local indexType:CompactRange size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
              ...
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "isPR: "
                            + isPr
                            + "  client local indexType: Range size of resultset: "
                            + rs[0][1].size()
                            + " for query: "
                            + queryString[i]);
                ;
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < 2; i++) {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .info(
                        "PR NULL Pdxstring test size of resultset: "
                            + res.size()
                            + " for query: "
                            + qs[i]);
                ;
  ...
  public boolean compareResultsOfWithAndWithoutIndex(SelectResults[][] r) {
    ...
    for (int j = 0; j < r.length; j++) {
      ...
      if (r[j][0].size() == r[j][1].size()) {
        ...
        System.out.println(
            "FAILED4: SelectResults size is different in both the cases. Size1="
                + r[j][0].size()
                + " Size2 = "
                + r[j][1].size());
        ok = false;
  ...
  public void testPRQueryForDuplicates() throws CacheException {
    ...
    vm2.invoke(
        new SerializableCallable("Create client") {
          @Override
          public Object call() throws Exception {
          ...
          public Object call() throws Exception {
            ...
            try {
              for (int i = 0; i < qs.length; i++) {
                ...
                assertEquals(
                    "Did not get expected result from query: " + qs[i] + " ", 2, sr.size());
              }
            } finally {
