...
public class QueryDataInconsistencyDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testCompactRangeIndex() {
    ...
    server.invoke(
        new CacheSerializableRunnable("create indexes") {
    ...
    AsyncInvocation putThread =
        server.invokeAsync(
            new CacheSerializableRunnable("update a Region Entry") {
    ...
    server.invoke(
        new CacheSerializableRunnable("query on server") {
          ...
          public void run2() throws CacheException {
            ...
            try {
              rs =
                  qs.newQuery("<trace> select * from /" + repRegionName + " where ID = 1")
                      .execute();
            } catch (Exception e) {
              e.printStackTrace();
        ...
        new CacheSerializableRunnable("query on server") {
          ...
          public void run2() throws CacheException {
            ...
            if (((SelectResults) rs).size() > 0) {
              ...
              if (p1.getID() != 1) {
                fail(
                    "Query thread did not verify index results even when RE is under update and "
        ...
        new CacheSerializableRunnable("create indexes") {
          ...
          public void run2() throws CacheException {
            ...
            try {
              Index index =
                  qs.createIndex(
                      "posIndex", "pos.secId", "/" + repRegionName + " p, p.positions.values pos");
        ...
        new CacheSerializableRunnable("query on server") {
          ...
          public void run2() throws CacheException {
            ...
            try {
              Object rs =
                  qs.newQuery(
                          "<trace> select pos from /"
                              + repRegionName
                              + " p, p.positions.values pos where pos.secId = 'APPL' AND p.ID = 1")
                      .execute();
  ...
  public void testRangeIndexWithIndexAndQueryFromCluaseMisMatch() { // TODO: fix misspelling
    ...
    ThreadUtils.join(
        putThread, 200); // GEODE-925 occurs here and this is very short join 200 millis
  }
