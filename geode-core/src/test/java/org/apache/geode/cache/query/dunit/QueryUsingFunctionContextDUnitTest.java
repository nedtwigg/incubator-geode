...
public class QueryUsingFunctionContextDUnitTest extends JUnit4CacheTestCase {
  ...
  public static String[] nonColocatedQueries =
      new String[] {
        ...
        "Select * from /"
            ...
            + " r5 where r1.ID = r4.ID and r4.ID = r5.ID"
      };
  ...
  public void testQueriesWithFilterKeysOnReplicatedRegion() {
    ...
    client.invoke(
        new CacheSerializableRunnable("Run function on RR") {
          @Override
  ...
  public void testQueriesWithFilterKeysOnPRLocal() {
    ...
    client.invoke(
        new CacheSerializableRunnable("Test query on client and server") {
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < queries.length; i++) {
              ...
              ArrayList queryResults2 =
                  test.runQueryOnClientUsingFunc(
                      function, PartitionedRegionName1, filter, queries[i]);
              if (queryResults2 == null) fail(queries[i] + "result is null from client function");

              ArrayList queryResults1 = test.runLDSQueryOnClientUsingFunc(func, filter, queries[i]);
              if (queryResults1 == null) fail(queries[i] + "result is null from LDS function");

              r[0][0] = queryResults1;
              r[0][1] = queryResults2;
              StructSetOrResultsSet ssORrs = new StructSetOrResultsSet();
              ssORrs.CompareQueryResultsAsListWithoutAndWithIndexes(
                  r, 1, false, new String[] {queries[i]});
            }
          }
        });
  }
        ...
        new CacheSerializableRunnable("Test query on client and server") {
          ...
          public void run2() throws CacheException {
            ...
            try {
              ...
              assertTrue(
                  "The exception message should mention QueryInvalidException. ",
  ...
  public void testQueriesWithFilterKeysOnPRLocalAndRemoteWithBucketDestroy() {
    ...
    server1.invoke(
        new CacheSerializableRunnable("Set QueryObserver in cache on server1") {
          @Override
          public void run2() throws CacheException {
              ...
              public void startQuery(Query query) {
                ...
                if (query.getQueryString().contains("ID>=0")) {
                  ...
                  for (int i = 3; i < 7; i++) {
                    ...
                    if (KeyRegion != null) KeyRegion.destroyRegion();
                  }
                }
              }
            };
    ...
    server1.invoke(
        new CacheSerializableRunnable("Reset Query Observer on server1") {
          @Override
          public void run2() throws CacheException {
  ...
  public void testQueriesWithFilterKeysOnPRWithBucketDestroy() {
    ...
    client.invoke(
        new CacheSerializableRunnable("Run function on PR") {
          @Override
          public void run2() throws CacheException {
            Set filter = new HashSet();
            ResultCollector rcollector = null;
  ...
  public void testNonColocatedRegionQueries() {
    ...
    client.invoke(
        new CacheSerializableRunnable("Test query on non-colocated regions on server") {
          @Override
          public void run2() throws CacheException {
            Set filter = new HashSet();
            filter.add(0);
            ...
            for (int i = 0; i < nonColocatedQueries.length; i++) {
              ...
              try {
                ArrayList queryResults2 =
                    test.runQueryOnClientUsingFunc(
                        function, PartitionedRegionName1, filter, nonColocatedQueries[i]);
                fail("Function call did not fail for query with function context");
              } catch (FunctionException e) {
                if (!(e.getCause() instanceof UnsupportedOperationException)) {
                  Assert.fail(
                      "Should have received an UnsupportedOperationException but received", e);
                }
              }
            }
          }
  ...
  public void testJoinQueryPRWithMultipleIndexes() {
    ...
    server1.invoke(
        new CacheSerializableRunnable("Test query with indexes") {
          ...
          public void run2() throws CacheException {
            ...
            for (int i = 3; i < 5; i++) {
              ArrayList queryResults1 =
                  test.runQueryOnClientUsingFunc(
                      function, PartitionedRegionName1, filter, queries[i]);
              if (queryResults1 == null) fail(queries[i] + "result is null from client function");
              r[j++][0] = queryResults1;
            }
                ...
                (ArrayList)
                    ((SelectResults)
                            ((LocalDataSet) localDataSet)
                                .executeQuery((DefaultQuery) query, null, buckets))
                        .asList());
      } catch (Exception e) {
        throw new FunctionException(e);
      }
  ...
  private void createServersWithRegions() {
    ...
    client.invoke(
        () ->
            QueryUsingFunctionContextDUnitTest.createCacheClientWithoutRegion(
  ...
  public static void createCacheClientWithoutRegion(
      String host, Integer port1, Integer port2, Integer port3) {
    new QueryUsingFunctionContextDUnitTest().createCacheClientWithoutReg(host, port1, port2, port3);
  }

  private void createCacheClientWithoutReg(
      String host, Integer port1, Integer port2, Integer port3) {
  ...
  protected ArrayList runQueryOnServerLDS(String queryStr, Set filter) {
    ...
    try {
      results =
          (SelectResults)
              ((LocalDataSet) localDataSet).executeQuery((DefaultQuery) query, null, buckets);
   ...
   */
  public static ArrayList runQueryOnClientUsingFunction(
      Function function, String regionName, Set filter, String query) {
    return new QueryUsingFunctionContextDUnitTest()
        .runQueryOnClientUsingFunc(function, regionName, filter, query);
  }

  private ArrayList runQueryOnClientUsingFunc(
      Function func, String regionName, Set filter, String query) {
    ResultCollector rcollector = null;

    // Filter can not be set as null if withFilter() is called.
    if (filter != null) {
      rcollector =
          FunctionService.onRegion(CacheFactory.getAnyInstance().getRegion(regionName))
              .withArgs(query)
              .withFilter(filter)
              .execute(func);
    } else {
  ...
  private ArrayList runLDSQueryOnClientUsingFunc(Function func, Set filter, String query) {
    ...
    // Filter can not be set as null if withFilter() is called.
    rcollector =
        FunctionService.onServer(ClientCacheFactory.getAnyInstance())
            .withArgs(new Object[] {query, filter})
            .execute(func);
    Object result = rcollector.getResult();
    assertTrue(result instanceof ArrayList);
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRPuts(
    ...
    SerializableRunnable puts =
        new CacheSerializableRunnable("Region Puts") {
          @Override
          public void run2() throws CacheException {
            Cache cache = CacheFactory.getAnyInstance();
