...
public class QueryUsingFunctionContextDUnitTest extends JUnit4CacheTestCase {
  ...
  public static String[] queries = new String[] { "select * from /" + PartitionedRegionName1 + " where ID>=0", "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName2 + " r2 where r1.ID = r2.ID", "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName2 + " r2 where r1.ID = r2.ID AND r1.status = r2.status", "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName2 + " r2, /" + PartitionedRegionName3 + " r3 where r1.ID = r2.ID and r2.ID = r3.ID", "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName2 + " r2, /" + PartitionedRegionName3 + " r3  , /" + repRegionName + " r4 where r1.ID = r2.ID and r2.ID = r3.ID and r3.ID = r4.ID",
      // "Select * from /" + PartitionedRegionName4 + " r4 , /" + PartitionedRegionName5 + " r5 where r4.ID = r5.ID"
  };

  public static String[] nonColocatedQueries = new String[] { "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName4 + " r4 where r1.ID = r4.ID", "Select * from /" + PartitionedRegionName1 + " r1, /" + PartitionedRegionName4 + " r4 , /" + PartitionedRegionName5 + " r5 where r1.ID = r4.ID and r4.ID = r5.ID" };
      ...
      public void run2() throws CacheException {
        ...
        for (int i = 0; i < queriesForRR.length; i++) {
          ...
          try {
            ...
            rcollector = FunctionService.onRegion(CacheFactory.getAnyInstance().getRegion(repRegionName)).withArgs(queriesForRR[i]).execute(function);
  ...
  */
      ...
      public void run2() throws CacheException {
        ...
        for (int i = 0; i < queries.length; i++) {
          ...
          try {
            ...
            rcollector = FunctionService.onRegion(CacheFactory.getAnyInstance().getRegion(PartitionedRegionName1)).withArgs(queries[i]).withFilter(filter).execute(function);
  ...
  public class TestQueryFunction extends FunctionAdapter {
    ...
    public void execute(FunctionContext context) {
      ...
      try {
        ...
        context.getResultSender().lastResult((ArrayList) ((SelectResults) query.execute((RegionFunctionContext) context)).asList());
      } catch (Exception e) {
  ...
  public class TestServerQueryFunction extends FunctionAdapter {
    ...
    public void execute(FunctionContext context) {
      ...
      try {
        ...
        context.getResultSender().lastResult((ArrayList) ((SelectResults) ((LocalDataSet) localDataSet).executeQuery((DefaultQuery) query, null, buckets)).asList());
      } catch (Exception e) {
        throw new FunctionException(e);
      }
    }
  ...
  public void fillValuesInRegions() {
    ...
    server1.invoke(getCacheSerializableRunnableForPRPuts(PartitionedRegionName1, portfolio, cnt, cntDest));
    server1.invoke(getCacheSerializableRunnableForPRPuts(PartitionedRegionName2, portfolio, cnt, cntDest));
    server1.invoke(getCacheSerializableRunnableForPRPuts(PartitionedRegionName3, portfolio, cnt, cntDest));
    server1.invoke(getCacheSerializableRunnableForPRPuts(PartitionedRegionName4, portfolio, cnt, cntDest));
    server1.invoke(getCacheSerializableRunnableForPRPuts(PartitionedRegionName5, portfolio, cnt, cntDest));
  }
  ...
  private void createServersWithRegions() {
    ...
    client.invoke(() -> QueryUsingFunctionContextDUnitTest.createCacheClientWithoutRegion(NetworkUtils.getServerHostName(server1.getHost()), port1, port2, port3));
  ...
  public void createColoPR() {
    ...
    cache.createRegionFactory(RegionShortcut.PARTITION).setPartitionAttributes(new PartitionAttributesFactory().setTotalNumBuckets(numOfBuckets).setPartitionResolver(testKeyBasedResolver).create()).create(PartitionedRegionName4); // not collocated

    cache.createRegionFactory(RegionShortcut.PARTITION).setPartitionAttributes(new PartitionAttributesFactory().setTotalNumBuckets(numOfBuckets).setPartitionResolver(testKeyBasedResolver).setColocatedWith(PartitionedRegionName4).create()).create(PartitionedRegionName5); // collocated with 4 
  }
  ...
  private ArrayList runQueryOnClientUsingFunc(Function func, String regionName, Set filter, String query) {
    ...
    if (filter != null) {
      rcollector = FunctionService.onRegion(CacheFactory.getAnyInstance().getRegion(regionName)).withArgs(query).withFilter(filter).execute(func);
    } else {
      rcollector = FunctionService.onRegion(CacheFactory.getAnyInstance().getRegion(regionName)).withArgs(query).execute(func);
    }
  ...
  private ArrayList runLDSQueryOnClientUsingFunc(Function func, Set filter, String query) {
    ...
    // Filter can not be set as null if withFilter() is called.
    rcollector = FunctionService.onServer(ClientCacheFactory.getAnyInstance()).withArgs(new Object[] { query, filter }).execute(func);
    Object result = rcollector.getResult();
    assertTrue(result instanceof ArrayList);
  ...
  public CacheSerializableRunnable getCacheSerializableRunnableForPRPuts(final String regionName, final Object[] portfolio, final int from, final int to) {
    SerializableRunnable puts = new CacheSerializableRunnable("Region Puts") {
      ...
      public void run2() throws CacheException {
        ...
        LogWriterUtils.getLogWriter().info("getCacheSerializableRunnableForPRPuts: Inserted Portfolio data on Region " + regionName);
      }
    };
