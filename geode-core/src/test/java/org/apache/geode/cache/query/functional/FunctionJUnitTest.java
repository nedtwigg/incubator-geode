...
public class FunctionJUnitTest {
  ...
  public void testIS_DEFINED() throws Exception {
    ...
    if (result instanceof Collection) {
      ...
      while (iter.hasNext()) {
        ...
        if (p.getP2() == null) fail(query.getQueryString());
      }
    }
    Object testData[][] = {
      {"string", Boolean.TRUE},
    ...
    for (int i = 0; i < testData.length; i++) {
      ...
      if (!result.equals(testData[i][1])) fail(query.getQueryString() + " for " + testData[i][0]);
    }
  }
  ...
  public void testELEMENT() throws Exception {
    ...
    Object result = query.execute();
    if (!result.equals("inactive")) fail(query.getQueryString());
    try {
      query =
          CacheUtils.getQueryService()
              .newQuery("ELEMENT(SELECT DISTINCT * FROM /Portfolios where ID <= 1).status");
  ...
  public void testNVL() throws Exception {
    ...
    Object result = query.execute();
    if (!result.equals("foundNull")) fail(query.getQueryString());
  ...
  public void testTo_Date() throws Exception {
    ...
    for (int i = 0; i < queries.length; i++) {
      ...
      if (!result.equals(date)) fail(query.getQueryString());
    }
