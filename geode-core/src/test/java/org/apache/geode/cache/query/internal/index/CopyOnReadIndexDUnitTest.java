...
public class CopyOnReadIndexDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testTransactionsOnReplicatedRegion() throws Exception {
    ...
    helpTestTransactionsOnReplicatedRegion(
        "select * from /portfolios p where p.ID = 1", 100, 1, true);
    ...
    helpTestTransactionsOnReplicatedRegion(
        "select * from /portfolios p where p.ID = 1", 100, 1, false);
  }
  ...
  public void helpTestPRQueryOnLocalNode(
        ...
        new SerializableCallable() {
          public Object call() throws Exception {
            ...
            if (hasIndex) {
              ...
              Wait.waitForCriterion(
                  verifyPortfolioCount(
                      (int) index.getStatistics().getNumberOfValues()
                          + numPortfoliosPerVM
                          + numExpectedResults),
                  5000,
                  200,
              ...
              Wait.waitForCriterion(
                  verifyPortfolioCount(
                      (int) ((PartitionedRegion) region).getLocalSize()
                          + numExpectedResults
                          + numPortfoliosPerVM),
                  5000,
                  200,
          ...
          public Object call() throws Exception {
            ...
            if (hasIndex) {
              ...
              Wait.waitForCriterion(
                  verifyPortfolioCount(
                      (int)
                          (((PartitionedRegion) region).getLocalSize()
                              + ((PartitionedRegion) region).getLocalSize()
                              + numExpectedResults
                              + numExpectedResults
                              + numPortfoliosPerVM)),
  ...
  public void helpTestTransactionsOnReplicatedRegion(
      final String queryString,
      final int numPortfolios,
      final int numExpectedResults,
      final boolean hasIndex)
    ...
    vm0.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
            ...
            Wait.waitForCriterion(
                verifyPortfolioCount(numExpectedResults + numPortfolios), 0, 200, true);
            return null;
          }
        });
    ...
    vm1.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
            ...
            Wait.waitForCriterion(
                verifyPortfolioCount(numPortfolios * 2 + numExpectedResults * 2), 0, 200, true);
            return null;
          }
        });
    ...
    vm2.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
            ...
            Wait.waitForCriterion(
                verifyPortfolioCount(numPortfolios + numExpectedResults), 0, 200, true);
            results = (SelectResults) query.execute();
            assertEquals(numExpectedResults, results.size());
            for (Object o : results) {
              if (o instanceof Portfolio) {
            ...
            if (hasIndex) {
              ...
              Wait.waitForCriterion(
                  verifyPortfolioCount(numPortfolios + numExpectedResults * 2), 0, 200, true);
            } else {
              //we never created index on vm1
              //so in this case, we always have to deserialize the value from the region
              Wait.waitForCriterion(
    ...
    vm0.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
            ...
            Wait.waitForCriterion(
                verifyPortfolioCount(numExpectedResults * 2 + numPortfolios), 0, 200, true);
            return null;
          }
        });
