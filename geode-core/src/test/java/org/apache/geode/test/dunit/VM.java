...
public class VM implements Serializable {
   ...
   *
   * <p>TODO: change pid to reflect value from {@link ProcessUtils#identifyPid()}
   */
  ...
  public Object invoke(final Class targetClass, final String methodName, final Object[] args) {
    ...
    if (!result.exceptionOccurred()) {
      ...
      throw new RMIException(
          this, targetClass.getName(), methodName, result.getException(), result.getStackTrace());
    }
  }
  ...
  public AsyncInvocation invokeAsync(
      final Object targetObject, final String methodName, final Object[] args) {
    return new AsyncInvocation(
            targetObject, methodName, () -> invoke(targetObject, methodName, args))
        .start();
  }
   ...
   */
  public AsyncInvocation invokeAsync(
      final Class<?> targetClass, final String methodName, final Object[] args) {
  ...
  /**
   * Invokes the {@code run} method of a {@link Runnable} in this VM. Recall that {@code run} takes
   * no arguments and has no return value. The {@code Runnable} is wrapped in a {@link
   * NamedRunnable} having the given name so it shows up in DUnit logs.
   *
   * @param runnable The {@code Runnable} to be run
   ...
   */
  public <T> AsyncInvocation<T> invokeAsync(
      final String name, final SerializableCallableIF<T> callable) {
  ...
  /**
   * Invokes the {@code run} method of a {@link Runnable} in this {@code VM}. Recall that {@code
   * run} takes no arguments and has no return value.
   *
   * @param runnable The {@code Runnable} to be run
   * @param name The name of the {@code Runnable}, which will be logged in DUnit output
   * @see SerializableRunnable
   ...
   * has passed.
   *
   * @deprecated Please use {@link Awaitility} to await condition and then {@link
   *     #invoke(SerializableCallableIF)} instead.
   */
  public void invokeRepeatingIfNecessary(
  ...
  /**
   * Invokes an instance method with no arguments on an object that is serialized into this {@code
   * VM}. The return type of the method can be either {@link Object} or {@code void}. If the return
   * type of the method is {@code void}, {@code null} is returned.
   *
   ...
   *
   * <p>This method is currently not supported by the standalone DUnit runner.
   *
   * @throws RMIException if an exception occurs while bouncing this {@code VM}, for example a
   *     {@code HydraTimeoutException} if the {@code VM} fails to stop within {@code
   *     hydra.Prms#maxClientShutdownWaitSec} or restart within {@code
  ...
  private MethExecutorResult execute(
      final Class targetClass, final String methodName, final Object[] args) {
    try {
