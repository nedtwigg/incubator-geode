...
public class ClientMultiUserAuthzDUnitTest extends ClientAuthorizationTestCase {
  ...
  public void testOps1() throws Exception {
    for (Iterator<AuthzCredentialGenerator> iter = getDummyGeneratorCombos().iterator();
        iter.hasNext();
        ) {
      ...
      if (!prepareClientsForOps(
          ...
          new OperationCode[] {OperationCode.DESTROY, OperationCode.DESTROY},
          javaProps,
          authInit,
          port1,
          port2)) {
      ...
      if (!prepareClientsForOps(
          ...
          cGen,
          new OperationCode[] {OperationCode.PUT, OperationCode.CONTAINS_KEY},
          new OperationCode[] {OperationCode.INVALIDATE, OperationCode.INVALIDATE},
          javaProps,
          authInit,
          port1,
          port2)) {
  ...
  public void testOps2() throws Exception {
    ...
    getLogWriter()
        .info(
            "testOps2: For first client credentials: "
                + client1Credentials[0]
                + "\n"
    ...
    Properties[] client2Credentials =
        new Properties[] {
          gen.getAllowedCredentials(
              new OperationCode[] {OperationCode.EXECUTE_FUNCTION}, new String[] {regionName}, 2),
          gen.getDisallowedCredentials(
              new OperationCode[] {OperationCode.EXECUTE_FUNCTION}, new String[] {regionName}, 9)
        };

    javaProps = cGen.getJavaProperties();
    getLogWriter()
        .info(
            "testOps2: For second client credentials: "
                + client2Credentials[0]
                + "\n"
    ...
    client1.invoke(
        () -> doMultiUserQueryExecute(2, new int[] {NO_EXCEPTION, NOTAUTHZ_EXCEPTION}, 4));
  ...
  private boolean prepareClientsForOps(
      final AuthzCredentialGenerator gen,
      final CredentialGenerator cGen,
      final OperationCode[] client1OpCodes,
      final OperationCode[] client2OpCodes,
      final Properties javaProps,
      final String authInit,
      final int port1,
      final int port2) {
    return prepareClientsForOps(
        gen,
        cGen,
        client1OpCodes,
        client2OpCodes,
        ...
        port1,
        port2,
        true /* both clients in multiuser mode */,
        false /* unused */);
  }
  ...
  private boolean prepareClientsForOps(
      ...
      final int port1,
      final int port2,
      final boolean bothClientsInMultiuserMode,
    ...
    Properties[] client1Credentials =
        new Properties[] {
          ...
          gen.getDisallowedCredentials(
              new OperationCode[] {client1OpCodes[1]}, new String[] {regionName}, 1)
        };
    ...
    getLogWriter()
        .info(
            "testOps1: For first client credentials: "
                + client1Credentials[0]
                + "\n"
                + client1Credentials[1]);
    final Properties finalJavaProps = javaProps;
    ...
    Properties[] client2Credentials =
        new Properties[] {
          gen.getAllowedCredentials(client2OpCodes, new String[] {regionName}, 2),
    ...
    getLogWriter()
        .info(
            "testOps1: For second client credentials: "
                + client2Credentials[0]
                + "\n"
                + client2Credentials[1]);
    ...
    if (bothClientsInMultiuserMode) {
      ...
      client2.invoke(
          () ->
              createCacheClient(
                  authInit,
                  client2Credentials[credentialsIndex],
                  finalJavaProps2,
                  new int[] {port1, port2},
                  -1,
                  false,
  ...
  private void verifyContainsKeyDestroys(final boolean isMultiUser, final boolean opAllowed)
    ...
    client1.invoke(
        () ->
            doMultiUserContainsKeys(
                1, 2, new int[] {NO_EXCEPTION, NOTAUTHZ_EXCEPTION}, new boolean[] {true, false}));
  ...
  private void verifyContainsKeyInvalidates(final boolean isMultiUser, final boolean opAllowed)
    ...
    if (isMultiUser) {
      client2.invoke(
          () -> doMultiUserInvalidates(2, 2, new int[] {NO_EXCEPTION, NOTAUTHZ_EXCEPTION}));
  ...
  private void verifyGetAllRegionDestroys() {
    ...
    client2.invoke(
        () -> doMultiUserRegionDestroys(2, new int[] {NO_EXCEPTION, NOTAUTHZ_EXCEPTION}));
  }
  ...
  private int createCacheServerOnVM(
      final VM server, final Properties javaProps, final Properties serverProps) {
