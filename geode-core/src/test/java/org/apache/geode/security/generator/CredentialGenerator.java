...
public abstract class CredentialGenerator {
   ...
   * method for convenience of implementations so that they do not need to store in {@link
   * #systemProperties}. The latter is convenient for the users who do not need to store these
   * properties rather can obtain it later by invoking {@link #getSystemProperties()}
   *
   ...
   *
   * <p>The following schemes are supported as of now: {@code DummyAuthenticator}, {@code
   * LdapUserAuthenticator}, {@code PKCSAuthenticator}. In addition SSL socket mode with mutual
   ...
   *   <li>Add an implementation for {@link CredentialGenerator}.
   ...
   * </ul>
   *
   * <p>All security dunit tests will automagically start testing the new implementation after this.
   *
   * @since GemFire 5.5
   */
