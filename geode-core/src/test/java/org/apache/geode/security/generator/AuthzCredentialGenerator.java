...
public abstract class AuthzCredentialGenerator {
   ...
   * Initialize the authorized credential generator.
   *
   * <p>Required to be implemented by concrete classes that implement this abstract class.
   *
   * @return A set of extra properties that should be added to Gemfire system properties when not
   *     null.
  ...
  /**
   * Get a set of credentials generated using the given index allowed to perform the given {@link
   * OperationCode}s for the given regions.
   *
   ...
   * efficiency.
   *
   * <p>This will be used by {@link #getAllowedCredentials} to step through different principals and
   ...
   */
  protected abstract int getNumPrincipalTries(
      final OperationCode[] opCodes, final String[] regionNames);

  /**
   * Get a {@link Principal} generated using the given index allowed to perform the given {@link
   * OperationCode}s for the given region.
   *
   * <p>Required to be implemented by concrete classes that implement this abstract class.
   *
   * @param opCodes the {@link OperationCode}s of the operations requiring authorization
   ...
   */
  protected abstract Principal getAllowedPrincipal(
      final OperationCode[] opCodes, final String[] regionNames, final int index);

  /**
   * Get a {@link Principal} generated using the given index not allowed to perform the given {@link
   * OperationCode}s for the given region.
   *
   * <p>Required to be implemented by concrete classes that implement this abstract class.
   *
   ...
   */
  protected abstract Principal getDisallowedPrincipal(
      final OperationCode[] opCodes, final String[] regionNames, final int index);
   ...
   *
   * <ul>
   *   <li>{@code DummyAuthorization} with {@code DummyAuthenticator}
   *   <li>{@code XMLAuthorization} with {@code DummyAuthenticator}
   *   <li>{@code XMLAuthorization} with {@code LDAPAuthenticator}
   *   <li>{@code XMLAuthorization} with {@code PKCSAuthenticator}
   *   <li>{@code XMLAuthorization} when using SSL sockets
   * </ul>
   *
   * <p>To add a new authorization scheme the following needs to be done:
   *
   * <ul>
   *   <li>Add implementation for {@link AccessControl}.
   *   <li>Choose the authentication schemes that it shall work with from {@link
   *       CredentialGenerator.ClassCode}
   *   <li>Add a new enumeration value for the scheme in this class. Notice the size of {@code
   *       VALUES} array and increase that if it is getting overflowed. Note the methods and fields
   *       for existing schemes and add for the new one in a similar manner.
   *   <li>Add an implementation for {@link AuthzCredentialGenerator}. Note the {@link
   *       AuthzCredentialGenerator#init} method where different authentication schemes can be
   *       passed and initialize differently for the authentication schemes that shall be handled.
   *   <li>Modify the {@link AuthzCredentialGenerator#create} method to add creation of an instance
   *       of the new implementation for the {@code ClassCode} enumeration value.
   * </ul>
   *
   * <p>All dunit tests will automagically start testing the new implementation after this.
   *
   * @since GemFire 5.5
   */
