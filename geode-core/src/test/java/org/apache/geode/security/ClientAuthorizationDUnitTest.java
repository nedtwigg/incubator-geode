...
public class ClientAuthorizationDUnitTest extends ClientAuthorizationTestCase {
  ...
  public void testDisallowPutsGets() {
    ...
    getCredentials =
        gen.getAllowedCredentials(
            new OperationCode[] {OperationCode.GET}, new String[] {regionName}, 5);
  ...
  public void testInvalidAccessor() {
    ...
    Properties getCredentials =
        gen.getAllowedCredentials(
            new OperationCode[] {OperationCode.GET}, new String[] {regionName}, 7);
    Properties getJavaProps = cGen.getJavaProperties();
    ...
    client1.invoke(
        () ->
            ClientAuthenticationTestUtils.createCacheClient(
                authInit,
                createCredentials,
                createJavaProps,
                port1,
    ...
    for (int opNum = 0; opNum < unregisterOps.length; ++opNum) {
      ...
      if (currentOp.equals(OperationWithAction.OPBLOCK_END)
        ...
        if (opBlock.size() > 0) {
          ...
          executeRIOpBlock(
              opBlock, port1, port2, authInit, extraAuthProps, extraAuthzProps, javaProps);
  ...
  private OperationWithAction[] unregisterOpsForTestUnregisterInterestWithFailover() {
    return new OperationWithAction[] {
      ...
      new OperationWithAction(
          OperationCode.REGISTER_INTEREST, OperationCode.UNREGISTER_INTEREST, 3, OpFlags.NONE, 4),
      ...
      new OperationWithAction(
          OperationCode.UNREGISTER_INTEREST, 3, OpFlags.USE_OLDCONN | OpFlags.CHECK_NOTAUTHZ, 4),
      ...
      new OperationWithAction(
          OperationCode.UNREGISTER_INTEREST,
          3,
          OpFlags.USE_OLDCONN | OpFlags.USE_LIST | OpFlags.CHECK_NOTAUTHZ,
          4),
      new OperationWithAction(
          OperationCode.UNREGISTER_INTEREST, 1, OpFlags.USE_OLDCONN | OpFlags.USE_LIST, 4),
      // UPDATE and test with GET for no updates
      ...
      new OperationWithAction(
          OperationCode.UNREGISTER_INTEREST,
          3,
          OpFlags.USE_OLDCONN | OpFlags.USE_REGEX | OpFlags.CHECK_NOTAUTHZ,
          4),
      new OperationWithAction(
          OperationCode.UNREGISTER_INTEREST, 2, OpFlags.USE_OLDCONN | OpFlags.USE_REGEX, 4),
      // UPDATE and test with GET for no updates
      new OperationWithAction(OperationCode.PUT, 1, OpFlags.USE_OLDCONN | OpFlags.USE_NEWVAL, 4),
      new OperationWithAction(OperationCode.GET, 2, OpFlags.USE_OLDCONN | OpFlags.LOCAL_OP, 4),
      OperationWithAction.OPBLOCK_END
  ...
  private OperationWithAction[] allOpsForAllOpsWithFailover() {
    return new OperationWithAction[] {
      ...
      new OperationWithAction(OperationCode.PUT),
      new OperationWithAction(
          OperationCode.GET, 3, OpFlags.CHECK_NOKEY | OpFlags.CHECK_NOTAUTHZ, 4),
      ...
      new OperationWithAction(
          OperationCode.PUTALL, 3, OpFlags.USE_NEWVAL | OpFlags.CHECK_NOTAUTHZ, 4),
      ...
      new OperationWithAction(
          OperationCode.DESTROY, 3, OpFlags.USE_NEWVAL | OpFlags.CHECK_NOTAUTHZ, 4),
      new OperationWithAction(OperationCode.DESTROY),
      new OperationWithAction(
          OperationCode.GET,
          2,
          OpFlags.USE_OLDCONN | OpFlags.CHECK_FAIL,
          4), // bruce: added check_nokey because we now bring tombstones to the client in 8.0
      ...
      new OperationWithAction(
          OperationCode.CONTAINS_KEY, 3, OpFlags.CHECK_FAIL | OpFlags.CHECK_NOTAUTHZ, 4),
      new OperationWithAction(
          OperationCode.CONTAINS_KEY, 1, OpFlags.USE_OLDCONN | OpFlags.CHECK_FAIL, 4),
      // Repopulate the region
      new OperationWithAction(OperationCode.PUT),
      OperationWithAction.OPBLOCK_END,
      ...
      new OperationWithAction(
          OperationCode.GET, 2, OpFlags.USE_GET_ENTRY_IN_TX | OpFlags.CHECK_FAIL, 4),
      OperationWithAction.OPBLOCK_END
    };
  }
  ...
  private void executeRIOpBlock(
    ...
    for (Iterator opIter = opBlock.iterator(); opIter.hasNext(); ) {
      ...
      if ((opFlags & OpFlags.USE_OLDCONN) == 0) {
        ...
        getLogWriter()
            .info(
                "executeRIOpBlock: For client"
                    + clientNum
                    + credentialsTypeStr
                    + " credentials: "
                    + opCredentials);
        if (useThisVM) {
          createCacheClientWithDynamicRegion(
              authInit, clientProps, javaProps, new int[] {port1, port2}, 0, false, NO_EXCEPTION);
        } else {
          clientVM.invoke(
              () ->
                  createCacheClient(
  ...
  private void createClient2NoException(
      final Properties javaProps,
      final String authInit,
      final int port1,
      final int port2,
      final Properties getCredentials) {
    client2.invoke(
        () ->
            ClientAuthenticationTestUtils.createCacheClient(
                authInit, getCredentials, javaProps, port1, port2, 0, NO_EXCEPTION));
  }

  private void createClient1NoException(
      final Properties javaProps,
      final String authInit,
      final int port1,
      final int port2,
      final Properties createCredentials) {
    client1.invoke(
        () ->
            ClientAuthenticationTestUtils.createCacheClient(
                authInit, createCredentials, javaProps, port1, port2, 0, NO_EXCEPTION));
  }

  private int createServer2(final Properties javaProps, final Properties serverProps) {
    return server2.invoke(
        () ->
            ClientAuthorizationTestCase.createCacheServer(
                getLocatorPort(), serverProps, javaProps));
  }
