...
public class PdxInstanceJUnitTest {
  ...
  public void testGetField() throws IOException, ClassNotFoundException {
    ...
    assertEquals(
        Arrays.asList(new String[] {"field1", "field2", "field3"}), instance.getFieldNames());
  ...
  public void testHashCodeAndEqualsSameType() throws IOException, ClassNotFoundException {
    ...
    for (int i = 1; i < allFieldCount + 1; i++) {
      ...
      assertFalse(
          "One field "
              + i
              + " hashcode have been unequal but were equal"
              + instance.getField("field" + (i - 1))
              + ", "
              + other.getField("field" + (i - 1))
              + ", "
              + instance
              + ", "
              + other,
        ...
        getPdx(
            new TestPdx() {
              public void toData(PdxWriter out) {
                ...
                out.writeCharArray(
                    "field" + x++, change == x ? new char[] {'a', 'b'} : new char[] {'a', 'c'});
                out.writeByteArray(
                    "field" + x++, change == x ? new byte[] {0, 1, 2} : new byte[] {0, 1, 3});
                out.writeShortArray(
                    "field" + x++, change == x ? new short[] {0, 1, 4} : new short[] {0, 1, 5});
                out.writeIntArray(
                    "field" + x++, change == x ? new int[] {0, 1, 4} : new int[] {0, 1, 5});
                out.writeLongArray(
                    "field" + x++, change == x ? new long[] {0, 1, 4} : new long[] {0, 1, 5});
                out.writeFloatArray(
                    "field" + x++, change == x ? new float[] {0, 1, 4} : new float[] {0, 1, 5});
                out.writeDoubleArray(
                    "field" + x++, change == x ? new double[] {0, 1, 4} : new double[] {0, 1, 5});
                out.writeStringArray(
                    "field" + x++, change == x ? new String[] {"a", "b"} : new String[] {"a", "c"});
                out.writeObjectArray(
                    "field" + x++,
                    change == x
                        ? new Object[] {collection1, serializable1}
                        : new Object[] {serializable2});
                out.writeArrayOfByteArrays(
                    "field" + x++, change == x ? new byte[][] {{1, 2}} : new byte[][] {{2, 2}});
                allFieldCount = x;
              }
            });
  ...
  private PdxInstance getPdx(PdxSerializable toData) throws IOException, ClassNotFoundException {
    ...
    return DataSerializer.readObject(
