...
public class PartitionedRegionDUnitTestCase extends JUnit4CacheTestCase {
  ...
  public static int setLogLevel(LogWriter l, int logLevl) {
    int ret = -1;
    l.config(
        "PartitionedRegionDUnitTest attempting to set log level on LogWriter instance class:"
            + l.getClass().getName());
    if (l instanceof PureLogWriter) {
      ...
      l.config(
          "PartitiionedRegionDUnitTest forcing log level to "
              + LogWriterImpl.levelToString(logLevl)
              + " from "
              + LogWriterImpl.levelToString(ret));
      pl.setLevel(logLevl);
    }
    return ret;
  }
  ...
  public CacheSerializableRunnable createMultiplePartitionRegion(
    ...
    return createMultiplePartitionRegion(
        PRPrefix, startIndexForRegion, endIndexForRegion, redundancy, localmaxMemory, false);
  }
        ...
        EvictionAttributes evictionAttrs =
            evict
                ? EvictionAttributes.createLRUEntryAttributes(
                    Integer.MAX_VALUE, EvictionAction.LOCAL_DESTROY)
                : null;
        ...
        for (int i = 0; i < maxIndex; i++) {
          ...
          try {
            getCache()
                .createRegion(
                    prPrefix + i,
                    PartitionedRegionTestHelper.createRegionAttrsForPR(
  ...
  public CacheSerializableRunnable validateMultiplePartitionRegion(
      final String PRPrefix, final int startIndexForRegion, final int endIndexForRegion) {
    CacheSerializableRunnable validateAllPRs;
    validateAllPRs =
        new CacheSerializableRunnable("validateAllPRs") {
          String innerPRPrefix = PRPrefix;
          ...
          public void run2() throws CacheException {
            ...
            //      allPR.size());
            assertEquals(
                "prIdToPR size is not correct",
                innerEndIndexForRegion,
                PartitionedRegion.prIdToPR.size());
            getCache()
                .getLogger()
                .info(
                    "validateMultiplePartitionRegion() - Partition Regions Successfully Validated ");
          }
        };
          ...
          public void run2() throws CacheException {
            for (int j = innerStartIndexNumOfRegions; j < innerEndIndexNumOfRegions; j++) {
              ...
              getCache()
                  .getLogger()
                  .info(
                      "putInMultiplePartitionRegion() - Put() done Successfully in Partition Region "
                          + pr.getName());
            }
          }
        };
  ...
  public CacheSerializableRunnable getInMultiplePartitionRegion(
    ...
    CacheSerializableRunnable getInPRs =
        new CacheSerializableRunnable("doGetOperations") {
          String innerPRPrefix = PRPrefix;
          ...
          public void run2() throws CacheException {
            for (int j = innerStartIndexNumOfRegions; j < innerEndIndexNumOfRegions; j++) {
              ...
              getCache()
                  .getLogger()
                  .info(
                      "putInMultiplePartitionRegion() - Get() done Successfully in Partition Region "
                          + pr.getName());
            }
          }
        };
  ...
  public CacheSerializableRunnable getRemovedOrDestroyedInMultiplePartitionRegion(
      ...
      final int startIndexNumOfRegions,
      final int endIndexNumOfRegions,
      final int afterPutFlag) {
    CacheSerializableRunnable getInPRs =
        new CacheSerializableRunnable("doDetroyedGetOperations") {
          String innerPRPrefix = PRPrefix;
          ...
          public void run2() throws CacheException {
            for (int j = innerStartIndexNumOfRegions; j < innerEndIndexNumOfRegions; j++) {
              ...
              for (int k = innerStartIndexForKey; k < innerEndIndexForKey; k++) {
                Object Obj = pr.get(j + innerPRPrefix + k);
                if (innerAfterPutFlag == 0) assertNull(Obj);
                else assertNotNull(Obj);
              }
              getCache()
                  .getLogger()
                  .info(
  ...
  public CacheSerializableRunnable disconnectVM() {
    CacheSerializableRunnable csr =
        new CacheSerializableRunnable("disconnectVM") {
          public void run2() throws CacheException {
            getCache();
