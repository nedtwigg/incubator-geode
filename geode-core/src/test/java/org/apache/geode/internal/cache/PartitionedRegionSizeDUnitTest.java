...
public class PartitionedRegionSizeDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void sizeOpsForDistAckSync() throws Exception {
        ...
        new CacheSerializableRunnable("createPrRegions") {
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < MAX_REGIONS; i++) {
              cache.createRegion(
                  PR_PREFIX + "DistAckSync" + i, createRegionAttributesForPR(1, 200));
            }
          }
        };
    ...
    vm3.invoke(
        new CacheSerializableRunnable("validateSize") {
          public void run2() {
            Cache cache = getCache();
            for (int j = 0; j < MAX_REGIONS; j++) {
              Region pr = cache.getRegion(Region.SEPARATOR + PR_PREFIX + "DistAckSync" + j);
    ...
    CacheSerializableRunnable createPRs =
        new CacheSerializableRunnable("createPrRegions") {
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < MAX_REGIONS; i++) {
              cache.createRegion(
                  PR_PREFIX + "DistAckASync" + i, createRegionAttributesForPR(1, 200));
            }
          }
        };
    ...
    CacheSerializableRunnable createPRs =
        new CacheSerializableRunnable("createPrRegions") {
          public void run2() throws CacheException {
            ...
            for (int i = 0; i < MAX_REGIONS; i++) {
              cache.createRegion(
                  PR_PREFIX + "DistAckSyncChangingVMCount" + i,
  ...
  public void testBug39868() throws Exception {
    ...
    SerializableRunnable createPRs =
        new SerializableRunnable("createPrRegion") {
    ...
    vm0.invoke(
        new SerializableRunnable("delete data") {
          public void run() {
            Cache cache = getCache();
            Region partitionedregion = cache.getRegion(PR_PREFIX);
            for (int i = 0; i < 100; i++) {
    ...
    vm1.invoke(
        new SerializableRunnable("check size") {
  ...
  public void testByteSize() throws Exception {
    ...
    SerializableRunnable checkMemSize =
        new SerializableRunnable("checkMemSize") {
          public void run() throws CacheException {
            Cache cache = getCache();
            PartitionedRegion partitionedregion = (PartitionedRegion) cache.getRegion(PR_PREFIX);
                ...
                new SerializableCallable("create data") {
                  public Object call() {
                    ...
                    PartitionedRegion partitionedregion =
                        (PartitionedRegion) cache.getRegion(PR_PREFIX);
                    PartitionedRegionDataStore dataStore = partitionedregion.getDataStore();
