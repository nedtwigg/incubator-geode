...
public class OffHeapStoredObjectJUnitTest extends AbstractStoredObjectTestBase {
  ...
  public void setSerializedShouldSetTheSerializedBit() {
    ...
    int headerBeforeSerializedBitSet =
        AddressableMemoryManager.readIntVolatile(
            chunk.getAddress() + OffHeapStoredObject.REF_COUNT_OFFSET);
  ...
  public void retainShouldThrowExceptionAfterMaxNumberOfTimesRetained() {
    ...
    for (int i = 0; i < OffHeapStoredObject.MAX_REF_COUNT - 1; i++) chunk.retain();
  ...
  public void testToString() {
    ...
    String expected =
        ":<dataSize="
            ...
            + chunk.getRefCount()
            + " addr="
            + Long.toHexString(chunk.getAddress())
            + ">";
