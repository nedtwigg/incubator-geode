...
public class CacheServerMaxConnectionsJUnitTest {
  ...
  public void testMaxCnxLimit() throws Exception {
    ...
    try {
      ...
      if (cnx != null) {
        fail(
            "should not have been able to connect more than "
                + MAX_CNXS
                + " times but was able to connect "
                + s.getInt("currentClientConnections")
                + " times. Last connection="
                + cnx);
      }
      ...
      fail(
          "expected acquireConnection to throw NoAvailableServersException but instead it threw "
              + ex);
    } finally {
      this.system
          .getLogWriter()
