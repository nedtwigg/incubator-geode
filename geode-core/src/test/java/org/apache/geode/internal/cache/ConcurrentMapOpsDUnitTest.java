...
public class ConcurrentMapOpsDUnitTest extends JUnit4CacheTestCase {
  ...
  private void createEmptyRegion(VM vm) {
    vm.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
            getCache()
                .createRegionFactory(RegionShortcut.REPLICATE_PROXY)
                .setConcurrencyChecksEnabled(true)
                .create(REP_REG_NAME);
            getCache()
  ...
  private void createClientRegion(
      VM vm, final int port1, final boolean isEmpty, final boolean ri, final int port2) {
    vm.invoke(
        new SerializableCallable() {
          public Object call() throws Exception {
  ...
  public void testListenerNotInvokedForRejectedOperation() {
    ...
    SerializableCallable addListenerToClientForInitialCreates =
        new SerializableCallable() {
          public Object call() throws Exception {
    ...
    SerializableCallable waitForInitialCreates =
        new SerializableCallable() {
          @Override
          public Object call() throws Exception {
            Region<Integer, String> r = getGemfireCache().getRegion(REP_REG_NAME);
          ...
          private void waitForCreates(Region region) {
            ...
            for (CacheListener listener : listeners) {
              if (listener instanceof InitialCreatesListener) {
                listenerFound = true;
                final InitialCreatesListener initialCreatesListener =
                    (InitialCreatesListener) listener;
                WaitCriterion wc =
                    new WaitCriterion() {
                      @Override
                      public boolean done() {
    ...
    SerializableCallable addListener =
        new SerializableCallable() {
          public Object call() throws Exception {
            Region r = getCache().getRegion(REP_REG_NAME);
    ...
    SerializableCallable addListenerToClient =
        new SerializableCallable() {
          public Object call() throws Exception {
            Region r = getCache().getRegion(REP_REG_NAME);
            r.getAttributesMutator().addCacheListener(new NotInvokedListener());
  ...
  private void doRetriedOperation(final Operation op, boolean usePR) {
    ...
    SerializableCallable getID =
        new SerializableCallable("get DM ID") {
          public Object call() {
    ...
    try {
      ...
      server1.invoke(
          new SerializableCallable("install crasher in server1") {
            public Object call() throws Exception {
              Region r = getCache().getRegion(regionName);
              r.put("key0", "value");
                        ...
                        private void killSender(EntryEvent event) {
                          if (event.isOriginRemote()) {
                            ...
                            try {
                              ...
                              fail(
                                  "failed to stop the other server for this test:"
                                      + e.getMessage());
                            }
                          }
                        }
      ...
      server2.invoke(
          new SerializableCallable("install crasher in server2") {
            public Object call() throws Exception {
              Region r = getCache().getRegion(regionName);
              // force client to use server1 for now
              //          getCache().getCacheServers().get(0).stop();
      ...
      client.invoke(
          new SerializableRunnable() {
            public void run() {
              ...
              if (op == Operation.PUT_IF_ABSENT) {
                assertTrue(
                    "expected putIfAbsent to succeed and return null",
                    r.putIfAbsent("key0", "newvalue") == null);
              } else if (op == Operation.REMOVE) {
                assertTrue(
                    "expected remove operation to succeed and return true",
                    r.remove("key0", "value"));
              } else if (op == Operation.REPLACE) {
                assertTrue(
                    "expected replace operation to succeed and return true",
