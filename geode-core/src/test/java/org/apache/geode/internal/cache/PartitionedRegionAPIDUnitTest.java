...
public class PartitionedRegionAPIDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void partitionedRegionTestAfterDestroyRegion(final String prName) {
     ...
     */
    vm0.invoke(
        new CacheSerializableRunnable("doPutCreateInvalidateOperations1") {
     ...
     */
    vm1.invoke(
        new CacheSerializableRunnable("doPutCreateInvalidateOperations2") {
          ...
          public void run2() throws CacheException {
            ...
            for (int i = invalidateRange_2Start; i <= invalidateRange_2End; i++) {
              ...
              assertNull(
                  "Key " + key + " should report val null, however it has " + shouldBeNull,
                  shouldBeNull);
              assertFalse(
                  "Key " + key + " should report False for containsValueForKey",
  ...
  public void partitionedRegionTest(final String prName) {
    ...
    vm0.invoke(
        new CacheSerializableRunnable("doPutCreateInvalidateOperations1") {
          public void run2() throws CacheException {
            ...
            assertEquals(
                "isEmpty doesnt return proper state of the PartitionedRegion", true, pr.isEmpty());
            ...
            assertEquals(
                "isEmpty doesnt return proper state of the PartitionedRegion", false, pr.isEmpty());
            ...
            size = pr.size();
            assertEquals(
                "Size doesnt return expected value = 0 instead it returns" + size, size, 0);
            assertEquals(
                "isEmpty doesnt return proper state of the PartitionedRegion", pr.isEmpty(), true);
            //				}
            for (int i = putRange_1Start; i <= putRange_1End; i++) {
              // System.out.println("Putting entry for key = " + i);
              pr.put(Integer.toString(i), Integer.toString(i));
    ...
    vm1.invoke(
        new CacheSerializableRunnable("doRemoveOperations1") {
     ...
     */
    vm2.invoke(
        new CacheSerializableRunnable("doPutCreateInvalidateOperations3") {
    ...
    vm2.invoke(
        new CacheSerializableRunnable("doRemoveOperations2") {
          ...
          public void run2() throws CacheException {
            ...
            for (i = removeRange_2Start; i <= removeRange_2End; i++) {
              ...
              try {
                pr.destroy(Integer.toString(i));
                fail(
                    "EntryNotFoundException is not thrown in destroy operation for key = "
                        + (Integer.toString(i)));
              } catch (EntryNotFoundException expected) {
              }
            }
    ...
    vm3.invoke(
        new CacheSerializableRunnable("doPutCreateInvalidateOperations4") {
          ...
          public void run2() {
            ...
            for (int i = putRange_1Start; i <= putRange_4End; i++) {
              ...
              if ((i >= removeRange_1Start && i <= removeRange_1End)
                  || (i >= removeRange_2Start && i <= removeRange_2End)) {
                assertFalse(
                    "containsValueForKey() remove validation failed for key = " + i, conKey);
              } else {
  ...
  /**
   * Test the PartitionedRegion operations when the Scope is set to {@link
  ...
  public void testPartitionedRegionsOperationsScopeDistEarlyAck() throws Exception {
    ...
    vm2 = host.getVM(2);
    vm3 = host.getVM(3);
    CacheSerializableRunnable create =
        new CacheSerializableRunnable("createPRWithEarlyAck") {
          public void run2() throws CacheException {
            Cache cache = getCache();
            AttributesFactory attr = new AttributesFactory();
  ...
  public void testCacherLoaderHelper() throws Exception {
    ...
    CacheSerializableRunnable createLoaderPR =
        new CacheSerializableRunnable("createLoaderPR") {
          public void run2() throws CacheException {
            getCache();
