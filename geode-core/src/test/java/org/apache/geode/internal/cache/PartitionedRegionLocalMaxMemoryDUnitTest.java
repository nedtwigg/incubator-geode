...
public class PartitionedRegionLocalMaxMemoryDUnitTest extends PartitionedRegionDUnitTestCase {
   ...
   * 1.Create Partition region with localMaxMemory = 1MB on all the VMs </br> <br>
   * 2.Put objects in partition region so that only one bucket gets created and size of that bucket
   * exceeds localMaxMemory <br>
   * 3.Put object such that new bucket gets formed</br> <br>
  ...
  public void testLocalMaxMemoryInPartitionedRegion() {
    ...
    createPartitionRegion(
        vmList, startIndexForRegion, endIndexForRegion, localMaxMemory, redundancy, false);
    System.setProperty(
  ...
  public void testLocalMaxMemoryInPartitionedRegionWithEviction() {
    ...
    System.setProperty(PartitionedRegion.RETRY_TIMEOUT_PROPERTY, "20000");
    createPartitionRegion(
        vmList, startIndexForRegion, endIndexForRegion, localMaxMemory, redundancy, true);
    System.setProperty(
        PartitionedRegion.RETRY_TIMEOUT_PROPERTY,
        Integer.toString(PartitionedRegionHelper.DEFAULT_TOTAL_WAIT_RETRY_ITERATION));
    putFromOneVm(vm[0], true);
    ...
    CacheSerializableRunnable putObject =
        new CacheSerializableRunnable("putObject") {
          public void run2() {
            ...
            if (objectFlg == true) {
              long size = 0;
              while ((size = pr.getDataStore().currentAllocatedMemory())
  ...
  private void destroyRegion(VM vm) {
    SerializableRunnable destroyObj =
        new CacheSerializableRunnable("destroyObj") {
          public void run2() {
            Cache cache = getCache();
            PartitionedRegion pr =
                (PartitionedRegion)
