...
public class PartitionedRegionWithSameNameDUnitTest extends PartitionedRegionDUnitTestCase {
   ...
   * <br>
   * 2. then creates distributed region with the same name as partition region on vm0 </br> In this
   * test RegionExistException is expected while creating region with the same as partition region.
   */
  @Test
   ...
   * 2. then creates Partition region with the same name as region on vm0 </br> In this test
   * RegionExistException is expected while creating Partition region with the same as region.
   */
  @Test
  ...
  /**
   * This test performs following operations <br>
   * 1. Creates partition region with scope = DISTRIBUTED_NO_ACK on vm0.</br> <br>
   ...
   * 2. Creates distributed subregion of parent region </br> OperationNotSupportedException is
   * expected.
   */
  @Test
   ...
   * This test performs following operations <br>
   * 1. Creates partitoned region as parent region on all vms </br> <br>
   * 2. Creates partitioned subregion of parent region </br> OperationNotSupportedException is
   * expected
   */
  @Test
   ...
   * 3.Creates a distributed region as subregion of parent on vm1,vm2,vm3 </br> In this case
   * InternalGemFireException is expected.
   */
  @Test
   ...
   * 3.Creates a partitioned region as subregion of parent on vm1,vm2,vm3 </br> In this case
        ...
        new CacheSerializableRunnable("subRegionOfPartiotionRegion") {
          ...
          public void run2() throws CacheException {
            ...
            PartitionedRegion parentRegion =
                (PartitionedRegion)
                    cache.getRegion(Region.SEPARATOR + "parent_partitioned_region0");
  ...
  private CacheSerializableRunnable createSubRegion(
    ...
    CacheSerializableRunnable createSubRegion =
        new CacheSerializableRunnable("createSubRegion") {
            ...
            if (innerFirstCreationFlag) {
              switch (innerRegionType) {
                ...
                case PARTITIONED_REGION:
                  {
                    Region childRegion =
                        parentRegion.createSubregion(
                            "child_region", createRegionAttrsForPR(0, 200));
                    LogWriterUtils.getLogWriter()
  ...
  /** This function createas multiple partition regions on nodes specified in the vmList */
  private void createDistributedRegion(
      List vmList,
      int startIndexForRegion,
      int endIndexForRegion,
      Scope scope,
    ...
    while (nodeIterator.hasNext()) {
      VM vm = (VM) nodeIterator.next();
      vm.invoke(
          createMultipleDistributedlRegion(
              prPrefix,
              startIndexForRegion,
              endIndexForRegion,
              scope,
  ...
  CacheSerializableRunnable createMultipleDistributedlRegion(
      ...
      final Scope scope,
      final boolean firstCreationFlag,
      final boolean multipleVMFlag) {
   ...
   *     <p>These indices Represents range of the Partition Region
                ...
                if (!multipleVMFlag) {
                  try {
                    cache.createRegion(
                        ...
                        createRegionAttrsForPR(innerRedundancy, innerlocalmaxMemory));
                    fail(
                        "test failed : partition region with same name as local region is created");
                  } catch (RegionExistsException expected) {
                    // getLogWriter()
                    // .info(
                    // "Expected exception RegionExistsException for creating
