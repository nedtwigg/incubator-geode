...
public class CompressionCacheListenerDUnitTest extends JUnit4CacheTestCase {
   ...
   *
   * <ul>
   *   <li>{@link CacheWriter#beforeCreate(EntryEvent)}
   *   <li>{@link CacheListener#afterCreate(EntryEvent)}
   *   <li>{@link CacheWriter#beforeUpdate(EntryEvent)}
   *   <li>{@link CacheListener#afterUpdate(EntryEvent)}
   *   <li>{@link CacheListener#afterInvalidate(EntryEvent)}
   *   <li>{@link CacheWriter#beforeDestroy(EntryEvent)}
   *   <li>{@link CacheListener#afterDestroy(EntryEvent)}
   * </ul>
   *
  ...
  private Region createRegion(String name, Compressor compressor, boolean offHeap) {
    return getCache()
        ...
        .addCacheListener(CACHE_LISTENER)
        .setCacheWriter(CACHE_WRITER)
