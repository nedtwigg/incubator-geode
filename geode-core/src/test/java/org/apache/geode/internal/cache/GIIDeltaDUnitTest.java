...
public class GIIDeltaDUnitTest extends JUnit4CacheTestCase {
  ...
  public final void postSetUp() throws Exception {
    Invoke.invokeInEveryVM(
        GIIDeltaDUnitTest.class, "setRegionName", new Object[] {getUniqueName()});
  ...
  public InternalDistributedMember getDistributedMemberID(VM vm) {
    SerializableCallable getID =
        new SerializableCallable("get member id") {
          public Object call() {
  ...
  public void testDeltaGIIWithOnlyUnfinishedOp_GCAtR() throws Throwable {
    ...
    R.invoke(
        new SerializableRunnable() {
          public void run() {
            Mycallback myAfterRequestRVV =
                new Mycallback(GIITestHookType.AfterRequestRVV, REGION_NAME);
   ...
   *
   * <p>This test also verify tombstoneGC happened in middle of GII, but BEFORE GII thread got
   * GIILock. i.e. before GII, P's RVVGC=P0,R0, upon received RequestImageMessage, it becomes P4,R0
  ...
  public void testFullGIITriggeredByHigherRVVGC() throws Throwable {
    ...
    P.invoke(
        () ->
            InitialImageOperation.resetGIITestHook(
   ...
   *
   * <p>When image (which contains P4, P5) arrives, it should fill the special exception
   */
  @Test
   ...
   * <p>When image (which contains P4, P5) arrives, it should fill the special exception
   *
   * <p>This test will let the 2 operations happen before RIM, so the rvv will match between R and P
  ...
  public void testHooks() throws Throwable {
    ...
    R.invoke(
        () ->
            InitialImageOperation.resetGIITestHook(GIITestHookType.AfterReceivedImageReply, true));
    // 11
   ...
   *
   * <p>There's member T doing GII from P at the same time.
   *
   * <p>In this test, GII thread will get the GIILock before tombstone GC, so tombstone GC should
   * wait for all GIIs to finish
   */
  @Category(
      FlakyTest
          .class) // GEODE-633: SLOW_DISTRIBUTION_MS, non-thread safe test hook, async actions, time sensitive, waitForCriterion, thread joins, forceGC
  @Test
  public void testTombstoneGCInMiddleOfGII() throws Throwable {
    ...
    verifyTombstoneExist(
        P, "key2", true, true); // expect key2 is still tombstone during and after GIIs
    verifyTombstoneExist(
        R, "key2", true, true); // expect key2 is still tombstone during and after GIIs
    verifyTombstoneExist(
        T, "key2", true, true); // expect key2 is still tombstone during and after GIIs
  ...
  @Category(
      FlakyTest
          .class) // GEODE-1068: time sensitive, SLOW_DISTRIBUTION_MS, waitForCriterion, possible thread unsafe test hooks, async actions, depends on stats
  @Test
  ...
  @Category(
      FlakyTest
          .class) // GEODE-1137: orphaned AsyncInvocations, time sensitive, GC, waitForCriterion, thread unsafe test hooks/observers, expiration
  @Test
  public void testTombstoneGCDuringFullGII() throws Throwable {
                ...
                new DistributionMessageObserver() {
                  ...
                  public void beforeSendMessage(
                    ...
                    if (message instanceof TombstoneMessage
                      ...
                      System.err.println(
                          "DAN DEBUG  about to send tombstone message, starting up R - "
                              + message.getSender());
                      ...
                      System.err.println(
                          "DAN DEBUG  R has received the RVV, sending tombstone message");
    ...
    System.err.println(
        "DAN DEBUG P has finished the tombstone GC, waiting for R to get the correct RVV");
        ...
        new SerializableRunnable("Create Region") {
          public void run() {
            try {
              ...
              LogWriterUtils.getLogWriter()
                  .fine(
                      "Unfinished Op limit=" + InitialImageOperation.MAXIMUM_UNFINISHED_OPERATIONS);
  ...
  protected void verifyDeltaSizeFromStats(
      VM vm, final int expectedKeyNum, final int expectedDeltaGIINum) {
  ...
  protected void assignVMsToPandR(final VM vm0, final VM vm1) {
    ...
    LogWriterUtils.getLogWriter()
        .info(
            "Before assignVMsToPandR, dsid0 is "
                + dsid0
                + ",dsid1 is "
                + dsid1
                + ",compare="
  ...
  private DiskStoreID getMemberID(VM vm) {
    SerializableCallable getDiskStoreID =
        new SerializableCallable("get DiskStoreID as member id") {
          public Object call() {
            LocalRegion lr = (LocalRegion) getCache().getRegion(REGION_NAME);
  ...
  private void forceGC(VM vm, final int count) {
    vm.invoke(
        new SerializableCallable("force GC") {
  ...
  private void forceAddGIICount(VM vm) {
    vm.invoke(
        new SerializableCallable("force to add gii count") {
          public Object call() throws Exception {
  ...
  private void assertDeltaGIICountBeZero(VM vm) {
    vm.invoke(
        new SerializableCallable("assert progressingDeltaGIICount == 0") {
          public Object call() throws Exception {
  ...
  public void waitForToVerifyRVV(
      final VM vm,
      final DiskStoreID member,
      final long expectedRegionVersion,
      final long[] exceptionList,
      final long expectedGCVersion) {
    SerializableRunnable waitForVerifyRVV =
        new SerializableRunnable() {

          private boolean verifyExceptionList(
              final DiskStoreID member,
              final long regionversion,
              final RegionVersionVector rvv,
              final long[] exceptionList) {
            ...
            if (exceptionList != null) {
              for (long i : exceptionList) {
                ...
                if (!exceptionListVerified) {
                  LogWriterUtils.getLogWriter()
                      .finer("DeltaGII:missing exception " + i + ":" + rvv);
                  break;
                }
              }
            } else {
                ...
                new WaitCriterion() {
                  public boolean done() {
                    RegionVersionVector rvv =
                        ((LocalRegion) getCache().getRegion(REGION_NAME))
                            .getVersionVector()
                    ...
                    LogWriterUtils.getLogWriter()
                        .info(
                            "DeltaGII:expected:" + expectedRegionVersion + ":" + expectedGCVersion);
                    LogWriterUtils.getLogWriter()
                        .info(
                            "DeltaGII:actual:"
                                + regionversion
  ...
  public void waitForCallbackStarted(final VM vm, final GIITestHookType callbacktype) {
    SerializableRunnable waitForCallbackStarted =
        new SerializableRunnable() {
          public void run() {
  ...
  private VersionTag getVersionTag(VM vm, final String key) {
    SerializableCallable getVersionTag =
        new SerializableCallable("get version tag") {
          public Object call() {
  ...
  public void waitToVerifyKey(final VM vm, final String key, final String expect_value) {
    SerializableRunnable waitToVerifyKey =
        new SerializableRunnable() {
          public void run() {
            ...
            WaitCriterion ev =
                new WaitCriterion() {
                  public boolean done() {
                    String value =
                        (String) ((LocalRegion) getCache().getRegion(REGION_NAME)).get(key);
  ...
  protected byte[] getRVVByteArray(VM vm, final String regionName)
    ...
    SerializableCallable getRVVByteArray =
        new SerializableCallable("getRVVByteArray") {
  ...
  private void checkIfFullGII(
      VM vm,
      final String regionName,
      final byte[] remote_rvv_bytearray,
      final boolean expectFullGII) {
    SerializableRunnable checkIfFullGII =
        new SerializableRunnable("check if full gii") {
          public void run() {
  ...
  private SerializableRunnable oneClearOp(
      final long regionVersionForThisOp, final DiskStoreID memberID) {
    SerializableRunnable clearOp =
        new SerializableRunnable("clear now") {
          public void run() {
            ...
            long region_gc_version =
                getRegionVersionForMember(rr.getVersionVector(), memberID, true);
  ...
  private SerializableRunnable onePutOp(
      final String key,
      final String value,
      final long regionVersionForThisOp,
      final DiskStoreID memberID) {
    SerializableRunnable putOp =
        new SerializableRunnable("put " + key) {
          public void run() {
            LocalRegion lr = (LocalRegion) getCache().getRegion(REGION_NAME);
            lr.put(key, value);
  ...
  private AsyncInvocation doOnePutAsync(
      final VM vm, final long regionVersionForThisOp, final String key) {
    SerializableRunnable putOp =
        onePutOp(key, generateValue(vm), regionVersionForThisOp, getMemberID(vm));
  ...
  private SerializableRunnable oneDestroyOp(
      final String key,
      final String value,
      final long regionVersionForThisOp,
      final DiskStoreID memberID) {
    SerializableRunnable destroyOp =
        new SerializableRunnable("destroy " + key) {
          public void run() {
            LocalRegion lr = (LocalRegion) getCache().getRegion(REGION_NAME);
            lr.destroy(key);
            long region_version = getRegionVersionForMember(lr.getVersionVector(), memberID, false);
  ...
  private AsyncInvocation doOneDestroyAsync(
      final VM vm, final long regionVersionForThisOp, final String key) {
    SerializableRunnable destroyOp =
        oneDestroyOp(key, generateValue(vm), regionVersionForThisOp, getMemberID(vm));
  ...
  private long getRegionVersionForMember(
      RegionVersionVector rvv, DiskStoreID member, boolean isRVVGC) {
    long ret = 0;
    if (isRVVGC) {
  ...
  protected void verifyTombstoneExist(
      VM vm, final String key, final boolean expectExist, final boolean expectExpired) {
    SerializableRunnable verify =
        new SerializableRunnable() {
          private boolean doneVerify() {
            ...
            if (entry == null || !entry.getRegionEntry().isTombstone()) {
              ...
              if (expectExpired) {
                return (ts + TombstoneService.REPLICATE_TOMBSTONE_TIMEOUT
                    <= ((GemFireCacheImpl) cache).cacheTimeMillis()); // use MAX_WAIT as timeout
              } else {
  ...
  protected int getDeltaGIICount(VM vm) {
    SerializableCallable getDelGIICount =
        new SerializableCallable("getDelGIICount") {
          public Object call() throws Exception {
