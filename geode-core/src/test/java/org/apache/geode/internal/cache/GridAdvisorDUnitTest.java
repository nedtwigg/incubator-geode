...
public class GridAdvisorDUnitTest extends JUnit4DistributedTestCase {
  ...
  public void test2by2() throws Exception {
    ...
    keeper2.release();
    vm3.invoke(
        new SerializableRunnable("Start locators on " + port2) {
          public void run() {
    ...
    bsKeeper2.release();
    vm2.invoke(
        new SerializableRunnable("start bridgeServer on " + bsPort2) {
          public void run() {
            try {
              Cache c = CacheFactory.getAnyInstance();
              CacheServer bs = c.addCacheServer();
    ...
    bsKeeper4.release();
    vm2.invoke(
        new SerializableRunnable("start bridgeServer on " + bsPort4) {
          public void run() {
            try {
              Cache c = CacheFactory.getAnyInstance();
              CacheServer bs = c.addCacheServer();
    ...
    vm0.invoke(
        new SerializableRunnable("Verify other locator on " + port2) {
          public void run() {
            ...
            for (int j = 0; j < others.size(); j++) {
              ...
              if (bsp.getPort() == bsPort1) {
                assertEquals(
                    Arrays.asList(new String[] {"bs1Group1", "bs1Group2"}),
                    Arrays.asList(bsp.getGroups()));
              } else if (bsp.getPort() == bsPort2) {
                assertEquals(
                    Arrays.asList(new String[] {"bs2Group1", "bs2Group2"}),
                    Arrays.asList(bsp.getGroups()));
              } else if (bsp.getPort() == bsPort3) {
                assertEquals(
                    Arrays.asList(new String[] {"bs3Group1", "bs3Group2"}),
                    Arrays.asList(bsp.getGroups()));
    ...
    vm3.invoke(
        new SerializableRunnable("Verify other locator on " + port1) {
          public void run() {
            assertTrue(Locator.hasLocator());
            InternalLocator l = (InternalLocator) Locator.getLocator();
            DistributionAdvisee advisee = l.getServerLocatorAdvisee();
    ...
    vm2.invoke(
        new SerializableRunnable("Verify bridge server saw locator stop") {
          public void run() {
            Cache c = CacheFactory.getAnyInstance();
            List bslist = c.getCacheServers();
            assertEquals(2, bslist.size());
    ...
    vm3.invoke(
        new SerializableRunnable("Verify bridge server restart ") {
          public void run() {
            assertTrue(Locator.hasLocator());
            InternalLocator l = (InternalLocator) Locator.getLocator();
            DistributionAdvisee advisee = l.getServerLocatorAdvisee();
  ...
  public void test2by2usingGroups() throws Exception {
    ...
    keeper1.release();
    vm0.invoke(
        new SerializableRunnable("Start locators on " + port1) {
          public void run() {
            File logFile = new File(getUniqueName() + "-locator" + port1 + ".log");
            try {
              Locator.startLocatorAndDS(port1, logFile, null, dsProps, true, true, null);
