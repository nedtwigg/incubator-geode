...
public class PartitionedRegionQueryDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testReevaluationDueToUpdateInProgress() {
    ...
    vm0.invoke(
        new SerializableRunnable("resetting sqt") {
          public void run() {
  ...
  public void testRebalanceDuringQueryEvaluation() {
    ...
    vm1.invoke(
        new SerializableRunnable("check rebalance happened") {
      ...
      vm1.invoke(
          new SerializableRunnable() {
            public void run() {
              ...
              assertTrue(
                  server1TestHook.getHooks().get("Create PR Query Trace Info From Local Node"));
            }
          });
      vm2.invoke(
          new SerializableRunnable() {
            public void run() {
              ...
              assertTrue(
                  server2TestHook.getHooks().get("Create PR Query Trace Info for Remote Query"));
            }
          });
    } finally {
  ...
  public void testOrderByOnPRWithReservedKeywords() throws Exception {
    ...
    final Integer port2 =
        (Integer)
            server2.invoke(
                new SerializableCallable("Create Server2") {
                  @Override
                  public Object call() throws Exception {
                    Region r1 =
                        getCache().createRegionFactory(RegionShortcut.PARTITION).create(regionName);
          ...
          public Object call() throws Exception {
            ...
            for (int i = 0; i < queries.length; i++) {
              try {
                ...
                assertTrue(
                    "Size of resultset should be greater than 0 for query: " + queries[i],
                    sr.size() > 0);
              } catch (Exception e) {
