...
public class PartitionedRegionBucketCreationDistributionDUnitTest
   ...
   * (c) Size of bucket2Node region should be 0 before any put() operation. </br><br>
   * (d) Parent region of the bucket2Node region should be root i.e. region with name "PRRoot".</br>
   * <br>
   * 2. Do put() operation from the different VMs so that buckets gets generated.</br><br>
   ...
   * scope of the partition regions.</br><br>
  ...
  public void testBucketCreationInMultiplePartitionRegion() throws Throwable {
    ...
    putInMultiplePartitionedRegion(
  ...
  public void testBucketCreationInPRPutFromOneNode() throws Throwable {
    ...
    createPartitionRegion(
        vmList, startIndexForRegion, endIndexForRegion, localMaxMemory, redundancy);

    LogWriterUtils.getLogWriter()
        .info(
            "testBucketCerationInMultiPlePartitionRegion() - Partition Regions successfully created ");
    // doing put() operation from vm0 only
    putInMultiplePartitionRegionFromOneVm(
        vm[0], startIndexForRegion, endIndexForRegion, startIndexForKey, endIndexForKey);
    LogWriterUtils.getLogWriter()
        .info(
    ...
    validateBucketsDistributionInMultiplePartitionRegion(
   ...
   * DISTRIBUTED_NO_ACK.</br><br>
   * 2. Performs Put() operation from all the vms for the keys 0 to 111.</br> <br>
   * 3. Validates bucket distribution over all the nodes for multiple partition regions.</br>
   */
  @Test
  public void testBucketCreationInMultiplePartitionRegionFromAllNodes() throws Throwable {
    ...
    putInMultiplePartitionedRegionFromAllVms(
        startIndexForRegion, endIndexForRegion, startIndexForKey, endIndexForKey);
    LogWriterUtils.getLogWriter()
        .info(
            "testBucketCerationInMultiPlePartitionRegion() - Put() Opereration done only from one VM ");
  ...
  public void testBucketDistributionAfterNodeAdditionInPR() throws Throwable {
    ...
    putInMultiplePartitionedRegionFrom3Nodes(
        startIndexForRegion, endIndexForRegion, startIndexForKey, endIndexForKey);
    LogWriterUtils.getLogWriter()
        .info(
  ...
  public void testTotalNumBucketProperty() throws Throwable {
    ...
    createPRWithTotalNumPropSetList(
        vmList, startIndexForRegion, endIndexForRegion, localMaxMemory, localRedundancy);
    putInMultiplePartitionRegionFromOneVm(
        vm[0], startIndexForRegion, endIndexForRegion, startIndexForKey, endIndexForKey);
    int expectedNumBuckets = 11;
    validateTotalNumBuckets(
        prPrefix, vmList, startIndexForRegion, endIndexForRegion, expectedNumBuckets);
  ...
  public void testCompleteBucketAllocation() throws Exception {
    ...
    this.vm[0]
        .invoke(
            new SerializableRunnable("Create keys") {
              public void run() {
                Cache c = getCache();
   ...
   *
   * <p>Used by test {@link #testEnforceUniqueHostForLonerDistributedSystem()}. Moved from
   ...
   */
  private void invalidateInMultiplePartitionedRegion(
      int startIndexForRegion,
      int endIndexForRegion,
      int startIndexForInvalidate,
      int endIndexForInvalidate)
      throws Throwable {
    prPrefix = "testMemoryOfPartitionRegion";
    int AsyncInvocationArrSize = 4;
    AsyncInvocation[] async = new AsyncInvocation[AsyncInvocationArrSize];
    ...
    async[0] =
        vm[0]
            .invokeAsync(
                invalidatesInMultiplePartitionRegion(
                    prPrefix,
                    startIndexForInvalidate,
                    startIndexForInvalidate + 1 * delta,
                    startIndexForRegion,
                    endIndexForRegion));
    async[1] =
        vm[1]
            .invokeAsync(
                invalidatesInMultiplePartitionRegion(
                    prPrefix,
                    startIndexForInvalidate + 1 * delta,
                    startIndexForInvalidate + 2 * delta,
                    startIndexForRegion,
                    endIndexForRegion));
    async[2] =
        vm[2]
            .invokeAsync(
                invalidatesInMultiplePartitionRegion(
                    prPrefix,
                    startIndexForInvalidate + 2 * delta,
                    startIndexForInvalidate + 3 * delta,
                    startIndexForRegion,
                    endIndexForRegion));
    async[3] =
        vm[3]
  ...
  private void putInMultiplePartitionedRegionFrom3Nodes(
    ...
    async[0] =
        vm[0]
            .invokeAsync(
                putInMultiplePartitionRegion(
                    prPrefix,
                    startIndexForKey,
                    startIndexForKey + 1 * delta,
                    startIndexForRegion,
                    endIndexForRegion));
    async[1] =
        vm[1]
   ...
   */
  private void putInMultiplePartitionRegionFromOneVm(
      VM vm0,
      final int startIndexForRegion,
      final int endIndexForRegion,
      final long startIndexForKey,
      final long endIndexForKey)
      throws Throwable {
    int AsyncInvocationArrSize = 3;
    AsyncInvocation[] async = new AsyncInvocation[AsyncInvocationArrSize];
    async[0] =
    ...
    async[2] =
        vm0.invokeAsync(
            putFromOneVm(
                startIndexForKey + 2 * totalBucketNumProperty,
                endIndexForKey + 2 * totalBucketNumProperty,
                startIndexForRegion,
                endIndexForRegion));
   ...
   * @return
   */
  private CacheSerializableRunnable putFromOneVm(
      final long startIndexForKey,
      final long endIndexForKey,
      final int startIndexForRegion,
      final int endIndexForRegion) {
    CacheSerializableRunnable putFromVm =
        new CacheSerializableRunnable("putFromOneVm") {
   ...
   * are put is startIndexForKey to endIndexForKey. Each Vm puts different set of keys.
   */
  private void putInMultiplePartitionedRegion(
      int startIndexForRegion, int endIndexForRegion, int startIndexForKey, int endIndexForKey)
      throws Throwable {
    int AsyncInvocationArrSize = 4;
    AsyncInvocation[] async = new AsyncInvocation[AsyncInvocationArrSize];
  ...
  private void putInMultiplePartitionedRegionFromAllVms(
      int startIndexForRegion, int endIndexForRegion, long startIndexForKey, long endIndexForKey)
      throws Throwable {
  ...
  private void validateBucket2NodeBeforePutInMultiplePartitionedRegion(
    ...
    for (int count = 0; count < AsyncInvocationArrSize; count++) {
      if (async[count].exceptionOccurred()) {
        LogWriterUtils.getLogWriter()
            .warning(
                "Failure in async invocation on vm "
                    + vm[count]
                    + " with exception "
  ...
  private void validateBucketsAfterPutInMultiplePartitionRegion(
      final int startIndexForRegion, final int endIndexForRegion) throws Throwable {
    int AsyncInvocationArrSize = 8;
    AsyncInvocation[] async = new AsyncInvocation[AsyncInvocationArrSize];
    ...
    for (int count = 4; count < AsyncInvocationArrSize; count++) {
      if (async[count].exceptionOccurred()) {
        LogWriterUtils.getLogWriter()
            .warning(
                "Failure of async invocation on VM "
                    + this.vm[count]
                    + " exception thrown "
                    + async[count].getException());
        throw async[count].getException();
  ...
  private void validateBucketsDistributionInMultiplePartitionRegion(
      final int startIndexForRegion, final int endIndexForRegion, int noBucketsExpectedOnEachNode)
      throws Throwable {
    int AsyncInvocationArrSize = 4;
    AsyncInvocation[] async = new AsyncInvocation[AsyncInvocationArrSize];
    async[0] =
    ...
    for (int count = 0; count < AsyncInvocationArrSize; count++) {
      if (async[count].exceptionOccurred()) {
        Assert.fail(
            "Validation of bucket distribution failed on " + count, async[count].getException());
      }
    }
  }

  /** This function is used for the validation of bucket on all the region. */
  private void validateBucketsOnAllNodes(
  ...
  private void validateRedundancy(
      ...
      final int startIndexForRegion,
      final int endIndexForRegion,
      final int redundancyManageFlag) {
    vm0.invoke(
        new CacheSerializableRunnable("validateRedundancy") {
          int innerStartIndexForRegion = startIndexForRegion;
            ...
            for (int i = innerStartIndexForRegion; i < endIndexForRegion; i++) {
              ...
              while (buckteIdItr.hasNext()) {
                ...
                if (redundancyManageFlag == 0) {
                  // checking redundancy
                  if (pr.getRegionAdvisor().getBucketRedundancy(bucketId.intValue())
                      >= redundancy) {
  ...
  private CacheSerializableRunnable validateBucketCreationAfterPut(
    ...
    CacheSerializableRunnable validateAfterPut =
        new CacheSerializableRunnable("validateAfterPut") {
          int innerStartIndexForRegion = startIndexForRegion;
          ...
          public void run2() {
            ...
            for (int i = innerStartIndexForRegion; i < innerEndIndexForRegion; i++) {
              ...
              assertTrue(
                  "Size of local region map should be > 0 for region: " + pr.getFullPath(),
        ...
        new CacheSerializableRunnable("validateBucketsDistribution") {
          ...
          public void run2() {
            ...
            for (int i = startIndexForRegion; i < endIndexForRegion; i++) {
              ...
              assertTrue(
                  "Bucket Distribution for region = "
                      + pr.getFullPath()
                      + " is not correct for member "
                      + pr.getDistributionManager().getId()
                      + " existing size "
                      + localBSize
                  ...
                  .visitBuckets(
                      new BucketVisitor() {
                        public void visit(Integer bucketId, Region r) {
                          Region bucketRegion =
                              root.getSubregion(pr.getBucketName(bucketId.intValue()));
  ...
  private void createPartitionRegion(
    ...
    while (nodeIterator.hasNext()) {
      ...
      vm.invoke(
          createMultiplePRWithTotalNumBucketPropSet(
              prPrefix, startIndexForRegion, endIndexForRegion, redundancy, localMaxMemory, 11));
    }
  }

  /** This function creates a partition region with TOTAL_BUCKETS_NUM_PROPERTY set to 11. */
  private void createPRWithTotalNumPropSetList(
      List vmList,
      int startIndexForRegion,
      int endIndexForRegion,
      int localMaxMemory,
  ...
  CacheSerializableRunnable createMultiplePRWithTotalNumBucketPropSet(
      final String prPrefix,
      final int startIndexForRegion,
      final int endIndexForRegion,
      final int redundancy,
      final int localMaxMem,
    ...
    CacheSerializableRunnable createPRWithTotalNumBucketPropSet =
        new CacheSerializableRunnable("createPRWithTotalNumBucketPropSet") {
          public void run2() throws CacheException {
            ...
            for (int i = startIndexForRegion; i < endIndexForRegion; i++) {
              cache.createRegion(
                  prPrefix + i, createRegionAttrs(redundancy, localMaxMem, numBuckets));
            }
            LogWriterUtils.getLogWriter()
                .info(
  ...
  private void validateTotalNumBuckets(
      String prPrefix,
      List vmList,
      int startIndexForRegion,
      int endIndexForRegion,
    ...
    while (nodeIterator.hasNext()) {
      VM vm = (VM) nodeIterator.next();
      vm.invoke(
          validateTotalNumberOfBuckets(
              prPrefix, expectedNumBuckets, startIndexForRegion, endIndexForRegion));
    }
  }

  /** This function validates total number of buckets from bucket2NodeRegion of partition region. */
  CacheSerializableRunnable validateTotalNumberOfBuckets(
      final String prPrefix,
  ...
  void createMultiplePR(int startIndexForRegion, int endIndexForRegion) {
    ...
    // redundancy = 0;
    createPartitionRegion(
        vmList, startIndexForRegion, midIndexForRegion, localMaxMemory, redundancy);
    ...
    createPartitionRegion(
        vmList, midIndexForRegion, endIndexForNode, localMaxMemory, redundancyTwo);
  ...
  private void calculateTotalMemoryOfPartitionRegion() {
    for (int i = 0; i < 4; i++) {
      if (vm[i] == null) LogWriterUtils.getLogWriter().fine("VM is null" + vm[i]);
  ...
  private void checkTotalMemoryOfPartitionRegion() {
    ...
    CacheSerializableRunnable testTotalMemory =
        new CacheSerializableRunnable("testTotalMemory") {
