...
public class PartitionedRegionEvictionDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testHeapLRUWithOverflowToDisk() {
    ...
    final CacheSerializableRunnable create = new CacheSerializableRunnable("Create Heap LRU with Overflow to disk partitioned Region") {
      public void run2() {
    ...
    final SerializableCallable assertBucketAttributesAndEviction = new SerializableCallable("Assert bucket attributes and eviction") {
      public Object call() throws Exception {
        ...
        try {
          ...
          final PartitionedRegion pr = (PartitionedRegion) getRootRegion(name);
          assertNotNull(pr);
          for (final Iterator i = ((PartitionedRegion) pr).getDataStore().getAllLocalBuckets().iterator(); i.hasNext();) {
            final Map.Entry entry = (Map.Entry) i.next();
            final BucketRegion bucketRegion = (BucketRegion) entry.getValue();
            if (bucketRegion == null) {
              continue;
            }
            assertTrue(bucketRegion.getAttributes().getEvictionAttributes().getAlgorithm().isLRUHeap());
  ...
  public void testHeapLRUWithLocalDestroy() {
    ...
    final SerializableRunnable create = new CacheSerializableRunnable("Create Heap LRU with local destroy on a partitioned Region") {
      public void run2() {
        System.setProperty(HeapLRUCapacityController.TOP_UP_HEAP_EVICTION_PERCENTAGE_PROPERTY, Float.toString(0));
        setEvictionPercentage(heapPercentage);
        final Properties sp = System.getProperties();
      ...
      public Object call() throws Exception {
        try {
          ...
          WaitCriterion wc = new WaitCriterion() {
            ...
            public boolean done() {
              // we have a primary
              if (((AbstractLRURegionMap) pr.entries)._getLruList().stats().getEvictions() == 9) {
                return true;
              }
              return false;
            }
  ...
  public void testMemoryLRUWithOverflowToDisk() {
    ...
    final String name = uniqName + "-PR";
    final SerializableRunnable create = new SerializableRunnable("Create Memory LRU with Overflow to disk partitioned Region") {
      public void run() {
        try {
          final AttributesFactory factory = new AttributesFactory();
          factory.setOffHeap(isOffHeap());
          factory.setPartitionAttributes(new PartitionAttributesFactory().setRedundantCopies(redundantCopies).setLocalMaxMemory(localMaxMem).setTotalNumBuckets(maxBuckets).create());

          factory.setEvictionAttributes(EvictionAttributes.createLRUMemoryAttributes(new ObjectSizerImpl(), EvictionAction.OVERFLOW_TO_DISK));
          factory.setDiskSynchronous(true);
          DiskStoreFactory dsf = getCache().createDiskStoreFactory();
          final File[] diskDirs = new File[1];
          diskDirs[0] = new File("overflowDir/" + uniqName + "_" + OSProcess.getId());
  ...
  public void testMemoryLRUWithLocalDestroy() {
    ...
    final SerializableRunnable create = new CacheSerializableRunnable("Create Memory LRU with local destroy on a partitioned Region") {
      public void run2() {
        ...
        factory.setEvictionAttributes(EvictionAttributes.createLRUMemoryAttributes(new ObjectSizerImpl(), EvictionAction.LOCAL_DESTROY));
        final PartitionedRegion pr = (PartitionedRegion) createRootRegion(name, factory.create());
        assertNotNull(pr);
      }
    };
  ...
  public void testEvictionValidationForLRUMemory() {
    ...
    final int redundantCopies = 1;
    final String name = uniqName + "-PR";
    final EvictionAttributes firstEvictionAttributes = EvictionAttributes.createLRUMemoryAttributes(PartitionAttributesFactory.GLOBAL_MAX_BUCKETS_DEFAULT * 2);
    ...
    firstDatastore.invoke(new CacheSerializableRunnable("First datastore setting LRU memory eviction attrs") {
      public void run2() {
        ...
        // to put it.
        {
          ...
          final AttributesFactory factory = new AttributesFactory();
          factory.setOffHeap(isOffHeap());
          factory.setPartitionAttributes(new PartitionAttributesFactory().setTotalNumBuckets(totalNumBuckets).setRedundantCopies(0).create());
          factory.setEvictionAttributes(EvictionAttributes.createLRUMemoryAttributes((totalNumBuckets / 2)));
          final PartitionedRegion pr = (PartitionedRegion) createRootRegion(name, factory.create());
          final Integer key = new Integer(1);
          pr.put(key, "testval");
          final BucketRegion b;
    ...
    class PRLRUMemoryRunnable extends CacheSerializableRunnable {
      ...
      public PartitionAttributes createPartitionAttributes(final int localMaxMemory) {
        if (this.isAccessor) {
          return new PartitionAttributesFactory().setLocalMaxMemory(0).setRedundantCopies(redundantCopies).create();
        } else {
          return new PartitionAttributesFactory().setLocalMaxMemory(localMaxMemory).setRedundantCopies(redundantCopies).create();
        }
      }
      ...
      public void run2() throws CacheException {
        ...
        try {
          ...
          final EvictionAttributes badEa = EvictionAttributes.createLRUMemoryAttributes(firstEvictionAttributes.getMaximum(), firstEvictionAttributes.getObjectSizer(), EvictionAction.OVERFLOW_TO_DISK);
          assertTrue(!badEa.equals(firstEvictionAttributes));
          factory.setEvictionAttributes(badEa);
          createRootRegion(name, factory.create());
          fail("Test to validate the Eviction Attribute : HeapLRU Interval");
        ...
        {
          assertTrue(firstEvictionAttributes.getAlgorithm().isLRUMemory());
          final EvictionAttributes okEa = EvictionAttributes.createLRUMemoryAttributes(firstEvictionAttributes.getMaximum() - 1, firstEvictionAttributes.getObjectSizer(), firstEvictionAttributes.getAction());
          if (!this.isAccessor) {
          ...
          assertEquals(firstEvictionAttributes.getMaximum(), pr.getAttributes().getEvictionAttributes().getMaximum());
          pr.localDestroyRegion();
        }
