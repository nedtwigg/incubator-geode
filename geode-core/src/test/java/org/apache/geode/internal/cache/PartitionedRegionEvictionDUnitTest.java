...
public class PartitionedRegionEvictionDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testHeapLRUWithOverflowToDisk() {
    ...
    final SerializableRunnable createBuckets =
        new SerializableRunnable("Create Buckets") {
          public void run() {
            setEvictionPercentage(heapPercentage);
            ...
            for (int i = 0; i < bucketsToCreate; i++) {
              ...
              pr.put(
                  new Integer(i + (pr.getPartitionAttributes().getTotalNumBuckets()) * 2),
                  "value2");
            }
          }
        };
          ...
          public Object call() throws Exception {
            ...
            try {
              ...
              for (final Iterator i =
                      ((PartitionedRegion) pr).getDataStore().getAllLocalBuckets().iterator();
                  i.hasNext();
                  ) {
        ...
        new SerializableCallable("Assert bucket attributes and eviction") {
          public Object call() throws Exception {
            try {
              ...
              for (final Iterator i = pr.getDataStore().getAllLocalBuckets().iterator();
                  i.hasNext();
                  ) {
                final Map.Entry entry = (Map.Entry) i.next();
              ...
              WaitCriterion wc =
                  new WaitCriterion() {
                    ...
                    public boolean done() {
                      // we have a primary
                      if (((AbstractLRURegionMap) pr.entries)._getLruList().stats().getEvictions()
                          == 9) {
                        return true;
                      }
                      return false;
  ...
  public void testMemoryLRUWithOverflowToDisk() {
    ...
    vm0.invoke(
        new SerializableRunnable("Test to see that we can get keys") {
          public void run() {
            final PartitionedRegion pr = (PartitionedRegion) getRootRegion(name);
            ...
            try {
              ...
              factory.setEvictionAttributes(
                  EvictionAttributes.createLRUEntryAttributes(
                      maxEntries, EvictionAction.OVERFLOW_TO_DISK));
              factory.setDiskSynchronous(true);
              DiskStoreFactory dsf = getCache().createDiskStoreFactory();
              final File[] diskDirs = new File[1];
              diskDirs[0] = new File("overflowDir/" + uniqName + "_" + OSProcess.getId());
  ...
  public void testEntryLRUWithLocalDestroy() {
    ...
    final SerializableCallable assertListenerCount =
        ...
        new CacheSerializableRunnable(
          ...
          public void run2() {
            ...
            // to put it.
            {
              ...
              factory.setPartitionAttributes(
                  new PartitionAttributesFactory()
                      .setTotalNumBuckets(buks)
                      .setRedundantCopies(0)
                      .create());
              factory.setEvictionAttributes(
                  EvictionAttributes.createLRUEntryAttributes((buks / 2)));
              final PartitionedRegion pr =
                  (PartitionedRegion) createRootRegion(name, factory.create());
    ...
    final SerializableRunnable create2 =
        new SerializableRunnable("Create Entry LRU with Overflow to disk partitioned Region") {
          public void run() {
            ...
            try {
              ...
              assertTrue(
                  expected
                      .getMessage()
                      .contains(
                          PartitionRegionConfigValidator
            ...
            assertTrue(firstEvictionAttrs.getAlgorithm().isLRUEntry());
            final EvictionAttributes brokenEa =
                EvictionAttributes.createLRUEntryAttributes(
        ...
        new SerializableRunnable("Create a datastore to test existing eviction attributes") {
          public void run() {
            ...
            assertNotSame(
                ...
                pr.getAttributes().getEvictionAttributes().getMaximum());
            assertEquals(
                pr.getAttributes().getPartitionAttributes().getLocalMaxMemory(),
                pr.getAttributes().getEvictionAttributes().getMaximum());
          }
        });
  }
  ...
  public void testEvictionValidationForLRUMemory() {
    ...
    final EvictionAttributes firstEvictionAttributes =
        EvictionAttributes.createLRUMemoryAttributes(
            PartitionAttributesFactory.GLOBAL_MAX_BUCKETS_DEFAULT * 2);

    firstDatastore.invoke(
        new CacheSerializableRunnable("First datastore setting LRU memory eviction attrs") {
          public void run2() {
    ...
    class PRLRUMemoryRunnable extends CacheSerializableRunnable {
      ...
      public void run2() throws CacheException {
        ...
        try {
          ...
          final EvictionAttributes badEa =
              EvictionAttributes.createLRUMemoryAttributes(
                  firstEvictionAttributes.getMaximum(),
  ...
  public void testEntryLRUDeadlock() {
    ...
    final SerializableRunnable doPuts =
        new SerializableRunnable("Do Puts") {
          public void run() {
            final PartitionedRegion pr = (PartitionedRegion) getRootRegion(name);
            assertNotNull(pr);
