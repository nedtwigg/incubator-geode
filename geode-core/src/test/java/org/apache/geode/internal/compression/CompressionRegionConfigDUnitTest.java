...
public class CompressionRegionConfigDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testReplicatePersistentRegion() throws Exception {
    Compressor compressor = new SnappyCompressor();
    assertTrue(
        createCompressedRegionOnVm(
            getVM(0), REGION_NAME, DataPolicy.PERSISTENT_REPLICATE, compressor, DISK_STORE));
    assertTrue(
        createCompressedRegionOnVm(
            getVM(1), REGION_NAME, DataPolicy.PERSISTENT_REPLICATE, compressor));
    assertNull(putUsingVM(getVM(0), KEY_1, VALUE_1));
    waitOnPut(getVM(1), KEY_1);
  ...
  public void testPartitionedPersistentRegion() {
    ...
    assertTrue(
        createCompressedRegionOnVm(
            getVM(0), REGION_NAME, DataPolicy.PERSISTENT_PARTITION, compressor, DISK_STORE));
    assertTrue(
        createCompressedRegionOnVm(
            getVM(1), REGION_NAME, DataPolicy.PERSISTENT_PARTITION, compressor));
    assertNull(putUsingVM(getVM(0), KEY_1, VALUE_1));
    waitOnPut(getVM(1), KEY_1);
    flushDiskStoreOnVM(getVM(0), DISK_STORE);
    closeRegionOnVM(getVM(1), REGION_NAME);
  ...
  public void testClientProxyRegion() {
    ...
    assertTrue(
        createCompressedClientRegionOnVm(
            getVM(1), REGION_NAME, compressor, ClientRegionShortcut.PROXY));
  ...
  public void testCachingClientProxyRegion() {
    ...
    assertTrue(
        createCompressedClientRegionOnVm(
            getVM(1), REGION_NAME, compressor, ClientRegionShortcut.CACHING_PROXY));
    assertNull(putUsingClientVM(getVM(1), KEY_1, VALUE_1));
    assertEquals(VALUE_1, getUsingClientVM(getVM(1), KEY_1));
    assertEquals(VALUE_1, getUsingVM(getVM(0), KEY_1));
    cleanupClient(getVM(1));
   ...
   */
  private boolean createCompressedServerRegionOnVm(
      final VM vm, final String name, final DataPolicy dataPolicy, final Compressor compressor) {
    return (Boolean)
        vm.invoke(
            new SerializableCallable() {
              @Override
   ...
   */
  private boolean createCompressedClientRegionOnVm(
      final VM vm,
      final String name,
      final Compressor compressor,
   ...
   */
  private Region<String, String> createClientRegion(
      String name, Compressor compressor, ClientRegionShortcut shortcut) {
    ClientCacheFactory factory = getClientCacheFactory(getLocatorPort());
    return getClientCache(factory)
        .<String, String>createClientRegionFactory(shortcut)
  ...
  private Region<String, String> createRegion(
      String name, DataPolicy dataPolicy, Compressor compressor) {
    return getCache()
   ...
   */
  private Region<String, String> createServerRegion(
      String name, DataPolicy dataPolicy, Compressor compressor) throws IOException {
   ...
   */
  private Region<String, String> createRegion(
      String name, DataPolicy dataPolicy, Compressor compressor, String diskStoreName) {
  ...
  private ClientCacheFactory getClientCacheFactory(int dunitLocatorPort) {
    return new ClientCacheFactory()
        .addPoolLocator("localhost", dunitLocatorPort)
