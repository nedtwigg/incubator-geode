...
public class Bug39356DUnitTest extends JUnit4CacheTestCase {
  ...
  public void testCrashWhileCreatingABucket() {
    ...
    SerializableRunnable createParReg =
        new SerializableRunnable("Create parReg") {
          public void run() {
    ...
    SerializableRunnable createParRegAccessor =
        new SerializableRunnable("Create parReg") {
          public void run() {
            Cache cache = getCache();
            AttributesFactory af = new AttributesFactory();
    ...
    SerializableRunnable verifyBuckets =
        new SerializableRunnable("Verify buckets") {
          ...
          public void run() {
            ...
            for (int i = 0;
                i < r.getAttributes().getPartitionAttributes().getTotalNumBuckets();
                i++) {
              List owners = null;
              while (owners == null) {
                try {
  ...
  protected final class MyRegionObserver extends DistributionMessageObserver
    ...
    public void beforeProcessMessage(DistributionManager dm, DistributionMessage message) {
      if (message instanceof ManageBucketMessage) {
        vm0.invoke(
            new SerializableRunnable("Disconnect VM 0") {
              public void run() {
                disconnectFromDS();
                try {
                  Thread.sleep(10000);
  ...
  public void testMoveBucketToHostThatHasTheBucketAlready() {
    ...
    //Move the bucket
    vm0.invoke(
        new SerializableRunnable("moveBucket") {
