...
public class PartitionedRegionDestroyDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void testDestroyRegion() throws Exception, Throwable {
    ...
    vm1.invoke(
        new CacheSerializableRunnable("doPutOperations-1") {
    ...
    async1 =
        vm2.invokeAsync(
            new CacheSerializableRunnable("doPutOperations-2") {
    ...
    vm0.invoke(
        new CacheSerializableRunnable("destroyPRRegions") {
    ...
    CacheSerializableRunnable validateMetaDataAfterDestroy =
        new CacheSerializableRunnable("validateMetaDataAfterDestroy") {
          ...
          public void run2() throws CacheException {
            ...
            if (cache.rootRegions().size() > 1) {
              fail(
                  "All Regions Not destroyed. # OF Regions Not Destroyed = "
            ...
            while (itr.hasNext()) {
              ...
              assertEquals(
                  "regionFound that should be gone!:" + rg.getName(),
                  -1,
  ...
  private void addExceptionTag(final String expectedException) {
    ...
    SerializableRunnable addExceptionTag =
        new CacheSerializableRunnable("addExceptionTag") {
          public void run2() {
            getCache()
                .getLogger()
                .info(
                    "<ExpectedException action=add>" + expectedException + "</ExpectedException>");
          }
        };
  ...
  private void removeExceptionTag(final String expectedException) {
    ...
    SerializableRunnable removeExceptionTag =
