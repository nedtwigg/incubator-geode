...
public class EventIdOptimizationDUnitTest extends JUnit4DistributedTestCase {
  ...
  public final void postSetUp() throws Exception {
    ...
    int PORT1 =
        ((Integer) server1.invoke(() -> EventIdOptimizationDUnitTest.createServerCache()))
            .intValue();
    int PORT2 =
        ((Integer) server2.invoke(() -> EventIdOptimizationDUnitTest.createServerCache()))
            .intValue();

    client1.invoke(
        () ->
            EventIdOptimizationDUnitTest.createClientCache1(
                NetworkUtils.getServerHostName(host), new Integer(PORT1)));
    client2.invoke(
        () ->
            EventIdOptimizationDUnitTest.createClientCache2(
  ...
  public static void generateEventsByDestroyEntryOperation() throws Exception {
    ...
    for (int i = 0; i < eventIds.length; i++) {
      srp.destroyOnForTestsOnly(
          connection, "KEY-" + i, null, Operation.DESTROY, new EventIDHolder(eventIds[i]), null);
    }
    srp.destroyOnForTestsOnly(
        connection, LAST_KEY, null, Operation.DESTROY, new EventIDHolder(eventIdForLastKey), null);
  }
