...
public class CacheAdvisorDUnitTest extends JUnit4CacheTestCase {
      ...
      vms[i]
          .invoke(
              new CacheSerializableRunnable(
                  "CacheAdvisorDUnitTest.testGenericAdvice;createRegion") {
  ...
  public void testNetWriteAdvice() throws Exception {
    ...
    for (int i = 0; i < vms.length; i++) {
      ...
      final int index = i;
      vm.invoke(
          new CacheSerializableRunnable("CacheAdvisorDUnitTest.testNetWriteAdvice") {
            public void run2() throws CacheException {
  ...
  public void testNetLoadAdvice() throws Exception {
    ...
    for (int i = 0; i < vms.length; i++) {
      ...
      final int index = i;
      vm.invoke(
          new CacheSerializableRunnable("CacheAdvisorDUnitTest.testNetLoadAdvice") {
            public void run2() throws CacheException {
              AttributesFactory fac = new AttributesFactory();
              if (index % 2 == 1) {
                fac.setCacheLoader(
