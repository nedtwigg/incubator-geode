...
public class EvictionTestBase extends JUnit4CacheTestCase {
  ...
  public void prepareScenario1(EvictionAlgorithm evictionAlgorithm, int maxEntries) {
    createCacheInAllVms();
    createPartitionedRegionInAllVMS(
        true, evictionAlgorithm, "PR1", totalNoOfBuckets, 1, 10000, maxEntries);
    createPartitionedRegionInAllVMS(
        true, evictionAlgorithm, "PR2", totalNoOfBuckets, 2, 10000, maxEntries);
  }
  ...
  public void prepareScenario2(
      ...
      final String partitionRegion1,
      final String partitionRegion2) {
    dataStore3.invoke(
  ...
  public static void setTolerance() {
    System.setProperty(
        DistributionConfig.GEMFIRE_PREFIX + "memoryEventTolerance", Integer.toString(0));
  }
  ...
  protected void createPartitionedRegionInAllVMS(
  ...
  public void createPartitionedRegion(
    ...
    PartitionAttributesFactory partitionAttributesFactory =
        new PartitionAttributesFactory()
            .setRedundantCopies(totalNoOfBuckets == 4 ? 0 : 1)
            .setTotalNumBuckets(totalNoOfBuckets);
    if (evictionAlgorithm.isLRUMemory()) partitionAttributesFactory.setLocalMaxMemory(maxEnteries);
  ...
  public static void putData(
      final String regionName, final int noOfElememts, final int sizeOfElement) {
    dataStore1.invoke(
  ...
  public void validateNoOfEvictions(final String regionName, final int noOfEvictions) {
    final SerializableCallable validate =
          ...
          public Object call() throws Exception {
            try {
              return getEvictor().getEvictorThreadPool() != null
                  ? getEvictor().getEvictorThreadPool().getTaskCount()
                  : 0;
            } finally {
            }
          }
  ...
  public static void putDataInDataStore3(
      final String regionName, final int noOfElememts, final int sizeOfElement) {
    dataStore3.invoke(
        new CacheSerializableRunnable("putData") {
          @Override
  ...
  public void validateNoOfEvictionsInDataStore3N4(
      final String regionName, final int noOfEvictions) {
    final SerializableCallable validate =
        new SerializableCallable("Validate evictions") {
          public Object call() throws Exception {
        ...
        vm.invoke(
            new SerializableCallable() {
              ...
              public Object call() throws Exception {
                return (int)
                        Math.ceil(
                            (getEvictor().getTotalBytesToEvict() / 2)
                                / (double) ((1024 * 1024) + 100))
                    * 2;
              }
            });
  }
