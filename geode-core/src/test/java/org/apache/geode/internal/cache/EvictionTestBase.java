...
public class EvictionTestBase extends JUnit4CacheTestCase {
  ...
  public void prepareScenario1(EvictionAlgorithm evictionAlgorithm, int maxEntries) {
    createCacheInAllVms();
    createPartitionedRegionInAllVMS(true, evictionAlgorithm, "PR1", totalNoOfBuckets, 1, 10000, maxEntries);
    createPartitionedRegionInAllVMS(true, evictionAlgorithm, "PR2", totalNoOfBuckets, 2, 10000, maxEntries);
  }
  ...
  public void createPartitionedRegion(boolean setEvictionOn, EvictionAlgorithm evictionAlgorithm, String regionName, int totalNoOfBuckets, int evictionAction, int evictorInterval, int maxEnteries) {
    ...
    if (setEvictionOn) {
      if (evictionAlgorithm.isLRUHeap()) {
        ...
        factory.setEvictionAttributes(EvictionAttributes.createLRUMemoryAttributes(new ObjectSizerImpl(), evictionAction == 1 ? EvictionAction.LOCAL_DESTROY : EvictionAction.OVERFLOW_TO_DISK));
      } else {
