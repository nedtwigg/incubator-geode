...
public class Bug37500JUnitTest extends DiskRegionTestingBase {
   ...
   *     5.Put 440 bytes , switching will be caused, it will go in oplog2, Roller will remained
   *     blocked (step 2)<br>
   *     6.Put 440 bytes , it will go in oplog2, oplog2 will now be full<br>
   *     7.Notify the Roller and put 440 bytes , this will try further switching. The put will fail
   *     with exception due to bug 37500. The put thread takes an entry level lock for entry2 ( the
   *     one with KEY2) and tries to write to disk but there is no free space left, so it goes into
   *     wait, expecting Roller to free up the space. The roller, which has now been notified to
   *     run, tries to roll entry2 for which it seeks entry level lock which has been acquired by
   *     put-thread. So the put thread eventually comes out of the wait with DiskAccessException<br>
   *     Another scenario for this bug is, once the disk space was getting exhausted , the entry
   *     operation threads which had already taken a lock on Entry got stuck trying to seek the
   *     Oplog Lock. The switching thread had acquired the Oplog.lock & was waiting for the roller
   *     thread to free disk space. Since the roller needed to acquire Entry lock to roll, it was
   *     unable to do so because of entry operation threads. This would cause the entry operation
   *     threads to get DiskAccessException after completing the stipulated wait. The Roller was
   *     able to free space only when it has rolled all the relevant entries which could happen only
   *     when the entry operation threads released the entry lock after getting DiskAccessException.
   *
   * @throws Exception
   */
  @Test
