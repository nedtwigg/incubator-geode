...
public class PartitionedRegionStatsDUnitTest extends PartitionedRegionDUnitTestCase {
  ...
  public void testClose() throws Exception {
     ...
     */
    CacheSerializableRunnable doRegionOps =
    ...
    CacheSerializableRunnable disconnectVM =
        new CacheSerializableRunnable( // TODO bug36296
            "disconnectVM") {
          public void run2() {
            Cache cache = getCache();
  ...
  public void testDataStoreEntryCountWithRebalance() throws InterruptedException {
    ...
    vm0.invoke(
        new SerializableRunnable("Put some data") {
          public void run() {
            Cache cache = getCache();
  ...
  public void testDataStoreEntryCount() throws InterruptedException {
    ...
    //2
    vm2.invoke(
        new SerializableRunnable("validate stats") {
          public void run() {
            Cache cache = getCache();
  ...
  public void testTotalNumBuckets() {
    ...
    SerializableRunnable putDataInRegion =
        new SerializableRunnable("Put some data") {
          public void run() {
            Cache cache = getCache();
            Region region = cache.getRegion("region1");
  ...
  private void validateEntryCount(final VM vm0, final long expectedCount) {
    SerializableRunnable validateStats =
        new SerializableRunnable("validate stats") {
          public void run() {
            Cache cache = getCache();
            PartitionedRegion region = (PartitionedRegion) cache.getRegion("region1");
