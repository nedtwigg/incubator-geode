...
public class HARegionQueueJUnitTest {
   ...
   *
   * <p>TODO:Dinesh : Work on optimizing the handling of receiving duplicate events
   */
  @Test
   ...
   */
  private void createAndRunProducers(
      boolean generateSameKeys,
      boolean generateSameIds,
      boolean conflationEnabled,
      int putPerProducer)
      throws Exception {
  ...
  public void testBlockQueue() {
    ...
    testFailed = false;
    try {
      final HARegionQueue bQ =
          HARegionQueue.getHARegionQueueInstance(
              "testing", cache, HARegionQueue.BLOCKING_HA_QUEUE, false);
  ...
  class Producer extends Thread {
    ...
    public void run() {
      ...
      for (long i = 0; i < totalPuts; i++) {
        ...
        try {
          ConflatableObject event =
              new ConflatableObject(
                  keyPrefix + i,
                  "val" + i,
                  new EventID(new byte[] {1}, startingId, startingId + i),
                  createConflatables,
                  REGION_NAME);
  ...
  public void testExpiryPositive() {
    try {
      ...
      regionqueue.put(
          new ConflatableObject(
              "key", "value", new EventID(new byte[] {1}, 1, 1), true, "testing"));
      ...
      assertTrue(
          " Expected region size to be zero since expiry time has been exceeded but it is  "
              + regionqueue.getRegion().keys().size(),
  ...
  public void testExpiryNegative() {
    try {
      ...
      regionqueue.put(
          new ConflatableObject(
              "key", "value", new EventID(new byte[] {1}, 1, 1), false, "testing"));
      Thread.sleep(1200);
      assertTrue(
          " Expected region size to be 2, since expiry time has not been exceeded but it is : "
              + regionqueue.getRegion().keys().size(),
          regionqueue.getRegion().keys().size() == 2);
  ...
  public void testExpiryPositiveWithConflation() {
    try {
      ...
      regionqueue.put(
          new ConflatableObject(
              "key", "newValue", new EventID(new byte[] {1}, 1, 2), true, "testing"));
      assertTrue(
      ...
      System.out.println(
          " it still contains thread id : " + regionqueue.getRegion().containsKey(tid));
      assertTrue(
  ...
  public void testExpiryNegativeWithConflation() {
    try {
      ...
      assertTrue(
          " Expected conflation map size not  to be zero since expiry time has not been exceeded but it is not so ",
          !(((Map) (regionqueue.getConflationMapForTesting().get("testing"))).size() == 0));
      assertTrue(
          " Expected eventID map size not to be zero since expiry time has not been exceeded but it is not so ",
          !(regionqueue.getEventsMapForTesting().size() == 0));
  ...
  public void testPutPath() {
    try {
      ...
      regionqueue.put(cf);
      assertTrue(
          " Expected region peek to return cf but it is not so ", (regionqueue.peek().equals(cf)));
      assertTrue(
          " Expected the available id's size not  to be zero since expiry time has not  been exceeded but it is not so ",
          !(regionqueue.getAvalaibleIds().size() == 0));
      assertTrue(
  ...
  public void testQRMDispatch() {
    try {
      ...
      for (int i = 0; i < 10; i++) {
        cf[i] =
            new ConflatableObject(
                "key" + i, "value", new EventID(new byte[] {1}, 1, i), true, "testing");
  ...
  public void testQRM() {
    try {
      ...
      for (int i = 0; i < 10; ++i) {
        regionqueue.put(
            new ConflatableObject(
                "key" + (i + 1), "value", new EventID(new byte[] {1}, 1, i + 1), true, "testing"));
      }
  ...
  public void testSafeConflationRemoval() {
    try {
      hrqFortestSafeConflationRemoval = new HARQTestClass("testSafeConflationRemoval", cache, this);
      ...
      Map map =
          (Map)
              hrqFortestSafeConflationRemoval
   ...
   *
   * <p>In this test, first we add x number of events for unique thread id for the same region then
   * we start two concurrent threads. One which does add to dispatched events map with sequence id's
   ...
   *
   * <p>After the two threads have operated, createMessageList is called again and the data is
   * stored in another list.
   *
   * <p>The data in the list is populated on a map against the threadId.
   *
   * <p>It is then verified to see that all the sequence should be greater than x
   */
  @Test
  public void testConcurrentDispatcherAndRemovalForSameRegionSameThreadId() {
    try {
      ...
      while (iterator.hasNext()) {
        ...
        assertTrue(
            " Expected all the sequence ID's to be greater than "
                + max
                + " but it is not so. Got sequence id "
                + next,
            next.compareTo(max) >= 0);
      }
    } catch (Exception e) {
      throw new AssertionError("Test failed due to : ", e);
    }
   ...
   * <p>The data in the list is populated on a map against the threadId.
   *
   * <p>It is then verified to see that the map size should be 2x
   */
  @Test
   ...
   *
   * <p>In this test, first we add x number of events for unique thread id for the 2 regions then we
   * start two concurrent threads. One which does add to dispatched events map to 3 new regions
   *
   * <p>the second thread does createMessageList (which removes from the dispatched events map) and
   * stores the list
   *
   * <p>After the two threads have operated, createMessageList is called again and the data is
   * stored in another list.
   ...
   * <p>The data in the list is populated on a map against the threadId.
   *
   * <p>It is then verified to see that a total of x entries are present in the map
   */
  @Test
   ...
   *
   * <p>In this test, first we add x number of events for unique thread id for the multiples regions
   ...
   * <p>The data in the list is populated on a map against the threadId.
   *
   * <p>It is then verified to see that the map size should be 2x * number of regions
   */
  @Test
  public void testConcurrentDispatcherAndRemovalForMultipleRegionsDifferentThreadId() {
    try {
      ...
      assertTrue(
          " Expected the map size to be "
              + (numberOfIterations * 2 * 5)
              + " but it is "
              + map.size(),
          map.size() == (numberOfIterations * 2 * 5));
   ...
   * exception
   *
   * <p>Test is commented as assertion for sequence violation is removed in the
  ...
  public void testExceptionInPutForSequenceViolationWithoutGII() {
    ...
    try {
      ...
      rq =
          HARegionQueue.getHARegionQueueInstance(
              "testException", cache, HARegionQueue.NON_BLOCKING_HA_QUEUE, false);
    } catch (Exception e) {
    ...
    try {
      rq.put(cf2);
      fail(
          "Test failed because asertion error was expected but there was'nt any"); // TODO: this is broken, the following catch will catch this or what's thrown by put
  ...
  private void testBatchPeekWithRemove(boolean createBlockingQueue) {
    ...
    try {
      HARegionQueue regionqueue =
          createBlockingQueue
              ? HARegionQueue.getHARegionQueueInstance(
                  "testing", cache, haa, HARegionQueue.BLOCKING_HA_QUEUE, false)
              : HARegionQueue.getHARegionQueueInstance(
                  "testing", cache, haa, HARegionQueue.NON_BLOCKING_HA_QUEUE, false);
      for (int i = 0; i < 10; ++i) {
      ...
      while (itr.hasNext()) {
        ...
        assertNotNull(conf);
        assertTrue(
            "The sequence ID of the objects in the queue are not as expected",
  ...
  public void testUpdationOfMessageSyncInterval() throws Exception {
    ...
    assertEquals(
        "messageSyncInterval not set properly",
        initialMessageSyncInterval,
    ...
    assertEquals(
        "messageSyncInterval not updated.",
        updatedMessageSyncInterval,
        HARegionQueue.getMessageSyncInterval());
  }
}
