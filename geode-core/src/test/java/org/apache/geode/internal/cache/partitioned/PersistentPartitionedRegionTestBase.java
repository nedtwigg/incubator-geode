...
public abstract class PersistentPartitionedRegionTestBase extends JUnit4CacheTestCase {
  ...
  public final void postSetUp() throws Exception {
    ...
    Invoke.invokeInEveryVM(
        PersistentPartitionedRegionTestBase.class, "setRegionName", new Object[] {getUniqueName()});
  ...
  protected void checkRecoveredFromDisk(VM vm, final int bucketId, final boolean recoveredLocally) {
    vm.invoke(
        new SerializableRunnable("check recovered from disk") {
          public void run() {
  ...
  protected void fakeCleanShutdown(VM vm, final int bucketId) {
    vm.invoke(
        new SerializableRunnable("mark clean") {
          public void run() {
            Cache cache = getCache();
            PartitionedRegion region = (PartitionedRegion) cache.getRegion(PR_REGION_NAME);
            DiskRegion disk = region.getRegionAdvisor().getBucket(bucketId).getDiskRegion();
  ...
  private PersistentMemberID getPersistentID(VM vm, final int bucketId) {
    Object id =
        vm.invoke(
            new SerializableCallable("get bucket persistent id") {
              public Object call() {
                Cache cache = getCache();
                PartitionedRegion region = (PartitionedRegion) cache.getRegion(PR_REGION_NAME);
  ...
  private void forceRecovery(VM vm) {
    vm.invoke(
        new SerializableRunnable("force recovery") {
          public void run() {
            Cache cache = getCache();
  ...
  protected void checkData(
    ...
    SerializableRunnable checkData =
        new SerializableRunnable("CheckData") {
  ...
  protected void closePR(VM vm0, String regionName) {
    SerializableRunnable close =
        new SerializableRunnable("Close PR") {
          public void run() {
            Cache cache = getCache();
            Region region = cache.getRegion(regionName);
            region.close();
  ...
  protected void destroyPR(VM vm0, String regionName) {
    SerializableRunnable destroy =
        new SerializableRunnable("Destroy PR") {
          public void run() {
            Cache cache = getCache();
            Region region = cache.getRegion(regionName);
  ...
  protected void localDestroyPR(VM vm0) {
    SerializableRunnable destroyPR =
        new SerializableRunnable("destroy pr") {
  ...
  protected void createPR(
      VM vm0, final int redundancy, final int recoveryDelay, int numBuckets, boolean synchronous) {
  ...
  private SerializableRunnable getCreatePRRunnable(
      final int redundancy, final int recoveryDelay, final int numBuckets) {
        ...
        new SerializableRunnable("create pr") {
          ...
          public void run() {
            ...
            RegionAttributes attr =
                getPersistentPRAttributes(
                    redundancy, recoveryDelay, cache, numBuckets, synchronous);
  ...
  protected AsyncInvocation createPRAsync(
      VM vm0, final int redundancy, int recoveryDelay, int numBuckets) {
    SerializableRunnable createPR = getCreatePRRunnable(redundancy, recoveryDelay, numBuckets);
  ...
  protected void waitForBuckets(
      VM vm, final Set<Integer> expectedBuckets, final String regionName) {
    SerializableCallable getBuckets =
        new SerializableCallable("get buckets") {
            ...
            Wait.waitForCriterion(
                new WaitCriterion() {
                  ...
                  public String description() {
                    return "Buckets on vm "
                        + getActualBuckets()
                        + " never became equal to expected "
                        + expectedBuckets;
                  }
  ...
  protected Set<Integer> getPrimaryBucketList(VM vm0, final String regionName) {
    SerializableCallable getPrimaryBuckets =
        new SerializableCallable("get primary buckets") {
        ...
        new SerializableRunnable("Revoke the member") {
          ...
          public void run() {
            ...
            try {
              ...
              try {
                final WaitCriterion wc =
                    new WaitCriterion() {

                      public boolean done() {
                        try {
                          final Set<PersistentID> missingIds =
          ...
          public void run() {
            ...
            try {
              ...
              adminDS.revokePersistentMember(
                  InetAddress.getLocalHost(), directory.getCanonicalPath());
            } catch (Exception e) {
              throw new RuntimeException(e);
            } finally {
              if (adminDS != null) {
  ...
  protected boolean moveBucket(final int bucketId, VM source, VM target) {
    ...
    SerializableCallable move =
        new SerializableCallable("move bucket") {
  ...
  protected void waitForBucketRecovery(
      final VM vm2, final Set<Integer> lostBuckets, final String regionName) {
    vm2.invoke(
        new SerializableRunnable() {
          public void run() {
            Cache cache = getCache();
  ...
  protected void waitForRedundancyRecovery(
      VM vm, final int expectedRedundancy, final String regionName) {
    vm.invoke(
        new SerializableRunnable() {
          ...
          public void run() {
            ...
            Wait.waitForCriterion(
                new WaitCriterion() {
                  ...
                  public String description() {
                    PartitionRegionInfo info = PartitionRegionHelper.getPartitionRegionInfo(region);
                    return "Did not reach expected redundancy "
                        + expectedRedundancy
                        + " redundancy info = "
  ...
  protected BackupStatus backup(VM vm) {
    return (BackupStatus)
        vm.invoke(
            new SerializableCallable("Backup all members") {
