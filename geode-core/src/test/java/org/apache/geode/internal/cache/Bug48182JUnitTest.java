...
public class Bug48182JUnitTest {
  ...
  public void test48182WithCacheClose() throws Exception {
    AbstractRegionMap.testHookRunnableFor48182 =
        new Runnable() {
          @Override
          public void run() {
            getCache().close();
    ...
    try {
      ...
      //      e.printStackTrace();
      fail(
          "Did not receive a CacheClosedException.  Received a "
              + e.getClass().getName()
              + " instead.");
    }
  ...
  /**
   * Simulates the conditions similar to 48182 by setting a test hook boolean in {@link
   * AbstractRegionMap}. This test hook forces a region destroy during a destroy operation in an
   * off-heap region. This test asserts that a RegionDestroyedException is thrown rather than an
  ...
  public void test48182WithRegionDestroy() throws Exception {
    ...
    try {
      ...
      //      e.printStackTrace();
      fail(
          "Did not receive a RegionDestroyedException.  Received a "
              + e.getClass().getName()
              + " instead.");
    }
