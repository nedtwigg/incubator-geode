...
public class TXLockServiceDUnitTest extends JUnit4DistributedTestCase {
  ...
  public void testTXRecoverGrantorMessageProcessor() throws Exception {
    ...
    */
    ...
    assertEquals(
        "testTXRecoverGrantor_replyCode_PASS is false", true, testTXRecoverGrantor_replyCode_PASS);
    assertEquals(
        "testTXRecoverGrantor_heldLocks_PASS is false", true, testTXRecoverGrantor_heldLocks_PASS);
  }
  ...
  public void testTXLock() {
    ...
    Host.getHost(0)
        .getVM(clientA)
        .invoke(
            new SerializableRunnable("[testTXLock] create clientA and request txLock") {
              public void run() {
                TXLockService dtls = TXLockService.getDTLS();
            ...
            new SerializableRunnable(
              ...
              public void run() {
                ...
                testTXOriginatorRecoveryProcessor_TXLockId =
                    dtls.txLock(regionLockReqs, participants);
                assertNotNull(
                    "testTXOriginatorRecoveryProcessor_TXLockId is null",
                    testTXOriginatorRecoveryProcessor_TXLockId);
              }
            });
  ...
  protected static Boolean isLockGrantor_DTLS() {
    ...
    if (true) {
      DLockService service =
          DLockService.getInternalServiceNamed(
              ((TXLockServiceImpl) dtls).getInternalDistributedLockService().getName());
      assertNotNull(service);

      assertEquals(
          "DTLS and DLock should both report same isLockGrantor result",
          true,
  ...
  protected static Boolean isDistributed_DTLS() {
    ...
    assertNotNull(svc);
    assertEquals(
        "DTLS InternalDistributedLockService should not be destroyed", false, svc.isDestroyed());
    ...
    if (true) {
      ...
      assertEquals(
          "DTLS and DLock should both report same isDistributed result",
          true,
  ...
  private void checkDLockRecoverGrantorMessageProcessor() {
    ...
    */
    ...
    assertEquals(
        "DTLS should use TXRecoverGrantorMessageProcessor",
        true,
