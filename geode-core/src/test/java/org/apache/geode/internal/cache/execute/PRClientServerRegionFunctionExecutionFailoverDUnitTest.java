...
public class PRClientServerRegionFunctionExecutionFailoverDUnitTest extends PRClientServerTestBase {
  ...
  public void testOnRegionFailoverWithTwoServerDownHA() throws InterruptedException {
    ...
    client.invoke(() -> PRClientServerRegionFunctionExecutionDUnitTest.regionExecutionHATwoServerDown(Boolean.FALSE, function, Boolean.FALSE));
  ...
  public void testOnRegionFailoverWithOneServerDownHA() throws InterruptedException {
    ...
    client.invoke(() -> PRClientServerRegionFunctionExecutionDUnitTest.regionExecutionHAOneServerDown(Boolean.FALSE, function, Boolean.FALSE));
  ...
  public void testServerBucketMovedException() throws InterruptedException {
    ...
    ArrayList commonAttributes = createCommonServerAttributes("TestPartitionedRegion", null, 1, 113, null);
    ...
    try {
      ...
      Integer port1 = (Integer) server1.invoke(() -> createServerWithLocator(locator, false, commonAttributes));

      Integer port2 = (Integer) server2.invoke(() -> createServerWithLocator(locator, false, commonAttributes));
      ...
      Integer port3 = (Integer) server3.invoke(() -> createServerWithLocator(locator, false, commonAttributes));
  ...
  public void testServerBucketMovedException_LocalServer() throws InterruptedException {
    ...
    ArrayList commonAttributes = createCommonServerAttributes("TestPartitionedRegion", null, 0, 113, null);
  ...
  public void createClientWithLocator(String host, int port0) {
    ...
    Pool p;
    try {
      p = PoolManager.createFactory().addLocator(host, port0).setPingInterval(250).setSubscriptionEnabled(true).setSubscriptionRedundancy(-1).setReadTimeout(2000).setSocketBufferSize(1000).setMinConnections(6).setMaxConnections(10).setRetryAttempts(3).create("Pool_" + regionName);
    } finally {
  ...
  public static void putIntoRegion() {
    ...
    LogWriterUtils.getLogWriter().info("Distributed Region " + regionName + " Have size :" + region.size());
  }

  public static Object executeFunction() {
    Execution execute = FunctionService.onRegion(region);
    ResultCollector rc = execute.withArgs(Boolean.TRUE).execute(new TestFunction(true, TestFunction.TEST_FUNCTION_LASTRESULT));
  ...
  public static void checkSize() {
    LogWriterUtils.getLogWriter().info("Partitioned Region " + regionName + " Have size :" + region.size());
  }
