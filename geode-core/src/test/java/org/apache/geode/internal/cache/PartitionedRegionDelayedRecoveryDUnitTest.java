...
public class PartitionedRegionDelayedRecoveryDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testNoRecovery() throws Exception {
    ...
    SerializableRunnable createPrRegions =
        new SerializableRunnable("createRegions") {
          public void run() {
    ...
    vm0.invoke(
        new SerializableRunnable("putData") {
          public void run() {
            Cache cache = getCache();
    ...
    SerializableRunnable checkNoBucket =
        new SerializableRunnable("Check for bucket") {
          public void run() {
            Cache cache = getCache();
            PartitionedRegion region1 = (PartitionedRegion) cache.getRegion("region1");
  ...
  @Category(
      FlakyTest
          .class) // GEODE-860: time sensitive, thread unsafe test hook, CountDownLatch, 1 minute timeout, waitForBucketRecovery loops eating InterruptedException
  @Test
  public void testDelay() {
    ...
    assertTrue(
        "Did not wait at least 5 seconds to create the bucket. Elapsed=" + elapsed,
        elapsed >= 5000);
  }

  @Category(
      FlakyTest
          .class) // GEODE-757: time sensitive, fails because event occurs 2 millis too soon, waitForBucketRecovery wait loop eats InterruptedException, thread unsafe test hook
  @Test
  public void testStartupDelay() {
    ...
    assertTrue(
        "Create region should not have waited to recover redundancy. Elapsed=" + elapsed,
        elapsed < 5000);
    ...
    vm2.invoke(
        new SerializableCallable("wait for primary move") {
  ...
  private long waitForBucketRecovery(VM vm2, final int numBuckets, final long begin) {
    ...
    Long elapsed =
        (Long)
            vm2.invoke(
                new SerializableCallable("putData") {
                  public Object call() {
                    Cache cache = getCache();
                    PartitionedRegion region1 = (PartitionedRegion) cache.getRegion("region1");
                        ...
                        try {
                          Thread.sleep(100);
                            ...
                            InterruptedException
                                e) { // TODO: don't catch InterruptedException -- let test fail!
                          e.printStackTrace();
                        }
                      }
                    }
                    assertEquals(
                        "Did not start managing the bucket within 30 seconds",
                        numBuckets,
