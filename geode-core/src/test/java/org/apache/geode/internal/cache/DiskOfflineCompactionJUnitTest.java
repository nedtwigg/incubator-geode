...
public class DiskOfflineCompactionJUnitTest {
  ...
  public void testTwoEntriesNoCompact() throws Exception {
    ...
    assertEquals(
        headerSize + Oplog.OPLOG_GEMFIRE_VERSION_REC_SIZE + getRVVSize(0, null, true),
        drfFile.length());
  ...
  public void testTwoEntriesWithUpdateAndDestroy() throws Exception {
    ...
    assertEquals(
        crfsize + createsize1 + createsize2 + updatesize1 + updatesize2 + tombstonesize1,
        crfFile.length());
    assertEquals(
        headerSize + Oplog.OPLOG_GEMFIRE_VERSION_REC_SIZE + getRVVSize(0, null, true),
        drfFile.length());
  ...
  public void testbug41862() throws Exception {
    ...
    crfsize +=
        (getRVVSize(1, new int[] {1}, false) - getRVVSize(0, null, false)); // adjust rvv size
  ...
  public void testTwoEntriesWithRegionClear() throws Exception {
    ...
    int crfsize =
        headerSize
            ...
            + getRVVSize(0, null, false)
            + Oplog.OPLOG_NEW_ENTRY_BASE_REC_SIZE;
    int clearsize_in_crf =
        getRVVSize(1, new int[] {1}, false); // write extra RVV and RVVGC for clear operation
    int clearsize_in_drf =
        getRVVSize(1, new int[] {1}, true); // write extra RVV and RVVGC for clear operation
    int createsize1 = getSize4Create(extra_byte_num_per_entry, "key1", "value1");
    int createsize2 = getSize4Create(extra_byte_num_per_entry, "key2", "value2");
    int updatesize1 = getSize4UpdateWithoutKey(extra_byte_num_per_entry, "update1");
    int updatesize2 = getSize4UpdateWithoutKey(extra_byte_num_per_entry, "update2");
  ...
  static int getSize4Create(int extra_byte_num_per_entry, String key, Object value) {
    int createsize =
        1 /* opcode */
            + 1 /* userbits */
            + versionsize
            + extra_byte_num_per_entry
            + getStrSizeInOplog(key)
            + 1 /* drid */
            + getValueSizeInOplog(value)
            + 1 /* END_OF_RECORD_ID */;
    return createsize;
  }
