...
public class GemfireDataCommandsDUnitTest extends CliCommandTestBase {
    ...
    SerializableRunnable checkRegionMBeans =
        new SerializableRunnable() {
          ...
          public void run() {
            ...
            WaitCriterion waitForRegionMBeans =
                new WaitCriterion() {
                  @Override
                  public boolean done() {
                    ...
                    if (!flag) {
                      getLogWriter()
                          .info(
                              "Still probing for regionMbeans "
                      ...
                      getLogWriter()
                          .info(
                              "Probing complete for regionMbeans "
                                  + DATA_REGION_NAME_PATH
                                  + "="
                                  + beans[0]
                                  + " "
            ...
            String regions[] = {
              ...
              DATA_PAR_REGION_NAME_PATH,
              DATA_PAR_REGION_NAME_VM1_PATH,
              DATA_PAR_REGION_NAME_VM2_PATH, /*DATA_REGION_NAME_CHILD_1_PATH, DATA_REGION_NAME_CHILD_1_2_PATH*/
            };
            ...
            for (String region : regions) {
              ...
              getLogWriter()
                  .info(
                      "Members Array for region "
                          + region
                          + " : "
                          + StringUtils.objectToString(membersName, true, 10));
              if (bean.getMemberCount() < 1)
                fail(
                    "Even after waiting mbean reports number of member hosting region "
                        + DATA_REGION_NAME_VM1_PATH
                        + " is less than one");
  ...
  private void doQueryRegionsAssociatedMembers(
      String queryTemplate, int expectedMembers, boolean returnAll, String... regions) {
    Cache cache = CacheFactory.getAnyInstance();
  ...
  public void doTestGetRegionAssociatedMembersForSelect() {
    ...
    manager.invoke(
        new SerializableRunnable() {
          @Override
          public void run() {
            doQueryRegionsAssociatedMembers(
                queryTemplate1,
                0,
                true,
  ...
  public void doTestSelectProjection() {
    ...
    String query =
        "query --query=\"select ID , status , createTime , pk, floatMinValue from "
            + DATA_PAR_REGION_NAME_PATH
            + " where ID <= "
            + randomInteger
            + "\" --interactive=false";
  ...
  public void doTestSelectProjectionWithNestedField() {
    ...
    int randomInteger = random.nextInt(COUNT);
    String query =
        "query --query=\"select employeeId, name, department, value2 from "
            + DATA_REGION_NAME_VM1_PATH
            + " where employeeId <= "
            + randomInteger
            + "\" --interactive=false";
  ...
  private void testGetPutLocateEntryFromShellAndGemfire(
      final Object key, final Object value, Class klass, boolean addRegionPath, boolean expResult) {

    final VM vm1 = Host.getHost(0).getVM(1);

    SerializableRunnable putTask =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
    ...
    SerializableRunnable removeTask =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
    ...
    SerializableRunnable clearTask =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
  ...
  public void testSimplePutCommand() {
    ...
    SerializableRunnable checkPutKeys =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
  ...
  private void validateSelectResult(
      CommandResult cmdResult, boolean expectedFlag, int expectedRows, String[] cols) {
    if (ResultData.TYPE_COMPOSITE.equals(cmdResult.getType())) {
      CompositeResultData rd = (CompositeResultData) cmdResult.getResultData();
      SectionResultData section = rd.retrieveSectionByIndex(0);
      boolean result = (Boolean) section.retrieveObject("Result");
  ...
  public void testSimplePutIfAbsentCommand() {
    ...
    SerializableRunnable checkPutIfAbsentKeys =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
  ...
  public void testSimpleRemoveCommand() {
    ...
    SerializableRunnable checkRemoveKeys =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
  ...
  public void testGetLocateEntryFromRegionOnDifferentVM() {
    ...
    for (int i = 0; i < COUNT; i++) {
      ...
      assertEquals(Result.Status.OK, cmdResult.getStatus());
      validateResult(cmdResult, true);
      validateLocationsResult(
          cmdResult, 1); //1 Regions X (2-1) 2 Copies but redundancy not satisfied = 1
    }
  }
  ...
  public void testPutFromRegionOnDifferentVM() {
    ...
    SerializableRunnable checkPutKeysInVM1 =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
    ...
    SerializableRunnable checkPutKeysInVM2 =
        new SerializableRunnable() {
          @Override
          public void run() {
            Cache cache = getCache();
  ...
  @Category(
      FlakyTest
          .class) // GEODE-1182: random ports, BindException, HeadlessGFSH, waitForCriterion, time sensitive
  @Test
  ...
  public void doBugCheck50449() {
    String command =
        "put --key-class="
            + ObjectWithCharAttr.class.getCanonicalName()
    ...
    command =
        "put --key-class="
            + ObjectWithCharAttr.class.getCanonicalName()
            + " --value=123 --key=\"('name':'hesdfdsfy2','t':123, 'c':'d')\""
            + " --region="
            + DATA_REGION_NAME_PATH;
    cmdResult = executeCommand(command);
    printCommandOutput(cmdResult);
                ...
                new WaitCriterion() {
                  ...
                  public boolean done() {
                    ...
                    if (bean1 == null) {
                      ...
                      getLogWriter()
                          .info(
                              "waitForListClientMbean Still probing for DistributedRegionMXBean="
                                  + bean2);
                      if (bean2 == null) {
  ...
  public void testRegionsViaMbeanAndFunctions() {
    ...
    String memSizeFromFunctionCall =
        (String)
            manager.invoke(
                new SerializableCallable() {
                  public Object call() {
  ...
  public void testRegionsViaMbeanAndFunctionsForPartRgn() {
            ...
            manager.invoke(
                new SerializableCallable() {
                  public Object call() {
                    Cache cache = GemFireCacheImpl.getInstance();
                    return ""
                        + CliUtil.getMembersForeRegionViaFunction(cache, REBALANCE_REGION_NAME)
                            .size();
                  }
                });
    ...
    getLogWriter()
        .info(
            "testRegionsViaMbeanAndFunctionsForPartRgn memSizeFromFunctionCall= "
