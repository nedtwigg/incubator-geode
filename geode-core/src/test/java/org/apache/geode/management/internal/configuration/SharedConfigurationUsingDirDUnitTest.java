...
public class SharedConfigurationUsingDirDUnitTest extends JUnit4CacheTestCase {
  ...
  public void basicClusterConfigDirWithOneLocator() throws Exception {
    ...
    for (int i = 2; i < 4; i++) {
      ...
      vm.invoke(
          "Checking for region presence",
          () -> {
            waitAtMost(15, TimeUnit.SECONDS)
                .until(() -> getRootRegion("newReplicatedRegion") != null);
          });
    }
  }
  ...
  private void copyClusterXml(final VM vm, final String clusterXml) {
    vm.invoke(
        "Copying new cluster.xml from " + clusterXml,
        () -> {
          ...
          Files.copy(
              cacheXml,
              Paths.get("cluster_config", "cluster", "cluster.xml"),
  ...
  private void waitForSharedConfiguration(final VM vm) {
    vm.invoke(
        "Waiting for shared configuration",
        () -> {
