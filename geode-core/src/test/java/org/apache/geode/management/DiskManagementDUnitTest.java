...
public class DiskManagementDUnitTest extends ManagementTestBase {
  ...
  protected void checkNavigation(
      final VM vm, final DistributedMember diskMember, final String diskStoreName) {
  ...
  public void invokeFlush(final VM vm) {
    SerializableRunnable invokeFlush =
        new SerializableRunnable("Invoke Flush On Disk") {
          public void run() {
            Cache cache = getCache();
  ...
  public void invokeForceRoll(final VM vm) {
    SerializableRunnable invokeForceRoll =
        new SerializableRunnable("Invoke Force Roll") {
          public void run() {
            Cache cache = getCache();
            DiskStoreFactory dsf = cache.createDiskStoreFactory();
  ...
  public void makeDiskCompactable(VM vm1) throws Exception {
    vm1.invoke(
        new SerializableRunnable("Make The Disk Compactable") {
  ...
  public void compactAllDiskStores(VM vm1) throws Exception {
    ...
    vm1.invoke(
        new SerializableCallable("Compact All Disk Stores") {
          ...
          public Object call() throws Exception {
            ...
            for (int i = 0; i < compactedDiskStores.length; i++) {
              org.apache.geode.test.dunit.LogWriterUtils.getLogWriter()
                  .info(
                      "<ExpectedString> Compacted Store "
                          + i
                          + " "
  ...
  public void backupAllMembers(final VM managingVM) throws Exception {
    ...
    managingVM.invoke(
        new SerializableCallable("Backup All Disk Stores") {

          public Object call() throws Exception {
            ManagementService service = getManagementService();
            DistributedSystemMXBean bean = service.getDistributedSystemMXBean();
            DiskBackupStatus status =
                bean.backupAllMembers(
                    getBackupDir("test_backupAllMembers").getAbsolutePath(), null);
  ...
  public void compactDiskStoresRemote(VM managingVM) throws Exception {
    {
      managingVM.invoke(
          new SerializableCallable("Compact All Disk Stores Remote") {
            ...
            public Object call() throws Exception {
              ...
              for (DistributedMember member : otherMemberSet) {
                ...
                for (int i = 0; i < compactedDiskStores.length; i++) {
                  org.apache.geode.test.dunit.LogWriterUtils.getLogWriter()
                      .info(
                          "<ExpectedString> Remote Compacted Store "
                              + i
                              + " "
                              + compactedDiskStores[i]
                              + "</ExpectedString> ");
  ...
  protected void validatePersistentRegion(final VM vm) {
    SerializableRunnable validateDisk =
        new SerializableRunnable("Validate persistent region") {
          public void run() {
            Cache cache = getCache();
            ManagementService service = getManagementService();
