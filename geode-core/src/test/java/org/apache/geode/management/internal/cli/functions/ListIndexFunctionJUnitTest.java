...
public class ListIndexFunctionJUnitTest {
  ...
  private void assertIndexDetailsEquals(
      final IndexDetails expectedIndexDetails, final IndexDetails actualIndexDetails) {
    assertEquals(expectedIndexDetails.getFromClause(), actualIndexDetails.getFromClause());
    assertEquals(
        expectedIndexDetails.getIndexedExpression(), actualIndexDetails.getIndexedExpression());
    assertEquals(expectedIndexDetails.getIndexName(), actualIndexDetails.getIndexName());
    assertIndexStatisticsDetailsEquals(
    ...
    assertEquals(
        expectedIndexDetails.getProjectionAttributes(),
  ...
  private void assertIndexStatisticsDetailsEquals(
    ...
    if (expectedIndexStatisticsDetails != null) {
      ...
      assertEquals(
          expectedIndexStatisticsDetails.getNumberOfKeys(),
          actualIndexStatisticsDetails.getNumberOfKeys());
      assertEquals(
          expectedIndexStatisticsDetails.getNumberOfUpdates(),
          actualIndexStatisticsDetails.getNumberOfUpdates());
      assertEquals(
          expectedIndexStatisticsDetails.getNumberOfValues(),
          actualIndexStatisticsDetails.getNumberOfValues());
      assertEquals(
          expectedIndexStatisticsDetails.getTotalUpdateTime(),
          actualIndexStatisticsDetails.getTotalUpdateTime());
      assertEquals(
          expectedIndexStatisticsDetails.getTotalUses(),
  ...
  private IndexDetails createIndexDetails(
  ...
  private IndexStatisticsDetails createIndexStatisticsDetails(
  ...
  private Index createMockIndex(final IndexDetails indexDetails) {
    ...
    if (indexDetails.getIndexStatisticsDetails() != null) {
      final IndexStatistics mockIndexStatistics =
          mockContext.mock(
        ...
        new Expectations() {
          {
            ...
            will(
                returnValue(
                    Arrays.asList(
                        createMockIndex(indexDetailsOne),
                        createMockIndex(indexDetailsTwo),
                        createMockIndex(indexDetailsThree),
                        createMockIndex(indexDetailsFour))));
    ...
    for (final IndexDetails expectedIndexDetails : expectedIndexDetailsSet) {
      final IndexDetails actualIndexDetails =
          CollectionUtils.findBy(
              actualIndexDetailsSet,
              new Filter<IndexDetails>() {
                @Override
