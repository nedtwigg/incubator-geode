  ...
  public void testRolesInDistributedVMs() {
    ...
    for (int i = 0; i < vmRoles.length; i++) {
      ...
      Host.getHost(0)
          .getVM(vm)
          .invoke(
              new SerializableRunnable("create system") {
                public void run() {
                  disconnectFromDS();
    ...
    for (int i = 0; i < vmRoles.length; i++) {
      ...
      Host.getHost(0)
          .getVM(vm)
          .invoke(
              new SerializableRunnable("verify roles") {
                public void run() {
                  ...
                  for (Iterator iter = allRoles.iterator(); iter.hasNext(); ) {
                    ...
                    for (int j = 0; j < roleCounts.length; j++) {
                      if (role.getName().equals(roleCounts[j][0])) {
                        ...
                        // assert count
                        assertEquals(
                            "count for role " + role + " is wrong", count, dm.getRoleCount(role));
                        assertEquals(
                            "isRolePresent for role " + role + " should be true",
                            true,
