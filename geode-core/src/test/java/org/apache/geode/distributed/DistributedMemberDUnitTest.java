...
public class DistributedMemberDUnitTest extends JUnit4DistributedTestCase {
                ...
                public void run() {
                  ...
                  for (Iterator iterMembers = members.iterator(); iterMembers.hasNext(); ) {
                    InternalDistributedMember member =
                        (InternalDistributedMember) iterMembers.next();
                    Set roles = member.getRoles();
   ...
   *
   * <p>Changing the id can result in bad keys in JMX and can result in numerous errors in Admin/JMX
   * tests.
   */
  @Test
  ...
  private DistributedMember createSystemAndGetId(VM vm, final String name) {
    return (DistributedMember)
        vm.invoke(
            new SerializableCallable("create system and get member") {
