...
public class TcpServerBackwardCompatDUnitTest extends JUnit4DistributedTestCase {
  ...
  public final void postSetUp() throws Exception {
    ...
    Invoke.invokeInEveryVM(
        new CacheSerializableRunnable("Set TcpServer.isTesting true") {
  ...
  public void testGossipVersionBackwardCompatibility() {
    ...
    locator0.invoke(
        new CacheSerializableRunnable("Starting first locator on port " + port0) {

          @Override
          public void run2() throws CacheException {
            try {
              TcpServer.getGossipVersionMapForTestOnly()
                  .put(TcpServer.TESTVERSION - 100, Version.CURRENT_ORDINAL);
    ...
    member.invoke(
        new CacheSerializableRunnable("Start a member") {
    ...
    locator1.invoke(
        new CacheSerializableRunnable("Starting second locator on port " + port1) {
          ...
          public void run2() throws CacheException {
            try {
              ...
              TcpServer.getGossipVersionMapForTestOnly()
                  .put(TcpServer.TESTVERSION, Version.CURRENT_ORDINAL);
              TcpServer.getGossipVersionMapForTestOnly()
                  .put(TcpServer.OLDTESTVERSION, Version.GFE_57.ordinal());
    ...
    locator0.invoke(
        new CacheSerializableRunnable("Stopping first locator") {
    ...
    //props.setProperty(DistributionConfig.START_LOCATOR_NAME, host.getHostName() + "["+port0+"]");
    locatorRestart0.invoke(
        new CacheSerializableRunnable("Restarting first locator on port " + port0) {
