...
public class DistributedLockServiceDUnitTest extends JUnit4DistributedTestCase {
        ...
        Integer count =
            (Integer)
                Host.getHost(0)
                    ...
                    .invoke(
                        () ->
                            DistributedLockServiceDUnitTest.get_count_testFairness(
                                new Integer(thread)));
    ...
    for (int attempt = 0; attempt < attempts; attempt++) {
      try {
        for (int vm = 0; vm < numVMs; vm++) {
          ...
          assertEquals(
              "New grantor must not equal the old grantor",
              true,
  ...
  public void testLockFailover() {
    ...
    assertEquals(
        "First member calling getLockGrantor failed to become grantor", Boolean.TRUE, isGrantor);
    ...
    Boolean locked =
        (Boolean)
            Host.getHost(0)
                ...
                .invoke(
                    () ->
                        DistributedLockServiceDUnitTest.lock(
                            serviceName, "KEY-" + originalGrantorVM));
    ...
    InternalDistributedMember oneID =
        (InternalDistributedMember)
            Host.getHost(0)
                .getVM(oneVM)
                .invoke(() -> DistributedLockServiceDUnitTest.identifyLockGrantor(serviceName));
    InternalDistributedMember twoID =
        (InternalDistributedMember)
            Host.getHost(0)
                .getVM(twoVM)
                .invoke(() -> DistributedLockServiceDUnitTest.identifyLockGrantor(serviceName));
    ...
    Boolean check =
        (Boolean)
            Host.getHost(0)
                ...
                .invoke(
                    () ->
                        DistributedLockServiceDUnitTest.unlock(
                            serviceName, "KEY-" + becomeGrantorVM));
    assertEquals(
        "Check of control failed... unlock succeeded but nothing locked", Boolean.FALSE, check);
    ...
    locked =
        (Boolean)
            Host.getHost(0)
                .getVM(thirdPartyVM)
                .invoke(
                    () -> DistributedLockServiceDUnitTest.lock(serviceName, "KEY-" + thirdPartyVM));
    assertEquals("Failed to get lock in testLockThenBecomeLockGrantor", Boolean.TRUE, locked);
    ...
    unlocked =
        (Boolean)
            Host.getHost(0)
                ...
                .invoke(
                    () ->
                        DistributedLockServiceDUnitTest.unlock(serviceName, "KEY-" + thirdPartyVM));
    assertEquals("Failed to release lock in testLockThenBecomeLockGrantor", Boolean.TRUE, unlocked);
  ...
  public void testBecomeLockGrantor() {
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      if (vm != originalVM) {
        ...
        Host.getHost(0)
            .getVM(finalvm)
            .invoke(DistributedLockServiceDUnitTest.class, "becomeLockGrantor", args);
        Boolean isGrantor =
            (Boolean)
                Host.getHost(0)
                    .getVM(finalvm)
                    .invoke(DistributedLockServiceDUnitTest.class, "isLockGrantor", args);
        assertEquals(
            "isLockGrantor is false after calling becomeLockGrantor", Boolean.TRUE, isGrantor);
        break;
      }
    }
      ...
      Boolean unlocked =
          (Boolean)
              Host.getHost(0)
                  .getVM(finalvm)
                  .invoke(
                      () -> DistributedLockServiceDUnitTest.unlock(serviceName, "obj-" + finalvm));
  ...
  public void testLeaseDoesntExpire() throws InterruptedException {
    ...
    assertEquals(
        Boolean.FALSE,
        vm.invoke(
            () -> this.getLockAndIncrement(serviceName, objName, new Long(1000), new Long(0))));
    // the unlock should succeed without throwing LeaseExpiredException
    service.unlock(objName);
  }
            ...
            new Runnable() {
              public void run() {
                try {
                  ...
                  LogWriterUtils.getLogWriter()
                      .fine(
                          "[testLockIsNotInterruptible] thread2 finished lock() - got " + gotLock);
                } catch (VirtualMachineError e) {
                  SystemFailure.initiateFailure(e);
                  throw e;
                } catch (Throwable ex) {
                  LogWriterUtils.getLogWriter()
                      .warning("[testLockIsNotInterruptible] Caught...", ex);
                  exception = ex;
                }
  ...
  public void testSuspendLockingProhibitsLocking() {
    ...
    vm1.invoke(
        new SerializableRunnable("setDebugHandleSuspendTimeouts") {
          public void run() {
    ...
    vm1.invoke(
        new SerializableRunnable("unsetDebugHandleSuspendTimeouts") {
          public void run() {
            DLockService dls = (DLockService) DistributedLockService.getServiceNamed(name);
            assertTrue(dls.isLockGrantor());
            DLockGrantor grantor = dls.getGrantorWithNoSync();
          ...
          new SerializableRunnable() {
            public void run() {
              try {
                ...
                LogWriterUtils.getLogWriter()
                    .error("Error in testSuspendLockingBehaves finally", t);
              }
              try {
  ...
  private void doTestSuspendLockingBehaves() throws Exception {
    ...
    vmGrantor.invoke(
        new SerializableRunnable("Create grantor " + dlsName) {
          public void run() {
            DistributedLockService.create(dlsName, getSystem());
  ...
  public void testSuspendLockingBlocksUntilNoLocks() throws InterruptedException {
    ...
    VM vm1 = Host.getHost(0).getVM(1);
    vm1.invokeAsync(
        new SerializableRunnable("Lock & unlock in vm1") {
          public void run() {
    ...
    vm1.invoke(
        new SerializableRunnable("notify vm1 to unlock") {
          public void run() {
            synchronized (monitor) {
              monitor.notify();
            }
   ...
   *
   * <p>VM-A locks/unlocks "lock", VM-B leases "lock" and disconnects, VM-C attempts to lock "lock"
   * and old dlock throws StuckLockException. VM-C should now succeed in acquiring the lock.
   */
  @Test
      ...
      final Thread thread1 =
          new Thread(
              new Runnable() {
                public void run() {
                  LogWriterUtils.getLogWriter()
                      .info("[testReleaseOrphanedGrant_Local] get the lock");
                  ...
                  LogWriterUtils.getLogWriter()
                      .info("[testReleaseOrphanedGrant_Local] unlock the lock");
                  service.unlock("obj");
                }
              });
      thread1.start();
      ...
      vm1.invokeAsync(
          new SerializableRunnable() {
            public void run() {
              ...
              LogWriterUtils.getLogWriter()
                  .info("[testReleaseOrphanedGrant_Remote] unlock the lock");
  ...
  public void testDestroyLockServiceAfterGrantResponse() throws Throwable {
    ...
    vm0.invoke(
        new SerializableRunnable("Create the grantor") {
    ...
    vm0.invoke(
        new SerializableRunnable("check to make sure the lock is not orphaned") {
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      ...
      for (int thread = 0; thread < numThreadsPerVM; thread++) {
        ...
        (new Thread(
                ...
                "doOneGetsAndOthersTimeOut-" + thread))
            .start();
      }
    }
    ...
    vmGrantor.invoke(
        new SerializableRunnable() {
          public void run() {
            ...
            assertEquals(
                "Failed with bug 38179: grantTokens=" + grantTokens, 0, grantTokens.size());
          ...
          vm1.invokeAsync(
              new SerializableRunnable() {
                public void run() {
                  ...
                  connectDistributedSystem();
                  DLockService dls =
                      (DLockService) DistributedLockService.create(dlsName, getSystem());
              ...
              vm1.invoke(
                  new SerializableCallable() {
                    public Object call() throws Exception {
                      LogWriterUtils.getLogWriter()
                          .info("[testLockQuery] vm1 waits for locking thread");
                      synchronized (testLockQuery_whileVM1Locks) {
                        long maxWait = 10000;
                        StopWatch timer = new StopWatch(true);
   ...
   *     name of the Region be the {@link #getUniqueName()} of the test, or at least derive from it.
   */
  ...
  private static Object getLockAndIncrement(
    ...
    if (got) {
      ...
      long count = blackboard.getCount();
      logInfo(
          "[getLockAndIncrement] In getLockAndIncrement - count is "
              + count
              + " for "
              + serviceName
              + "/"
      ...
      blackboard.setIsLocked(false);
      logInfo(
          "[getLockAndIncrement] In getLockAndIncrement: "
              + "cleared blackboard lock for "
              + serviceName
              + "/"
              + objectName);
      service.unlock(objectName);
