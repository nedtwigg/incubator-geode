...
public class DistributedLockServiceDUnitTest extends JUnit4DistributedTestCase {
  ...
  public void testFairness() throws Exception {
    ...
    for (int i = 0; i < vmThreads.length; i++) {
      final int vm = i;
      LogWriterUtils.getLogWriter().info("[testFairness] lining up " + vmThreads[vm] + " threads in vm " + vm);
    ...
    LogWriterUtils.getLogWriter().info("[testFairness] totalLocks=" + totalLocks + " minLocks=" + minLocks + " maxLocks=" + maxLocks);
    ...
    LogWriterUtils.getLogWriter().info("[testFairness] deviation=" + deviation + " expectedLocks=" + expectedLocks + " lowThreshold=" + lowThreshold + " highThreshold=" + highThreshold);
    ...
    for (int attempt = 0; attempt < attempts; attempt++) {
      try {
        for (int vm = 0; vm < numVMs; vm++) {
          ...
          assertEquals("New grantor must not equal the old grantor", true, !id.equals(oldGrantor)); // new grantor != old grantor
        } // loop thru vms
  ...
  public void testLockFailover() {
    ...
    assertEquals("First member calling getLockGrantor failed to become grantor", Boolean.TRUE, isGrantor);
  ...
  public void testLockThenBecomeLockGrantor() {
    ...
    assertEquals("Check of control failed... unlock succeeded but nothing locked", Boolean.FALSE, check);
  ...
  public void testBecomeLockGrantor() {
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      final int finalvm = vm;
      Boolean isGrantor = (Boolean) Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "isLockGrantor", args);
      if (isGrantor.booleanValue()) {
        originalVM = vm;
        oldGrantor = (InternalDistributedMember) Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "identifyLockGrantor", args);
        break;
      }
    }
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      if (vm != originalVM) {
        final int finalvm = vm;
        Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "becomeLockGrantor", args);
        Boolean isGrantor = (Boolean) Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "isLockGrantor", args);
        assertEquals("isLockGrantor is false after calling becomeLockGrantor", Boolean.TRUE, isGrantor);
        break;
      }
    }
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      ...
      Boolean isGrantor = (Boolean) Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "isLockGrantor", args);
      if (isGrantor.booleanValue()) {
        newGrantor = (InternalDistributedMember) Host.getHost(0).getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "identifyLockGrantor", args);
        break;
      }
    }
  ...
  public void testLockDifferentNames() throws Exception {
    ...
    assertEquals(Boolean.TRUE, vm.invoke(() -> this.getLockAndIncrement(serviceName, "otherVMobj", new Integer(-1), new Integer(0))));
  ...
  public void testRemoteGetLockAndIncrement() {
    ...
    VM vm = Host.getHost(0).getVM(0);
    vm.invoke(() -> this.remoteCreateService(serviceName));
    assertEquals(Boolean.TRUE, vm.invoke(() -> this.getLockAndIncrement(serviceName, "remoteVMobj", new Integer(-1), new Integer(0))));
  }
  ...
  public void testLockSameNameDifferentService() {
    ...
    assertTrue(service1.lock(objName, -1, -1));
    assertEquals(Boolean.TRUE, vm.invoke(() -> this.getLockAndIncrement(serviceName2, objName, new Integer(-1), new Integer(0))));
    service1.unlock(objName);
  }
  ...
  public void testLeaseDoesntExpire() throws InterruptedException {
    ...
    assertEquals(Boolean.FALSE, vm.invoke(() -> this.getLockAndIncrement(serviceName, objName, new Long(1000), new Long(0))));
    // the unlock should succeed without throwing LeaseExpiredException
    service.unlock(objName);
  }
  ...
  private void leaseExpiresTest(boolean tryToLockBeforeExpiration) throws InterruptedException {
    ...
    assertEquals(Boolean.TRUE, vm.invoke(() -> this.getLockAndIncrement(serviceName, objName, new Long(-1), new Long(0))));
  ...
  public void testSuspendLockingAfterExpiration() throws Exception {
    ...
    Host.getHost(0).getVM(0).invoke(new SerializableRunnable() {
      public void run() {
        final DistributedLockService dlock = DistributedLockService.create(serviceName, getSystem());
  ...
  public void testLockInterruptiblyIsInterruptible() {
    ...
    assertFalse(gotLock);
    if (throwable != null) {
      LogWriterUtils.getLogWriter().warning("testLockInterruptiblyIsInterruptible threw unexpected Throwable", throwable);
    }
  ...
  public void testSuspendLockingBasic() throws InterruptedException {
    final DistributedLockService service = DistributedLockService.create(getUniqueName(), dlstSystem);
  ...
  public void testSuspendLockingIsNotInterruptible() {
    ...
    // Now thread2 should have gotten the lock, not the exception, but the
    // thread's flag should be set
    LogWriterUtils.getLogWriter().info("[testSuspendLockingIsNotInterruptible]" + " gotLock=" + gotLock + " wasFlagSet=" + wasFlagSet + " exception=" + exception, exception);
    assertTrue(gotLock);
    assertNull(exception);
    assertTrue(wasFlagSet);
  }
    ...
    try {
      ...
      vm1.invokeAsync(new SerializableRunnable() {
        public void run() {
          ...
          connectDistributedSystem();
          DistributedLockService service_vm1 = DistributedLockService.create(serviceName, getSystem());
      ...
      vm2.invokeAsync(new SerializableRunnable() {
        public void run() {
          ...
          DistributedLockService service_vm2 = DistributedLockService.create(serviceName, getSystem());
  ...
  private void doOneGetsAndOthersTimeOut(int numVMs, int numThreadsPerVM) throws Exception {
    ...
    final Object[] args = new Object[] { serviceName, objectName, new Long(timeout), new Long(holdTime) };
    final Host host = Host.getHost(0);
    for (int vm = 0; vm < numVMs; vm++) {
      final int finalvm = vm;
      for (int thread = 0; thread < numThreadsPerVM; thread++) {
        ...
        (new Thread(new Runnable() {
          public void run() {
            ...
            Boolean result = (Boolean) host.getVM(finalvm).invoke(DistributedLockServiceDUnitTest.class, "getLockAndIncrement", args);
    ...
    while ((completes < numVMs * numThreadsPerVM) && (System.currentTimeMillis() - start < holdTime * 10)) {
      sleep(200);
    }
  ...
  private static Object getLockAndIncrement(String serviceName, Object objectName, long timeout, long holdTime) throws Exception {
