...
public class LocatorDUnitTest extends JUnit4DistributedTestCase {
  ...
  public void testCollocatedLocatorWithSecurity() throws Exception {
    ...
    properties.put(
        SECURITY_PEER_AUTHENTICATOR, "org.apache.geode.distributed.MyAuthenticator.create");
    ...
    assertEquals(
        "expected the VM to have NORMAL vmKind",
    ...
    SerializableRunnable startSystem =
        new SerializableRunnable("start system") {
          public void run() {
            system = (InternalDistributedSystem) DistributedSystem.connect(properties);
          }
        };
    ...
    vm1.invoke(
        new SerializableRunnable("get the lock service and lock something") {
          public void run() {
    ...
    try {
      vm1.invoke(
          new SerializableRunnable("ensure grantor failover") {
            public void run() {
              final DistributedLockService service =
              ...
              Wait.waitForCriterion(
                  ...
                  DistributionConfig.DEFAULT_MEMBER_TIMEOUT * 2,
                  1000,
                  true);
      ...
      system = (InternalDistributedSystem) DistributedSystem.connect(properties);
      System.out.println(
          "done connecting distributed system.  Membership view is "
      ...
      assertNotSame(
          "should not be the first member in the view (" + view + ")",
          system.getDistributedMember(),
          view.get(0));
      ...
      vm3.invoke(
          new SerializableRunnable("get the lock service and lock something(2)") {
            public void run() {
              final DistributedLockService service =
                  DistributedLockService.create("test service", system);
  ...
  private Boolean startLocatorWithPortAndProperties(final int port, final Properties properties)
    ...
    try {
      System.setProperty(
          "p2p.joinTimeout", "5000"); // set a short join timeout.  default is 17000ms
  ...
  private void assertLeadMember(
      final DistributedMember member, final DistributedSystem sys, long timeout) {
    WaitCriterion ev =
        new WaitCriterion() {
          public boolean done() {
   ...
   *
   * <p>So, we start two locators with admin distributed systems, then start two regular distributed
   * members.
   *
   * <p>We kill the second locator (which is not the view coordinator) and then kill the non-lead
   * member. That should be okay - the lead and remaining locator continue to run.
   *
   * <p>We then kill the lead member and demonstrate that the original locator (which is now the
   * sole remaining member) shuts itself down.
   */
  @Test
  public void testLeadAndCoordFailure() throws Exception {
    ...
    try {
      ...
      assertTrue(
          "Distributed system should not have disconnected",
      ...
      assertTrue(
          "expected suspect processing initiated by TCPConduit",
      ...
      bLogger.info(
          "<ExpectedException action=remove>java.net.ConnectException</ExpectedException>");
      bLogger.info(
   ...
   * enabled.
   *
   * <p>Start two locators with admin distributed systems, then start two regular distributed
   * members.
   *
   * <p>We kill the lead member and demonstrate that the other members continue to operate normally.
   *
   * <p>We then shut down the group coordinator and observe the second locator pick up the job and
   * the remaining member continues to operate normally.
   */
  @Test
  public void testLeadFailureAndCoordShutdown() throws Exception {
    ...
    try {
      ...
      assertEquals(
          ...
          mem2,
          vm2.invoke(() -> LocatorDUnitTest.getLeadMember()));
   ...
   * enabled.
   *
   * <p>Start one locators with admin distributed systems, then start two regular distributed
   * members.
   *
   * <p>We kill the lead member and demonstrate that the other members continue to operate normally.
   *
  ...
  public void testForceDisconnectAndPeerShutdownCause() throws Exception {
    ...
    try {
      ...
      SerializableRunnable crashSystem =
          new SerializableRunnable("Crash system") {
            public void run() {
              ...
              msys.getLogWriter()
                  .info(
                      "<ExpectedException action=add>Possible loss of quorum</ExpectedException>");
              hook = new TestHook();
      ...
      vm2.invoke(
          new SerializableRunnable("Request Member Removal") {
            ...
            public void run() {
              ...
              try {
                ...
                assertTrue(
                    "This should have been ForceDisconnectException but found " + cause,
   ...
   * members.
   *
   * <p>We kill the coordinator and shut down the lead member and observe the second locator pick up
   ...
   * correct.
   *
   * <p>The locator is then restarted and is shown to take over the role of membership coordinator.
   */
  @Test
    ...
    try {
      ...
      WaitCriterion ev =
          new WaitCriterion() {
            ...
            public String description() {
              return "expected the coordinator to not be "
                  + coord
                  + " but it is "
                  + MembershipManagerHelper.getCoordinator(system);
            }
    ...
    try {
      ...
      WaitCriterion ev =
          new WaitCriterion() {
            ...
            public String description() {
              return "expected the coordinator to be "
                  + coord
                  + " but it is "
                  + MembershipManagerHelper.getCoordinator(system);
            }
      ...
      if (newCoord == null || coord.equals(newCoord)) {
        fail(
            "another member should have become coordinator after the locator was stopped: "
                + newCoord);
      }
  ...
  private void waitUntilLocatorBecomesCoordinator(VM vm) {
    SerializableRunnable sr =
        new SerializableRunnable("waitUntilLocatorBecomesCoordinator") {
  ...
  private void startLocatorSync(VM vm, Object[] args) {
    vm.invoke(
        new SerializableRunnable("Starting locator process on " + args[0], args) {
          public void run() {
            File logFile = new File("");
            try {
              ...
              org.apache.geode.test.dunit.Assert.fail(
                  "While starting process on port " + args[0], ex);
            }
          }
        });
  }
  ...
  private void startLocatorAsync(VM vm, Object[] args) {
    vm.invokeAsync(
        new SerializableRunnable("Starting Locator process async on " + args[0], args) {
          public void run() {
            File logFile = new File("");
            try {
              Locator.startLocatorAndDS((int) args[0], logFile, (Properties) args[1]);
  ...
  public void testMultipleMcastLocators() throws Exception {
    disconnectAllFromDS();
    IgnoredException.addIgnoredException(
        "Could not stop  Distribution Locator"); // shutdown timing issue in InternalLocator
    Host host = Host.getHost(0);
    VM vm0 = host.getVM(0);
    VM vm1 = host.getVM(1);
    VM vm2 = host.getVM(2);
    ...
    vm0.invoke(
        new SerializableRunnable("Start locator on " + port1) {
          public void run() {
            File logFile = new File("");
            try {
              Properties props = new Properties();
              ...
              org.apache.geode.test.dunit.Assert.fail(
                  "While starting locator on port " + port1, ex);
            }
          }
        });
    vm3.invoke(
        new SerializableRunnable("Start locator on " + port2) {
          public void run() {
            File logFile = new File("");
            try {
              Properties props = new Properties();
              ...
              org.apache.geode.test.dunit.Assert.fail(
                  "While starting locator on port " + port2, ex);
            }
          }
        });
