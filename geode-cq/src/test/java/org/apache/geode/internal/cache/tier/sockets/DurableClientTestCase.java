...
public class DurableClientTestCase extends JUnit4DistributedTestCase {
  ...
  public void testSimpleDurableClient() {
    ...
    this.durableClientVM.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(durableClientVM.getHost()), serverPort, true),
                regionName,
    ...
    this.server1VM.invoke(
        new CacheSerializableRunnable("Verify durable client") {
          public void run2() throws CacheException {
            // Find the proxy
            ...
            assertEquals(
                DistributionConfig.DEFAULT_DURABLE_CLIENT_TIMEOUT, proxy.getDurableTimeout());
  ...
  public void testDurableClientPrimaryUpdate() {
    ...
    this.publisherClientVM.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(publisherClientVM.getHost()), serverPort, false),
                regionName));
    ...
    this.server1VM.invoke(
        new CacheSerializableRunnable("Wait for paused") {
          public void run2() throws CacheException {
            WaitCriterion wc =
                new WaitCriterion() {
                  public boolean done() {
  ...
  protected void publishEntries(final int numberOfEntries) {
    this.publisherClientVM.invoke(
        new CacheSerializableRunnable("Publish entries") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  public void testStartStopStartDurableClientUpdate() {
    ...
    // Publish some more entries
    this.publisherClientVM.invoke(
        new CacheSerializableRunnable("Publish more entries") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  public void verifyListenerUpdatesEntries(
      final int numEntries, final int numEntriesBeforeDisconnect) {
    this.durableClientVM.invoke(
        new CacheSerializableRunnable("Verify updates") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  public void testDurableClientConnectServerStopStart() {
    ...
    Integer[] ports =
        ((Integer[])
            this.server1VM.invoke(
                () ->
                    CacheServerTestUtil.createCacheServerReturnPorts(
    ...
    this.server1VM.invoke(
        () ->
            CacheServerTestUtil.createCacheServer(
                regionName, new Boolean(true), new Integer(serverPort)));
  ...
  protected void verifyDurableClientEvents(
      VM durableClientVm, final int numberOfEntries, final int eventType) {
    verifyDurableClientEvents(durableClientVm, numberOfEntries, eventType, numberOfEntries);
  }

  protected void verifyNoDurableClientEvents(
      VM durableClientVm, final int numberOfEntries, final int eventType) {
    verifyDurableClientEvents(durableClientVm, numberOfEntries, eventType, 0);
  }

  private void verifyDurableClientEvents(
      VM durableClientVm,
  ...
  public void durableFailover(int redundancyLevel) throws InterruptedException {
    ...
    if (redundancyLevel == 1) {
      clientPool =
          getClientPool(
              NetworkUtils.getServerHostName(Host.getHost(0)), server1Port, server2Port, true);
    } else {
      clientPool =
          getClientPool(
              NetworkUtils.getServerHostName(Host.getHost(0)), server1Port, server2Port, true, 0);
    }
    ...
    this.server2VM.invoke(
        () ->
            CacheServerTestUtil.createCacheServer(
                regionName, new Boolean(true), new Integer(server2Port)));
    ...
    this.publisherClientVM.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(publisherClientVM.getHost()),
                    server1Port,
                    server2Port,
                    false),
                regionName));
    ...
    this.publisherClientVM.invoke(
        new CacheSerializableRunnable("Publish entries before failover") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
    ...
    this.publisherClientVM.invoke(
        new CacheSerializableRunnable("Publish entries after failover") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  protected Pool getClientPool(
      String host, int server1Port, int server2Port, boolean establishCallbackConnection) {
  ...
  protected Properties getClientDistributedSystemProperties(String durableClientId) {
    return getClientDistributedSystemProperties(
        durableClientId, DistributionConfig.DEFAULT_DURABLE_CLIENT_TIMEOUT);
  }
  ...
  public void setPrimaryRecoveryCheck() {
    this.durableClientVM.invoke(
        new CacheSerializableRunnable("Set observer") {
          public void run2() {
  ...
  public void checkPrimaryRecovery() {
    this.durableClientVM.invoke(
        new CacheSerializableRunnable("Check observer") {
          public void run2() {
            WaitCriterion waitForPrimaryRecovery =
                new WaitCriterion() {
                  public boolean done() {
  ...
  protected void registerInterest(VM vm, final String regionName, final boolean durable) {
    vm.invoke(
        new CacheSerializableRunnable("Register interest on region : " + regionName) {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  protected void createCq(VM vm, final String cqName, final String cqQuery, final boolean durable) {
    vm.invoke(
        new CacheSerializableRunnable("Register cq " + cqName) {
          public void run2() throws CacheException {
  ...
  protected void publishEntries(VM vm, final String regionName, final int numEntries) {
    vm.invoke(
        new CacheSerializableRunnable("publish " + numEntries + " entries") {
          public void run2() throws CacheException {
            // Get the region
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
            assertNotNull(region);
  ...
  protected void flushEntries(VM server, VM client, final String regionName) {
    ...
    server.invoke(
        new CacheSerializableRunnable("flush entries") {
          public void run2() throws CacheException {
  ...
  protected void checkCqStatOnServer(
      VM server, final String durableClientId, final String cqName, final int expectedNumber) {
    server.invoke(
          ...
          public void run2() throws CacheException {
            ...
            WaitCriterion ev =
                new WaitCriterion() {
                  ...
                  public String description() {
                    return "cq numHAQueuedEvents stat was expected to be "
                        + expectedNumber
                        + " but was instead "
                        + cqQuery.getVsdStats().getNumHAQueuedEvents();
                  }
                };
   ...
   */
  protected void checkHAQueueSize(
      VM server, final String durableClientId, final int expectedNumber, final int remaining) {
    server.invoke(
        new CacheSerializableRunnable(
          ...
          public void run2() throws CacheException {
            ...
            WaitCriterion ev =
                new WaitCriterion() {
                  ...
                  public String description() {
                    return "queue size stat was expected to be "
                        + expectedNumber
                        + " but was instead "
            ...
            assertTrue(
                clientProxy.getQueueSizeStat() == expectedNumber
                    || clientProxy.getQueueSizeStat() == remaining);
          }
        });
  }

  protected void checkNumDurableCqs(
      VM server, final String durableClientId, final int expectedNumber) {
    server.invoke(
        new CacheSerializableRunnable(
  ...
  * @param secondsToWait
  */
  protected void checkCqListenerEvents(
      VM vm,
      final String cqName,
      final int numEvents,
      final int numEventsToWaitFor,
      final int secondsToWait) {
    vm.invoke(
        new CacheSerializableRunnable("Verify events for cq: " + cqName) {
          public void run2() throws CacheException {
            QueryService qs = CacheServerTestUtil.getCache().getQueryService();
  ...
  protected void checkInterestEvents(VM vm, final String regionName, final int numEvents) {
    vm.invoke(
        new CacheSerializableRunnable("Verify interest events") {
          public void run2() throws CacheException {
            Region region = CacheServerTestUtil.getCache().getRegion(regionName);
  ...
  protected void startDurableClient(
      VM vm,
    ...
    vm.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(durableClientVM.getHost()), serverPort1, true),
                regionName,
  ...
  protected void startDurableClient(
      VM vm, String durableClientId, int serverPort1, String regionName) {
    vm.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
  ...
  protected void startDurableClient(
      VM vm, String durableClientId, int serverPort1, int serverPort2, String regionName) {
    vm.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(vm.getHost()), serverPort1, serverPort2, true),
                regionName,
                getClientDistributedSystemProperties(durableClientId),
                Boolean.TRUE));
  }
  ...
  protected void startClient(VM vm, int serverPort1, int serverPort2, String regionName) {
    vm.invoke(
        () ->
            CacheServerTestUtil.createCacheClient(
                getClientPool(
                    NetworkUtils.getServerHostName(vm.getHost()), serverPort1, serverPort2, false),
                regionName));
  }
