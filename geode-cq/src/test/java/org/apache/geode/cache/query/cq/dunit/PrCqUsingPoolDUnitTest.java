...
public class PrCqUsingPoolDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testPRWithCQsAndProfileUpdates() throws Exception {
    ...
    // register cq.
    createCQ(
        client, poolName1, "testPRWithCQsAndProfileUpdates_0", cqs[0]); // SAME CQ REGISTERED TWICE.
  ...
  @Category(
      FlakyTest
          .class) // GEODE-1181, 1253: random ports, eats exceptions (fixed some), async behavior
  @Test
  ...
  public void testCQsWithPutallsValidateStats() throws Exception {
    ...
    cqStatsHelper.validateCQStats(
        client, "testCQEvents_0", size, size, 0, size + size, size + size);
  ...
  public void testCQsWithPutallsWithTx() throws Exception {
    ...
    server1.invoke(
        new CacheSerializableRunnable("begin transaction") {
          public void run2() throws CacheException {
    ...
    server1.invoke(
        new CacheSerializableRunnable("commit transaction") {
          public void run2() throws CacheException {
  ...
  public void updateValuesToGenerateDestroyCQEvent(VM vm, final String regionName, final int size) {
    vm.invoke(
        new CacheSerializableRunnable("Update values for region : " + regionName) {
          public void run2() throws CacheException {
            Region region1 = getRootRegion().getSubregion(regionName);
            for (int i = 1; i <= size; i++) {
