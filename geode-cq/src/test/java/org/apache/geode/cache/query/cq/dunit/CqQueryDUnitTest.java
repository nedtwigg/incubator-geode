...
public class CqQueryDUnitTest extends JUnit4CacheTestCase {
                ...
                getCache()
                    ...
                    .setEvictionAttributes(
                        EvictionAttributes.createLIFOEntryAttributes(
                            10, EvictionAction.LOCAL_DESTROY));
            for (int i = 0; i < regionNames.length; i++) {
              rf.create(regionNames[i]);
            }
          }
  ...
  public void crashServer(VM server) {
    server.invoke(
        new SerializableRunnable("Crash CacheServer") {
          public void run() {
            org.apache.geode.cache.client.internal.ConnectionImpl.setTEST_DURABLE_CLIENT_CRASH(
                true);
    ...
    SerializableRunnable createQService =
        new CacheSerializableRunnable("Create Local Region") {
          public void run2() throws CacheException {
            ...
            if (redundancyLevel != null) {
              ClientServerTestCase.configureConnectionPool(
                  af, serverHost, serverPorts, true, Integer.parseInt(redundancyLevel), -1, null);
            } else {
              ClientServerTestCase.configureConnectionPool(
                  af, serverHost, serverPorts, true, -1, -1, null);
            }
  ...
  public void createClientWith2Pools(
      VM client,
      final int[] serverPorts1,
      final int[] serverPorts2,
      final String serverHost,
      final String redundancyLevel) {
    SerializableRunnable createQService =
        new CacheSerializableRunnable("Create Client") {
          public void run2() throws CacheException {
            ...
            if (redundancyLevel != null) {
              ...
              ClientServerTestCase.configureConnectionPoolWithName(
                  regionFactory0, serverHost, serverPorts1, true, -1, -1, null, "testPoolA");
              ClientServerTestCase.configureConnectionPoolWithName(
                  regionFactory1, serverHost, serverPorts2, true, -1, -1, null, "testPoolB");
            }
  ...
  public void deleteValues(VM vm, final String regionName, final int size) {
    vm.invoke(
        new CacheSerializableRunnable("Delete values") {
          public void run2() throws CacheException {
            Region region1 = getRootRegion().getSubregion(regionName);
            for (int i = 1; i <= size; i++) {
  ...
  public void createCQ(
      VM vm, final String cqName, final String queryStr, final boolean isBridgeMemberTest) {
    vm.invoke(
            ...
            for (int i = 0; i < 20; ++i) {
              ...
              if (i % 2 == 0) {
                try {
                  ...
                  ex.printStackTrace();
                  fail(
                      "Failed to execute CQ with initial results, cq name: "
                          + cqName
                          + " . "
                          + ex.getMessage());
                }
  ...
  public void executeAndCloseAndExecuteIRMultipleTimes(
      VM vm, final String cqName, final String queryStr) {
    vm.invoke(
        new CacheSerializableRunnable("Create CQ :" + cqName) {
          public void run2() throws CacheException {
            LogWriterUtils.getLogWriter().info("### Create CQ. ###" + cqName);
  ...
  protected void waitForCqsDisconnected(VM vm, final String cqName, final int count) {
    vm.invoke(
        new CacheSerializableRunnable("validate cq disconnected count") {
          public void run2() throws CacheException {
            QueryService cqService = null;
            try {
              cqService = getCache().getQueryService();
  ...
  protected void waitForCqsConnected(VM vm, final String cqName, final int count) {
    vm.invoke(
        new CacheSerializableRunnable("validate cq connected count") {
          public void run2() throws CacheException {
            QueryService cqService = null;
            try {
              cqService = getCache().getQueryService();
  ...
  private void performGC(VM server, final String regionName) {
    SerializableRunnable task =
        new CacheSerializableRunnable("perform GC") {
          public void run2() throws CacheException {
            Region subregion = getCache().getRegion("root/" + regionName);
            DistributedTombstoneOperation gc =
                DistributedTombstoneOperation.gc(
                    (DistributedRegion) subregion, new EventID(getCache().getDistributedSystem()));
            gc.distribute();
          }
        };
    server.invoke(task);
  ...
  private void ensureCQExists(VM server, final String regionName, final String cqName) {
    SerializableRunnable task =
        new CacheSerializableRunnable("check CQs") {
          public void run2() throws CacheException {
  ...
  public void createMapValues(VM vm, final String regionName, final int size) {
    vm.invoke(
        new CacheSerializableRunnable("Create map values") {
          public void run2() throws CacheException {
  ...
  public void testCQEquals() throws Exception {
    ...
    // Create CQs.
    createCQ(
        client, "equalsStatusQuery2", "select * from /root/regionA p where p.status='inactive'");
  ...
  public void testCQEventsWithNotEqualsUndefined() throws Exception {
    ...
    validateCQ(
        client,
        "testCQEventsWithUndefined_0", /* resultSize: */
        noTest, /* creates: */
        size, /* updates: */
        0, /* deletes; */
        0, /* queryInserts: */
  ...
  public void restartBridgeServer(VM server, final int port) {
    server.invoke(
        new CacheSerializableRunnable("Start bridge server") {
          public void run2() {
            try {
