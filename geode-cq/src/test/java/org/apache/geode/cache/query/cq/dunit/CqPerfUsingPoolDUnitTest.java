...
public class CqPerfUsingPoolDUnitTest extends JUnit4CacheTestCase {
  ...
  protected CqQueryUsingPoolDUnitTest cqDUnitTest =
      new CqQueryUsingPoolDUnitTest(); // TODO: get rid of this!
  ...
  public void testKeyMaintenance() throws Exception {
    ...
    server.invoke(
        new CacheSerializableRunnable("LookForCachedEventKeys1") {
          public void run2() throws CacheException {
            ...
            for (InternalCqQuery cq : cqs) {
              ...
              if (serverCqName.startsWith("testKeyMaintainance_0")) {
                assertEquals(
                    "The number of keys cached for cq testKeyMaintainance_0 is wrong.",
                    1,
                    cqQuery.getCqResultKeysSize());
              } else if (serverCqName.startsWith("testKeyMaintainance_1")) {
                assertEquals(
                    "The number of keys cached for cq testKeyMaintainance_1 is wrong.",
                    0,
                    cqQuery.getCqResultKeysSize());
              }
            }
    ...
    server.invoke(
        new CacheSerializableRunnable("LookForCachedEventKeysAfterUpdate1") {
          public void run2() throws CacheException {
            CqService cqService = null;
            try {
              cqService = ((DefaultQueryService) getCache().getQueryService()).getCqService();
    ...
    // testKeyMaintainance_0 with 1 entry and testKeyMaintainance_1 with 1
    server.invoke(
        new CacheSerializableRunnable("LookForCachedEventKeysAfterUpdate2") {
          public void run2() throws CacheException {
            CqService cqService = null;
            try {
              cqService = ((DefaultQueryService) getCache().getQueryService()).getCqService();
        ...
        server.invoke(
            () ->
                CqQueryUsingPoolDUnitTest
                    .getCacheServerPort()); // TODO: move static methods from other dunit into util class
    final String host0 = NetworkUtils.getServerHostName(server.getHost());
    //cqDUnitTest.createClient(client, port, host0);
  ...
  public void testMatchingCQWithMultipleClients() throws Exception {
    ...
    for (int clientIndex = 0; clientIndex < 3; clientIndex++) {
      ...
      for (int i = 0; i < 4; i++) {
        cqDUnitTest.createCQ(
            clients[clientIndex],
            cPoolName,
            "testMatchingCQWithMultipleClients_" + i,
            cqDUnitTest.cqs[0]);
        cqDUnitTest.executeCQ(
            clients[clientIndex], "testMatchingCQWithMultipleClients_" + i, false, null);
      }
    }
    ...
    for (int clientIndex = 0; clientIndex < 3; clientIndex++) {
      cqDUnitTest.waitForCreated(
          clients[clientIndex],
          "testMatchingCQWithMultipleClients_0",
    ...
    // Update - 2.
    for (int clientIndex = 0; clientIndex < 3; clientIndex++) {
      cqDUnitTest.clearCQListenerEvents(
          clients[clientIndex], "testMatchingCQWithMultipleClients_3");
    }
    ...
    cqDUnitTest.validateCQ(
        client2,
        "testMatchingCQWithMultipleClients_1", /* resultSize: */
        CqQueryUsingPoolDUnitTest.noTest, /* creates: */
        size, /* updates: once */
        size, /* deletes; */
        0, /* queryInserts: */
    ...
    for (int clientIndex = 0; clientIndex < 3; clientIndex++) {
      ...
      cqDUnitTest.createCQ(
          clients[clientIndex],
          cPoolName,
          "testMatchingCQWithMultipleClients_4",
          cqDUnitTest.cqs[1]);
      cqDUnitTest.executeCQ(
          clients[clientIndex], "testMatchingCQWithMultipleClients_4", false, null);

      cqDUnitTest.createCQ(
          clients[clientIndex],
          cPoolName,
          "testMatchingCQWithMultipleClients_5",
          cqDUnitTest.cqs[1]);
      cqDUnitTest.executeCQ(
          clients[clientIndex], "testMatchingCQWithMultipleClients_5", false, null);

      cqDUnitTest.createCQ(
          clients[clientIndex],
          cPoolName,
          "testMatchingCQWithMultipleClients_6",
          cqDUnitTest.cqs[2]);
      cqDUnitTest.executeCQ(
          clients[clientIndex], "testMatchingCQWithMultipleClients_6", false, null);
    }
  ...
  public void testMatchingCQsWithMultipleServers() throws Exception {
    ...
    cqDUnitTest.createPool(
        client1, poolName1, new String[] {host0, host0}, new int[] {port1, ports[0]});
    cqDUnitTest.createPool(
        client2, poolName2, new String[] {host0, host0}, new int[] {port1, ports[0]});
    ...
    for (int i = 0; i < numCQs; i++) {
      cqDUnitTest.createCQ(
          client1, poolName1, "testMatchingCQsWithMultipleServers_" + i, cqDUnitTest.cqs[i]);
      cqDUnitTest.executeCQ(client1, "testMatchingCQsWithMultipleServers_" + i, false, null);

      cqDUnitTest.createCQ(
          client2, poolName2, "testMatchingCQsWithMultipleServers_" + i, cqDUnitTest.cqs[i]);
    ...
    for (int i = 1; i <= 10; i++) {
      cqDUnitTest.waitForCreated(
          client1, "testMatchingCQsWithMultipleServers_0", CqQueryUsingPoolDUnitTest.KEY + i);
    }
    ...
    System.out.println(
        "### Port on which server1 running : " + port1 + " Server2 running : " + port2);
    ...
    for (int i = 1; i <= 10; i++) {
      cqDUnitTest.waitForUpdated(
          client1, "testMatchingCQsWithMultipleServers_0", CqQueryDUnitTest.KEY + i);
      cqDUnitTest.waitForUpdated(
          client2, "testMatchingCQsWithMultipleServers_0", CqQueryDUnitTest.KEY + i);
    }

    // Wait for updates on regions[1] - Waiting for last key is good enough.
    cqDUnitTest.waitForUpdated(
        client1, "testMatchingCQsWithMultipleServers_2", CqQueryDUnitTest.KEY + 4);
    cqDUnitTest.waitForUpdated(
        client2, "testMatchingCQsWithMultipleServers_2", CqQueryDUnitTest.KEY + 4);
    ...
    for (int i = 1; i <= resultsCnt[0]; i++) {
      cqDUnitTest.waitForUpdated(
          client1, "testMatchingCQsWithMultipleServers_0", CqQueryUsingPoolDUnitTest.KEY + i);
      cqDUnitTest.waitForUpdated(
          client2, "testMatchingCQsWithMultipleServers_0", CqQueryUsingPoolDUnitTest.KEY + i);
    }

    // Wait for cq3. on region[1]
    {
      cqDUnitTest.waitForUpdated(
          client1, "testMatchingCQsWithMultipleServers_2", CqQueryUsingPoolDUnitTest.KEY + "4");
      cqDUnitTest.waitForUpdated(
          client2, "testMatchingCQsWithMultipleServers_2", CqQueryUsingPoolDUnitTest.KEY + "4");
    }
  ...
  public void testPerformanceForMatchingCQs() throws Exception {
    ...
    for (int k = 1; k <= size; k++) {
      cqDUnitTest.waitForUpdated(
          client1, "testPerformanceForMatchingCQs_0", CqQueryUsingPoolDUnitTest.KEY + k);
    }
  ...
  public void validateMatchingCqs(
      VM server, final int mapSize, final String query, final int numCqSize) {
    server.invoke(
        new CacheSerializableRunnable("validateMatchingCqs") {
          public void run2() throws CacheException {
            ...
            assertEquals(
                "The number of matched cq is not as expected.", mapSize, matchedCqMap.size());
  ...
  public void printCqQueryExecutionTime(VM server) {
    server.invoke(
        new CacheSerializableRunnable("printCqQueryExecutionTime") {
          public void run2() throws CacheException {
            CqServiceImpl cqService = null;
            try {
              cqService =
            ...
            LogWriterUtils.getLogWriter()
                .info("Total Time taken to Execute CQ Query :" + timeTaken);
