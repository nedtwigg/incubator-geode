...
public class ServerCQProxyImpl extends ServerProxy {
  ...
  public Object createOn(
    ...
    return CreateCQOp.executeOn(
        this.pool, conn, cqName, queryStr, cqState, isDurable, regionDataPolicy);
  }
  ...
  public SelectResults createWithIR(ClientCQ cq) {
    ...
    byte regionDataPolicyOrdinal =
        cq.getCqBaseRegion() == null
            ? (byte) 0
            : cq.getCqBaseRegion().getAttributes().getDataPolicy().ordinal;
    return CreateCQWithIROp.execute(
        this.pool,
        cq.getName(),
        cq.getQueryString(),
        CqStateImpl.RUNNING,
