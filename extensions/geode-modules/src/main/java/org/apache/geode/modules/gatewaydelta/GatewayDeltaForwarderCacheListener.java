...
public class GatewayDeltaForwarderCacheListener extends CacheListenerAdapter<String, GatewayDelta>
  ...
  public void afterCreate(EntryEvent<String, GatewayDelta> event) {
    ...
    if (event.getCallbackArgument() == null) {
      ...
      if (scv == null) {
        getGatewayDeltaRegion()
            .put(
                sessionId,
                new GatewayDeltaCreateEvent(
                    regionName, sessionId, EntryEventImpl.serialize(event.getNewValue())));
      } else {
        System.out.println(
  ...
  public void afterDestroy(EntryEvent<String, GatewayDelta> event) {
    ...
    if (event.getCallbackArgument() != null) {
      ...
      String sessionId = event.getKey();
      getGatewayDeltaRegion()
          .put(sessionId, new GatewayDeltaDestroyEvent(event.getRegion().getFullPath(), sessionId));
    } else {
      if (this.cache.getLogger().fineEnabled()) {
        StringBuilder builder = new StringBuilder();
        builder
