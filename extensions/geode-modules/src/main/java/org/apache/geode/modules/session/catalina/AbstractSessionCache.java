...
public abstract class AbstractSessionCache implements SessionCache {
  ...
  /**
   * The sessionRegion is the <code>Region</code> that actually stores and replicates the <code>
   * Session</code>s.
   */
  ...
  protected void createStatistics() {
    this.statistics =
        new DeltaSessionStatistics(
            getCache().getDistributedSystem(), getSessionManager().getStatisticsName());
  }
  ...
  protected RegionConfiguration createRegionConfiguration() {
    ...
    if (getSessionManager().getMaxInactiveInterval()
    ...
    configuration.setEnableGatewayDeltaReplication(
        getSessionManager().getEnableGatewayDeltaReplication());
