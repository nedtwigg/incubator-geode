...
@Api(
  value = "queries",
  description = "Rest api for gemfire query execution",
...
public class QueryAccessController extends AbstractBaseController {
  ...
  public ResponseEntity<?> create(
      @RequestParam("id") final String queryId,
    ...
    logger.debug(
        "Creating a named, parametrized Query ({}) with ID ({})...", oqlStatement, queryId);
    ...
    if (existingOql != null) {
      ...
      return new ResponseEntity<String>(
          JSONUtils.formulateJsonForExistingQuery(queryId, existingOql),
          headers,
          HttpStatus.CONFLICT);
    } else {
  ...
  @ApiOperation(
    ...
    notes =
        "run the specified named query passing in scalar values for query parameters in the GemFire cluster",
    response = void.class
  )
  @ApiResponses({
  ...
  @ResponseStatus(HttpStatus.OK)
  @PreAuthorize("@securityService.authorize('DATA', 'WRITE')")
  public ResponseEntity<String> runNamedQuery(
      @PathVariable("query") String queryId, @RequestBody String arguments) {
  ...
  public ResponseEntity<?> update(
      @PathVariable("query") final String queryId,
      @RequestParam(value = "q", required = false) String oqlInUrl,
      @RequestBody(required = false) final String oqlInBody) {
    final String oqlStatement = validateQuery(oqlInUrl, oqlInBody);

    logger.debug(
        "Updating a named, parametrized Query ({}) with ID ({})...", oqlStatement, queryId);
