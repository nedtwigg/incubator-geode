...
public class LuceneIndexCommandsJUnitTest {
  ...
  public void testListIndexWithoutStats() {
    ...
    assertEquals(
        Arrays.asList("memberFive", "memberSix", "memberTen"),
        data.retrieveAllValues("Index Name"));
    assertEquals(
        Arrays.asList("/Employees", "/Employees", "/Employees"),
    ...
    assertEquals(
        Arrays.asList("Initialized", "Defined", "Initialized"), data.retrieveAllValues("Status"));
  }
  ...
  public void testListIndexWithStats() {
    ...
    final LuceneIndexDetails indexDetails1 =
        createIndexDetails(
            "memberFive", "/Employees", searchableFields, fieldAnalyzers, mockIndexStats1, true);
    final LuceneIndexDetails indexDetails2 =
        createIndexDetails(
            "memberSix", "/Employees", searchableFields, fieldAnalyzers, mockIndexStats2, true);
    final LuceneIndexDetails indexDetails3 =
        createIndexDetails(
            "memberTen", "/Employees", searchableFields, fieldAnalyzers, mockIndexStats3, true);
  ...
  public void testDescribeIndex() throws Exception {
    ...
    indexDetails.add(
        createIndexDetails(
            "memberFive", "/Employees", searchableFields, fieldAnalyzers, mockIndexStats, true));
    ...
    assertEquals(
        Collections.singletonList("[field1, field2, field3]"),
        data.retrieveAllValues("Indexed Fields"));
    assertEquals(
        Collections.singletonList("{field1=StandardAnalyzer, field2=KeywordAnalyzer}"),
        data.retrieveAllValues("Field Analyzer"));
  ...
  public void testSearchIndexWithPaging() throws Exception {
    ...
    when(mockGfsh.interact(anyString()))
        .thenReturn("n")
        .thenReturn("n")
        .thenReturn("n")
        .thenReturn("n")
  ...
  private String getPage(final LuceneSearchResults[] expectedResults, int[] indexList) {
    ...
    while (commandResult.hasNextLine()) buffer.append(commandResult.nextLine());
  ...
  private List<Set<LuceneSearchResults>> getSearchResults(LuceneSearchResults... results) {
    final List<Set<LuceneSearchResults>> queryResultsList = new ArrayList<>();
    HashSet<LuceneSearchResults> queryResults = new HashSet<>();
    for (LuceneSearchResults result : results) queryResults.add(result);
    queryResultsList.add(queryResults);
  ...
  private LuceneIndexCommands createIndexCommands(
      final Cache cache, final Execution functionExecutor) {
    return new LuceneTestIndexCommands(cache, functionExecutor);
  }

  private LuceneIndexDetails createIndexDetails(
      final String indexName,
      final String regionPath,
      final String[] searchableFields,
      final Map<String, Analyzer> fieldAnalyzers,
      LuceneIndexStats indexStats,
      boolean status) {
    return new LuceneIndexDetails(
        indexName, regionPath, searchableFields, fieldAnalyzers, indexStats, status);
  }
  ...
  private LuceneIndexDetails createIndexDetails(
      ...
      boolean status) {
    return new LuceneIndexDetails(
        indexName, regionPath, searchableFields, fieldAnalyzers, null, status);
  }

  private LuceneSearchResults createQueryResults(
      final String key, final String value, final float score) {
