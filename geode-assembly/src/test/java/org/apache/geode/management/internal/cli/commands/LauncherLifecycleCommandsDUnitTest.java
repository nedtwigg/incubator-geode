...
public class LauncherLifecycleCommandsDUnitTest extends CliCommandTestBase {
  ...
  protected static String getMemberId(
      final String jmxManagerHost, final int jmxManagerPort, final String memberName)
      throws Exception {
    ...
    while ((pid = processIds.poll()) != null) {
      if (launcherLifecycleCommands.isVmWithProcessIdRunning(pid)) {
        try {
          String killCommand =
              String.format(
                  "%1$s %2$d", SystemUtils.isWindows() ? "taskkill /F /PID" : "kill -9", pid);
  ...
  protected Integer readPid(final File workingDirectory) throws IOException {
    assertTrue(
        String.format("The working directory (%1$s) must exist!", workingDirectory),
  ...
  protected String serviceStateStatusStringNormalized(final String serviceStateStatus) {
    ...
    assertTrue(
        "serviceStateStatus is missing 'Uptime': " + serviceStateStatus,
        serviceStateStatus.contains("Uptime"));
    assertTrue(
        "serviceStateStatus is missing 'JVM Arguments': " + serviceStateStatus,
        serviceStateStatus.contains("JVM Arguments"));

    return serviceStateStatus
        .substring(0, serviceStateStatus.indexOf("Uptime"))
  ...
  protected Status waitForGemFireProcessToStop(
      final ServiceState serviceState, final String workingDirectory) {
    if (!Status.STOPPED.equals(serviceState.getStatus())) {
      try {
        final Integer pid = readPid(new File(workingDirectory));

        if (pid != null) {
          WaitCriterion waitCriteria =
              new WaitCriterion() {
          ...
          waitForCriterion(
              waitCriteria, TimeUnit.SECONDS.toMillis(15), TimeUnit.SECONDS.toMillis(5), false);
  ...
  public void test000StartLocatorCapturesOutputOnError() throws IOException {
    ...
    command.addOption(
        CliStrings.START_LOCATOR__ENABLE__SHARED__CONFIGURATION, Boolean.FALSE.toString());
    command.addOption(
        CliStrings.START_LOCATOR__J,
  ...
  public void test002StartLocatorFailsFastOnMissingGemFireSecurityPropertiesFile()
    ...
    command.addOption(CliStrings.START_LOCATOR__PORT, "0");
    command.addOption(
        CliStrings.START_LOCATOR__SECURITY_PROPERTIES, gemfireSecurityPropertiesPathname);
    command.addOption(
        CliStrings.START_LOCATOR__J,
        "-D" + DistributionConfig.GEMFIRE_PREFIX + "http-service-port=0");
    command.addOption(
  ...
  public void test003StartServerFailsFastOnMissingCacheXmlFile() throws IOException {
    ...
    assertTrue(
        resultString,
        resultString.contains(
            MessageFormat.format(CliStrings.CACHE_XML_NOT_FOUND_MESSAGE, cacheXmlPathname)));
  }
  ...
  public void test005StartServerFailsFastOnMissingGemFireSecurityPropertiesFile()
    ...
    command.addOption(CliStrings.START_SERVER__DIR, workingDirectory.getCanonicalPath());
    command.addOption(
        CliStrings.START_SERVER__SECURITY_PROPERTIES, gemfireSecuritiesPropertiesFile);
  ...
  public void test006StartLocatorInRelativeDirectory() {
    ...
    try {
      ...
      assertTrue(
          "Locator output was: " + locatorOutput,
  ...
  public void test008StatusLocatorUsingMemberName() throws IOException {
    ...
    try {
      ...
      assertEquals(
          CliStrings.format(
              CliStrings.STATUS_LOCATOR__NO_LOCATOR_FOUND_FOR_MEMBER_ERROR_MESSAGE,
      ...
      assertEquals(Result.Status.OK, result.getStatus());
      assertTrue(
          serviceStateStatusStringNormalized(toString(result))
    ...
    try {
      ...
      result =
          executeCommand(
              String.format(
                  "%1$s --name=%2$s",
                  CliStrings.STATUS_LOCATOR, getMemberId(jmxManagerPort, pathname)));
  ...
  public void test014GemFireServerJvmProcessTerminatesOnOutOfMemoryError() throws Exception {
    ...
    command.addOption(CliStrings.START_SERVER__DIR, workingDirectory.getCanonicalPath());
    command.addOption(
        CliStrings.START_SERVER__CACHE_XML_FILE,
    ...
    try {
      ...
      waitForCriterion(
          waitCriteria, TimeUnit.SECONDS.toMillis(30), TimeUnit.SECONDS.toMillis(10), true);
      ...
      fileWriter =
          new BufferedWriter(
              new OutputStreamWriter(
                  new FileOutputStream(cacheXml, false), Charset.forName("UTF-8").newEncoder()));
  ...
  private ClientCache setupClientCache(final String durableClientId, final int serverPort) {
    ...
    assertNotNull(
        "The 'Example' Client Region was not properly configured and initialized", exampleProxy);
