...
public class RestSecurityDUnitTest extends AbstractSecureServerDUnitTest {
  ...
  public void testFunctions() {
    client1.invoke(
        () -> {
          ...
          response =
              doPost(
                  "/functions/AddFreeItemsToOrder?onRegion=" + REGION_NAME,
                  "dataWriter",
                  "1234567",
                  json);
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPost("/queries/id", "unknown-user", "1234567", "{\"id\" : \"foo\"}");
          assertEquals(401, getCode(response));
  ...
  public void testPutQuery() {
    client1.invoke(
        () -> {
          HttpResponse response =
              doPut("/queries/id", "unknown-user", "1234567", "{\"id\" : \"foo\"}");
          assertEquals(401, getCode(response));
  ...
  public void postRegionKey() {
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPost("/" + REGION_NAME + "?key9", "unknown", "1234567", "{ \"key9\" : \"foo\" }");
          assertEquals(401, getCode(response));
        });
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPost(
                  "/" + REGION_NAME + "?key9", "dataReader", "1234567", "{ \"key9\" : \"foo\" }");
          assertEquals(403, getCode(response));
        });
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPost(
                  "/" + REGION_NAME + "?key9", "dataWriter", "1234567", "{ \"key9\" : \"foo\" }");
  ...
  public void putRegionKey() {
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPut("/" + REGION_NAME + "/key1?op=CAS", "dataReader", "1234567", casJSON);
          assertEquals(403, getCode(response));
        });
    ...
    client1.invoke(
        () -> {
          HttpResponse response =
              doPut("/" + REGION_NAME + "/key1?op=REPLACE", "key1User", "1234567", json);
          assertEquals(200, getCode(response));
          assertTrue(isOK(response));
        });
  }

  protected HttpResponse doHEAD(String query, String username, String password)
      throws MalformedURLException {
    HttpHead httpHead = new HttpHead(CONTEXT + query);
    return doRequest(httpHead, username, password);
  }

  protected HttpResponse doPost(String query, String username, String password, String body)
      throws MalformedURLException {
  ...
  protected HttpResponse doGet(String uri, String username, String password)
      throws MalformedURLException {
    HttpGet getRequest = new HttpGet(CONTEXT + uri);
    return doRequest(getRequest, username, password);
  }

  protected HttpResponse doDelete(String uri, String username, String password)
      throws MalformedURLException {
