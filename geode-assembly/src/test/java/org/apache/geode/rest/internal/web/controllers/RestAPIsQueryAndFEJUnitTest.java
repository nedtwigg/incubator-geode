...
public class RestAPIsQueryAndFEJUnitTest {
  ...
  private final String MALFORMED_CAS_JSON =
      "{"
          ...
          + "}"
          + "}";
  final String[][] PARAMETERIZED_QUERIES =
      new String[][] {
        {
          "selectOrders",
  ...
  public String createRestURL(String baseURL, Object requestPart) {
    if (StringUtils.isEmpty(requestPart)) {
      return baseURL
          + RestTestUtils.GEMFIRE_REST_API_CONTEXT
    ...
    for (int index = 0; index < totalRequests; index++) {
      try {
        ...
        ResponseEntity<String> result =
            RestTestUtils.getRestTemplate()
                .exchange(
                    restRequestUrl,
                    (HttpMethod) TEST_DATA[index][METHOD_INDEX],
                    entity,
                    String.class);
        ...
        assertEquals(
            result.hasBody(), ((Boolean) TEST_DATA[index][RESPONSE_HAS_BODY_INDEX]).booleanValue());
        ...
        if (VALID_409_URL_INDEXS.contains(index)) {
          ...
          fail(
              "Index:"
                  + index
                  + " "
                  + TEST_DATA[index][METHOD_INDEX]
                  + " "
                  + TEST_DATA[index][URL_INDEX]
        ...
        caught(
            "caught Exception in executeQueryTestCases "
                + "Index:"
                + index
                + " "
                + TEST_DATA[index][METHOD_INDEX]
